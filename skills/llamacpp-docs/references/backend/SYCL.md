...
| GGML_SYCL_DEV2DEV_MEMCPY | 0 (default), 1, 2 | Choose the method of dev2dev memory copy.Value: *  0: SYCL API (default), only support dGPUs.* 1: L0 API -- Better performance, only support dGPUs, found to lead to abnormal crash in some case. * 2: Host Forward -- Most stable method for all cases (including iGPU + dGPU*N), but with lower performance (-2% to -5%).SYCL & L0 API are easy to be impacted by Intel GPU driver issue. When you meet the garbled output or crash issues in multiple GPUs case, try with this debug flag to work around or check the issue.|
...
| GGML_SYCL_MEMTRACE | 0 (default), 1, 2 | Enable record and output memory allocation diagnostics. Requires `-lv 4`. 0 -  Disable1 - Basic memory info, including current and peak allocations, as well allocations from other sources, around 50 lines per model load.2 - More verbose, logging around 900 specific allocations and deallocations. |
