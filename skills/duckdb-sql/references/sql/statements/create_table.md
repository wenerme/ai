...
Temporary tables reside in memory rather than on disk even when connecting to a persistent DuckDB, but if the `temp_directory` [configuration](https://duckdb.org/docs/current/configuration/overview.html) is set, data will be spilled to disk if memory becomes constrained.
...
This syntax can be used in combination with the [CSV reader](https://duckdb.org/docs/current/data/csv/overview.html), the shorthand to read directly from CSV files without specifying a function, the [`FROM`-first syntax](https://duckdb.org/docs/current/sql/query_syntax/from.html), and the [HTTP(S) support](https://duckdb.org/docs/current/core_extensions/httpfs/https.html), yielding concise SQL commands such as the following:
