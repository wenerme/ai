...
| `GLOBAL` | Variables with a `GLOBAL` scope are specific [configuration option variables](https://duckdb.org/docs/current/configuration/overview.html#global-configuration-options) that affect the entire DuckDB instance and all sessions. For example, see [Set a Global Variable](https://duckdb.org/docs/current/sql/statements/set.html#set-a-global-variable). |
...
The `getvariable` function can also be used in a [`COLUMNS` expression](https://duckdb.org/docs/current/sql/expressions/star.html#columns-expression):
