...
The `SHOW` statement is an alias for [`DESCRIBE`](https://duckdb.org/docs/current/sql/statements/describe.html).
...
The `current` column indicates which schema is the default schema (set via the [`USE` statement](https://duckdb.org/docs/current/sql/statements/use.html)).
