...
> Instead of
> `ALTER ⟨column_name⟩ TYPE ⟨type⟩`, you can also use the equivalent
...
There are two options to change the sub-schema of a [`STRUCT`](https://duckdb.org/docs/current/sql/data_types/struct.html)-typed column.
...
[`ADD COLUMN`, `DROP COLUMN` and `RENAME COLUMN` clauses](https://duckdb.org/docs/current/sql/data_types/struct.html#updating-the-schema)
