...
For details on how the `PIVOT` statement is implemented, see the [Pivot Internals site](https://duckdb.org/docs/current/internals/pivot.html#pivot).

> The [`UNPIVOT` statement](https://duckdb.org/docs/current/sql/statements/unpivot.html) is the inverse of the `PIVOT` statement.
...
> Note The SQL syntax permits [`FILTER` clauses](https://duckdb.org/docs/current/sql/query_syntax/filter.html) with aggregate functions in the `USING` clause.
...
The `PIVOT` statement may be included within a `SELECT` statement as a CTE ([a Common Table Expression, or `WITH` clause](https://duckdb.org/docs/current/sql/query_syntax/with.html)), or a subquery.
...
To work around this limitation, perform the `PIVOT` with the aggregation only, then use the [`COLUMNS` expression](https://duckdb.org/docs/current/sql/expressions/star.html#columns-expression):
