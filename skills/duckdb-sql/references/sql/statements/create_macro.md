...
* Table macros return a tabular result: when called, they act essentially as [table functions](https://duckdb.org/docs/current/sql/query_syntax/from.html#table-functions) and return a table value. Their expression can be a `SELECT` statement, or a call to another table function.
...
* Optionally, a parameter may explicitly specify a particular datatype. This can be any of the existing DuckDB [data types](https://duckdb.org/docs/current/sql/data_types/overview.html). Note: there is no way to specify a parameter of a `TABLE` type.
...
This is similar to calling a [function](https://duckdb.org/docs/current/sql/functions/overview.html).
...
* If the expression of a scalar macro references an [aggregate function](https://duckdb.org/docs/current/sql/functions/aggregates.html), then the macro behaves like an aggregate function too.
* A call to a table macro can appear in the `FROM` clause of a `SELECT` statement, or in a [`CALL` statement](https://duckdb.org/docs/current/sql/statements/call.html).
...
To define macros on arbitrary tables, use the [`query_table` function](https://duckdb.org/docs/current/guides/sql_features/query_and_query_table_functions.html). For example, the following macro computes a column-wise checksum on a table:
