...
Attach a SQLite database for reading and writing (see the [`sqlite` extension](https://duckdb.org/docs/current/core_extensions/sqlite.html) for more information):
...
For more details, see the [“Storage” page](https://duckdb.org/docs/current/internals/storage.html#explicit-storage-versions).
...
To encrypt data, DuckDB can use either the built-in `mbedtls` library or the OpenSSL library from the [`httpfs` extension](https://duckdb.org/docs/current/core_extensions/httpfs/overview.html). Note that the OpenSSL versions are much faster due to hardware acceleration, so make sure to load the `httpfs` for good encryption performance:
...
> Warning Closing the connection, e.g., invoking the [`close()` function in Python](https://duckdb.org/docs/current/clients/python/dbapi.html#connection), does not release the locks held on the database files as the file handles are held by the main DuckDB instance (in Python's case, the `duckdb` module).
...
Also note the rules on [identifiers and database names in particular](https://duckdb.org/docs/current/sql/dialect/keywords_and_identifiers.html#database-names).
