...
Optionally, the `SELECT` statement can be prefixed with a [`WITH` clause](https://duckdb.org/docs/current/sql/query_syntax/with.html).
...
The [`FROM` clause](https://duckdb.org/docs/current/sql/query_syntax/from.html) specifies the *source* of the data on which the remainder of the query should operate. Logically, the `FROM` clause is where the query starts execution. The `FROM` clause can contain a single table, a combination of multiple tables that are joined together, or another `SELECT` query inside a subquery node.

### `SAMPLE` Clause

The [`SAMPLE` clause](https://duckdb.org/docs/current/sql/query_syntax/sample.html) allows you to run the query on a sample from the base table. This can significantly speed up processing of queries, at the expense of accuracy in the result. Samples can also be used to quickly see a snapshot of the data when exploring a dataset. The `SAMPLE` clause is applied right after anything in the `FROM` clause (i.e., after any joins, but before the where clause or any aggregates). See the [Samples](https://duckdb.org/docs/current/sql/samples.html) page for more information.

### `WHERE` Clause

The [`WHERE` clause](https://duckdb.org/docs/current/sql/query_syntax/where.html) specifies any filters to apply to the data. This allows you to select only a subset of the data in which you are interested. Logically the `WHERE` clause is applied immediately after the `FROM` clause.

### `GROUP BY` and `HAVING` Clauses

The [`GROUP BY` clause](https://duckdb.org/docs/current/sql/query_syntax/groupby.html) specifies which grouping columns should be used to perform any aggregations in the `SELECT` clause. If the `GROUP BY` clause is specified, the query is always an aggregate query, even if no aggregations are present in the `SELECT` clause.

### `WINDOW` Clause

The [`WINDOW` clause](https://duckdb.org/docs/current/sql/query_syntax/window.html) allows you to specify named windows that can be used within window functions. These are useful when you have multiple window functions, as they allow you to avoid repeating the same window clause.

### `QUALIFY` Clause

The [`QUALIFY` clause](https://duckdb.org/docs/current/sql/query_syntax/qualify.html) is used to filter the result of [`WINDOW` functions](https://duckdb.org/docs/current/sql/functions/window_functions.html).

### `ORDER BY`, `LIMIT` and `OFFSET` Clauses

[`ORDER BY`](https://duckdb.org/docs/current/sql/query_syntax/orderby.html), [`LIMIT` and `OFFSET`](https://duckdb.org/docs/current/sql/query_syntax/limit.html) are output modifiers.
...
[A `VALUES` list](https://duckdb.org/docs/current/sql/query_syntax/values.html) is a set of values that is supplied instead of a `SELECT` statement.
