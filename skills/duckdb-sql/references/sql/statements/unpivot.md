...
Both can utilize a [`COLUMNS` expression](https://duckdb.org/docs/current/sql/expressions/star.html#columns) to automatically detect the columns to unpivot.
`PIVOT_LONGER` may also be used in place of the `UNPIVOT` keyword.

For details on how the `UNPIVOT` statement is implemented, see the [Pivot Internals site](https://duckdb.org/docs/current/internals/pivot.html#unpivot).

> The [`PIVOT` statement](https://duckdb.org/docs/current/sql/statements/pivot.html) is the inverse of the `UNPIVOT` statement.
...
The [`COLUMNS` expression](https://duckdb.org/docs/current/sql/expressions/star.html#columns-expression) can be used to select all columns that are not `empid` or `dept`.
...
The `UNPIVOT` statement may be included within a `SELECT` statement as a CTE ([a Common Table Expression, or WITH clause](https://duckdb.org/docs/current/sql/query_syntax/with.html)), or a subquery.
...
DuckDB allows expressions within the `UNPIVOT` statements, provided that they only involve a single column. These can be used to perform computations as well as [explicit casts](https://duckdb.org/docs/current/sql/data_types/typecasting.html#explicit-casting). For example:
...
The [`COLUMNS` expression](https://duckdb.org/docs/current/sql/expressions/star.html#columns) can be used to determine the `IN` list of columns dynamically.
