...
Sequences can be used as `DEFAULT` values in [`CREATE TABLE` statements](https://duckdb.org/docs/current/sql/statements/create_table.html).

The example below uses a sequence to create an integer [primary key](https://duckdb.org/docs/current/sql/constraints.html#primary-key-and-unique-constraint):
...
Sequences can also be added using the [`ALTER TABLE` statement](https://duckdb.org/docs/current/sql/statements/alter_table.html). The following example adds an `id` column and fills it with values generated by the sequence:
