...
DuckDB has two built-in index types. Indexes can also be defined via [extensions](https://duckdb.org/docs/current/extensions/overview.html).

### Min-Max Index (Zonemap)

A [min-max index](https://en.wikipedia.org/wiki/Block_Range_Index) (also known as zonemap or block range index) is _automatically created_ for columns of all [general-purpose data types](https://duckdb.org/docs/current/sql/data_types/overview.html).
...
DuckDB supports [R-trees for spatial indexing](https://duckdb.org/docs/current/core_extensions/spatial/r-tree_indexes.html) via the `spatial` extension.
...
To create an [ART index](#adaptive-radix-tree-art), use the [`CREATE INDEX` statement](https://duckdb.org/docs/current/sql/statements/create_index.html#create-index).
To drop an [ART index](#adaptive-radix-tree-art), use the [`DROP INDEX` statement](https://duckdb.org/docs/current/sql/statements/create_index.html#drop-index).
...
> As expected, indexes have a strong effect on performance, slowing down loading and updates, but speeding up certain queries. Please consult the [Performance Guide](https://duckdb.org/docs/current/guides/performance/indexing.html) for details.
