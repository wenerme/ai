...
This section describes functions and operators for examining and manipulating [`TIMESTAMP` values](https://duckdb.org/docs/current/sql/data_types/timestamp.html).
See also the related [`TIMESTAMPTZ` functions](https://duckdb.org/docs/current/sql/functions/timestamptz.html).
...
Adding to or subtracting from [infinite values](https://duckdb.org/docs/current/sql/data_types/timestamp.html#special-values) produces the same infinite value.
...
| [`date_diff(part, starttimestamp, endtimestamp)`](#date_diffpart-starttimestamp-endtimestamp) | The number of [`part`](https://duckdb.org/docs/current/sql/functions/datepart.html) boundaries between `starttimestamp` and `endtimestamp`, inclusive of the larger timestamp and exclusive of the smaller timestamp. |
| [`date_part([part, ...], timestamp)`](#date_partpart--timestamp) | Get the listed [subfields](https://duckdb.org/docs/current/sql/functions/datepart.html) as a `struct`. The list must be constant. |
| [`date_part(part, timestamp)`](#date_partpart-timestamp) | Get [subfield](https://duckdb.org/docs/current/sql/functions/datepart.html) (equivalent to `extract`). |
| [`date_sub(part, starttimestamp, endtimestamp)`](#date_subpart-starttimestamp-endtimestamp) | The signed length of the interval between `starttimestamp` and `endtimestamp`, truncated to whole multiples of [`part`](https://duckdb.org/docs/current/sql/functions/datepart.html). |
| [`date_trunc(part, timestamp)`](#date_truncpart-timestamp) | Truncate to specified [precision](https://duckdb.org/docs/current/sql/functions/datepart.html). |
...
| [`extract(field FROM timestamp)`](#extractfield-from-timestamp) | Get [subfield](https://duckdb.org/docs/current/sql/functions/datepart.html) from a timestamp. |
...
| [`strftime(timestamp, format)`](#strftimetimestamp-format) | Converts timestamp to string according to the [format string](https://duckdb.org/docs/current/sql/functions/dateformat.html#format-specifiers). |
| [`strptime(text, format-list)`](#strptimetext-format-list) | Converts the string `text` to timestamp applying the [format strings](https://duckdb.org/docs/current/sql/functions/dateformat.html) in the list until one succeeds. Throws an error on failure. To return `NULL` on failure, use [`try_strptime`](#try_strptimetext-format-list). |
| [`strptime(text, format)`](#strptimetext-format) | Converts the string `text` to timestamp according to the [format string](https://duckdb.org/docs/current/sql/functions/dateformat.html#format-specifiers). Throws an error on failure. To return `NULL` on failure, use [`try_strptime`](#try_strptimetext-format). |
...
| [`try_strptime(text, format-list)`](#try_strptimetext-format-list) | Converts the string `text` to timestamp applying the [format strings](https://duckdb.org/docs/current/sql/functions/dateformat.html) in the list until one succeeds. Returns `NULL` on failure. |
| [`try_strptime(text, format)`](#try_strptimetext-format) | Converts the string `text` to timestamp according to the [format string](https://duckdb.org/docs/current/sql/functions/dateformat.html#format-specifiers). Returns `NULL` on failure. |

There are also dedicated extraction functions to get the [subfields](https://duckdb.org/docs/current/sql/functions/datepart.html).
...
| **Description** | The signed number of [`part`](https://duckdb.org/docs/current/sql/functions/datepart.html) boundaries between `starttimestamp` and `endtimestamp`, inclusive of the larger timestamp and exclusive of the smaller timestamp. |
| **Example** | `date_diff('hour', TIMESTAMP '1992-09-30 23:59:59', TIMESTAMP '1992-10-01 01:58:00')` |
| **Result** | `2` |

#### `date_part([part, ...], timestamp)`

| **Description** | Get the listed [subfields](https://duckdb.org/docs/current/sql/functions/datepart.html) as a `struct`. The list must be constant. |
| **Example** | `date_part(['year', 'month', 'day'], TIMESTAMP '1992-09-20 20:38:40')` |
| **Result** | `{year: 1992, month: 9, day: 20}` |

#### `date_part(part, timestamp)`

| **Description** | Get [subfield](https://duckdb.org/docs/current/sql/functions/datepart.html) (equivalent to `extract`). |
| **Example** | `date_part('minute', TIMESTAMP '1992-09-20 20:38:40')` |
| **Result** | `38` |

#### `date_sub(part, starttimestamp, endtimestamp)`

| **Description** | The signed length of the interval between `starttimestamp` and `endtimestamp`, truncated to whole multiples of [`part`](https://duckdb.org/docs/current/sql/functions/datepart.html). |
| **Example** | `date_sub('hour', TIMESTAMP '1992-09-30 23:59:59', TIMESTAMP '1992-10-01 01:58:00')` |
| **Result** | `1` |

#### `date_trunc(part, timestamp)`

| **Description** | Truncate to specified [precision](https://duckdb.org/docs/current/sql/functions/datepart.html). |
...
| **Description** | Get [subfield](https://duckdb.org/docs/current/sql/functions/datepart.html) from a timestamp. |
...
| **Description** | Converts timestamp to string according to the [format string](https://duckdb.org/docs/current/sql/functions/dateformat.html#format-specifiers). |
| **Example** | `strftime(timestamp '1992-01-01 20:38:40', '%a, %-d %B %Y - %I:%M:%S %p')` |
| **Result** | `Wed, 1 January 1992 - 08:38:40 PM` |

#### `strptime(text, format-list)`

| **Description** | Converts the string `text` to timestamp applying the [format strings](https://duckdb.org/docs/current/sql/functions/dateformat.html) in the list until one succeeds. Throws an error on failure. To return `NULL` on failure, use [`try_strptime`](#try_strptimetext-format-list). |
| **Example** | `strptime('4/15/2023 10:56:00', ['%d/%m/%Y %H:%M:%S', '%m/%d/%Y %H:%M:%S'])` |
| **Result** | `2023-04-15 10:56:00` |

#### `strptime(text, format)`

| **Description** | Converts the string `text` to timestamp according to the [format string](https://duckdb.org/docs/current/sql/functions/dateformat.html#format-specifiers). Throws an error on failure. To return `NULL` on failure, use [`try_strptime`](#try_strptimetext-format). |
...
| **Description** | Converts the string `text` to timestamp applying the [format strings](https://duckdb.org/docs/current/sql/functions/dateformat.html) in the list until one succeeds. Returns `NULL` on failure. |
| **Example** | `try_strptime('4/15/2023 10:56:00', ['%d/%m/%Y %H:%M:%S', '%m/%d/%Y %H:%M:%S'])` |
| **Result** | `2023-04-15 10:56:00` |

#### `try_strptime(text, format)`

| **Description** | Converts the string `text` to timestamp according to the [format string](https://duckdb.org/docs/current/sql/functions/dateformat.html#format-specifiers). Returns `NULL` on failure. |
