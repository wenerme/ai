...
(or `TIMESTAMPTZ`) values](https://duckdb.org/docs/current/sql/data_types/timestamp.html). See also the related [`TIMESTAMP` functions](https://duckdb.org/docs/current/sql/functions/timestamp.html).

Time zone support is provided by the built-in [ICU extension](https://duckdb.org/docs/current/core_extensions/icu.html).
...
| [`date_diff(part, starttimestamptz, endtimestamptz)`](#date_diffpart-starttimestamptz-endtimestamptz) | The number of [`part`](https://duckdb.org/docs/current/sql/functions/datepart.html) boundaries between `starttimestamptz` and `endtimestamptz` inclusive of the larger timestamp and exclusive of the smaller timestamp. |
| [`date_part([part, ...], timestamp)`](#date_partpart--timestamptz) | Get the listed [subfields](https://duckdb.org/docs/current/sql/functions/datepart.html) as a `struct`. The list must be constant. |
| [`date_part(part, timestamp)`](#date_partpart-timestamptz) | Get [subfield](https://duckdb.org/docs/current/sql/functions/datepart.html) (equivalent to `extract`). |
| [`date_sub(part, starttimestamptz, endtimestamptz)`](#date_subpart-starttimestamptz-endtimestamptz) | The signed length of the interval between `starttimestamptz` and `endtimestamptz`, truncated to whole multiples of [`part`](https://duckdb.org/docs/current/sql/functions/datepart.html). |
| [`date_trunc(part, timestamptz)`](#date_truncpart-timestamptz) | Truncate to specified [precision](https://duckdb.org/docs/current/sql/functions/datepart.html). |
...
| [`extract(field FROM timestamptz)`](#extractfield-from-timestamptz) | Get [subfield](https://duckdb.org/docs/current/sql/functions/datepart.html) from a `TIMESTAMP WITH TIME ZONE`. |
...
| [`strftime(timestamptz, format)`](#strftimetimestamptz-format) | Converts a `TIMESTAMP WITH TIME ZONE` value to string according to the [format string](https://duckdb.org/docs/current/sql/functions/dateformat.html#format-specifiers). |
| [`strptime(text, format)`](#strptimetext-format) | Converts string to `TIMESTAMP WITH TIME ZONE` according to the [format string](https://duckdb.org/docs/current/sql/functions/dateformat.html#format-specifiers) if `%Z` is specified. |
...
| **Description** | The signed number of [`part`](https://duckdb.org/docs/current/sql/functions/datepart.html) boundaries between `starttimestamptz` and `endtimestamptz`, inclusive of the larger timestamp and exclusive of the smaller timestamp. |
...
| **Description** | The signed length of the interval between `starttimestamptz` and `endtimestamptz`, truncated to whole multiples of [`part`](https://duckdb.org/docs/current/sql/functions/datepart.html). |
...
| **Description** | Get [subfield](https://duckdb.org/docs/current/sql/functions/datepart.html) from a `TIMESTAMP WITH TIME ZONE`. |
...
| **Description** | Converts a `TIMESTAMP WITH TIME ZONE` value to string according to the [format string](https://duckdb.org/docs/current/sql/functions/dateformat.html#format-specifiers). |
| **Example** | `strftime(timestamptz '1992-01-01 20:38:40', '%a, %-d %B %Y - %I:%M:%S %p')` |
| **Result** | `Wed, 1 January 1992 - 08:38:40 PM` |

#### `strptime(text, format)`

| **Description** | Converts string to `TIMESTAMP WITH TIME ZONE` according to the [format string](https://duckdb.org/docs/current/sql/functions/dateformat.html#format-specifiers) if `%Z` is specified. |
...
| [`timezone(text, timestamp)`](#timezonetext-timestamp) | Use the [date parts](https://duckdb.org/docs/current/sql/functions/datepart.html) of the timestamp in GMT to construct a timestamp in the given time zone. Effectively, the argument is a “local” time. |
| [`timezone(text, timestamptz)`](#timezonetext-timestamptz) | Use the [date parts](https://duckdb.org/docs/current/sql/functions/datepart.html) of the timestamp in the given time zone to construct a timestamp. Effectively, the result is a “local” time. |
...
| **Description** | Use the [date parts](https://duckdb.org/docs/current/sql/functions/datepart.html) of the timestamp in GMT to construct a timestamp in the given time zone. Effectively, the argument is a “local” time. |
| **Example** | `timezone('America/Denver', TIMESTAMP '2001-02-16 20:38:40')` |
| **Result** | `2001-02-16 19:38:40-08` |

#### `timezone(text, timestamptz)`

| **Description** | Use the [date parts](https://duckdb.org/docs/current/sql/functions/datepart.html) of the timestamp in the given time zone to construct a timestamp. Effectively, the result is a “local” time. |
...
The ICU extension also supports [non-Gregorian calendars](https://duckdb.org/docs/current/sql/data_types/timestamp.html#calendar-support).
