...
The `strftime` and `strptime` functions can be used to convert between [`DATE`](https://duckdb.org/docs/current/sql/data_types/date.html) / [`TIMESTAMP`](https://duckdb.org/docs/current/sql/data_types/timestamp.html) values and strings. This is often required when parsing CSV files, displaying output to the user or transferring information between programs. Because there are many possible date representations, these functions accept a [format string](#format-specifiers) that describes how the date or timestamp should be structured.

## `strftime` Examples

The [`strftime(timestamp, format)`](https://duckdb.org/docs/current/sql/functions/timestamp.html#strftimetimestamp-format) converts timestamps or dates to strings according to the specified pattern.
...
The [`strptime(text, format)` function](https://duckdb.org/docs/current/sql/functions/timestamp.html#strptimetext-format) converts strings to timestamps according to the specified pattern.
...
To return `NULL` on failure, use the [`try_strptime` function](https://duckdb.org/docs/current/sql/functions/timestamp.html#try_strptimetext-format):
