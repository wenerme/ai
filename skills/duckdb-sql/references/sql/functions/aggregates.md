...
Return two values, the total sum of `amount` and the sum of `amount` minus columns where the region is `north` using the [`FILTER` clause](https://duckdb.org/docs/current/sql/query_syntax/filter.html):
...
To exclude `NULL`s from `list`, you can use a [`FILTER` clause](https://duckdb.org/docs/current/sql/query_syntax/filter.html).
...
| [`histogram(arg, boundaries)`](#histogramarg-boundaries) | Returns a `MAP` of key-value pairs representing the provided upper `boundaries` and counts of elements in the corresponding bins (left-open and right-closed partitions) of the datatype. A boundary at the largest value of the datatype is automatically added when elements larger than all provided `boundaries` appear, see [`is_histogram_other_bin`](https://duckdb.org/docs/current/sql/functions/utility.html#is_histogram_other_binarg). Boundaries may be provided, e.g., via [`equi_width_bins`](https://duckdb.org/docs/current/sql/functions/utility.html#equi_width_binsminmaxbincountnice). |
| [`histogram_exact(arg, elements)`](#histogram_exactarg-elements) | Returns a `MAP` of key-value pairs representing the requested elements and their counts. A catch-all element specific to the data-type is automatically added to count other elements when they appear, see [`is_histogram_other_bin`](https://duckdb.org/docs/current/sql/functions/utility.html#is_histogram_other_binarg). |
