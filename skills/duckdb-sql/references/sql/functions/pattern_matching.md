...
Additionally, the function `like_escape` has the same functionality as a `LIKE` expression with an `ESCAPE` clause, but using function syntax. See the [Text Functions page](https://duckdb.org/docs/current/sql/functions/text.html) for details.
...
The `SIMILAR TO` operator returns true or false depending on whether its pattern matches the given string. It is similar to `LIKE`, except that it interprets the pattern using a [regular expression](https://duckdb.org/docs/current/sql/functions/regular_expressions.html). Like `LIKE`, the `SIMILAR TO` operator succeeds only if its pattern matches the entire string; this is unlike common regular expression behavior where the pattern can match any part of the string.
...
> see the [PostgreSQL compatibility page](https://duckdb.org/docs/current/sql/dialect/postgresql_compatibility.html).
...
DuckDB's regular expression support is documented on the [Regular Expressions page](https://duckdb.org/docs/current/sql/functions/regular_expressions.html).
