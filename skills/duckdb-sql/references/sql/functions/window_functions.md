...
Window functions are [blocking operators](https://duckdb.org/docs/current/guides/performance/how_to_tune_workloads.html#blocking-operators), i.e., they require their entire input to be buffered, making them one of the most memory-intensive operators in SQL.
...
> Tip If you only need a number for each row in a table, you can use the [`rowid` pseudocolumn](https://duckdb.org/docs/current/sql/statements/select.html#row-ids).
...
Window functions can only be used in the `SELECT` clause. To share `OVER` specifications between functions, use the statement's [`WINDOW` clause](https://duckdb.org/docs/current/sql/query_syntax/window.html) and use the `OVER ⟨window_name⟩` syntax.
...
All [aggregate functions](https://duckdb.org/docs/current/sql/functions/aggregates.html) can be used in a windowing context, including the optional [`FILTER` clause](https://duckdb.org/docs/current/sql/query_syntax/filter.html).
...
Ordering is also optional, but without it the results of [general-purpose window functions](#general-purpose-window-functions) and [order-sensitive aggregate functions](https://duckdb.org/docs/current/sql/functions/aggregates.html#order-by-clause-in-aggregate-functions), and the order of [framing](#framing) are not well-defined.
...
the canonical order of the [`SELECT statement`](https://duckdb.org/docs/current/sql/statements/select.html).

### Filtering the Results of Window Functions Using `QUALIFY`

Window functions are executed after the [`WHERE`](https://duckdb.org/docs/current/sql/query_syntax/where.html) and [`HAVING`](https://duckdb.org/docs/current/sql/query_syntax/having.html) clauses have been already evaluated, so it's not possible to use these clauses to filter the results of window functions
The [`QUALIFY` clause](https://duckdb.org/docs/current/sql/query_syntax/qualify.html) avoids the need for a subquery or [`WITH` clause](https://duckdb.org/docs/current/sql/query_syntax/with.html) to perform this filtering.
