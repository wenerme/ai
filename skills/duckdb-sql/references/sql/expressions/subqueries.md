...
Subqueries are parenthesized query expressions that appear as part of a larger, outer query. Subqueries are usually based on `SELECT ... FROM`, but in DuckDB other query constructs such as [`PIVOT`](https://duckdb.org/docs/current/sql/statements/pivot.html) can also appear as a subquery.
...
In the section on [scalar subqueries](#scalar-subquery), a scalar expression was compared directly to a subquery using the equality [comparison operator](https://duckdb.org/docs/current/sql/expressions/comparison_operators.html#comparison-operators) (`=`).
...
> The subqueries in the examples above make use of the fact that you can omit the `SELECT *` in DuckDB thanks to the [`FROM`-first syntax](https://duckdb.org/docs/current/sql/query_syntax/from.html). The `SELECT` clause is required in subqueries by other SQL systems but cannot fulfill any purpose in `EXISTS` and `NOT EXISTS` subqueries.
