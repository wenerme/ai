...
See the [`STRUCT` data type](https://duckdb.org/docs/current/sql/data_types/struct.html) and [`STRUCT` functions](https://duckdb.org/docs/current/sql/functions/struct.html) pages for more details on working with structs.
...
The [pattern matching operators](https://duckdb.org/docs/current/sql/functions/pattern_matching.html) `LIKE`, `GLOB`, `SIMILAR TO` and their variants allow you to select columns by matching their names to patterns.
...
`COLUMNS` expressions can be used in [`DISTINCT ON`](https://duckdb.org/docs/current/sql/query_syntax/select.html#distinct-on-clause) clauses to specify distinct columns by pattern:
...
`COLUMNS` also supports passing in a [lambda function](https://duckdb.org/docs/current/sql/functions/lambda.html).
