...
Casting can occur either implicitly or explicitly. The syntax described here performs an explicit cast. More information on casting can be found on the [typecasting page](https://duckdb.org/docs/current/sql/data_types/typecasting.html).

## Explicit Casting

The standard SQL syntax for explicit casting is `CAST(expr AS TYPENAME)`, where `TYPENAME` is a name (or alias) of one of [DuckDB's data types](https://duckdb.org/docs/current/sql/data_types/overview.html). DuckDB also supports the shorthand `expr::TYPENAME`, which is also present in PostgreSQL.
...
Casting between numeric types might have rounding implications. The applicable rounding rules can be found on the [`NUMERIC type page`](https://duckdb.org/docs/current/sql/data_types/numeric.html#rounding).
...
This function is primarily useful in [macros](https://duckdb.org/docs/current/guides/snippets/sharing_macros.html), as it allows you to maintain types.
