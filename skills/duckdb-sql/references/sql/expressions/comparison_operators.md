...
When performing comparison on different types, DuckDB performs [Combination Casting](https://duckdb.org/docs/current/sql/data_types/typecasting.html#combination-casting).
...
> It is not possible to enforce stricter type-checking for DuckDB's comparison operators. If you require stricter type-checking, consider creating a [macro](https://duckdb.org/docs/current/sql/statements/create_macro.html) with the [`typeof` function](https://duckdb.org/docs/current/sql/functions/utility.html#typeofexpression) or implementing a [user-defined function](https://duckdb.org/docs/current/clients/python/function.html).
...
An explicit [collation](https://duckdb.org/docs/current/sql/expressions/collations.html) on any input to `BETWEEN` applies to the entire `BETWEEN` expression. This can differ from writing the comparisons separately, where the explicit collation applies only to the comparison that contains it:
