...
> The `TRY` expression was inspired by the [`TRY_CAST` expression](https://duckdb.org/docs/current/sql/expressions/cast.html#try_cast).
...
`TRY` cannot be used in combination with a volatile function, an aggregate function, or a [scalar subquery](https://duckdb.org/docs/current/sql/expressions/subqueries.html#scalar-subquery).
For example:
