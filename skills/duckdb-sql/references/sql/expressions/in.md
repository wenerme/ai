...
The `IN` operator works on [maps](https://duckdb.org/docs/current/sql/data_types/map.html) according to the semantics used in Python, i.e., it checks for the presence of keys (not values):
...
The `IN` operator works with [subqueries](https://duckdb.org/docs/current/sql/expressions/subqueries.html) that return a single column.
For example:
...
The `IN` operator can be used as a shorthand for the [`contains` string function](https://duckdb.org/docs/current/sql/functions/text.html#containsstring-search_string).
For example:
