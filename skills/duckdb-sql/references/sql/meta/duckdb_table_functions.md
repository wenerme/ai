...
| `character_maximum_length` | Always `NULL`. DuckDB [text types](https://duckdb.org/docs/current/sql/data_types/text.html) do not enforce a value length restriction based on a length type parameter. | `INTEGER` |
...
The [`information_schema.columns`](https://duckdb.org/docs/current/sql/meta/information_schema.html#columns-columns) system view provides a more standardized way to obtain metadata about database columns, but the `duckdb_columns` function also returns metadata about DuckDB internal objects. (In fact, `information_schema.columns` is implemented as a query on top of `duckdb_columns()`)
...
The [`information_schema.referential_constraints`](https://duckdb.org/docs/current/sql/meta/information_schema.html#referential_constraints-referential-constraints) and [`information_schema.table_constraints`](https://duckdb.org/docs/current/sql/meta/information_schema.html#table_constraints-table-constraints) system views provide a more standardized way to obtain metadata about constraints, but the `duckdb_constraints` function also returns metadata about DuckDB internal objects. (In fact, `information_schema.referential_constraints` and `information_schema.table_constraints` are implemented as a query on top of `duckdb_constraints()`)

## `duckdb_coordinate_systems`

The `duckdb_coordinate_systems()` function lists the available [coordinate reference systems](https://duckdb.org/docs/current/sql/data_types/geometry.html#how-are-coordinate-reference-systems-stored-in-duckdb) (CRS) for use with the [`GEOMETRY`](https://duckdb.org/docs/current/sql/data_types/geometry.html) data type.
...
This function lists both built-in CRSs as well as the ones that are installed by extensions (such as the [`SPATIAL`](https://duckdb.org/docs/current/core_extensions/spatial/overview.html) extension.

## `duckdb_databases`

The `duckdb_databases()` function lists the databases that are accessible from within the current DuckDB process.
Apart from the database associated at startup, the list also includes databases that were [attached](https://duckdb.org/docs/current/sql/statements/attach.html) later on to the DuckDB process.
...
| `macro_definition` | If this is a [macro](https://duckdb.org/docs/current/sql/statements/create_macro.html), the SQL expression that defines it. | `VARCHAR` |
...
| `internal` | `true` if the function is built-in (defined by DuckDB or an extension), `false` if it was defined using the [`CREATE MACRO` statement](https://duckdb.org/docs/current/sql/statements/create_macro.html). | `BOOLEAN` |
...
Note that `duckdb_indexes` only provides metadata about secondary indexes, i.e., those indexes created by explicit [`CREATE INDEX`](https://duckdb.org/docs/current/sql/indexes.html#create-index) statements. Primary keys, foreign keys, and `UNIQUE` constraints are maintained using indexes, but their details are included in the `duckdb_constraints()` function.
...
| `keyword_category` | The category of the keyword: `reserved` (cannot be used as an identifier without quoting), `unreserved` (can be used as an identifier), `column_name` (usable as a column name but not as a type or function name), or `type_function` (usable as a type or function name but not as a column name). See [Keywords and Identifiers](https://duckdb.org/docs/current/sql/dialect/keywords_and_identifiers.html). | `VARCHAR` |
...
These can be selectively turned off using [`PRAGMA disabled_optimizers`](https://duckdb.org/docs/current/configuration/pragmas.html#selectively-disabling-optimizers).
...
The `duckdb_prepared_statements()` function provides metadata about the [prepared statements](https://duckdb.org/docs/current/sql/query_syntax/prepared_statements.html) that exist in the current DuckDB session.
...
The [`information_schema.schemata`](https://duckdb.org/docs/current/sql/meta/information_schema.html#schemata-database-catalog-and-schema) system view provides a more standardized way to obtain metadata about database schemas.
...
Attributes like `temporary`, `start_value` etc. correspond to the various options available in the [`CREATE SEQUENCE`](https://duckdb.org/docs/current/sql/statements/create_sequence.html) statement and are documented there in full. Note that the attributes will always be filled out in the `duckdb_sequences` resultset, even if they were not explicitly specified in the `CREATE SEQUENCE` statement.
...
The various settings are described in the [configuration page](https://duckdb.org/docs/current/configuration/overview.html).
...
| `sql` | The definition of this object, expressed as SQL [`CREATE TABLE`-statement](https://duckdb.org/docs/current/sql/statements/create_table.html). | `VARCHAR` |

The [`information_schema.tables`](https://duckdb.org/docs/current/sql/meta/information_schema.html#tables-tables-and-views) system view provides a more standardized way to obtain metadata about database tables that also includes views. But the resultset returned by `duckdb_tables` contains a few columns that are not included in `information_schema.tables`.
...
The [`information_schema.tables`](https://duckdb.org/docs/current/sql/meta/information_schema.html#tables-tables-and-views) system view provides a more standardized way to obtain metadata about database views that also includes base tables. But the resultset returned by `duckdb_views` contains also definitions of internal view objects as well as a few columns that are not included in `information_schema.tables`.
