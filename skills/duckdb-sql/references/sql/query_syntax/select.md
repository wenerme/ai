...
> Column names are case-insensitive. See the [Rules for Case Sensitivity](https://duckdb.org/docs/current/sql/dialect/keywords_and_identifiers.html#rules-for-case-sensitivity) for more details.
...
The [star expression](https://duckdb.org/docs/current/sql/expressions/star.html) is a special expression that expands to *multiple expressions* based on the contents of the `FROM` clause. In the simplest case, `*` expands to **all** expressions in the `FROM` clause. Columns can also be selected using regular expressions or lambda functions. See the [star expression page](https://duckdb.org/docs/current/sql/expressions/star.html) for more details.
...
The [`unnest`](https://duckdb.org/docs/current/sql/query_syntax/unnest.html) function is a special function that can be used together with [arrays](https://duckdb.org/docs/current/sql/data_types/array.html), [lists](https://duckdb.org/docs/current/sql/data_types/list.html), or [structs](https://duckdb.org/docs/current/sql/data_types/struct.html). The unnest function strips one level of nesting from the type. For example, `INTEGER[]` is transformed into `INTEGER`. `STRUCT(a INTEGER, b INTEGER)` is transformed into `a INTEGER, b INTEGER`. The unnest function can be used to transform nested types into regular scalar types, which makes them easier to operate on.
