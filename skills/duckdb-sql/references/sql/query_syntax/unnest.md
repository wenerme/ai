...
Unnesting is an operation that decomposes values of a [composite types](https://duckdb.org/docs/current/sql/data_types/overview.html#nested--composite-types) into its components.
Values of the [`LIST`](https://duckdb.org/docs/current/sql/data_types/list.html) and [`STRUCT`](https://duckdb.org/docs/current/sql/data_types/struct.html) type may be unnested using the [`unnest()` function](https://duckdb.org/docs/current/sql/functions/list.html#unnestlist-recursive-max_depth).
...
* Unnesting a `STRUCT`-typed value creates a column for each member. The member key becomes the column name, and the member value becomes a column value. Values of a `STRUCT`-type may also be unnested using [dot-star (`⟨struct⟩.*`) shorthand syntax](https://duckdb.org/docs/current/sql/data_types/struct.html#unnest--struct), but the `unnest()` function offers some additional functionality.
...
The `unnest` function can be called in the [`SELECT`-clause](https://duckdb.org/docs/current/sql/query_syntax/select.html) as if it is a scalar function.
...
Unnesting a value of the `LIST`-type yields only the element values. To also keep track of their indices (the subscripts), you can use the built-in macro [`generate_subscripts()`](https://duckdb.org/docs/current/sql/functions/list.html#generate_subscriptslist-dimension).
The `generate_subscripts` macro takes a value of the `LIST`-type as first argument.

### `unnest()` as Table Function

Since a call to `unnest` on a value of the `LIST`-type yields a rowset, it may also be treated as a [table function](https://duckdb.org/docs/current/sql/query_syntax/from.html#table-functions).
This means it may appear in the [`FROM`-clause](https://duckdb.org/docs/current/sql/query_syntax/from.html) or a [`CALL`-statement](https://duckdb.org/docs/current/sql/statements/call.html).
...
To keep track of each entry's position within the original list, `unnest` may be combined with [`generate_subscripts`](https://duckdb.org/docs/current/sql/functions/list.html#generate_subscripts):
