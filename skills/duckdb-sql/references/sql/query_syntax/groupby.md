...
The values of the grouping columns themselves are unchanged, and any other columns can be combined using an [aggregate function](https://duckdb.org/docs/current/sql/functions/aggregates.html) (such as `count`, `sum`, `avg`, etc).
...
See examples below and additional examples in the [“Friendlier SQL with DuckDB” blog post](https://duckdb.org/2022/05/04/friendlier-sql.html#group-by-all).

## Multiple Dimensions

Normally, the `GROUP BY` clause groups along a single dimension.
Using the [`GROUPING SETS`, `CUBE` or `ROLLUP` clauses](https://duckdb.org/docs/current/sql/query_syntax/grouping_sets.html) it is possible to group along multiple dimensions.
See the [`GROUPING SETS`](https://duckdb.org/docs/current/sql/query_syntax/grouping_sets.html) page for more information.
