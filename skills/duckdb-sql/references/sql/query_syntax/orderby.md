...
`ORDER BY` is an output modifier. Logically it is applied near the very end of the query (just prior to [`LIMIT`](https://duckdb.org/docs/current/sql/query_syntax/limit.html) or [`OFFSET`](https://duckdb.org/docs/current/sql/query_syntax/limit.html), if present).
...
For more information on collations, see the [Collation page](https://duckdb.org/docs/current/sql/expressions/collations.html).
