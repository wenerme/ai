...
One way to achieve this is to store the path of a traversal in a [list](https://duckdb.org/docs/current/sql/data_types/list.html) and, before extending the path with a new edge, check whether its endpoint has been visited before (see the example later).
...
`WITH RECURSIVE` can also be used to find **all (unweighted) shortest paths between two nodes**. To ensure that the recursive query is stopped as soon as we reach the end node, we use a [window function](https://duckdb.org/docs/current/sql/functions/window_functions.html) which checks whether the end node is among the newly added nodes.
