...
> The few exceptions to this are listed on the [PostgreSQL compatibility page](https://duckdb.org/docs/current/sql/dialect/postgresql_compatibility.html).

In the examples that follow, we assume that you have installed the DuckDB Command Line Interface (CLI) shell. See the [installation page](https://duckdb.org/install/index.html?environment=cli) for information on how to install the CLI.

> Tip If you are looking for a comprehensive to SQL introduction,
> check the slide decks of the [“Tabular Database Systems” course](https://duckdb.org/_library/2026-03-19-tabular-database-systems.html):
>
...
Where the file name for the source file must be available on the machine running the process. There are many other ways of loading data into DuckDB, see the [corresponding documentation section](https://duckdb.org/docs/current/data/overview.html) for more information.
...
More information about the `LIKE` operator can be found in the [pattern matching page](https://duckdb.org/docs/current/sql/functions/pattern_matching.html).
