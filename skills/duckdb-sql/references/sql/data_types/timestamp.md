...
Timestamps represent points in time. As such, they combine [`DATE`](https://duckdb.org/docs/current/sql/data_types/date.html) and [`TIME`](https://duckdb.org/docs/current/sql/data_types/time.html) information.
...
Despite the name, a `TIMESTAMP WITH TIME ZONE` does not store time zone information. Instead, it only stores the `INT64` number of non-leap microseconds since the Unix epoch `1970-01-01 00:00:00+00`, and thus unambiguously identifies a point in absolute time, or [*instant*](https://duckdb.org/docs/current/sql/data_types/timestamp.html#instants). The reason for the labels *time zone aware* and `WITH TIME ZONE` is that timestamp arithmetic, [*binning*](https://duckdb.org/docs/current/sql/data_types/timestamp.html#temporal-binning), and string formatting for this type are performed in a [configured time zone](https://duckdb.org/docs/current/sql/data_types/timestamp.html#time-zone-support), which defaults to the system time zone and is just `UTC+00:00` in the examples above.
...
See [Timestamp Functions](https://duckdb.org/docs/current/sql/functions/timestamp.html).
...
The built-in [ICU extension](https://duckdb.org/docs/current/core_extensions/icu.html) implements all the binning and arithmetic functions using the
...
You can also find a reference table of [available time zones](https://duckdb.org/docs/current/sql/data_types/timezones.html).

## Calendar Support

The [ICU extension](https://duckdb.org/docs/current/core_extensions/icu.html) also supports non-Gregorian calendars using the `SET Calendar` command.
