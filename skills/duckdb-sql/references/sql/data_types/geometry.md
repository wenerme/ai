...
The `GEOMETRY` type was part of the [`spatial` extension](https://duckdb.org/docs/current/core_extensions/spatial/overview.html) but became a built-in data type in DuckDB v1.5. Most of the benefits of having `GEOMETRY` as a built-in type (e.g., storage optimizations, statistics, etc.) are therefore only available in databases using [storage version v1.5](https://duckdb.org/docs/current/internals/storage.html) and above. However, almost all of the associated functions for working with geometries (e.g., calculating distances, areas, intersections) are still part of `spatial`.
...
You can list all available CRSs known to DuckDB using the [`duckdb_coordinate_systems()`](https://duckdb.org/docs/current/sql/meta/duckdb_table_functions.html#duckdb_coordinate_systems) function:
...
- See [geometry functions](https://duckdb.org/docs/current/sql/functions/geometry.html) for the list of built-in geometry functions.
- See the documentation of the [`spatial` extension](https://duckdb.org/docs/current/core_extensions/spatial/overview.html) for the large set of additional geometry functions provided by the extension, including functions for calculating areas, distances, intersections, unions, and much more.
