...
An `ARRAY` column stores fixed-sized arrays. All fields in the column must have the same length and the same underlying type. Arrays are typically used to store arrays of numbers, but can contain any uniform data type, including `ARRAY`, [`LIST`](https://duckdb.org/docs/current/sql/data_types/list.html) and [`STRUCT`](https://duckdb.org/docs/current/sql/data_types/struct.html) types.
...
To store variable-length lists, use the [`LIST` type](https://duckdb.org/docs/current/sql/data_types/list.html). See the [data types overview](https://duckdb.org/docs/current/sql/data_types/overview.html) for a comparison between nested data types.
...
Retrieving one or more values from an array can be accomplished using brackets and slicing notation, or through [list functions](https://duckdb.org/docs/current/sql/functions/list.html#list-functions) like `list_extract` and `array_extract`. Using the example in [Defining an Array Field](#defining-an-array-field).
...
All [`LIST` functions](https://duckdb.org/docs/current/sql/functions/list.html) work with the `ARRAY` type. Additionally, several `ARRAY`-native functions are also supported.
See the [`ARRAY` functions](https://duckdb.org/docs/current/sql/functions/array.html#array-native-functions).
...
For more functions, see [List Functions](https://duckdb.org/docs/current/sql/functions/list.html).
