...
Structs can be created using the [`struct_pack(name := expr, ...)`](https://duckdb.org/docs/current/sql/functions/struct.html) function, the equivalent array notation `{'name': expr, ...}`, using a row variable, or using the `row` function.
...
Retrieving a value from a struct can be accomplished using dot notation, bracket notation, or through [struct functions](https://duckdb.org/docs/current/sql/functions/struct.html) like `struct_extract`.
...
The same can be achieved with the star notation (`*`), which additionally allows [modifications of the returned columns](https://duckdb.org/docs/current/sql/expressions/star.html):
...
The `STRUCT` type can be compared using all the [comparison operators](https://duckdb.org/docs/current/sql/expressions/comparison_operators.html).
...
Structs of different types are implicitly cast to a struct type with the union of the involved keys, following the rules for [combination casting](https://duckdb.org/docs/current/sql/data_types/typecasting.html#structs).
...
using the [`ALTER TABLE` clause](https://duckdb.org/docs/current/sql/statements/alter_table.html).
...
See [Struct Functions](https://duckdb.org/docs/current/sql/functions/struct.html).
