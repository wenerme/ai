...
Explicit typecasting is performed by using a `CAST` expression. For example, `CAST(col AS VARCHAR)` or `col::VARCHAR` explicitly cast the column `col` to `VARCHAR`. See the [cast page](https://duckdb.org/docs/current/sql/expressions/cast.html) for more information.
...
> Tip To check whether a type can be implicitly cast to another type, use the [`can_cast_implicitly` function](https://duckdb.org/docs/current/sql/functions/utility.html#can_cast_implicitlysource_value-target_value).
...
> Version 0.10.0 introduced a [breaking change which no longer allows implicit casts to `VARCHAR`](https://duckdb.org/2024/02/13/announcing-duckdb-0100.html#breaking-sql-changes).
> The [`old_implicit_casting` configuration option](https://duckdb.org/docs/current/configuration/pragmas.html#implicit-casting-to-varchar) setting can be used to revert to the old behavior.
...
The applicable rounding rules can be found on the [`NUMERIC type page`](https://duckdb.org/docs/current/sql/data_types/numeric.html#rounding).
...
So even though the cast operation from `INTEGER` to `TINYINT` is supported, it is not possible for this particular value. [TRY_CAST](https://duckdb.org/docs/current/sql/expressions/cast.html) can be used to convert the value into `NULL` instead of throwing an error.

### Varchar

The [`VARCHAR`](https://duckdb.org/docs/current/sql/data_types/text.html) type acts as a universal target: any arbitrary value of any arbitrary type can always be cast to the `VARCHAR` type. This type is also used for displaying values in the shell.
...
Integer literals (such as `42`) and string literals (such as `'string'`) have special implicit casting rules. See the [literal types page](https://duckdb.org/docs/current/sql/data_types/literal_types.html) for more information.
...
For [combination casting](https://duckdb.org/docs/current/sql/data_types/typecasting.html#combination-casting), the fields of the resulting struct are the superset of all fields of the input structs.
...
Union casting rules can be found on the [`UNION type page`](https://duckdb.org/docs/current/sql/data_types/union.html#casting-to-unions).
