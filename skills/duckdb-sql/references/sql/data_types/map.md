...
In contrast, `STRUCT`s must have string keys, but each value may have a different type. See the [data types overview](https://duckdb.org/docs/current/sql/data_types/overview.html) for a comparison between nested data types.
...
Nested types can be compared using all the [comparison operators](https://duckdb.org/docs/current/sql/expressions/comparison_operators.html).
...
for both `WHERE` and `HAVING` clauses, as well as for creating [Boolean values](https://duckdb.org/docs/current/sql/data_types/boolean.html).
...
See [Map Functions](https://duckdb.org/docs/current/sql/functions/map.html).
