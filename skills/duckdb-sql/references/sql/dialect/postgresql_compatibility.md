...
Therefore, DuckDB achieves case insensitivity by making identifiers fully case insensitive throughout the system but [_preserving their case_](https://duckdb.org/docs/current/sql/dialect/keywords_and_identifiers.html#rules-for-case-sensitivity).
...
PostgreSQL's behavior of lowercasing identifiers is accessible using the [`preserve_identifier_case` option](https://duckdb.org/docs/current/configuration/overview.html#local-configuration-options):
...
In DuckDB, the [`VACUUM` statement](https://duckdb.org/docs/current/sql/statements/vacuum.html) is only used to rebuild statistics.
For instruction on reclaiming space, refer to the [“Reclaiming space” page](https://duckdb.org/docs/current/operations_manual/footprint_of_duckdb/reclaiming_space.html).
...
Instead, please use the [`strptime` function](https://duckdb.org/docs/current/sql/functions/dateformat.html#strptime-examples).

### `date_part` Function

Most parts extracted by the [`date_part` function](https://duckdb.org/docs/current/sql/functions/datepart.html) are returned as integers. Since there are no infinite integer values in DuckDB, `NULL`s are returned for infinite timestamps.

## Resolution of Type Names in the Schema

For [`CREATE TABLE` statements](https://duckdb.org/docs/current/sql/statements/create_table.html), DuckDB attempts to resolve type names in the schema where a table is created. For example:
...
To work around this, add the other attributes or use the [`GROUP BY ALL` clause](https://duckdb.org/docs/current/sql/query_syntax/groupby.html#group-by-all).
