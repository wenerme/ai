...
> **tip**: This feature is supported since Apache Doris version 4.1.0 and is currently an experimental feature.
...
> **note**: - `file_path` is a file name prefix. The actual generated file name format is `{prefix}{query_id}_{idx}.{ext}`, where `idx` starts from 0 and increments.
...
> **tip**: - Properties in the Resource serve as default values; properties specified in the TVF will override properties with the same name in the Resource
...
> **note**: - If the column type does not match (e.g., the file contains a string but `int` is specified), the column returns `null`
