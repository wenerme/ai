...
> **note**: - This is an experimental feature, supported since version 5.0.0.
...
> **caution**: At `CREATE CATALOG` the FE resolves `driver_url` into an absolute path and sends that path to the BEs unchanged. Each BE loads the driver from exactly that path. Therefore:
...
> **note**: - Doris has no unsigned integer types, so an unsigned type widens by one step. `uint32` maps to `BIGINT` rather than `INT`, because the narrower type would silently wrap every value above 2^31 into a negative number.
...
> **tip**: On an Arrow Flight SQL data source, the call that asks for partitions is itself the query's execution. So when you run `EXPLAIN` on an ADBC table, Doris does not ask the data source for partitions, and the data source does not actually execute the query.
...
> **caution**: An ADBC connection reaches the data source as the single identity configured on the catalog, whoever runs the query. Cache entries are therefore shared across all users.
