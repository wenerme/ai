...
> **note**: Lance Catalog is supported starting from Apache Doris 4.2.
...
> **caution**: The current BE Reader does not support Lance tables whose versions are managed by REST Namespace (Managed Versioning).
...
> **note**: Some Lance Java SDK versions may lose the Dictionary marker while reading a Schema and expose a Dictionary column as its physical index type. This behavior does not mean that Doris supports the logical Dictionary values and must not be relied upon.
...
> **note**: Multi-Scanner vector search is planned as a future optimization. It will require index-aware search partitioning and an internal global candidate merge so that `top_k`, `offset`, Prefilter, and `_distance` semantics remain unchanged.
