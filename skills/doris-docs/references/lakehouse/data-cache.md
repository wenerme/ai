...
> **note**: - This control applies only to Data Cache fills after SELECT query misses in compute-storage decoupled mode. It does not limit remote bytes read or affect explicit cache warmup. Remote-only-on-miss mode may increase remote I/O and query latency.
