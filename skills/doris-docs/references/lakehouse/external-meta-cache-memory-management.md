...
> **tip**: `meta.cache.max-weight` works even when no FE-wide limit is configured. In that case, it limits only this Catalog and does not limit the total across all Catalogs on the FE.
...
> **note**: When `iceberg.rest.session` is `user`, the `table`, `partition`, `partition_view`, latest-snapshot, and format caches are not created, because a shared cache would bypass per-user authorization. A REST Catalog with vended credentials does not create the `table` cache either.
