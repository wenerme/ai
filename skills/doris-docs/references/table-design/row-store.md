...
> **note**: `"store_row_column" = "true"` is required for Duplicate tables. The `row_store_columns` property is not supported with Duplicate tables — all columns are stored in the row store.
