...
| [DECIMAL](../sql-manual/basic-element/sql-data-types/numeric/DECIMAL)       | 4/8/16/32          | An exact fixed-point number defined by precision (total number of digits) and scale (number of digits to the right of the decimal point). Format: DECIMAL(P[,S]), where P is precision and S is scale. The range for P is [1, MAX_P], where MAX_P=38 when `enable_decimal256`=false, and MAX_P=76 when `enable_decimal256`=true, and for S is [0, P]. The default value of `enable_decimal256` is false. Setting it to true can get more accurate results, but it will bring some performance loss. Storage requirements: <ul> <li>4 bytes for 0 < precision <= 9.</li><li>8 bytes for 9 < precision <= 18.<li>16 bytes for 18 < precision <= 38.<li>32 bytes for 38 < precision <= 76.</ul> |
