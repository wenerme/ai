...
> **tip**: In AUTO RANGE PARTITION, the `AUTO` keyword can be omitted; the meaning of auto partitioning is preserved.
...
> **info**: Doris supports using auto partitioning together with dynamic partitioning to implement lifecycle management, but this combination is no longer recommended.
