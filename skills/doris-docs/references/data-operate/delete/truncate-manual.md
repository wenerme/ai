...
> **caution**: After using `FORCE`, data cannot be recovered via `RECOVER`. Confirm before executing.
...
> **tip**: The recycle bin retention duration is controlled by the FE configuration item `catalog_trash_expire_second`. Expired data is permanently cleaned up.
