...
Before using Doris continuous load to synchronize data from Amazon RDS PostgreSQL, you need to ensure that the RDS instance has logical replication enabled. This guide walks you through all prerequisite configuration steps.
...
> **caution**: Modifying the `rds.logical_replication` parameter requires restarting the RDS instance to take effect. Please perform this during off-peak hours.
...
> **tip**: Amazon RDS PostgreSQL uses the `rds_replication` role to grant replication permissions, instead of the standard PostgreSQL `REPLICATION` attribute.
...
> **caution**: Currently Doris only supports the Publication named `dbz_publication` with `FOR ALL TABLES`. Custom Publication names or specifying individual tables are not supported.
