...
> **caution**: Modifying the `binlog_format` parameter requires restarting the Aurora writer instance to take effect. Perform this operation during off-peak hours.
...
> **caution**: If this configuration item is not set, or is set to too short an interval, gaps may appear in the binary logs, which can affect Doris's ability to resume replication.
