...
> **tip**: 1. The columns in the table allow null values to be imported.
...
> **tip**: The columns in the table allow null values to be imported.
...
> **tip**: For Insert Into statements, `insert_max_filter_ratio` only takes effect when `enable_insert_strict = false`. Its default value is `1.0`, which means all abnormal data is allowed to be filtered.
