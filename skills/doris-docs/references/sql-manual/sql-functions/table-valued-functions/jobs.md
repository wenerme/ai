...
-  **`jobs("type"="insert")`** Job return value of type insert
    ...
    | LagBytes           | Backlog in bytes of the source-side log (such as MySQL binlog or PostgreSQL WAL). `-1` means it is currently unavailable, for example for an S3 data source or during the full snapshot phase.**Since 4.1.4**, this column is renamed from `Lag` (in seconds) to `LagBytes` (in bytes) |
