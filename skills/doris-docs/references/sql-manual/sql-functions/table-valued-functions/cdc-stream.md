...
> **note**: When used alone, CDC Stream TVF supports only incremental data synchronization and does not support reading a full snapshot. When used with [CREATE STREAMING JOB](../../sql-statements/job/CREATE-STREAMING-JOB.md), it supports full and incremental synchronization.
