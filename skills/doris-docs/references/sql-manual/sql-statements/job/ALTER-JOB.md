...
> **note**: Auto Table Creation Sync does not allow changes to `jdbc_url`, `database`, `schema`, `include_tables`, `exclude_tables`, the source-side `offset`, `snapshot_split_size`, `snapshot_parallelism`, per-table mapping parameters, `slot_name`, or `publication_name`. To reset a CDC position, use the Job Property `offset`.
