...
|---|---|---|
| `type` | `append_only` / `min_delta` / `detail`default `min_delta` | Consumption type. `append_only` emits only inserted rows; `min_delta` emits the net change of each key between two consumptions; `detail` emits every change row by row. `min_delta` and `detail` require the base table to be a Unique Key Merge-on-Write table with `binlog.need_historical_value` enabled; on Duplicate Key tables `min_delta` is handled as `append_only` |
| `show_initial_rows` | `true` / `false`default `false` | Whether the data that already exists when the Stream is created is emitted as changes. When `true`, the first read returns the full current data of the base table (change type `APPEND`) and the Stream switches to incremental after that consumption |
