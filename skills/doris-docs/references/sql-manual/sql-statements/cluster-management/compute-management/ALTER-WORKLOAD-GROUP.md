...
> **note**: The `ALTER` statement only modifies the workload group's properties; it **cannot change the binding between a workload group and a compute group**. The `FOR <compute_group>` clause is used to locate which compute group's workload group is being modified.
