...
> **note**: 1. In the Doris Version column, `-` means the feature is supported by Doris 2.0 and later, and by all CCR versions. Doris 2.0.15, 2.1.6, or later is recommended.
...
> **note**: add/drop value column requires the property `"light_schema_change" = "true"` to be set at table creation.
...
> **note**: Due to limitations in the Doris implementation, the column name and view name within a view cannot be the same as the database name.
