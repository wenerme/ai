...
| --- | --- |
| VERY\_HIGH | REDUNDANT: shards with redundant replicas. Although the urgency is the lowest, this is processed the fastest and can quickly free up disk space and other resources, so it is processed first.FORCE\_REDUNDANT: same as above. |
| HIGH | REPLICA\_MISSING with most replicas missing (such as 2 out of 3 replicas lost).VERSION\_INCOMPLETE with most replicas having missing versions.COLOCATE\_MISMATCH: shards related to Colocation tables should be repaired as soon as possible.COLOCATE\_REDUNDANT. |
| NORMAL | REPLICA\_MISSING but most replicas alive (such as 1 out of 3 replicas lost).VERSION\_INCOMPLETE but most replicas have complete versions.REPLICA\_RELOCATING with most replicas needing to relocate (such as 2 out of 3 replicas). |
| LOW | REPLICA\_MISSING\_IN\_CLUSTER.REPLICA\_RELOCATING but most replicas are stable. |
