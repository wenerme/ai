...
> **tip**: - For ingestion methods such as Stream Load and Broker Load, the header `timezone` overrides the Doris cluster `time_zone`, so keep them consistent during ingestion.
