...
> **tip**: To enable LDAPS (encrypted connection to the LDAP server), see the [LDAPS (Encrypted Connection)](#ldaps-encrypted-connection) section below.
...
> **tip**: You can refer to the related examples in [this code repository](https://github.com/morningman/doris-debug-tools/tree/main/jdbc-test). Running `build-auth-plugin.sh` directly generates the plugin jar described above, which can then be placed in the designated location on the client.
...
> **caution**: - Enabling `ldap_allow_empty_pass` reopens the security risk described above. Keep the default `false` in production environments.
