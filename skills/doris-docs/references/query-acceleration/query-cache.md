...
[Incremental execution dataflow inside one instance]
...
[Decision flow and the main fallback reasons]

Putting it all together, from the FE authorization to the storage-level guards:

[Incremental merge overview across FE, BE and storage]
...
> **note**: The `query_cache_max_size_mb` and `query_cache_elasticity_size_mb` settings in `be.conf` control the legacy SQL Result Cache. They are **not** the pipeline-level Query Cache described in this article. Do not confuse them.
