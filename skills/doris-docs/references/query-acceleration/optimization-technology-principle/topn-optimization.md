...
> **note**: `TopNLazyMaterializationSecondPhaseWriteCacheBytes` covers only phase 2 of lazy materialization. If general File Cache metrics still report writes, first determine whether they come from phase 1 or another operator. For information about obtaining a Profile, see [Query Profile Analysis](../query-profile.md).
