...
[How IVM works: base table changes flow into Row Binlog and then into internal Table Streams; the delta plan combines unconsumed changes with a consistent snapshot to update the materialized view and advances the consumption offsets in the same transaction]
