...
> **caution**: The adapter currently does not support External Catalog three-part namespaces using Catalog, Database, and Table.
...
> **note**: The FE version gate for this materialization accepts Doris 2.x at 2.1.5 or later, Doris 3.x except 3.0.0, Doris 4.x and later, and identifiable source builds reported as `doris-0.0.0-<git-sha>`. Source builds are for development testing only. These gates are code admission conditions, not evidence that every accepted version has passed compatibility testing.
