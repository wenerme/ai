...
- Bun support [introduced](https://gitlab.com/gitlab-org/gitlab/-/work_items/592701) in GitLab 19.2.
...
| JavaScript/TypeScript     | Bun             | `bun.lock`                                      | Lockfiles generated by `bun`.                                                                                                                                                          | Yes                     | Yes                 |
...
| Objective-C               | CocoaPods       | `Podfile.lock`                                  | Lockfiles generated by `cocoapods`. Detection only: the SBOM is generated, but CocoaPods components do not appear in the [dependency list](../../dependency_list/_index.md), are not matched against vulnerability advisories, and are not included in license scanning.[^cocoapods-detection-only] | No                      | No                  |
...
| Python                    | uv[^lockfile-contains-multiple]  | `uv.lock`                                       | Lockfiles generated by `uv`.                                                                                                                                                          | Yes                     | Yes                 |
...
[^cocoapods-detection-only]: The `cocoapods` PURL type is not a supported package type.
    To get full dependency list and vulnerability scanning coverage for iOS projects,
    use [Swift Package Manager](https://www.swift.org/documentation/package-manager/) with `Package.resolved` instead.
[^lockfile-contains-multiple]: If a lockfile contains multiple entries for the same package with different environment markers (for example, numpy==2.2.6 for Python <3.11 and numpy==2.4.1 for Python ≥3.11), only the first entry is parsed and reported.
...
| `resolution_jobs_checkout_timeout`          | number  | `60`                                                                                                      | Timeout in seconds that the resolution service waits for the repository checkout. Must be a positive integer.                                                                                                                                                         |
| `resolution_jobs_script_timeout`            | number  | `60`                                                                                                      | Timeout in seconds to wait for the dependency resolution service to generate the resolution script. Must be a positive integer. If the service is still waiting for the repository checkout, increase `resolution_jobs_checkout_timeout` instead.                     |
