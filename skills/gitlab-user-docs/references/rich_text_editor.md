...
- [Introduced](https://gitlab.com/gitlab-org/gitlab/-/issues/371931) for editing issue descriptions in GitLab 15.5 [with a feature flag](../administration/feature_flags/_index.md) named `content_editor_on_issues`. Disabled by default.
...
[A text editor in rich text editing mode with the "Switch to plain text editing" text box in the lower-left]
...
| `code`                                                    | Inline code          |
| ` ```rb ` + <kbd>Enter</kbd>  ` ```js ` + <kbd>Enter</kbd> | Code block      |
...
[A table size selector with 3 rows and 3 columns.]

### Edit a table

Inside a table cell, you can use a menu to insert or delete rows or columns.

To open the menu: In the upper-right corner of a cell, select the chevron .

[An active chevron menu showing table actions.]
...
1. In the upper-right corner of a cell, select the chevron  > **Merge N cells**.

To split merged cells: In the upper-right corner of a cell, select the chevron  > **Split cell**.
...
1. In the top bar of a text box, select  **More options** and then **Mermaid diagram** or **PlantUML diagram**.
1. Enter the code for your diagram. The diagram preview appears in the text box.

[A mermaid diagram preview in rich text editor with LR syntax creating a left to right flowchart]
