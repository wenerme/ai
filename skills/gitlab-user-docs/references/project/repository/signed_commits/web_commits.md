...
## Turn on web-based commit signing for a group or project
...
You can turn on web-based commit signing for all projects in a group,
or for an individual project.

When web-based commit signing is turned on, all commits made through
the GitLab UI (Web Editor, Web IDE, and merge requests) are
automatically signed with the instance's configured signing key.
...
> If the group setting is on, the project checkbox is not available.
