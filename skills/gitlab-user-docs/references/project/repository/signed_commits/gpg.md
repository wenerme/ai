...
Sign commits and tags in your GitLab repository with GPG (GNU Privacy Guard) keys.
...
- Go to the user's profile (such as `https://gitlab.example.com/<USERNAME>`). In the upper-right corner
  of the user's profile, select **View public GPG keys** ().
...
1. Select **Edit profile**.
1. In the left sidebar, select **Access** > **GPG keys**.
1. Select **Remove** () next to the GPG key you want to delete.
