...
1. Find the lines of code you want to change.
   - To select a single line, hover over the line number and
     select **Add a comment to this line** ().
   - To select more lines:
     1. Hover over the line number, and select **Add a comment to this line** ():

        [The add comment icon displayed when hovering over a line number.]
   ...
   - To comment on an entire file instead of specific lines, in the header for the file,
     select **Comment on this file** ().
1. In the comment toolbar, select **Insert suggestion** (). GitLab
...
[The preview of a suggestion spanning multiple lines.]
...
[The line numbers indicating the range of a multi-line comment.]
...
[The rich text editor controls for adjusting the range of a multi-line suggestion.]
...
| `%{co_authored_by}`    | Names and emails of suggestion authors in a `Co-authored-by` Git commit trailer format. | `Co-authored-by: Zane Doe <zdoe@example.com>`  `Co-authored-by: Blake Smith <bsmith@example.com>` |
