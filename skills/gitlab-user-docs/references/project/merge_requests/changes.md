...
[A merge request diff showing added and removed lines of code.]
...
- **Comment on this file** () to leave a general comment on the file, without
...
- Show more context: Select **Previous 20 lines** () to display
  the previous 20 unchanged lines, or **Next 20 lines** () to
...
1. Select **Show file browser** () or press <kbd>F</kbd> to show
   the file tree.
...
1. If the merge request changes many files, you can jump directly to a specific file:
   1. Select **Show file browser** () or press <kbd>F</kbd> to show the file tree.
...
1. When you visit that link, your chosen file is shown at the top of the list. The file browser
   shows a link icon () next to the filename:

   [A merge request listing files, with the selected YAML file at the top.]
...
1. Select **Preferences** (). Select either **Side-by-side** or **Inline**.
...
### Inline changes
   ...
   [Merge request code changes in inline mode.]
...
### Side-by-side changes
   ...
   [Merge request code changes in side-by-side mode.]
...
1. Select **Changes**.
1. On the file you would like explained, select the three dots () and select **View File @ $SHA**.
...
1. On the left side, select the question mark (). You might have to scroll to the first line of your selection to view it.

   [Icon to explain the selected code snippet using GitLab Duo in a merge request.]
...
1. Scroll to the line the comment is attached to. In the gutter margin, select **Collapse** ():
   [Icon to collapse a comment in a merge request diff.]
...
1. Scroll to the line the comment is attached to. In the gutter margin, select the user avatar:
   [Icon to expand a comment in a merge request diff.]
...
1. In the left sidebar, select **Code** > **Merge requests** and find your merge request.
1. Below the title, select **Changes**.
1. Before the list of changed files, select **Preferences** ().
1. Select or clear **Show whitespace changes**:

   [A merge request diff with Preferences menu expanded and 'Show whitespace changes' option selected.]
...
[A conflict alert in a merge request diff.]
...
1. In the left sidebar, select **Code** > **Merge requests** and find your merge request.
1. Select **Changes**.
1. In the header for the file you want to comment on, select **Comment on this file** ().
