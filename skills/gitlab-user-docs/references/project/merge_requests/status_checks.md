...
In the **Status checks** sub-section, select **Edit** ()
...
Within the **Status checks** sub-section, select **Remove** ()
...
1. Select **Retry** () on the failed external status check row. The status check is put back into a pending state.
