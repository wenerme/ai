...
* The following [dot commands](https://duckdb.org/docs/current/clients/cli/dot_commands.html) are disabled:
    * `.cd`
    * `.excel`
    * `.import`
    * `.log`
...
* The [`getenv` function](https://duckdb.org/docs/current/clients/cli/overview.html#reading-environment-variables) is disabled.
* The [`enable_external_access` option](https://duckdb.org/docs/current/configuration/overview.html#configuration-reference) is set to `false`. This implies that:
...
For more information on running DuckDB in secure environments, see the [“Securing DuckDB” page](https://duckdb.org/docs/current/operations_manual/securing_duckdb/overview.html).
