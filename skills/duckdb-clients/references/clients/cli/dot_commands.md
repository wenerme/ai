...
| `.dump ⟨TABLE⟩`                            | Render database content as SQL. `TABLE` is a [`LIKE` pattern](https://duckdb.org/docs/current/sql/functions/pattern_matching.html) for the tables to dump                            |
...
| `.highlight_mode ⟨mixed/dark/light⟩`       | Toggle the highlight mode. See the [dark/light mode section](https://duckdb.org/docs/current/clients/cli/friendly_cli.html#darklight-mode) for details                               |
...
| `.maxrows ⟨COUNT⟩`                         | Sets the maximum number of rows for display. Only for [duckbox mode](https://duckdb.org/docs/current/clients/cli/output_formats.html)                                                |
| `.maxwidth ⟨COUNT⟩`                        | Sets the maximum width in characters. 0 defaults to terminal width. Only for [duckbox mode](https://duckdb.org/docs/current/clients/cli/output_formats.html)                         |
| `.mode ⟨MODE⟩ ⟨TABLE⟩`                     | Set [output mode](https://duckdb.org/docs/current/clients/cli/output_formats.html)                                                                                                   |
...
| `.pager ⟨OPTIONS⟩`                         | Control pager usage for output. See the [paging section](https://duckdb.org/docs/current/clients/cli/output_formats.html#paging) for details                                         |
...
| `.safe_mode`                               | Activates [safe mode](https://duckdb.org/docs/current/clients/cli/safe_mode.html)                                                                                                    |
...
| `.tables ⟨TABLE⟩`                          | List tables [matching `LIKE` pattern](https://duckdb.org/docs/current/sql/functions/pattern_matching.html) `TABLE` with column names, types and row counts, grouped by database and schema |
| `.timer ⟨on/off⟩`                          | Turn SQL timer `on` or `off`. SQL statements separated by `;` but _not_ separated via newline are measured together                                                        |
...
A common output format is CSV, or comma separated values. DuckDB supports [SQL syntax to export data as CSV or Parquet](https://duckdb.org/docs/current/sql/statements/copy.html#copy-to), but the CLI-specific commands may be used to write a CSV instead if desired.
...
All DuckDB clients support [querying the database schema with SQL](https://duckdb.org/docs/current/sql/meta/information_schema.html), but the CLI has additional [dot commands](https://duckdb.org/docs/current/clients/cli/dot_commands.html) that can make it easier to understand the contents of a database.
The `.tables` command will return a list of tables in the database. It has an optional argument that will filter the results according to a [`LIKE` pattern](https://duckdb.org/docs/current/sql/functions/pattern_matching.html#like).
...
An optional `TABLE` argument filters the output using a [`LIKE` pattern](https://duckdb.org/docs/current/sql/functions/pattern_matching.html#like). Multiple patterns can be provided as additional arguments.
...
<img src="https://duckdb.org/images/progress_bar/duckdb_progressbar_time.gif"
...
<img src="https://duckdb.org/images/progress_bar/duckdb_progressbar_cache_usage.gif"
