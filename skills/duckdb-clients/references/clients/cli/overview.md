...
The `⟨OPTIONS⟩` part encodes [arguments for the CLI client](https://duckdb.org/docs/current/clients/cli/arguments.html). Common options include:

* `-csv`: sets the output mode to CSV
* `-json`: sets the output mode to JSON
* `-readonly`: open the database in read-only mode (see [concurrency in DuckDB](https://duckdb.org/docs/current/connect/concurrency.html#handling-concurrency))

For a full list of options, see the [command line arguments page](https://duckdb.org/docs/current/clients/cli/arguments.html).

### In-Memory vs. Persistent Database

When no `⟨FILENAME⟩` argument is provided, the DuckDB CLI will open a temporary [in-memory database](https://duckdb.org/docs/current/connect/overview.html#in-memory-database).
...
To open or create a [persistent database](https://duckdb.org/docs/current/connect/overview.html#persistent-database), simply include a path as a command line argument:
...
The CLI supports all of DuckDB's rich [SQL syntax](https://duckdb.org/docs/current/sql/introduction.html) including `SELECT`, `CREATE` and `ALTER` statements.

### Editor Features

The CLI supports [autocompletion](https://duckdb.org/docs/current/clients/cli/autocomplete.html), and has sophisticated [editor features](https://duckdb.org/docs/current/clients/cli/editing.html) and [syntax highlighting](https://duckdb.org/docs/current/clients/cli/syntax_highlighting.html) on macOS, Linux and Windows.
...
Below, we summarize a few important dot commands. To see all available commands, see the [dot commands page](https://duckdb.org/docs/current/clients/cli/dot_commands.html) or use the `.help` command.
...
> To keep the current database, while adding a new database, use the [`ATTACH` statement](https://duckdb.org/docs/current/sql/statements/attach.html).
...
The `.mode` [dot command](https://duckdb.org/docs/current/clients/cli/dot_commands.html#mode) may be used to change the appearance of the tables returned in the terminal output.
...
For details, see the documentation for the [output dot command](https://duckdb.org/docs/current/clients/cli/dot_commands.html#output-writing-results-to-a-file).
...
For details, see the [Extension docs](https://duckdb.org/docs/current/extensions/overview.html).
...
The output of the `getenv` function can be used to set [configuration options](https://duckdb.org/docs/current/configuration/overview.html). For example, to set the `NULL` order based on the environment variable `DEFAULT_NULL_ORDER`, use:
...
The `getenv` function can only be run when the [`enable_external_access`](https://duckdb.org/docs/current/configuration/overview.html#configuration-reference) option is set to `true` (the default setting).
It is only available in the CLI client and is not supported in other DuckDB clients.

## Prepared Statements

The DuckDB CLI supports executing [prepared statements](https://duckdb.org/docs/current/sql/query_syntax/prepared_statements.html) in addition to regular `SELECT` statements.
