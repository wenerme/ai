...
Whether two `getConnection` calls share a `dbAddress` follows the same rules as [instance caching](https://duckdb.org/docs/current/clients/java/connecting.html#database-instances-and-instance-caching):
...
* [Define Connections](https://duckdb.org/docs/current/clients/java/connecting.html) — the `jdbc_jfr_memory_monitor` property and the query-cancel scheduler thread the driver runs.
* [Profiling](https://duckdb.org/docs/current/dev/profiling.html) — DuckDB's query profiling output and the formats `getProfilingInformation()` returns.
* [Run Queries](https://duckdb.org/docs/current/clients/java/querying.html) — running the queries whose progress, timing, and profiles this page observes.
