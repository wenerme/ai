...
The [Appender](https://duckdb.org/docs/current/data/appender.html) is available in the DuckDB JDBC driver via the `org.duckdb.DuckDBAppender` class.
...
* [Appender](https://duckdb.org/docs/current/data/appender.html) — the engine-level Appender interface that the Java `DuckDBAppender` wraps.
* [Data Import](https://duckdb.org/docs/current/data/overview.html) — DuckDB's other bulk-loading options, including reading directly from CSV, Parquet, and JSON files.
* [Run Queries](https://duckdb.org/docs/current/clients/java/querying.html) — sending the [`INSERT`](https://duckdb.org/docs/current/sql/statements/insert.html) and [`CREATE TABLE`](https://duckdb.org/docs/current/sql/statements/create_table.html) statements the batch writer builds on.
* [Define Connections](https://duckdb.org/docs/current/clients/java/connecting.html) — obtaining the `DuckDBConnection` an Appender is created from.
