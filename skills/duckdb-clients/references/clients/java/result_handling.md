...
Chunk contents are read through the same `DuckDBDataChunkReader` API that is used by [user-defined functions](https://duckdb.org/docs/current/clients/java/functions.html).
...
* [Run Queries](https://duckdb.org/docs/current/clients/java/querying.html) — sending the queries whose results this page reads, and the standard `ResultSet` accessors.
* [Define Functions](https://duckdb.org/docs/current/clients/java/functions.html) — the `DuckDBDataChunkReader` API shared with chunked results is also used to write user-defined functions.
* [Define Connections](https://duckdb.org/docs/current/clients/java/connecting.html) — the `jdbc_stream_results` option that enables result streaming.
* [C API](https://duckdb.org/docs/current/clients/c/api.html) — the `duckdb_fetch_chunk` function that the chunked result API exposes to Java.
