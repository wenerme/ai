...
The [Appender](https://duckdb.org/docs/current/data/appender.html) is the fastest way to insert rows generated in Rust. Create one from a `Connection` with `appender()`, passing the target table name, then push rows with `append_row()`. Each row is a parameter list built with the `params!` macro:
...
With the `appender-arrow` feature enabled, an Appender can append an [Apache Arrow](https://arrow.apache.org/) `RecordBatch` directly, which avoids building a parameter list per row. See [Handle Results](https://duckdb.org/docs/current/clients/rust/result_handling.html#apache-arrow) for the Arrow integration.

## Reading Data Files

DuckDB can read many file formats directly in SQL, which is often the simplest way to load external data. The Rust client just sends the query. For [Parquet](https://duckdb.org/docs/current/data/parquet/overview.html), enable the `parquet` feature and query `read_parquet()`, adapted from the crate's [`parquet` example](https://github.com/duckdb/duckdb-rs/blob/main/crates/duckdb/examples/parquet.rs):
...
The same pattern reads [CSV](https://duckdb.org/docs/current/data/csv/overview.html) with `read_csv()` and [JSON](https://duckdb.org/docs/current/data/json/overview.html) with `read_json()` (the latter needs the `json` feature). To load a file into a table rather than query it in place, wrap the reader in a [`CREATE TABLE ... AS SELECT`](https://duckdb.org/docs/current/sql/statements/create_table.html#create-table--as-select-ctas) or [`COPY`](https://duckdb.org/docs/current/sql/statements/copy.html) statement sent with `execute()`.

## Further Reading

* [Appender](https://duckdb.org/docs/current/data/appender.html) — the engine-level Appender interface that the Rust `Appender` wraps.
* [Data Import](https://duckdb.org/docs/current/data/overview.html) — DuckDB's full set of bulk-loading options.
* [Handle Results](https://duckdb.org/docs/current/clients/rust/result_handling.html) — the Apache Arrow integration used to append and read record batches.
* [Run Queries](https://duckdb.org/docs/current/clients/rust/querying.html) — sending the `CREATE TABLE` and `COPY` statements the file readers build on.
