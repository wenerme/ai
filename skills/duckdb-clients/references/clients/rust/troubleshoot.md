...
Because crates.io imposes a 10 MB package-size limit, the `bundled` source excludes the [ICU extension](https://duckdb.org/docs/current/core_extensions/icu.html). As a result, some collation and time-zone-aware date/time operations fail out of the box. Install and load ICU at runtime to restore them:
...
Do not enable `loadable-extension` for a client application. Use it only when building an extension. See [Write User Defined Functions](https://duckdb.org/docs/current/clients/rust/functions.html#building-a-loadable-extension) for how loadable extensions are built and packaged.
...
* [Rust Client](https://duckdb.org/docs/current/clients/rust/overview.html) — installation, the `bundled` feature, and the full list of Cargo feature flags.
* [Connect](https://duckdb.org/docs/current/clients/rust/connecting.html) — opening connections, whose failures are often build or linking problems.
* [Write User Defined Functions](https://duckdb.org/docs/current/clients/rust/functions.html) — the `loadable-extension` feature and how to package an extension correctly.
