...
* [Run Queries](https://duckdb.org/docs/current/clients/rust/querying.html) — sending the queries whose results this page reads, and reading them row by row.
* [Write User Defined Functions](https://duckdb.org/docs/current/clients/rust/functions.html) — writing table functions, of which the built-in `ArrowVTab` is one.
* [Import Data](https://duckdb.org/docs/current/clients/rust/data_import.html) — appending Arrow record batches into a table with the `appender-arrow` feature.
