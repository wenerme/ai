...
Prepared statements work with both. The sections below walk through sending statements, binding parameters, and turning rows into typed Rust values. For opening the `Connection` these run on, see [Connect](https://duckdb.org/docs/current/clients/rust/connecting.html).

## Sending Statements

Use `execute()` for a single statement that does not return rows, such as [`INSERT`](https://duckdb.org/docs/current/sql/statements/insert.html) or [`UPDATE`](https://duckdb.org/docs/current/sql/statements/update.html). It takes the SQL and a list of parameters, and returns the number of affected rows. Pass an empty slice `[]` when there are no parameters:
...
> Warning Do *not* use prepared statements to insert large amounts of data into DuckDB. See [Import Data](https://duckdb.org/docs/current/clients/rust/data_import.html) for the Appender and other faster options.
...
`Value`, `ValueRef`, and `Type` are marked non-exhaustive, so a `match` over them should include a wildcard arm to stay forward-compatible as DuckDB adds types. To read whole result sets as columnar batches instead of row by row, see [Handle Results](https://duckdb.org/docs/current/clients/rust/result_handling.html).

## Further Reading

* [Handle Results](https://duckdb.org/docs/current/clients/rust/result_handling.html) — reading results as Apache Arrow record batches or Polars data frames instead of row by row.
* [Import Data](https://duckdb.org/docs/current/clients/rust/data_import.html) — the Appender, the recommended alternative to prepared statements for bulk inserts.
* [Prepared Statements](https://duckdb.org/docs/current/sql/query_syntax/prepared_statements.html) — DuckDB's SQL-level support for the parameterized queries used here.
* [Connect](https://duckdb.org/docs/current/clients/rust/connecting.html) — opening the `Connection` that these statements run on.
