...
The full list of settings is on the [Configuration page](https://duckdb.org/docs/current/configuration/overview.html). Many of them can also be changed after connecting with a [`SET` statement](https://duckdb.org/docs/current/sql/statements/set.html) or the equivalent [`PRAGMA`](https://duckdb.org/docs/current/configuration/pragmas.html).
...
DuckDB runs queries on its own pool of native threads, sized by the `threads` option and shared by all connections to a database instance. See the [DuckDB concurrency documentation](https://duckdb.org/docs/current/connect/concurrency.html) for how connections and threads interact.
...
* [Run Queries](https://duckdb.org/docs/current/clients/rust/querying.html) — using the `Connection` to send queries and read results.
* [Configuration](https://duckdb.org/docs/current/configuration/overview.html) — the full list of DuckDB settings that can be passed to `Config`.
* [Concurrency](https://duckdb.org/docs/current/connect/concurrency.html) — how DuckDB handles multiple connections and threads.
* [Troubleshoot](https://duckdb.org/docs/current/clients/rust/troubleshoot.html) — linking and build problems encountered when opening a connection.
