...
* [Handle Results](https://duckdb.org/docs/current/clients/rust/result_handling.html) — the built-in `ArrowVTab` table function and the Arrow vector API shared with user-defined functions.
* [C API](https://duckdb.org/docs/current/clients/c/api.html) — the table function interface that the Rust `VTab` API mirrors.
* [Extensions](https://duckdb.org/docs/current/core_extensions/overview.html) — DuckDB's extension system that a loadable Rust extension plugs into.
* [Run Queries](https://duckdb.org/docs/current/clients/rust/querying.html) — calling registered functions from SQL.
