...
Profiling must be enabled on a connection before metrics are collected. Enable it with the [`enable_profiling` and `profiling_mode` PRAGMAs](https://duckdb.org/docs/current/dev/profiling.html), run a query, then read the result with `get_profiling_info()`:
...
* [Run Queries](https://duckdb.org/docs/current/clients/rust/querying.html) — running the queries whose metrics this page reads and whose execution it interrupts.
* [Connect](https://duckdb.org/docs/current/clients/rust/connecting.html) — the `Connection` that profiling and the interrupt handle operate on.
