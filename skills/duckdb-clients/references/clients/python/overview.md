...
>
> The latest stable version of the DuckDB Python client is current release.
...
DuckDB can ingest data from a wide variety of formats – both on-disk and in-memory. See the [data ingestion page](https://duckdb.org/docs/current/clients/python/data_ingestion.html) for more information.
...
Note that these are read-only, i.e., editing these tables via [`INSERT`](https://duckdb.org/docs/current/sql/statements/insert.html) or [`UPDATE` statements](https://duckdb.org/docs/current/sql/statements/update.html) is not possible.
...
DuckDB supports converting query results efficiently to a variety of formats. See the [result conversion page](https://duckdb.org/docs/current/clients/python/conversion.html) for more information.
...
DuckDB supports writing Relation objects directly to disk in a variety of formats. The [`COPY` statement](https://duckdb.org/docs/current/sql/statements/copy.html) can be used to write data to disk using SQL as an alternative.
...
The `duckdb.connect()` accepts a `config` dictionary, where [configuration options](https://duckdb.org/docs/current/configuration/overview.html#configuration-reference) can be specified. For example:
...
To specify the [storage version](https://duckdb.org/docs/current/internals/storage.html), pass the `storage_compatibility_version` option:
...
A [`DuckDBPyConnection.cursor()` method](https://duckdb.org/docs/current/clients/python/reference/#duckdb.DuckDBPyConnection.cursor) creates another handle on the same connection. It does not open a new connection. Therefore, all cursors created from one connection cannot run queries at the same time.

### Community Extensions

To load [community extensions](https://duckdb.org/community_extensions/), use the `repository="community"` argument with the `install_extension` method.
...
To load [unsigned extensions](https://duckdb.org/docs/current/extensions/overview.html#unsigned-extensions), use:
...
> Consult the [Securing DuckDB page](https://duckdb.org/docs/current/operations_manual/securing_duckdb/securing_extensions.html) for guidelines on how set up DuckDB in a secure manner.
