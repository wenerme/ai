...
* [Run Queries](https://duckdb.org/docs/current/clients/go/querying.html) — calling registered functions from SQL.
* [Handle Results](https://duckdb.org/docs/current/clients/go/result_handling.html) — registering an Arrow stream as a queryable view, a related way to expose external data.
* [Connect](https://duckdb.org/docs/current/clients/go/connecting.html) — checking out the connection that functions are registered on, and the connector for replacement scans.
