...
Profiling information is connection-local, so it is read from a `*sql.Conn` checked out with `db.Conn()`. Enable profiling on that connection with the [`enable_profiling` and `profiling_mode` PRAGMAs](https://duckdb.org/docs/current/dev/profiling.html), run the query to profile, then read the result with `duckdb.GetProfilingInfo()`:
...
Which metrics are collected depends on the `profiling_mode` set on the connection; see the [Profiling page](https://duckdb.org/docs/current/dev/profiling.html) for the available modes and metric names.

## Log Storage

DuckDB can forward its [log messages](https://duckdb.org/docs/current/operations_manual/logging/overview.html) to a custom storage registered from Go. Register one on a `Connector` with `duckdb.RegisterLogStorage()`, giving it a name and a `duckdb.LoggerCallbacks` whose `DefaultLoggerCallback` receives each entry's level, type, and message:
...
* [Profiling](https://duckdb.org/docs/current/dev/profiling.html) — DuckDB's query profiling output and the `enable_profiling` and `profiling_mode` PRAGMAs.
* [Logging](https://duckdb.org/docs/current/operations_manual/logging/overview.html) — enabling logging with `enable_logging` and querying log entries through `duckdb_logs`.
* [Run Queries](https://duckdb.org/docs/current/clients/go/querying.html) — running the queries whose metrics this page reads.
* [Connect](https://duckdb.org/docs/current/clients/go/connecting.html) — checking out the connection that profiling operates on, and the connector that log storage is registered on.
