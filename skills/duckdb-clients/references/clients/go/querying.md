...
The `Context` variants take a [`context.Context`](https://pkg.go.dev/context) as their first argument, which is the recommended form because it lets a query be cancelled or time out. The sections below cover sending statements, binding parameters, prepared statements, transactions, and scanning results. For opening the database these run on, see [Connect](https://duckdb.org/docs/current/clients/go/connecting.html).

## Sending Statements

Use `Exec` (or `ExecContext`) for a statement that does not return rows, such as [`INSERT`](https://duckdb.org/docs/current/sql/statements/insert.html) or [`UPDATE`](https://duckdb.org/docs/current/sql/statements/update.html). It returns a `sql.Result`, from which `RowsAffected()` reports how many rows changed:
...
> Warning Do *not* use prepared statements to insert large amounts of data into DuckDB. See [Import Data](https://duckdb.org/docs/current/clients/go/data_import.html) for the Appender, which is far faster for bulk inserts.

### Forcing a Parameter Type

The client infers the DuckDB type of a bound value from its Go type. When that inference is not what you want, wrap the value with `duckdb.Typed()` to pin the DuckDB [logical type](https://duckdb.org/docs/current/sql/data_types/overview.html). This matters most for timestamps: a bare `time.Time` binds as `TIMESTAMP_TZ`, so binding against a `TIMESTAMP_NS` column needs the hint:
...
`duckdb.Typed()` is a scalar binding hint; DuckDB validates the value against the requested type when the parameter is bound. See [Troubleshoot](https://duckdb.org/docs/current/clients/go/troubleshoot.html#timestamp-versus-timestamp_tz) for the details of how the client maps `time.Time` to DuckDB's timestamp types.
...
DuckDB's [nested types](https://duckdb.org/docs/current/sql/data_types/overview.html#nested--composite-types), such as `LIST`, `STRUCT`, and `MAP`, and its [`JSON`](https://duckdb.org/docs/current/data/json/overview.html) type, are scanned through the generic `duckdb.Composite[T]` wrapper. Choose the Go type parameter to match the SQL shape, then read the decoded value with `Get()`:
...
This follows the client's [`json` example](https://github.com/duckdb/duckdb-go/tree/main/examples/json). To read whole result sets as columnar batches instead of row by row, see [Handle Results](https://duckdb.org/docs/current/clients/go/result_handling.html).

## Further Reading

* [Handle Results](https://duckdb.org/docs/current/clients/go/result_handling.html) — reading results as Apache Arrow record batches instead of row by row.
* [Import Data](https://duckdb.org/docs/current/clients/go/data_import.html) — the Appender, the recommended alternative to prepared statements for bulk inserts.
...
* [Connect](https://duckdb.org/docs/current/clients/go/connecting.html) — opening the database that these statements run on.
