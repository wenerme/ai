...
DuckDB [configuration options](https://duckdb.org/docs/current/configuration/overview.html) are appended to the DSN as URL-style `name=value` query parameters, separated by `&`:

```go
db, err := sql.Open("duckdb", "/path/to/foo.db?access_mode=read_only&threads=4")
```

The `access_mode=read_only` option opens a database file so that several processes can read it at once. The full list of options is on the [Configuration page](https://duckdb.org/docs/current/configuration/overview.html). Options can also be changed after connecting with a [`SET` statement](https://duckdb.org/docs/current/sql/statements/set.html).
...
Pass `nil` as the callback when no initialization is needed. A `Connector` is also the entry point for the [Appender](https://duckdb.org/docs/current/clients/go/data_import.html#appender) and [Arrow](https://duckdb.org/docs/current/clients/go/result_handling.html) interfaces, which operate on an individual connection obtained from it, and for [replacement scans](https://duckdb.org/docs/current/clients/go/functions.html#replacement-scans) and [log storage](https://duckdb.org/docs/current/clients/go/profiling.html#log-storage), which are registered on the `Connector` itself.
...
A `*sql.DB` is safe for concurrent use by multiple goroutines: it manages its own pool of connections and hands each in-flight query a connection of its own. A single `*sql.Conn` checked out with `db.Conn()`, and the DuckDB [Appender](https://duckdb.org/docs/current/clients/go/data_import.html#appender) and [Arrow](https://duckdb.org/docs/current/clients/go/result_handling.html) handles built on one, are not safe for concurrent use. DuckDB runs queries on its own pool of native threads, sized by the `threads` option and shared across the pool. See the [DuckDB concurrency documentation](https://duckdb.org/docs/current/connect/concurrency.html) for how connections and threads interact.

## Connection Lifetime

Temporary objects such as [temporary tables](https://duckdb.org/docs/current/sql/statements/create_table.html#temporary-tables) are scoped to a connection. When code closes a connection, `database/sql` may return it to the pool as an idle connection rather than actually closing it, so a temporary table can outlive the code that created it. To make closing a connection tear it down, disable idle connections:
...
* [Run Queries](https://duckdb.org/docs/current/clients/go/querying.html) — using the database and connections opened here to send queries and read results.
* [Configuration](https://duckdb.org/docs/current/configuration/overview.html) — the full list of DuckDB settings that can be passed in the DSN or the connector callback.
* [Concurrency](https://duckdb.org/docs/current/connect/concurrency.html) — how DuckDB handles multiple connections and threads.
* [Troubleshoot](https://duckdb.org/docs/current/clients/go/troubleshoot.html) — cgo, linking, and other build problems encountered when opening a database.
