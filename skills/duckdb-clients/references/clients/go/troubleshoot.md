...
Then build the Go module against that archive with the `duckdb_use_static_lib` [build tag](https://duckdb.org/docs/current/clients/go/overview.html#build-tags), passing the linker flags for the archive. For example, on macOS ARM64:
...
See [Run Queries](https://duckdb.org/docs/current/clients/go/querying.html#forcing-a-parameter-type) for the full example.

## Scanning JSON Values

Starting with v2, scanning a [`JSON`](https://duckdb.org/docs/current/data/json/overview.html) value directly into a `string` or `[]byte` is no longer supported. Scan into `any` or into the `duckdb.Composite` wrapper instead, as shown in [Run Queries](https://duckdb.org/docs/current/clients/go/querying.html#reading-nested-and-composite-types). If a plain string or byte result is needed and the JSON structure is not, cast the column to `::VARCHAR` or `::BLOB` in SQL before scanning.

## Further Reading

* [Go Client](https://duckdb.org/docs/current/clients/go/overview.html) — installation, build tags, and the bundled extensions.
* [Connect](https://duckdb.org/docs/current/clients/go/connecting.html) — opening databases and the connection lifetime, whose failures are often build or linking problems.
* [Run Queries](https://duckdb.org/docs/current/clients/go/querying.html) — parameter binding, including the `duckdb.Typed()` hint and JSON scanning.
