...
DuckDB can read many file formats directly in SQL, which is often the simplest way to load external data: the Go client just sends the query. Because the [Parquet](https://duckdb.org/docs/current/data/parquet/overview.html), [CSV](https://duckdb.org/docs/current/data/csv/overview.html), and [JSON](https://duckdb.org/docs/current/data/json/overview.html) readers are among the [bundled extensions](https://duckdb.org/docs/current/clients/go/overview.html#bundled-extensions), no extra installation is needed. Query a file in place with the matching table function:

```go
rows, err := db.QueryContext(ctx, `SELECT * FROM read_parquet('data.parquet')`)
```

The same pattern reads CSV with `read_csv()` and JSON with `read_json()`. To load a file into a table rather than query it in place, wrap the reader in a [`CREATE TABLE ... AS SELECT`](https://duckdb.org/docs/current/sql/statements/create_table.html#create-table--as-select-ctas) or [`COPY`](https://duckdb.org/docs/current/sql/statements/copy.html) statement sent with `Exec`:
...
* [Appender](https://duckdb.org/docs/current/data/appender.html) — the engine-level Appender interface that the Go `Appender` wraps.
* [Data Import](https://duckdb.org/docs/current/data/overview.html) — DuckDB's full set of bulk-loading options.
* [Run Queries](https://duckdb.org/docs/current/clients/go/querying.html) — sending the `CREATE TABLE` and `COPY` statements the file readers build on.
* [Connect](https://duckdb.org/docs/current/clients/go/connecting.html) — creating the `Connector` and connection the Appender operates on.
