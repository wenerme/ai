...
DuckDB is columnar, and beyond reading a result set [row by row](https://duckdb.org/docs/current/clients/go/querying.html#scanning-results) through `database/sql`, the Go client can exchange whole results with [Apache Arrow](https://arrow.apache.org/) as a stream of record batches, and register an Arrow stream as a queryable view. Both are described below.

## Enabling the Arrow Interface

The DuckDB Arrow interface is a heavy dependency, so starting with v2 it is opt-in. Enable it with the `duckdb_arrow` [build tag](https://duckdb.org/docs/current/clients/go/overview.html#build-tags):
...
`QueryContext()` accepts bound parameters as trailing arguments, the same as [`database/sql` queries](https://duckdb.org/docs/current/clients/go/querying.html#binding-parameters).
...
* [Run Queries](https://duckdb.org/docs/current/clients/go/querying.html) — sending the queries whose results this page reads, and reading them row by row.
* [Build Tags](https://duckdb.org/docs/current/clients/go/overview.html#build-tags) — the `duckdb_arrow` tag that enables the Arrow interface, and the other build tags.
* [Write User Defined Functions](https://duckdb.org/docs/current/clients/go/functions.html) — replacement scans, another way to expose external data to SQL.
