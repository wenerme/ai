...
Within a Julia process, tasks are able to concurrently read and write to the database, as long as each task maintains its own connection to the database. In the example below, a single task is spawned to periodically read the database and many tasks are spawned to write to the database using both [`INSERT` statements](https://duckdb.org/docs/current/sql/statements/insert.html) as well as the [Appender API](https://duckdb.org/docs/current/data/appender.html).
