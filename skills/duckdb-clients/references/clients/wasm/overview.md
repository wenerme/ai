...
>
> The latest stable version of the DuckDB WebAssembly client is current release.
...
Alternatively, load a prebuilt bundle directly from a CDN such as jsDelivr without installing anything; see [Instantiate](https://duckdb.org/docs/current/clients/wasm/instantiation.html) for the bundle-selection options. To try it interactively with no setup, use the [DuckDB-Wasm Web shell](https://shell.duckdb.org/).

## Basic API Usage

The example below instantiates DuckDB-Wasm from a CDN bundle, opens a connection, runs a query, and reads the result. [Instantiate](https://duckdb.org/docs/current/clients/wasm/instantiation.html) covers the other ways to load the bundle, such as with webpack, Vite, or self-hosted files.

Refer to the externally hosted [DuckDB-Wasm API Reference](https://shell.duckdb.org/docs/modules/index.html) for details on every class and method. The [DuckDB-Wasm launch blog post](https://duckdb.org/2021/10/29/duckdb-wasm.html) is another good introduction.
...
* [Instantiate](https://duckdb.org/docs/current/clients/wasm/instantiation.html) — the bundle-selection patterns for jsDelivr, webpack, Vite, and statically served files.
* [Import Data](https://duckdb.org/docs/current/clients/wasm/data_ingestion.html) — registering files and inserting Apache Arrow, CSV, JSON, and Parquet data.
