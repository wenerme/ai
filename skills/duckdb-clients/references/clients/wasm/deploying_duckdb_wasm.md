...
Changes the default extension repository from the public `https://extensions.duckdb.org` to the one specified. Note that extensions are still signed, so the best path is downloading and serving the extensions with a similar structure to the original repository. See our additional notes on [Creating a Custom Repository](https://duckdb.org/docs/current/extensions/extension_distribution.html#creating-a-custom-repository).
