...
When DuckDB-Wasm reads a remote file — through the [Wasm-flavored httpfs extension](https://duckdb.org/docs/current/clients/wasm/extensions.html#httpfs) or `registerFileURL` — the browser issues the request, so it is subject to the browser's [CORS policy](https://developer.mozilla.org/en-US/docs/Web/HTTP/CORS) and every request is upgraded to HTTPS.
...
* Stream large results with [`send()`](https://duckdb.org/docs/current/clients/wasm/query.html#query-execution) instead of materializing them all at once with `query()`.
...
See [Limitations](https://duckdb.org/docs/current/clients/wasm/overview.html#limitations) for the other constraints of the WebAssembly client.
...
The threaded bundle relies on `SharedArrayBuffer`, which browsers only expose to pages that are [cross-origin isolated](https://duckdb.org/docs/current/clients/wasm/deploying_duckdb_wasm.html#cross-origin-isolation). If the top-level document is not served with the `Cross-Origin-Embedder-Policy` and `Cross-Origin-Opener-Policy` headers, DuckDB-Wasm falls back to the single-threaded `mvp` or `eh` bundle.

See [Cross-Origin Isolation](https://duckdb.org/docs/current/clients/wasm/deploying_duckdb_wasm.html#cross-origin-isolation) for the required headers, and [Threading](https://duckdb.org/docs/current/clients/wasm/instantiation.html#threading) for how to select and configure the `coi` bundle.

## Further Reading

* [DuckDB Wasm Client](https://duckdb.org/docs/current/clients/wasm/overview.html) — installation, basic usage, and the limitations behind several issues above.
* [Import Data](https://duckdb.org/docs/current/clients/wasm/data_ingestion.html) — registering and reading remote files that are subject to the CORS rules above.
* [Load Extensions](https://duckdb.org/docs/current/clients/wasm/extensions.html) — how extensions are fetched, signed, and served, including the httpfs CORS behavior.
* [Deploy](https://duckdb.org/docs/current/clients/wasm/deploying_duckdb_wasm.html) — serving components and configuring cross-origin isolation for threading.
