...
>
> The latest stable version of the DuckDB Node.js (Neo) client is current release.
...
It depends on low-level bindings that adhere closely to [DuckDB's C API](https://duckdb.org/docs/current/clients/c/overview.html),
...
Set [configuration options](https://duckdb.org/docs/current/configuration/overview.html):
```ts
const instance = await DuckDBInstance.create('my_duckdb.db', {
  threads: '4'
});
