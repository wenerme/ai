...
[Create a Check dialog showing Browser Test, HTTP / API, TCP Port, SSL / TLS Certificate, and SSH options]
...
[Browser check gate with Starting URL and Name fields, and Record journey and Build manually buttons]
...
[Recorder setup screen with three numbered steps and an Open and Record button]
...
[Journey step with no steps yet, offering Record journey and Add a step manually]

Every journey must begin with a **Navigate** step.

[Journey step editor showing action, step name, locator field, and a notice that the journey verifies nothing]
...
[Journey showing the First step must be Navigate validation error]
...
[Configure step showing the Check Details and Authentication and Network cards]
...
[Step editor showing the action dropdown open with all nine available actions]
...
[Assert step configured with the Page title is assertion and an expected value]
...
[Advanced group expanded showing step timeout and page settle budget fields]
...
[Edit view of a browser check showing collapsed Navigate and Assert steps]
