...
[Private Locations tab showing the empty state and the Set up an agent action]
...
[Set up a private agent drawer with location name, agent type tabs, platform tabs, and the generated install command]

Switching to **Browser** changes both the command and the available platforms, since browser agents run a container image.

[Agent setup drawer on the Browser tab, offering only Docker and Kubernetes platforms]
...
[Synthetics Tokens page under IAM showing the default token with its status, agent count, and actions]
