...
[Alerts tab of an SLO listing a burn-rate alert, a slow-burn alert, and an error-budget alert]
...
[SLO alert form in error budget mode, with critical at 75% and warning at 50% of budget consumed]
...
[Alert history for an SLO alert showing runs recorded as Skipped with no condition and no error]
...
[Alerts list with three SLO alerts, each carrying an SLO badge and a link to its SLO]
...
[SLO alert detail page showing the linked SLO, burn-rate condition, windows, and schedule]
