...
[count-based alert with warning threshold field]
...
[aggregation alert with warning value and warning group count]
...
[PromQL alert with warning value and per-series toggle]
...
[aggregation section with multi-alert toggle enabled]
...
[multi-alert group table showing group keys, levels, and timestamps]
...
[alert detail drawer with Groups tab showing per-group state rows]
...
[priority dropdown in alert form]
...
[alerts list page with priority column, filter, and sort]
...
[tags input field in alert form]
...
[tags filter dropdown with autocomplete facet]
...
[alerts list with last outcome, level, level-since, and groups-firing columns]
...
[alert history with value context columns]
...
[SLO alert form in burn rate mode, showing the fast, mid, and slow suggested configurations]
