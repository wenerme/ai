...
[Pipeline Editor in OpenObserve]
...
2. Click the edit icon to configure the selected node.
    ...
    [condition in realtime pipeline]  
    ...
    To create a new function:
        ...
        [new function in realtime pipeline]
    ...
    **Note**: In the **Associate Function** fom, the **After Flattening** toggle is enabled by default. Disable it only if necessary.
    ...
    For more details, see the [Functions Guide](../functions/functions-in-openobserve.md).
...
- [remove icon]: Use the **remove icon** to remove any incorrect connection.
- [connection icon]: Use the **connection icon** to build a connection between two nodes.

[realtime pipeline node connection] 
