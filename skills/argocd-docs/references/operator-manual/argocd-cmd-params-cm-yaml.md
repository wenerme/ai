...
data:
  ...
  # Path to the CA certificate file for verifying the repo server's TLS certificate 
  controller.repo.server.ca.cert.path: ""
  # Path to the client certificate file for mTLS connections to repo server
  controller.repo.server.client.cert.path: ""
  # Path to the client certificate key file for mTLS connections to repo server
  ...
  # Maximum number of concurrent cluster operations during sync. Any value less than 1 means no limit.
  ...
  # Path to the CA certificate file for verifying the repo server's TLS certificate (enables mTLS CA verification)
  ...
  # Include the source IP address of the client in API request logs
  server.enable.source.ip.logging: "false"
  # Comma separated CIDRs or addresses of proxies in front of the API server. When a request arrives from one of them, the
  # source IP is taken from server.client.ip.header or from X-Forwarded-For instead (default: none)
  server.trusted.proxies: ""
  # Header a trusted proxy sets to the client IP, e.g. CF-Connecting-IP, True-Client-IP or X-Real-IP (default: none)
  server.client.ip.header: ""
  ...
  # Path to the client CA certificate file for mTLS. if the file is not found or empty, mTLS is disabled.
  reposerver.client.ca.path: ""
  ...
  # Path to the ASCII-armored GPG private key used as the commit server's default
  # signing key for hydrated commits (default ""). Setting this enables signing:
  # there is no separate on/off toggle. When set, the commit server fails at
  # startup if the key cannot be loaded — it will not silently fall back to
  # unsigned commits.
  commitserver.signing.key.path: ""
  # Optional path to a file containing the passphrase for the signing key.
  commitserver.signing.key.passphrase.file: ""
  ...
  # Workqueue rate limiter bucket size (default "500")
  applicationsetcontroller.wq.bucket.size: "500"
  # Workqueue rate limiter bucket QPS. Defaults to MaxFloat64, which disables the bucket limiter
  applicationsetcontroller.wq.bucket.qps: "500"
  # Workqueue per-item rate limiter base delay, in nanoseconds (default "1000000", 1ms)
  applicationsetcontroller.wq.basedelay.ns: "1000000"
  # Workqueue per-item rate limiter max delay, in nanoseconds (default "1000000000", 1s)
  applicationsetcontroller.wq.maxdelay.ns: "1000000000"
  # Workqueue per-item rate limiter backoff factor (default "1.5")
  applicationsetcontroller.wq.backoff.factor: "1.5"
  # Workqueue per-item rate limiter cooldown duration, in nanoseconds. 0 disables the per-item rate limiter (default "0")
  applicationsetcontroller.wq.cooldown.ns: "0"
  # QPS (Queries Per Second) limit for K8s API client requests (default "50")
  # Can also be set via ARGOCD_K8S_CLIENT_QPS environment variable
