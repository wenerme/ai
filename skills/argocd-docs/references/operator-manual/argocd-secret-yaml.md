...
data:
  ...
  webhook.harbor.secret: shhhh! it's a harbor secret
  # docker hub webhook secret (OCI registry); supplied as the `secret` query parameter on the webhook URL
  webhook.dockerhub.secret: shhhh! it's a docker hub secret
