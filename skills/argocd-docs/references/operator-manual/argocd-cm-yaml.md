...
data:
  ...
  dex.config: |
    # Optional: configure Dex storage backend to persist sessions and signing keys
    storage:
      type: kubernetes
      config:
        inCluster: true
  ...
  # A default dex connector id to force HTTP redirect in Dex Auth endpoint.
  # Helpful when dex configured with two connectors for human users and for machines.
  dex.auth.connectorId: github
  ...
  # Enable manifest compression in the cluster cache. When enabled, cached resource manifests are serialized and
  # compressed using the configured storage format and compression algorithm. When disabled (default), manifests are
  # stored as raw in-memory objects.
  resource.manifest.compression.enabled: "false"
  # Serialization format for cached manifests. Supported values are "json" (default) and "msgpack".
  # resource.manifest.storage: "json"
  # Compression algorithm for cached manifests. Supported values are "gzip-bestspeed" (default), "gzip-default",
  # "s2-encode", "s2-encodebetter", "zlib", and "none".
  ...
  # Set to "true" to disable the managed Resources view in the UI. When disabled, the "Resources"
  # navigation item is hidden and the /resources page shows a "feature not enabled" message.
  # Defaults to "false" (the Resources view is enabled).
  ui.view.resources.disabled: "false"

  # Optional override for the SSO login button label.
  ...
  # application.sync.impersonation.enforced enforces that a service account must be configured for impersonation.
  # When set to false, if no matching service account is found, the controller's service account will be used.
  # Defaults to true for security. Only disable if you trust all applications.
  ...
  ### SourceHydrator readme message template.
  # This template defines the content of the README.md file that is automatically
  # generated during the hydration process. It can use placeholders such as
  # `.RepoURL`, `.DrySHA`, and `.Commands` to provide reproducible instructions.
  #
  # The template is dynamically loaded from a ConfigMap, which allows customizing
  # the README format depending on the environment or requirements.
