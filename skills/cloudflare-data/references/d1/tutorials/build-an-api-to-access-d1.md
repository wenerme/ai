...
1. Create a Worker named `d1-http` by running:npmyarnpnpm
...
1. Add the following snippet into your `src/index.ts`.
   ...
   *src/index.tsts*
   ...
   - POST `/api/exec`
   - POST `/api/batch`
2. Start the development server by running the following command:npmyarnpnpm
...
1. Start the development server by executing the following command:npmyarnpnpm
