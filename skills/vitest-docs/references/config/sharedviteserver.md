...
> **tip**: If every project repeats the same `plugins` entry, move it to the declaring config. The projects inherit it from the shared server and keep sharing:
...
> **warning**: When projects share a server, the declaring config file is executed once instead of once per project. Plugins are instantiated once, and their `config` hooks cannot observe per-project test options. If a plugin needs to behave differently per project, disable this option or don't share the server for that project (for example, set `extends: false` or define the project in its own config file).
