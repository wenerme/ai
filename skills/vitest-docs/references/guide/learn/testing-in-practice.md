...
> **tip**: If you're unsure whether an edge case matters, ask yourself: could a real user or a real caller trigger this? If yes, test it.
...
> **tip**: Only reach for mocks when the real thing is slow, flaky, or has side effects you can't control in a test.
...
> **tip**: If you use AI agents to fix bugs, configure them to follow the same principle: reproduce the issue with a failing test first, then fix the code. This prevents the agent from "fixing" a bug by changing the test instead of the code, and gives you confidence that the fix actually works.
