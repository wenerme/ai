...
> **tip**: Vitest UI access is protected. If the direct URL shows an error, open the URL with a token printed by Vitest in the terminal, for example `http://localhost:51204/__vitest__/?token=...`.

> **warning**: The UI is interactive and requires a running Vite server, so make sure to run Vitest in `watch` mode (the default). Alternatively, you can generate a static HTML report that looks identical to the Vitest UI by specifying `html` in config's `reporters` option.
...
> **tip**: To view the HTML report from CI, for example in GitHub Actions, upload the output directory as an artifact:
...
> **info**: All of the provided images use [Zammad](https://github.com/zammad/zammad) repository as an example.

[The module graph view]
[The module graph view]
...
<center>
  [The 'Show Full Graph' button located close to the legend]
  [The 'Show Full Graph' button located close to the legend]
</center>

> **warning**: Note that if your graph is too big, it may take some time before the node positions are stabilized.
...
[The module info view for an external module]
[The module info view for an external module]
...
[The module info view for an internal module]
[The module info view for an internal module]

> **warning**: Note that type-only imports are not executed at runtime and do not display a total duration. They also cannot be opened.
...
> **tip**: If you are developing a custom integration on top of Vitest, you can use [`vitest.experimental_getSourceModuleDiagnostic`](/api/advanced/vitest#getsourcemodulediagnostic) to retrieve this information.
...
[Import breakdown with a list of top 10 modules that take the longest time to load]
[Import breakdown with a list of top 10 modules that take the longest time to load]
