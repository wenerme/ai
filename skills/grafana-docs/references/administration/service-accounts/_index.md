...
```bash
curl -H "Authorization: Bearer glsa_EXAMPLE_SERVICE_ACCOUNT_TOKEN" -X GET '<grafana_url>/api/access-control/user/permissions' | jq
```
...
```bash
curl -H "Authorization: Bearer glsa_EXAMPLE_SERVICE_ACCOUNT_TOKEN" -X GET '<grafana_url>/api/access-control/user/permissions' | jq '."dashboards:read"'
```
