...
--header 'Authorization: Bearer glsa_EXAMPLE_SERVICE_ACCOUNT_TOKEN' \
...
- Add the following permissions to the `basic:editor` role, using provisioning or the [RBAC HTTP API](ref:api-rbac-update-a-role):
  ...
  curl -H 'Authorization: Bearer glsa_EXAMPLE_SERVICE_ACCOUNT_TOKEN' \
  ...
  # Update the role
  curl -H 'Authorization: Bearer glsa_EXAMPLE_SERVICE_ACCOUNT_TOKEN' -H 'Content-Type: application/json' \
