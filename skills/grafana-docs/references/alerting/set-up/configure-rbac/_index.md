...
| `alert.rules:read`   | `folders:*``folders:uid:*` | Read Grafana alert rules in a folder and its subfolders. Combine this permission with `folders:read` in a scope that includes the folder. To view alert state history, add `datasources:query` in the scope of the data source that records it.   |
