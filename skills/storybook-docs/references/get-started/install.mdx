...
If your browser meets Storybook 10's requirements but not Storybook 11's raised floors, stay on Storybook 10. For browsers below the requirements introduced in Storybook 9, use a version prior to `9.0.0` whose requirements your browser meets.

You can also develop or build your Storybook in ["preview-only" mode](../sharing/publish-storybook.mdx#build-storybook-for-older-browsers). This omits the manager UI; browser compatibility depends on your builder and its configuration. Open `/iframe.html?navigator=true` instead of `/index.html`. This does not add older-browser support to the Storybook manager.
...
> Code snippet: `_snippets/create-command-custom-version.md`

To install a Storybook version prior to 8.3, you must use the `init` command:

> Code snippet: `_snippets/init-command-custom-version.md`
...
> Code snippet: `_snippets/create-command-custom-package-manager.md`
...
> Code snippet: `_snippets/create-command-manual-framework.md`
