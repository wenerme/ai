...
Sharing an MCP server means sharing its docs toolset, which needs a [components manifest](../manifests.mdx).
See [framework support](./overview.mdx#framework-support).
