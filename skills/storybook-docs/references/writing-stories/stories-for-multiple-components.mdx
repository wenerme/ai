...
2. The table for each documented subcomponent does _not_ include [controls](../essentials/controls.mdx) to change the value of the props, because controls always apply to the main component's args.
...
> Code snippet: `_snippets/list-story-with-unchecked-children.md`
...
> Code snippet: `_snippets/list-story-template.md`

This approach requires more setup, but it means you can reuse the `args` to each story in a composite component. It also means you can alter the args to the component with the [Controls panel](../essentials/controls.mdx).
