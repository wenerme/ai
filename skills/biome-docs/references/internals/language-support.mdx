...
Biome supports the following languages embedded in template literals in JavaScript:

- CSS (`css` or `styled` tag)
- GraphQL (`gql` or `graphql` tag)

This feature is still experimental and disabled by default. To enable this feature, set the `javascript.experimentalEmbeddedSnippetsEnabled` option.
...
If you **lint** `.svelte`, `.astro` or `.vue` files and `html.experimentalFullSupportEnabled` is `false`, it's advised to turn off a few additional rules to prevent false positive linting errors caused by our partial support. Use the option `overrides` for that:
...
> **note**: This last section might become outdated, so it's advised to use the [playground](https://biomejs.dev/playground), and check if your feature has been implemented.
