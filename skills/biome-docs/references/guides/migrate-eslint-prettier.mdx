...
**Workaround:** If that happens, temporarily comment out the plugins or shared configurations may cause the issue, run the migration, and then enable them again one by one to identify the problematic configuration.

For example, if your `eslint.config.js` looks like this, you can comment out the problematic entries:

```js title="eslint.config.js" del={8,9,12,13} ins={10,11,14,15}
...
export default defineConfig([
  react.configs.recommended,   // may cause cyclic reference
  jsxA11y.flatConfigs.recommended, // may cause cyclic reference
  // react.configs.recommended,
  // jsxA11y.flatConfigs.recommended,
  {
    plugins: {
      '@stylistic': stylistic,           // may cause cyclic reference
      '@typescript-eslint': tsEslint.plugin, // may cause cyclic reference
      // '@stylistic': stylistic,
...
Once the migration is complete, you can restore and manually configure any
rules from those plugins in your new `biome.json`.
