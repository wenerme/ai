...
> **note**: Using `!test` to completely exclude a directory is only supported in `files.includes`. In other places where `includes` is used (`linter.includes`, `formatter.includes`, etc.), you need to use `!/test/**` to exclude the directory.
...
> **caution**: This option has been **deprecated** and may be removed in a future release.
Please use the
...
> **note**: Due to a technical limitation, `linter.includes` also cannot match folders
...
> **caution**: This options is deprecated. Use [`linter.rules.preset`](/reference/configuration#lintrulespreset) instead.
...
> **note**: Due to a technical limitation, `assist.includes` also cannot match folders
while `files.includes` can. If you want to match all files inside a folder,
you should explicitly add `/**` at the end.
...
> **note**: Due to a technical limitation, `formatter.includes` also cannot match folders
while `files.includes` can. If you want to match all files inside a folder,
you should explicitly add `/**` at the end.
...
> **note**: This property is ignored when `indentStyle` is set to `tab`.
...
> **danger**: Setting this option to `false` is **highly discouraged** because it could cause many problems with other tools:
...
> **caution**: The HTML formatter is **disabled** by default because it is still experimental.
...
> **note**: Enabling this option merely switches this behavior for these files, not plain `.html` files. To format HTML files, you also
...
> **caution**: Glob patterns can be used in a Biome configuration file, but they can also be
