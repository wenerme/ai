...
> **note**: CST node names like `JsConditionalExpression` are specific to Biome's parser.
...
> **note**: The regex matches against the full node text, which includes the surrounding
...
> **note**: These node names are specific to Biome's parser and may change between versions.
