...
> **note**: This command cannot update Biome binaries installed through npm. Update `@biomejs/biome` with your package manager instead.
...
> **tip**: Put the query inside single quotes when the shell supports them. This prevents the shell from interpreting GritQL's backticks as commands.
...
> **caution**: The `json` and `json-pretty` reporters are experimental and may change in patch releases.
