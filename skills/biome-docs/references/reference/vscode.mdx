...
> **tip**: If you're upgrading from the `2.x` version of the extension, check out the [migration instructions](#migrating-from-the-2x-extension) at the end of this document.
...
To enable **Fix on Save**, update VS Code's [`editor.codeActionsOnSave`](vscode://settings/editor.codeActionsOnSave) setting to include the following. This will apply only [the safe fixes](/linter/#safe-fixes):
...
> **caution**: The `biome.enabled` setting must be set to `true` (default) in the same scope for this setting to take effect.
...
However, during local development, you want to disable this rule because it's useful, and you don't want to see red squiggles. In your `inlineConfig`, you would write something like the following:
...
:::caution[Biome `2.4.0+` required]
This setting will only take effect when using Biome `2.4.0` or later; otherwise, this setting will be ignored.
