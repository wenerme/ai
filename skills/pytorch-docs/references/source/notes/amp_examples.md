...
after which it's impossible to recover the accumulated unscaled grads {meth}`step ` must apply.

Therefore, if you want to {meth}`unscale_ <unscale_>` grads (e.g., to allow clipping unscaled grads),
call {meth}`unscale_ <unscale_>` just before {meth}`step `, after all (scaled) grads for the upcoming
{meth}`step ` have been accumulated. Also, only call {meth}`update <update>` at the end of iterations
where you called {meth}`step ` for a full effective batch:
...
and you must call {meth}`scaler.step ` on each of them individually.
