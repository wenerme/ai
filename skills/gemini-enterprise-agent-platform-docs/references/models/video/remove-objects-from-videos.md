...
You can use Veo on Gemini Enterprise Agent Platform to remove objects from videos that you generate. You
provide a mask and a video object, and then use a text prompt to describe the
output that you want. The Agent Platform API is supported during Preview.
Supported interfaces include the Google Cloud console and the Gemini Enterprise Agent Platform
API.
