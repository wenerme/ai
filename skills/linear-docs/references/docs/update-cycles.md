...
The Cycles view in the Team sidebar provides a list of previous, current, and upcoming cycles. Most adjustments to a cycle can be made in this view.

> [!NOTE]
> **Note:** The Cycles view doesn't include archived cycles.
...
Update a cycle's dates from the contextual menu on the Cycles page. For upcoming cycles, you can change both the start and end date. For the current cycle, you can change only the end date.

> [!NOTE]
> **Note:** You can't change a cycle's date into the past, so past cycle dates aren't editable.
...
If cooldown periods are enabled and cycle dates are adjusted, the cooldown label updates to reflect the actual gap. If cooldown periods are not enabled, any gap between cycles is shown as "Cycles paused".
