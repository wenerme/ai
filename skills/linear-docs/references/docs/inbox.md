...
Inbox shows in-app notifications for work that needs attention. Other notification channels can be managed in [notification settings](https://linear.app/docs/notifications).
...
You can delete the selected notification with `Backspace` or delete all read notifications with `Shift` `Backspace`.

Select a notification and then use `U` to mark as read or unread, `Option/Alt` `U` to mark all notifications as read, and `H` to snooze the selected notification (more below).
...
Press `Cmd/Ctrl` `F` to bring up the quick search bar. Use quick search to narrow notifications by fields such as issue title, issue ID, notification type, assignee, team, project, and priority. `Esc` will clear the search.
...
Snoozing hides a notification from your Inbox until the selected time. When that time arrives, the notification reappears in Inbox.
...
Yes, you can have up to 2,000 open notifications in your Inbox at one time. Older notifications beyond that count will not be retained.
</details>
