...
> [!NOTE]
> For pre-existing issues, you can complete a one-time import from Jira to Linear, use the CSV or API credential options in our [importer](https://linear.app/docs/jira-to-linear).
>
...
Jira Sync connects your Jira spaces to Linear teams, automatically keeping _new_ issues and projects in sync across both tools. It's designed for teams that aren't ready to fully switch over yet—whether you're running a small pilot or working through a gradual transition.

If you'd prefer to switch over entirely, you can [import your existing issues and projects](https://linear.app/docs/import-issues#jira) instead.

For a blend of both, start with the [import guide](https://linear.app/docs/jira-to-linear) to bring over historical data, then use Jira Sync to stay current going forward.
...
1. Enable the integration in Linear from [_Workspace Settings > Integrations > Jira_](https://linear.app/settings/integrations/jira).
2. Enter the following information:
  1. Personal access token
  2. Email address
  3. Installation or cloud hostname. Be sure to remove the `http://` and anything after `.net`.
3. Select which Jira space to link to which Linear team.

For best performance, users should link their individual Jira accounts in _Settings > Integrations > Jira > Connect accounts_. This allows better handling of fields in Jira like Assignee and Creator.
