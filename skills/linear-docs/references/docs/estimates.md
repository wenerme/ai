...
Go to _Team Settings > General > Estimates_ to enable the feature. Teams can use different estimate scales and configurations, even if they're working together on the same project. Sub-teams can inherit estimate settings from their parent team.
...
Allow zero estimates to assign an explicit estimate of 0. This is different from leaving an issue unestimated. By default, unestimated issues count as 1 point, but you can disable that in estimate settings.
