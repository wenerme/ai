...
Workspace admins can export this data. On Enterprise plans, only owners can do this. Export workspace issue data in CSV format from [Settings > Administration > Import Export](https://linear.app/settings/import-export) and click **Export data** at the bottom. There is a toggle option to include private teams, if any. This export action is recorded in the audit log.

Once the export is ready, we’ll email you a download link. The link expires after 12 hours.
...
Workspace admins can export this data. On Enterprise plans, only owners can do this. Export the list of members in CSV format from your [Settings > Administration > Members](https://linear.app/settings/members) and click **Export CSV** button.
...
* When in projects or custom views, click the project or view name and choose "Export issues as CSV…" from the dropdown.
...
CSV exports don’t include attachment files. Links to attachments may appear in exported issue descriptions.
...
If you need to save an individual issue as a PDF, open the Print dialog (`Cmd`/`Ctrl` + `P`) while viewing the issue and save it as a PDF. On doing this, we'll automatically change the timestamps on each issue event from relative to absolute, which can be helpful if your auditors require seeing precise timestamps for each event on an issue.
...
Members can export up to 200 projects or initiatives at a time.
...
## Customer request CSV exports

Export customer requests associated with an issue, project, or customer by opening the command menu and selecting **Export customer requests as CSV…**. This option is available when customer requests exist.

## Integrations

Use [Airbyte](https://linear.app/docs/airbyte) or [Google Sheets](https://linear.app/docs/google-sheets) to sync Linear data for reporting. Google Sheets supports issues, projects, and initiatives from public teams. If you're interested in exporting data for analysis, consider using [Insights](https://linear.app/docs/insights) as well for in-product reporting.
...
No, there isn't a CSV export for teams. If you need to move teams between workspaces, you can [import](https://linear.app/docs/import-issues#linear) them from one Linear workspace to another.
</details>
