...
By default, we do not refresh the Figma preview so that you maintain the context of related comments and issue descriptions. To update a Figma preview in Linear, use the refresh button on the embed.

## Linear Plugin for Figma

Run the plugin to see all Linear issues linked to your designs. From the plugin, select issues or the linked elements below the issue description to be taken to the linked frame, section, or page.
...
Filter your issues that appear in the plugin window to hide completed or cancelled issues, show only assigned or created by you, and to sort by status, priority, name, or created date. Use filters in the plugin to hide canceled issues or narrow the list.
...
If your Brave shield is up, you must manually allow "cross-site cookies" to be able to view authenticated Figma embeds.
