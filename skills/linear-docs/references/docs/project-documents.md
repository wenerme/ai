...
Create documents in projects to keep relevant information and updates in one central place. Use documents to create specs, PRDs, and share status updates with the project team.

## Create documents

To create a document in a project, click + in the Resources section, then select Create document. You can also navigate to the [project overview](https://linear.app/docs/project-overview) and add a document under the `Resources` section. You'll be required to give the document a title and can pick a custom icon and icon color.
...
Document templates can be created from workspace settings or from an individual team's settings. Once you've created the document template, press `save `to make the template available for your team and workspace members.
...
Notification types include document changes, comments, and replies. You do not get notifications for actions you have taken on a document, but you will receive notifications when:
...
While in a project view, you can open the document list with the shortcut `O` then `D` or by selecting the relevant document from the Overview tab on a specific project.

## Reference documents

Reference documents in issue descriptions, comments, agent guidance fields and even in other documents by typing @ while in edit mode followed by the document name. This action can also be used to reference issues or projects; matching documents will appear in the lowest section. To reference a document from a Linear editor, type @ followed by part of the document’s title, then select the document.

If you mention a document an agent guidance field, the agent can reference the instructions in that document before performing another action.

## Link to headers

To copy a URL to a specific header, hover over the left side and click _Copy link_. Sharing this URL with a colleague in your workspace will direct them to this specific section.

![Copy link button on the left side of header formatted text](https://webassets.linear.app/images/ornj730p/production/08ec9b37746e989992e0a0369b2645fdbaa2aa40-998x594.png?q=95&auto=format&dpr=2)

### External URLs

Add external URLs to a project’s `Resources` section alongside project documents.
