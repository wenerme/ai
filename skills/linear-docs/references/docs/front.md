...
`G` then `S` to go to _Settings > Integrations > Front_

`Ctrl` `L` (Mac) or `Alt` `Ctrl` `L` (Windows) to link from Linear issue

### Mouse

* Click the Avatar to go to _Settings > Workspace > Integrations > Front_
* Select the more menu (three dots) after saving the Linear issue to link conversations from Linear

### Command menu

`settings` to go to _Settings > Integrations > Front_
...
* Click on contextual menu icon `…` at the top right of the issue
* Click _Add link_
...
In the [Front settings page](https://linear.app/settings/integrations/front) in Linear, you can automate the reopening of the linked Front conversation when it's completed, cancelled or a comment is made. The integration will post an internal note and re-open the conversation in Front so it's easy to follow up with customers.
