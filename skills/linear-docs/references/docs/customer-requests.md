...
Watch this video to see how Customer Requests works:
...
From settings, add further customizations. Choose a team to receive new issues created from a request by default (we recommend selecting a team [exclusively used to track feedback](https://linear.app/now/cx-in-linear)). You can also customize revenue units and tiers for different plans or products you offer. Revenue, Tier and Size fields will be automatically populated if you use the Intercom integration, or can be filled when creating customers manually.
...
Requests can be created manually in Linear from the Customers section on an issue, from the button on a customer page, or by using the keyboard shortcut `Ctrl` `R` (macOS)/ `Ctrl` `Alt` `R` (Windows). This is useful when a request comes in through a channel not necessarily associated with an integration, like if you have Linear open during an in-person meeting.

From the project page, you can add customer requests using CTRL+R, or the + Customer Request button. Once your first Customer Request is added to a project, a dedicated tab for Customer requests will appear next to the Overview tab.
...
![Customer field highlighted when creating an issue from the Slack integration](https://webassets.linear.app/images/ornj730p/production/3ae6cb7e67184343ca3cb4fd11b73d949cd7e824-918x1133.png?q=95&auto=format&dpr=2)
