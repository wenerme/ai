...
A webhook push is an `HTTP POST` request, sent to the URL of your choosing. The push is automatically triggered by Linear when data updates. For an example of what data a payload contains, see [Webhook Payload](https://linear.app/developers/webhooks#webhook-payload).
...
Once your consumer is ready to receive updates, you can enable it for your Linear team. You can configure webhooks in Linear from API settings.
...
To create a new webhook via the API, call the `webhookCreate` mutation with the `teamId` (or `allPublicTeams: true`) and `url` of your webhook, and the preferred `resourceTypes` (`[Comment, Issue, IssueLabel, Project, Cycle, Reaction]`):
...
The server responds with a success flag and the ID of the newly created webhook:
...
--- | ---
`Linear-Delivery` | A UUID (v4) that uniquely identifies this payload
...
`Linear-Timestamp` | The Unix timestamp, in milliseconds, when the webhook was sent.
...
`actor` | The actor who triggered the action. Could be a `User`, OAuth client, or `Integration`.
