...
See [Securing Quack with a Reverse Proxy](https://duckdb.org/docs/current/quack/setup/reverse_proxy.html) for [nginx](https://nginx.org/) and [Caddy recipes](https://caddyserver.com/) (production and local-test).
...
The cleanest way to plug in custom authentication is through using a [`MACRO`](https://duckdb.org/docs/current/sql/statements/create_macro.html).
...
```sql
CREATE TABLE quack_tokens (auth_token VARCHAR, user_name VARCHAR);
INSERT INTO quack_tokens VALUES ('EXAMPLE_QUACK_TOKEN', 'analytics');
...
CALL quack_serve('quack:localhost', token => 'EXAMPLE_QUACK_TOKEN');
...
> Warning The prefix regex above is illustrative, not a robust read-only filter: a query like `WITH x AS (SELECT 1) INSERT INTO t SELECT * FROM x` starts with `WITH` yet still mutates data. For production read-only enforcement, attach the database in [read-only mode](https://duckdb.org/docs/current/sql/statements/attach.html#options) or use an authorization function that inspects the *parsed statement type* rather than the raw query text.
