...
Last updated Sep 22, 2026|Copy as Markdown| [View as Markdown](https://developers.cloudflare.com/realtime/sfu/get-started/connection-patterns/index.md)| [Agent setup](https://developers.cloudflare.com/agent-setup/)
...
Create the SFU session when ready to begin that connection's SDP exchange. [Unconnected sessions can expire before their first operation](https://developers.cloudflare.com/realtime/sfu/concepts/sessions-tracks/#session-setup).
...
Create an [SFU app](https://developers.cloudflare.com/realtime/sfu/get-started/#create-your-first-app). Your backend holds its App Secret and makes every SFU API request shown here. Use [Connection API authentication](https://developers.cloudflare.com/realtime/sfu/api/). Paths are relative to `https://rtc.live.cloudflare.com/v1/apps/{appId}`.
...
Check the HTTP status, public errors, and individual resource results on every API call. Retain successful allocations for cleanup. Complete each request and its required SDP exchange before starting the next mutation on the same session. Different sessions can proceed independently. [Negotiation and session lifecycle](https://developers.cloudflare.com/realtime/sfu/concepts/negotiation/) explains how to coordinate overlapping handlers.
...
3. **Publish the tracks.** On the backend, call `POST /sessions/{sessionId}/tracks/new` with the browser's `pc.localDescription` as `sessionDescription`:
   ...
   Replace the placeholders with the generated SDP and assigned `mid`. To publish camera and microphone together, include both tracks in this request. Each entry uses its own `mid` and `trackName`. Refer to [batch resource operations](https://developers.cloudflare.com/realtime/sfu/api/#batch-resource-operations).
...
5. **Verify reception.** Wait for the connection to become `connected` and confirm that the requested media plays. Use [media troubleshooting](https://developers.cloudflare.com/realtime/sfu/observability/#inspect-browser-media-statistics) when the connection succeeds but playback does not.
...
You now have publisher-to-subscriber messages. To add a return path, use [DataChannel replies](https://developers.cloudflare.com/realtime/sfu/features/datachannels/#return-to-publisher-canreply). For another delivery policy, publish a separate named channel and have its subscribers mirror that policy. Refer to [delivery settings](https://developers.cloudflare.com/realtime/sfu/features/datachannels/#configure-message-delivery).
...
The [cloud-gaming guide](https://developers.cloudflare.com/realtime/sfu/examples/cloud-gaming/) shows media and input on one browser connection, including controller ownership and separate delivery settings for discrete and replaceable input.

## Add media to an existing connection

Start with a connected PeerConnection and keep its SFU session. Serialize additions with [other operations on that session](https://developers.cloudflare.com/realtime/sfu/concepts/negotiation/#serialize-mutations-per-session).
...
3. Wait for API acceptance and a `connected` PeerConnection. You can now [add application channels](https://developers.cloudflare.com/realtime/sfu/features/datachannels/#add-an-application-channel) using `datachannels/new`.
...
Use [close results](https://developers.cloudflare.com/realtime/sfu/observability/error-codes/#interpret-close-results) to distinguish successful closure, an already-closed track, and an unresolved failure.
...
Check each track result. No `/renegotiate` call is needed. Forced closure can leave records in [session information](https://developers.cloudflare.com/realtime/sfu/api/#inspect-a-session). Follow [teardown ordering](https://developers.cloudflare.com/realtime/sfu/concepts/negotiation/#teardown) for outstanding negotiation or requests.

## Clean up

Stop accepting new operations and withdraw ended publications from discovery. Close allocated [media tracks](#close-media-tracks) and [DataChannels](https://developers.cloudflare.com/realtime/sfu/features/datachannels/#close-channels), then close the endpoint's PeerConnection and stop captures when no longer needed. Follow [cleanup responsibilities](https://developers.cloudflare.com/realtime/sfu/best-practices/#retain-cleanup-state) if the endpoint leaves before finishing.
