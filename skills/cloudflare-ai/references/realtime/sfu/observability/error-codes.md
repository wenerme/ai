...
For a symptom without an API error, such as connected video that does not play, start with [troubleshooting](https://developers.cloudflare.com/realtime/sfu/observability/#check-the-failing-layer).
...
The following tables cover common public codes. Use the [API schema](https://developers.cloudflare.com/realtime/static/realtime-api-2024-05-21.yaml) for each operation's response structure.
...
Use the [complete message recipe](https://developers.cloudflare.com/realtime/sfu/get-started/connection-patterns/#send-messages-between-two-endpoints) for a new connection, or [add DataChannels to an existing media connection](https://developers.cloudflare.com/realtime/sfu/get-started/connection-patterns/#add-datachannels-to-a-media-connection).

## Session and negotiation errors

| Code | Meaning and next action |
| --- | --- |
| `session_error` | With top-level HTTP `425`, finish setup and wait for connection before later operations. With `410`, check the session ID and endpoint state. Replace an [expired session](https://developers.cloudflare.com/realtime/sfu/concepts/sessions-tracks/#session-setup) or closed connection and rebuild any publications and subscriptions. Retrying the old session does not reconnect it. |
...
HTTP `406` alone does not establish that requests overlapped. Inspect the public error and endpoint state. [Negotiation and session lifecycle](https://developers.cloudflare.com/realtime/sfu/concepts/negotiation/) explains the full exchange boundary and concurrent handlers.
...
| `update_track_error` | The track is not configured for the requested simulcast update. Check the publication and use [simulcast updates](https://developers.cloudflare.com/realtime/sfu/features/simulcast/#change-the-received-layer) on an applicable receiving track. |
...
For simulcast updates, use the receiving session's `mid`. For media closure, use the `mid` from the session on which you are closing the track. DataChannel closure uses that endpoint's allocated channel ID. Refer to [identifier ownership](https://developers.cloudflare.com/realtime/sfu/concepts/sessions-tracks/#media-tracks) and [cleanup state](https://developers.cloudflare.com/realtime/sfu/best-practices/#retain-cleanup-state).
...
A request-level error without item results does not establish resource absence. Follow [recovery and teardown guidance](https://developers.cloudflare.com/realtime/sfu/concepts/negotiation/#retry-and-reconnect) after an interrupted request.
...
Retry failed items individually instead of repeating a partially successful creation batch. A timeout with no response leaves the result uncertain. Follow [retry and reconnect guidance](https://developers.cloudflare.com/realtime/sfu/concepts/negotiation/#retry-and-reconnect), and retain [diagnostic context](https://developers.cloudflare.com/realtime/sfu/observability/#record-useful-context) for persistent failures.
...
```json
{"@context":"https://schema.org","@type":"TechArticle","@id":"https://developers.cloudflare.com/realtime/sfu/observability/error-codes/#page","headline":"Error codes","description":"Look up public Realtime SFU API errors and find the next action for request, session, media, DataChannel, or adapter failures.","url":"https://developers.cloudflare.com/realtime/sfu/observability/error-codes/","inLanguage":"en","image":"https://developers.cloudflare.com/og-docs.png","dateModified":"2026-09-22","publisher":{"@type":"Organization","name":"Cloudflare","description":"One platform for your apps, agents, and workforce. Build, secure, and scale without managing infrastructure","url":"https://www.cloudflare.com/","sameAs":["https://github.com/cloudflare","https://www.linkedin.com/company/cloudflare","https://x.com/cloudflare"],"logo":{"@type":"ImageObject","url":"https://developers.cloudflare.com/logo.svg"},"address":{"@type":"PostalAddress","streetAddress":"101 Townsend St","addressLocality":"San Francisco","addressRegion":"CA","postalCode":"94107","addressCountry":"US"},"contactPoint":[{"@type":"ContactPoint","contactType":"Customer Support","url":"https://support.cloudflare.com/","availableLanguage":["English"]},{"@type":"ContactPoint","contactType":"Sales","url":"https://www.cloudflare.com/contact/","availableLanguage":["English"]}]},"isPartOf":{"@type":"WebSite","@id":"https://developers.cloudflare.com/#website","name":"Cloudflare Docs","url":"https://developers.cloudflare.com/"}}
```
