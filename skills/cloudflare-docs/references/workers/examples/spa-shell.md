...
* [  wrangler.jsonc ](#tab-panel-11774)
* [  wrangler.toml ](#tab-panel-11775)

JSONC

```
{  "name": "my-spa",  "main": "src/worker.ts",  // Set this to today's date  "compatibility_date": "2026-06-19",  "compatibility_flags": ["nodejs_compat"],  "assets": {    "directory": "./dist",    "binding": "ASSETS",    "not_found_handling": "single-page-application",    "run_worker_first": ["/*", "!/assets/*"],  },}
```

TOML

```
name = "my-spa"main = "src/worker.ts"# Set this to today's datecompatibility_date = "2026-06-19"compatibility_flags = [ "nodejs_compat" ]
...
```
{  "name": "my-spa-proxy",  "main": "src/worker.ts",  // Set this to today's date  "compatibility_date": "2026-06-19",  "compatibility_flags": ["nodejs_compat"],  "vars": {    "SPA_ORIGIN": "https://my-spa.example-hosting.com",    "API_BASE_URL": "https://api.example.com",  },}
```

TOML

```
name = "my-spa-proxy"main = "src/worker.ts"# Set this to today's datecompatibility_date = "2026-06-19"compatibility_flags = [ "nodejs_compat" ]
[vars]SPA_ORIGIN = "https://my-spa.example-hosting.com"API_BASE_URL = "https://api.example.com"
```
...
* [  JavaScript ](#tab-panel-11780)
* [  TypeScript ](#tab-panel-11781)
...
function App() {  const [data, setData] = useState(window.__BOOTSTRAP_DATA__ || null);  const [loading, setLoading] = useState(!data);
  ...
  if (loading) return <LoadingSpinner />;  return <Dashboard data={data} />;}
```
...
```
declare global {  interface Window {    __BOOTSTRAP_DATA__?: unknown;  }}
```
...
```
new HTMLRewriter()  .on("head", {    element(el) {      el.append(`<meta property="og:title" content="${title}" />`, {        html: true,      });    },  })  .transform(shell);
```
...
```
new HTMLRewriter()  .on("body", {    element(el) {      el.prepend(        `<script>window.__APP_CONFIG__=${JSON.stringify({          apiBase: env.API_BASE_URL,          featureFlags: { darkMode: true },        })}</script>`,        { html: true },      );    },  })  .transform(shell);
```
