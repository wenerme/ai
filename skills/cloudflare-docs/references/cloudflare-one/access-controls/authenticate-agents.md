...
```
CF_TOKEN=$(cloudflared access login https://example.com)curl --header "cf-access-token: $CF_TOKEN" https://example.com/api/endpoint
```
...
Terminal window  
```  
curl --header "CF-Access-Client-Id: $CF_ACCESS_CLIENT_ID" \     --header "CF-Access-Client-Secret: $CF_ACCESS_CLIENT_SECRET" \     https://example.com/api/endpoint  
```
