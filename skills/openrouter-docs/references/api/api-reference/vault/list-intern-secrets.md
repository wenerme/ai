...
````yaml /openapi/openapi.yaml get /vault/interns/{internId}/secrets
openapi: 3.1.0
info:
  contact:
...
paths:
  /vault/interns/{internId}/secrets:
    get:
      tags:
        - Vault
      summary: List intern secrets
      description: >-
        Lists secret metadata stored for one intern. Responses contain names,
        bound hosts, fingerprints and creation times, never secret values.
        Results are ordered by name and paginated with `limit` and `offset`. The
