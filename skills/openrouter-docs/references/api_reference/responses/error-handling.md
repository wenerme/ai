...
These codes are a reduced mapping of the [internal typed error codes](/docs/api_reference/errors-and-debugging#typed-error-codes). Multiple internal error types collapse into each Responses API code — for example, `context_length_exceeded` and `invalid_request` both surface as `invalid_prompt` here.
...
The `error_type` value matches one of the [typed error codes](/docs/api_reference/errors-and-debugging#typed-error-codes) and is stable across all OpenRouter API formats. Use it to programmatically distinguish error categories when the native `code` is ambiguous.
