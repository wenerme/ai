...
[Parallel](https://parallel.ai) supports domain filtering and context size control (`search_context_size`). OpenRouter uses Parallel's Turbo mode at \$0.001 per request, including up to 10 results, then \$0.001 per additional result.
...
| **Parallel**   | \$0.001 per request using OpenRouter credits. Includes up to 10 results in a request, then \$0.001 per additional result                                                                                                                                                                                                                                                              |
