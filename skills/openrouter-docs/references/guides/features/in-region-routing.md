...
* **Web search, except Exa in the US.** The [web search plugin](/docs/guides/features/plugins/web-search) only runs on a regional domain when the request explicitly selects an engine whose backend is resident in that region. Today that is `engine: "exa"` on `us.openrouter.ai`: Exa serves US requests from in-region indexes, so the query never leaves the region. Every other configuration is rejected on regional domains. Perplexity, Parallel, and Firecrawl process queries outside your data region. `engine: "auto"` (or an omitted engine) may resolve to one of those vendors at execution time. `engine: "native"` is not covered by the provider's regional inference commitment. No web search plugin engine is available on `eu.openrouter.ai`.
* **Web search and web fetch server tools.** The [`web_search`](/docs/guides/features/server-tools/web-search) server tool follows the same rule as the plugin: `engine: "exa"` works on `us.openrouter.ai`, and every other configuration is rejected on regional domains. The [`web_fetch`](/docs/guides/features/server-tools/web-fetch) server tool is backed by out-of-region fetch vendors, so it is not available on regional domains.
