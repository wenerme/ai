...
Command line options `--settings`, `--on-init` and `--init-script` can be used to specify [DuckDB configuration](https://duckdb.org/docs/current/configuration/overview.html).
...
| `--settings <list[struct]>` | Set [DuckDB configuration](https://duckdb.org/docs/current/configuration/overview.html), struct should have keys name and value |
