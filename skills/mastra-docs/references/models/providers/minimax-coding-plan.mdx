---
title: "MiniMax Token Plan (minimax.io) Models: IDs, Context & Usage"
description: "Browse MiniMax Token Plan (minimax.io) model IDs, capabilities, and context windows. Learn how to use them in TypeScript with Mastra's model router."
---

# [MiniMax Token Plan (minimax.io) logo]MiniMax Token Plan (minimax.io)
...
> **note**: Mastra uses the OpenAI-compatible `/chat/completions` endpoint. Some provider-specific features may not be available. Check the [MiniMax Token Plan (minimax.io) documentation](https://platform.minimax.io/docs/token-plan/intro) for details.
