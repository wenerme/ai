---
title: "Mixlayer Models: IDs, Pricing & Usage"
description: "Browse Mixlayer model IDs, capabilities, context windows, and pricing. Learn how to use them in TypeScript with Mastra's model router."
---

# [Mixlayer logo]Mixlayer

Access 5 Mixlayer models through Mastra's model router. Authentication is handled automatically using the `MIXLAYER_API_KEY` environment variable.

Learn more in the [Mixlayer documentation](https://docs.mixlayer.com).

```bash title=".env"
MIXLAYER_API_KEY=your-api-key
```

```typescript title="src/mastra/agents/my-agent.ts" {7}
import { Agent } from "@mastra/core/agent";

const agent = new Agent({
  id: "my-agent",
  name: "My Agent",
  instructions: "You are a helpful assistant",
  model: "mixlayer/qwen/qwen3.5-122b-a10b"
});

// Generate a response
const response = await agent.generate("Hello!");

// Stream a response
const stream = await agent.stream("Tell me a story");
for await (const chunk of stream) {
  console.log(chunk);
}
```

> **note**: Mastra uses the OpenAI-compatible `/chat/completions` endpoint. Some provider-specific features may not be available. Check the [Mixlayer documentation](https://docs.mixlayer.com) for details.

## Models

<ProviderModelsTable
  models={[
  {
    "model": "mixlayer/qwen/qwen3.5-122b-a10b",
    "imageInput": false,
    "audioInput": false,
    "videoInput": false,
    "toolUsage": true,
    "reasoning": true,
    "contextWindow": 262144,
    "maxOutput": 262144,
    "inputCost": 0.4,
    "outputCost": 3.2
  },
  {
    "model": "mixlayer/qwen/qwen3.5-27b",
    "imageInput": false,
    "audioInput": false,
    "videoInput": false,
    "toolUsage": true,
    "reasoning": true,
    "contextWindow": 262144,
    "maxOutput": 262144,
    "inputCost": 0.3,
    "outputCost": 2.4
  },
  {
    "model": "mixlayer/qwen/qwen3.5-35b-a3b",
    "imageInput": false,
    "audioInput": false,
    "videoInput": false,
    "toolUsage": true,
    "reasoning": true,
    "contextWindow": 262144,
    "maxOutput": 262144,
    "inputCost": 0.25,
    "outputCost": 1.3
  },
  {
    "model": "mixlayer/qwen/qwen3.5-397b-a17b",
    "imageInput": false,
    "audioInput": false,
    "videoInput": false,
    "toolUsage": true,
    "reasoning": true,
    "contextWindow": 262144,
    "maxOutput": 262144,
    "inputCost": 0.6,
    "outputCost": 3.6
  },
  {
    "model": "mixlayer/qwen/qwen3.5-9b",
    "imageInput": false,
    "audioInput": false,
    "videoInput": false,
    "toolUsage": true,
    "reasoning": true,
    "contextWindow": 262144,
    "maxOutput": 262144,
    "inputCost": 0.1,
    "outputCost": 0.4
  }
]}
/>

Model availability, capabilities, context windows, and pricing are sourced from [models.dev](https://models.dev) and may change.

## Advanced configuration

### Custom headers

```typescript title="src/mastra/agents/my-agent.ts"
const agent = new Agent({
  id: "custom-agent",
  name: "custom-agent",
  model: {
    url: "https://models.mixlayer.ai/v1",
    id: "mixlayer/qwen/qwen3.5-122b-a10b",
    apiKey: process.env.MIXLAYER_API_KEY,
    headers: {
      "X-Custom-Header": "value"
    }
  }
});
```

### Dynamic model selection

```typescript title="src/mastra/agents/my-agent.ts"
const agent = new Agent({
  id: "dynamic-agent",
  name: "Dynamic Agent",
  model: ({ requestContext }) => {
    const useAdvanced = requestContext.task === "complex";
    return useAdvanced
      ? "mixlayer/qwen/qwen3.5-9b"
      : "mixlayer/qwen/qwen3.5-122b-a10b";
  }
});
```
