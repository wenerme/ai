---
title: "Fireworks AI Models: IDs, Pricing & Usage"
description: "Browse Fireworks AI model IDs, capabilities, context windows, and pricing. Learn how to use them in TypeScript with Mastra's model router."
---

# [Fireworks AI logo]Fireworks AI

Access 34 Fireworks AI models through Mastra's model router. Authentication is handled automatically using the `FIREWORKS_API_KEY` environment variable.

Learn more in the [Fireworks AI documentation](https://fireworks.ai/docs/).

```bash title=".env"
FIREWORKS_API_KEY=your-api-key
```

```typescript title="src/mastra/agents/my-agent.ts" {7}
import { Agent } from "@mastra/core/agent";

const agent = new Agent({
  id: "my-agent",
  name: "My Agent",
  instructions: "You are a helpful assistant",
  model: "fireworks-ai/accounts/fireworks/models/deepseek-v4-flash-0731"
});

// Generate a response
const response = await agent.generate("Hello!");

// Stream a response
const stream = await agent.stream("Tell me a story");
for await (const chunk of stream) {
  console.log(chunk);
}
```

> **note**: Mastra uses the OpenAI-compatible `/chat/completions` endpoint. Some provider-specific features may not be available. Check the [Fireworks AI documentation](https://fireworks.ai/docs/) for details.

## Models

<ProviderModelsTable
  models={[
  {
    "model": "fireworks-ai/accounts/fireworks/models/deepseek-v4-flash-0731",
    "imageInput": false,
    "audioInput": false,
    "videoInput": false,
    "toolUsage": true,
    "reasoning": true,
    "contextWindow": 1000000,
    "maxOutput": 384000,
    "inputCost": 0.22,
    "outputCost": 0.66
  },
  {
    "model": "fireworks-ai/accounts/fireworks/models/deepseek-v4-flash-vision-exp",
    "imageInput": true,
    "audioInput": false,
    "videoInput": false,
    "toolUsage": true,
    "reasoning": true,
    "contextWindow": 1000000,
    "maxOutput": 384000,
    "inputCost": 0.22,
    "outputCost": 0.66
  },
  {
    "model": "fireworks-ai/accounts/fireworks/models/deepseek-v4-pro",
    "imageInput": false,
    "audioInput": false,
    "videoInput": false,
    "toolUsage": true,
    "reasoning": true,
    "contextWindow": 1000000,
    "maxOutput": 384000,
    "inputCost": 1.2,
    "outputCost": 1.2
  },
  {
    "model": "fireworks-ai/accounts/fireworks/models/deepseek-v4-pro-0813",
    "imageInput": false,
    "audioInput": false,
    "videoInput": false,
    "toolUsage": true,
    "reasoning": true,
    "contextWindow": 1000000,
    "maxOutput": 384000,
    "inputCost": 1.32,
    "outputCost": 3.96
  },
  {
    "model": "fireworks-ai/accounts/fireworks/models/deepseek-v4p1-flash",
    "imageInput": true,
    "audioInput": false,
    "videoInput": false,
    "toolUsage": true,
    "reasoning": true,
    "contextWindow": 1000000,
    "maxOutput": 384000,
    "inputCost": 0.22,
    "outputCost": 0.66
  },
  {
    "model": "fireworks-ai/accounts/fireworks/models/glm-5p2",
    "imageInput": false,
    "audioInput": false,
    "videoInput": false,
    "toolUsage": true,
    "reasoning": true,
    "contextWindow": 1048575,
    "maxOutput": 131072,
    "inputCost": 1.4,
    "outputCost": 4.4
  },
  {
    "model": "fireworks-ai/accounts/fireworks/models/glm-5p3",
    "imageInput": false,
    "audioInput": false,
    "videoInput": false,
    "toolUsage": true,
    "reasoning": true,
    "contextWindow": 1048573,
    "maxOutput": 262144,
    "inputCost": 1.4,
    "outputCost": 4.4
  },
  {
    "model": "fireworks-ai/accounts/fireworks/models/glm-5p3-flash",
    "imageInput": true,
    "audioInput": false,
    "videoInput": false,
    "toolUsage": true,
    "reasoning": true,
    "contextWindow": 1048573,
    "maxOutput": 131072,
    "inputCost": 0.15,
    "outputCost": 0.5
  },
  {
    "model": "fireworks-ai/accounts/fireworks/models/gpt-oss-120b",
    "imageInput": false,
    "audioInput": false,
    "videoInput": false,
    "toolUsage": true,
    "reasoning": true,
    "contextWindow": 131072,
    "maxOutput": 32768,
    "inputCost": 0.15,
    "outputCost": 0.6
  },
  {
    "model": "fireworks-ai/accounts/fireworks/models/inkling",
    "imageInput": true,
    "audioInput": true,
    "videoInput": false,
    "toolUsage": true,
    "reasoning": true,
    "contextWindow": 1048576,
    "maxOutput": 1048576,
    "inputCost": 1,
    "outputCost": 4.05
  },
  {
    "model": "fireworks-ai/accounts/fireworks/models/kimi-k2p6",
    "imageInput": true,
    "audioInput": false,
    "videoInput": false,
    "toolUsage": true,
    "reasoning": true,
    "contextWindow": 262000,
    "maxOutput": 262000,
    "inputCost": 0.95,
    "outputCost": 4
  },
  {
    "model": "fireworks-ai/accounts/fireworks/models/kimi-k2p7-code",
    "imageInput": true,
    "audioInput": false,
    "videoInput": false,
    "toolUsage": true,
    "reasoning": true,
    "contextWindow": 262000,
    "maxOutput": 262000,
    "inputCost": 0.95,
    "outputCost": 4
  },
  {
    "model": "fireworks-ai/accounts/fireworks/models/kimi-k3",
    "imageInput": true,
    "audioInput": false,
    "videoInput": false,
    "toolUsage": true,
    "reasoning": true,
    "contextWindow": 1048576,
    "maxOutput": 131072,
    "inputCost": 3,
    "outputCost": 15
  },
  {
    "model": "fireworks-ai/accounts/fireworks/models/minimax-m2p7",
    "imageInput": false,
    "audioInput": false,
    "videoInput": false,
    "toolUsage": true,
    "reasoning": true,
    "contextWindow": 196608,
    "maxOutput": 131072,
    "inputCost": 1.2,
    "outputCost": 1.2
  },
  {
    "model": "fireworks-ai/accounts/fireworks/models/minimax-m3",
    "imageInput": false,
    "audioInput": false,
    "videoInput": false,
    "toolUsage": true,
    "reasoning": true,
    "contextWindow": 512000,
    "maxOutput": 512000,
    "inputCost": 0.3,
    "outputCost": 1.2
  },
  {
    "model": "fireworks-ai/accounts/fireworks/models/mistral-large-3-fp8",
    "imageInput": true,
    "audioInput": false,
    "videoInput": false,
    "toolUsage": true,
    "reasoning": false,
    "contextWindow": 262144,
    "maxOutput": 262144,
    "inputCost": null,
    "outputCost": null
  },
  {
    "model": "fireworks-ai/accounts/fireworks/models/muse-glimmer-30b",
    "imageInput": true,
    "audioInput": false,
    "videoInput": false,
    "toolUsage": true,
    "reasoning": true,
    "contextWindow": 131072,
    "maxOutput": 131072,
    "inputCost": 0.35,
    "outputCost": 1.5
  },
  {
    "model": "fireworks-ai/accounts/fireworks/models/nemotron-3-ultra-nvfp4",
    "imageInput": false,
    "audioInput": false,
    "videoInput": false,
    "toolUsage": true,
    "reasoning": true,
    "contextWindow": 262144,
    "maxOutput": 128000,
    "inputCost": 0.6,
    "outputCost": 2.4
  },
  {
    "model": "fireworks-ai/accounts/fireworks/models/nemotron-lightning-3p5-30b-a3b",
    "imageInput": false,
    "audioInput": false,
    "videoInput": false,
    "toolUsage": true,
    "reasoning": true,
    "contextWindow": 262144,
    "maxOutput": 262144,
    "inputCost": 0.05,
    "outputCost": 0.2
  },
  {
    "model": "fireworks-ai/accounts/fireworks/models/qwen3p7-plus",
    "imageInput": true,
    "audioInput": false,
    "videoInput": false,
    "toolUsage": true,
    "reasoning": true,
    "contextWindow": 262144,
    "maxOutput": 65536,
    "inputCost": 0.4,
    "outputCost": 1.6
  },
  {
    "model": "fireworks-ai/accounts/fireworks/models/qwen3p8-2p4t-a95b",
    "imageInput": false,
    "audioInput": false,
    "videoInput": false,
    "toolUsage": true,
    "reasoning": true,
    "contextWindow": 262144,
    "maxOutput": 131072,
    "inputCost": 2,
    "outputCost": 6
  },
  {
    "model": "fireworks-ai/accounts/fireworks/models/qwen3p8-max",
    "imageInput": true,
    "audioInput": false,
    "videoInput": false,
    "toolUsage": true,
    "reasoning": true,
    "contextWindow": 262144,
    "maxOutput": 131072,
    "inputCost": 2,
    "outputCost": 6
  },
  {
    "model": "fireworks-ai/accounts/fireworks/routers/deepseek-flash-latest",
    "imageInput": true,
    "audioInput": false,
    "videoInput": false,
    "toolUsage": true,
    "reasoning": true,
    "contextWindow": 1000000,
    "maxOutput": 384000,
    "inputCost": 0.22,
    "outputCost": 0.66
  },
  {
    "model": "fireworks-ai/accounts/fireworks/routers/deepseek-pro-latest",
    "imageInput": false,
    "audioInput": false,
    "videoInput": false,
    "toolUsage": true,
    "reasoning": true,
    "contextWindow": 1000000,
    "maxOutput": 384000,
    "inputCost": 1.32,
    "outputCost": 3.96
  },
  {
    "model": "fireworks-ai/accounts/fireworks/routers/glm-5p2-fast",
    "imageInput": false,
    "audioInput": false,
    "videoInput": false,
    "toolUsage": true,
    "reasoning": true,
    "contextWindow": 1048575,
    "maxOutput": 131072,
    "inputCost": 2.1,
    "outputCost": 6.6
  },
  {
    "model": "fireworks-ai/accounts/fireworks/routers/glm-5p3-fast",
    "imageInput": false,
    "audioInput": false,
    "videoInput": false,
    "toolUsage": true,
    "reasoning": true,
    "contextWindow": 1048572,
    "maxOutput": 262144,
    "inputCost": 2.1,
    "outputCost": 6.6
  },
  {
    "model": "fireworks-ai/accounts/fireworks/routers/glm-fast-latest",
    "imageInput": false,
    "audioInput": false,
    "videoInput": false,
    "toolUsage": true,
    "reasoning": true,
    "contextWindow": 1048572,
    "maxOutput": 262144,
    "inputCost": 2.1,
    "outputCost": 6.6
  },
  {
    "model": "fireworks-ai/accounts/fireworks/routers/glm-flash-latest",
    "imageInput": true,
    "audioInput": false,
    "videoInput": false,
    "toolUsage": true,
    "reasoning": true,
    "contextWindow": 1048573,
    "maxOutput": 131072,
    "inputCost": 0.15,
    "outputCost": 0.5
  },
  {
    "model": "fireworks-ai/accounts/fireworks/routers/glm-latest",
    "imageInput": false,
    "audioInput": false,
    "videoInput": false,
    "toolUsage": true,
    "reasoning": true,
    "contextWindow": 1048573,
    "maxOutput": 262144,
    "inputCost": 1.4,
    "outputCost": 4.4
  },
  {
    "model": "fireworks-ai/accounts/fireworks/routers/kimi-fast-latest",
    "imageInput": true,
    "audioInput": false,
    "videoInput": false,
    "toolUsage": true,
    "reasoning": true,
    "contextWindow": 1048576,
    "maxOutput": 131072,
    "inputCost": 4.5,
    "outputCost": 22.5
  },
  {
    "model": "fireworks-ai/accounts/fireworks/routers/kimi-k3-fast",
    "imageInput": true,
    "audioInput": false,
    "videoInput": false,
    "toolUsage": true,
    "reasoning": true,
    "contextWindow": 1048576,
    "maxOutput": 131072,
    "inputCost": 4.5,
    "outputCost": 22.5
  },
  {
    "model": "fireworks-ai/accounts/fireworks/routers/kimi-latest",
    "imageInput": true,
    "audioInput": false,
    "videoInput": false,
    "toolUsage": true,
    "reasoning": true,
    "contextWindow": 1048576,
    "maxOutput": 131072,
    "inputCost": 3,
    "outputCost": 15
  },
  {
    "model": "fireworks-ai/accounts/fireworks/routers/minimax-latest",
    "imageInput": false,
    "audioInput": false,
    "videoInput": false,
    "toolUsage": true,
    "reasoning": true,
    "contextWindow": 512000,
    "maxOutput": 512000,
    "inputCost": 0.3,
    "outputCost": 1.2
  },
  {
    "model": "fireworks-ai/accounts/fireworks/routers/qwen-max-latest",
    "imageInput": true,
    "audioInput": false,
    "videoInput": false,
    "toolUsage": true,
    "reasoning": true,
    "contextWindow": 262144,
    "maxOutput": 131072,
    "inputCost": 2,
    "outputCost": 6
  }
]}
/>

Model availability, capabilities, context windows, and pricing are sourced from [models.dev](https://models.dev) and may change.

## Advanced configuration

### Custom headers

```typescript title="src/mastra/agents/my-agent.ts"
const agent = new Agent({
  id: "custom-agent",
  name: "custom-agent",
  model: {
    url: "https://api.fireworks.ai/inference/v1/",
    id: "fireworks-ai/accounts/fireworks/models/deepseek-v4-flash-0731",
    apiKey: process.env.FIREWORKS_API_KEY,
    headers: {
      "X-Custom-Header": "value"
    }
  }
});
```

### Dynamic model selection

```typescript title="src/mastra/agents/my-agent.ts"
const agent = new Agent({
  id: "dynamic-agent",
  name: "Dynamic Agent",
  model: ({ requestContext }) => {
    const useAdvanced = requestContext.task === "complex";
    return useAdvanced
      ? "fireworks-ai/accounts/fireworks/routers/qwen-max-latest"
      : "fireworks-ai/accounts/fireworks/models/deepseek-v4-flash-0731";
  }
});
```
