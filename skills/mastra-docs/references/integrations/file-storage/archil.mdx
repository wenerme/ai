...
description: "Store Mastra workspace files on Archil elastic serverless disks with S3-compatible operations, POSIX shell access, snapshots, and persistent volumes."
packages:
  - "@mastra/archil"
---
...
Stores files on [Archil](https://docs.archil.com) elastic, serverless disks. Combines an S3-compatible object API for fast reads/writes with `exec()` for POSIX shell operations and `diskGrep()` for parallel server-side search. For interface details, see [WorkspaceFilesystem Interface](/reference/workspace/filesystem).
