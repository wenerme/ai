...
description: "Use trajectory accuracy scorers to compare an agent or workflow's tool and step sequence with an expected execution path or ordered trajectory."
packages:
  - "@mastra/evals"
  - "@mastra/core"
---
    ...
    properties: [
      {
        ...
        description:
          "How step ordering is evaluated. 'strict' gives full credit only for an exact match and penalizes extra steps. 'relaxed' allows additional steps as long as expected steps appear in order. 'unordered' only checks that expected steps are present.",
        isOptional: true,
