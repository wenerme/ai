...
<PropertiesTable
  ...
  },
  {
    ...
    description: 'Step type to match. If omitted, matches any step type with the given name.',
    isOptional: true,
  },
  {
    name: 'data',
    type: 'Record<string, unknown>',
    description:
      "Expected step data. Compared against the actual step's type-specific data (toolArgs for tool_call, output for workflow_step, etc.).",
    isOptional: true,
  },
  {
    name: 'children',
...
<PropertiesTable
  content={[
  {
    ...
    type: 'TrajectoryExpectation',
    description:
      'Static expected trajectory to compare against. When provided, all dataset items use this trajectory. When omitted, the scorer reads expectedTrajectory from each dataset item at runtime.',
    isOptional: true,
    properties: [
      {
        name: 'steps',
        type: 'ExpectedStep[]',
        description: 'Ordered list of expected steps. See [Expected steps](#expected-steps) for the type definition.',
        isOptional: true,
      },
    ],
  },
  {
    ...
    properties: [
      ...
      {
        name: 'compareStepData',
        type: 'boolean',
        description:
          'Whether to compare step-specific data (toolArgs/toolResult for tool calls, output for workflow steps) in addition to step names.',
        isOptional: true,
        defaultValue: 'false',
      },
      {
...
<PropertiesTable
  ...
  },
  {
    ...
    type: 'TrajectoryExpectation',
    description:
      'Optional static expected trajectory to compare against. When omitted, the LLM evaluates the trajectory based on the task requirements alone. Can also come from dataset items at runtime.',
    isOptional: true,
  },
]}
/>
...
<PropertiesTable
  content={[
  {
    ...
    properties: [
      ...
      },
      {
        name: 'compareStepData',
        type: 'boolean',
        description: 'Whether to compare step-specific data in addition to step names.',
        isOptional: true,
        defaultValue: 'false',
      },
      {
  ...
  },
  {
    name: 'weights',
    type: 'object',
    description: 'Weights for combining dimension scores into the final score.',
    isOptional: true,
    properties: [
      {
        name: 'accuracy',
      ...
      },
      {
        name: 'toolFailures',
        type: 'number',
        description: 'Weight for tool failures dimension.',
        isOptional: true,
        defaultValue: '0.3',
      },
    ],
...
1. **Tool failures** — Detects retry patterns, fallback patterns, and argument correction patterns.
