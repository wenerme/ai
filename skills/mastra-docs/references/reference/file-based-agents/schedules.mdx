...
**Storage required.** Because schedules are persisted rows and rows in an in-memory store don't survive a restart, the instance needs [storage](/reference/file-based-agents/storage) configured.
