...
description: "UnixSocketPubSub is a PubSub implementation that delivers events across processes on a single host using a Unix domain socket."
packages:
  - "@mastra/core"
---
  ...
  },
  {
    ...
    properties: [
      ...
      },
      {
        ...
        type: 'number',
        description:
          'Maximum size in bytes of a single incoming frame. Must be a positive finite number. A connection that sends a larger frame, or an unterminated partial frame beyond this size, is disconnected so one peer cannot exhaust process memory.',
        isOptional: true,
        defaultValue: '67108864',
      },
    ],
