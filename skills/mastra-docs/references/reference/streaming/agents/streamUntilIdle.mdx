...
description: "streamUntilIdle() streams an agent's response and keeps the stream open until every background task dispatched during the run completes."
packages:
  - "@mastra/core"
---
...
> **note**: `streamUntilIdle()` requires both a [`BackgroundTaskManager`](/reference/configuration#backgroundtasks) and a [memory](/docs/memory/overview) backend. Without either, it uses a plain `agent.stream()` call.
