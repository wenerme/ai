...
Return a short display name for the active model: the last segment of the model ID (for example, `__AI_SDK_ANTHROPIC_MODEL_SONNET__` from `__GATEWAY_ANTHROPIC_MODEL_SONNET__`). Returns `'unknown'` when no model is selected.
