...
<PropertiesTable
  ...
  },
  {
    ...
    type: 'boolean',
    description:
      'Adds a required `description` argument, listed before `command`, where the model says in a few words what the command does. Use it to show a readable label in place of the raw command. When `false`, the tool schema has no `description` argument.',
    isOptional: true,
    defaultValue: 'false',
  },
]}
