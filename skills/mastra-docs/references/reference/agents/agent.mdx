...
description: "The Agent class is the foundation for creating AI agents in Mastra. It provides methods for generating responses and streaming interactions."
packages:
  - "@mastra/core"
---
...
<PropertiesTable
  content={[
  {
    ...
    type: 'string',
    isOptional: true,
    description: 'Resource ID for the memory thread. Required with queueOwnerId.',
  },
  { name: 'threadId', type: 'string', description: 'Thread containing the pending signals.' },
  {
    ...
    isOptional: true,
    description:
      'IDs to cancel across Agents on the thread. Duplicate and unknown IDs are ignored. Supply this or queueOwnerId, not both.',
  },
  {
    name: 'queueOwnerId',
    type: 'string',
    isOptional: true,
    description:
      "Owner group to cancel within the calling Agent's queued messages. Supply this or signalIds, not both.",
  },
]}
/>
...
<PropertiesTable
  ...
  { name: 'threadId', type: 'string', description: 'Thread to abort.' },
  {
    ...
    defaultValue: 'false',
    description:
      'Clear pending signals before aborting. Includes signals waiting for preparation or a lease, but not running work or continuations from continueWithMessages().',
  },
]}
/>
    ...
    properties: [
      {
        ...
        parameters: [
          ...
          },
          {
            ...
            defaultValue: '[]',
            description:
              'Use true to hide all recognized signals, false to show all, or an array to hide selected types from this subscription, including live, idle-persisted, and replayed signals. Other subscribers and the initiating stream keep their own policies.',
          },
          {
            name: 'withInitialHistory',
...
Exclusions don't change model context, storage, shared broadcasts, or another caller's output. They aren't a security boundary and don't replace `ifActive`/`ifIdle` delivery policies or `transient` persistence behavior. This option is supported by the in-process core API, not HTTP or client-js subscription requests. See [stream signal visibility](/reference/streaming/agents/stream#signal-visibility) for transform ordering and the distinction from recall's exact stored-type matching and reminder-hidden history default.
...
<PropertiesTable
  ...
  {
    name: 'abort',
    type: '(options?: { clearPendingSignals?: boolean }) => boolean',
    description:
      'Requests an abort for the active run. Pending signals survive unless clearPendingSignals is true. Uses the same cancellation and return semantics as abortThreadStream().',
  },
  {
    name: 'unsubscribe',
    type: '() => void',
