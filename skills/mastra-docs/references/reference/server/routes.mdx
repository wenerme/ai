...
<PropertiesTable
  content={[
  { name: 'resourceId', type: 'string', isOptional: true, description: 'Resource ID for the memory thread.' },
  { name: 'threadId', type: 'string', description: 'Thread containing the pending signals.' },
  { name: 'signalIds', type: 'string[]', description: 'Between 1 and 1,000 nonempty signal IDs to cancel.' },
]}
/>
...
`DELETE /api/datasets/:datasetId/items/:itemId/purge` permanently scrubs the item's stored content from every historical version, deletion tombstone, and linked experiment result. It doesn't create a new dataset version and can't be undone. See [`dataset.purgeItem()`](/reference/datasets/purgeItem) for the complete purge behavior.
