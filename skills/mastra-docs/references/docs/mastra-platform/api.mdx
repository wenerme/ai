...
The endpoints share their query parameters, request bodies, and response types with the Mastra runtime feedback routes. See the [feedback reference](/reference/observability/feedback) for the full contract, including [list query parameters](/reference/observability/feedback#list-query-parameters) and [`FeedbackFilter`](/reference/observability/feedback#feedbackfilter).
...
Responses contain a `feedback` array and page or delta metadata. See [list query parameters](/reference/observability/feedback#list-query-parameters) in the feedback reference for the full parameter contract, including JSON-encoded object filters such as `timestamp`.
