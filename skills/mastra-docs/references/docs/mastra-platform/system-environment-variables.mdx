...
1. Variables you stored on the environment being deployed. Other environments' variables are never read. Projects still on the [legacy pipeline](/docs/mastra-platform/deploy#migrating-from-server-and-studio-deploys) use the variables stored on the project instead.
