...
```typescript title="src/mastra/workflows/ticket-triage.ts"
...
```mermaid
flowchart LR
  accTitle: Mapping data between workflow steps
  accDescr: The workflow runs step1, which invokes an agent and outputs foo. The map operation transforms foo into an object with a bar property before the workflow commits and ends.
  start(( start )) -->|".then(step1)"<br/>in| step1([step1])
  step1 -- ".map()"<br/>out --> mapping["&quot;foo&quot; → { bar: &quot;foo&quot; }"]
  mapping -->|".commit()<br/>out"| endNode(( end ))
  step1 <--> agent([agent])

  class endNode accent
```
...
```mermaid
flowchart LR
  accTitle: Repeat a workflow step with dountil
  accDescr: The workflow runs step1, then repeats step2 until its condition becomes true before continuing to the end.
  start(( start )) -->|".then(step1)"<br/>in| step1([step1])
  step1 -->|".dountil(step2)"<br/>out| step2([step2])
  step2 -->|".commit()"<br/>out| endNode(( end ))
  step2 -- dountil --> step2
...
```mermaid
flowchart LR
  accTitle: Repeat a workflow step with dowhile
  accDescr: The workflow runs step1, then repeats step2 while its condition remains true before continuing to the end.
  start(( start )) -->|".then(step1)"<br/>in| step1([step1])
  step1 -->|".dowhile(step2)"<br/>out| step2([step2])
  step2 -->|".commit()"<br/>out| endNode(( end ))
  step2 -- dowhile --> step2
...
```mermaid
flowchart LR
  accTitle: Run a workflow step for each array item
  accDescr: The foreach operation passes three example items into separate runs of step1, then passes their collected results to step2 before the workflow ends.
  subgraph foreach[".foreach()"]
    direction TB
    step1a([step1])
    step1b([step1])
    step1c([step1])
  ...
  start(( start )) -->|in| step1a
  start -->|in| step1b
  start -->|in| step1c
  foreach -->|".then(step2)"<br/>out| step2([step2])
  step2 -->|".commit()"<br/>out| endNode(( end ))
