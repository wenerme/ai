...
- [Configuration](/docs/observability/config): Base observability config, multiple configs, and serverless flushing
...
> **tip**: For more granular control over sensitive data, consider using the [Sensitive Data Filter](/docs/observability/integrations/processors/sensitive-data-filter) processor, which can redact specific fields (like passwords, tokens, and keys) while preserving the rest of the input/output.
