...
OpenTelemetry-based exporters and the Langfuse exporter build their span and trace names from the entity id, so they're not affected. To name a Langfuse trace, set `metadata.traceName` as described in the [Langfuse integration](/integrations/observability/langfuse).
