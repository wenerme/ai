...
> **note**: Notification delivery requires a storage adapter with notification support, such as [libSQL](/reference/storage/libsql), [PostgreSQL](/reference/storage/postgresql), or [MongoDB](/reference/storage/mongodb). Configure storage on the Mastra instance so `notify()` can store notification records.
...
> **note**: For a complete polling-provider build with notification storage, agent registration, thread subscription, and testing, follow [Building a signal provider](/guides/guide/signal-provider).
