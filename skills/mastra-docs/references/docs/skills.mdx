---
title: "Agent skills"
description: "Attach reusable Agent Skills directly to Mastra agents using inline definitions, filesystem paths, dynamic loaders, and programmatic skill access."
packages:
  - "@mastra/core"
---
...
Attach skills directly through an agent's `skills` config, or configure them on a [workspace](/docs/sandbox/skills). Agent-level skills belong to one agent and can be defined in code without a workspace, while workspace skills are discovered from the filesystem and shared by every agent using that workspace. This page explains how to define and load agent-level skills with per-request resolution.
