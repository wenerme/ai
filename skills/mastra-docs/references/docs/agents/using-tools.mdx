...
> **note**: Visit the [`createTool`](/reference/tools/create-tool) reference for more information on available properties, configurations, and examples.
...
> **note**: See the [`Agent.generate()` reference](/reference/agents/generate) for all runtime options including `toolsets`, `clientTools`, and `prepareStep`.
