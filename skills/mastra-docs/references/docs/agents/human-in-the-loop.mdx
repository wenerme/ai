...
> **note**: Human-in-the-loop uses snapshots to capture request state. Configure a [storage provider](/docs/storage) on your Mastra instance or you'll see a "snapshot not found" error.
...
> **note**: Suspended runs only survive restarts when your Mastra instance is configured with a persistent [storage provider](/docs/storage). The default in-memory store loses snapshots when the process exits.
