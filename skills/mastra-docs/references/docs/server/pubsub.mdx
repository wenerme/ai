...
- **Background tasks**: A task manager dispatches work to a worker group and fans task lifecycle updates out to subscribers, which is how background task streams stay live. See [Background task streaming](/docs/streaming/background-task-streaming).
...
> **note**: Visit the [PubSub reference](/reference/pubsub/base) for the full delivery contract and the configuration options for each backend.
