...
> **info**: This page covers the [`server`](/reference/configuration#server-options) configuration options passed to the `Mastra` constructor. For running Mastra with your own HTTP server (Hono, Express, etc.), visit [Server Adapters](/docs/server/server-adapters).
...
> **info**: Visit the [configuration reference](/reference/configuration#server-options) for a full list of available server options.
