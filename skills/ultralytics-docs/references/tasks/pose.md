...
- **mAP<sup>val</sup>** values are for single-model single-scale on [COCO Keypoints val2017](https://cocodataset.org/) dataset. Reproduce with `yolo pose val data=coco-pose.yaml device=0 nms=False`
- **Speed** averaged over COCO val images using an [Amazon EC2 P4d](https://aws.amazon.com/ec2/instance-types/p4/) instance. Reproduce with `yolo pose val data=coco-pose.yaml batch=1 device=0|cpu nms=False`
