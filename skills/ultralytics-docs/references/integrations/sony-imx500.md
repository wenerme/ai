...
| `quantize` | `int` or `str`   | `8`/auto       | Quantization precision. `8` (INT8/PTQ) is required and auto-enabled for IMX500, compressing the model and speeding up inference with minimal [accuracy](https://www.ultralytics.com/glossary/accuracy) loss on edge devices. Replaces the deprecated `half`/`int8` flags. |
