...
| `nms`      | `bool`, optional          | `None`   | Select raw output (`None`, default), embedded NMS (`True`), or the NMS-free head (`False`). IMX requires `nms=True` for detect, segment and pose and selects it automatically.                                                                                                |
