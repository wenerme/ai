...
| `quantize` | `int` or `str`   | `None`     | Quantization precision: unset (default) or `16` builds the floating-point RKNN model for FP16-capable targets; `8` (INT8) builds an INT8-quantized model and is required for INT8-only targets such as RV1103 and RV1106. RKNN export has no separate FP32 mode. Replaces the deprecated `half`/`int8` flags. |
