...
| `quantize` | `int` or `str`   | `None`          | Quantization precision: `16` (FP16) or `8` (INT8/PTQ; needs calibration `data`/`fraction`); `32`/unset is FP32. Replaces the deprecated `half`/`int8` flags. INT8 further compresses the model and speeds up inference with minimal [accuracy](https://www.ultralytics.com/glossary/accuracy) loss on edge devices. |
