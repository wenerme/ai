...
| Model                                                                  | size<sup>(pixels)</sup> | mAP<sup>val50-95</sup>baseline | mAP<sup>val50-95</sup>distilled | mAP<sup>val50-95 (e2e)</sup>baseline | mAP<sup>val50-95 (e2e)</sup>distilled |
| ---------------------------------------------------------------------- | --------------------------- | -------------------------------------- | --------------------------------------- | -------------------------------------------- | --------------------------------------------- |
...
- **mAP<sup>val</sup>** values are for single-model single-scale on the [COCO val2017](https://cocodataset.org/) dataset. Reproduce a distilled row with `yolo val detect model=yolo26n-distill.pt data=coco.yaml device=0`; add `end2end=False` for the non-e2e column.
