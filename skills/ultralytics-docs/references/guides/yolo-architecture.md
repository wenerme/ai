...
=== "YOLOv3"
    ...
    ```mermaid
    flowchart TD
        IN[Input 640x640]:::start --> ST[Conv stem5x stride-2 down to P1-P5]:::proc
        ST --> BB[Darknet-53 backbonestacked Bottleneck]:::proc
        BB --> FPN[Neck FPN onlytop-down Upsample + Concat]:::decide
        FPN --> HD[Detect head3 scales, anchor-based]:::out
...
=== "YOLOv5"
    ...
    ```mermaid
    flowchart TD
        IN[Input 640x640]:::start --> BB[CSP backboneC3 blocks]:::proc
        BB --> SP[SPPF]:::extern
        SP --> FPN[Neck FPN top-downUpsample + Concat]:::decide
        FPN --> PAN[Neck PAN bottom-upConv + Concat]:::decide
        PAN --> HD[Detect headanchor-based]:::out
        HD --> O[Predictions + NMS]:::out
        classDef start fill:#4CAF50,color:#fff
        classDef proc fill:#2196F3,color:#fff
        classDef decide fill:#FF9800,color:#fff
...
=== "YOLOv8"
    ...
    ```mermaid
    flowchart TD
        IN[Input 640x640]:::start --> BB[CSP backboneC2f blocks]:::proc
        BB --> SP[SPPF]:::extern
        SP --> FPN[Neck FPN top-downUpsample + Concat]:::decide
        FPN --> PAN[Neck PAN bottom-upConv + Concat]:::decide
        PAN --> HD[Detect headanchor-free, decoupled, DFL reg_max 16]:::out
        HD --> O[Predictions + NMS]:::out
        classDef start fill:#4CAF50,color:#fff
        classDef proc fill:#2196F3,color:#fff
        classDef decide fill:#FF9800,color:#fff
...
=== "YOLO11"
    ...
    ```mermaid
    flowchart TD
        IN[Input 640x640]:::start --> BB[CSP backboneC3k2 blocks]:::proc
        BB --> SP[SPPF]:::extern
        SP --> PSA[C2PSA attention]:::extern
        PSA --> FPN[Neck FPN top-downUpsample + Concat]:::decide
        FPN --> PAN[Neck PAN bottom-upConv + Concat]:::decide
        PAN --> HD[Detect headanchor-free, DFL reg_max 16]:::out
        HD --> O[Predictions + NMS]:::out
        classDef start fill:#4CAF50,color:#fff
        classDef proc fill:#2196F3,color:#fff
        classDef decide fill:#FF9800,color:#fff
...
=== "YOLO26"
    ...
    flowchart TD
        ...
        PSA --> FPN[Neck FPN top-downUpsample + Concat]:::decide
        FPN --> PAN[Neck PAN bottom-upConv + Concat]:::decide
        PAN --> HD[Detect headanchor-free, reg_max 1, dual heads]:::out
        HD --> O[PredictionsNMS by default; optional NMS-free]:::out
        classDef start fill:#4CAF50,color:#fff
        classDef proc fill:#2196F3,color:#fff
        classDef decide fill:#FF9800,color:#fff
        classDef out fill:#9C27B0,color:#fff
