...
> **note**: This filtering functionality is only enabled in production, so in development and in debug build, the full context-menu is always available everywhere.

> **warning**: This filtering functionality is NOT a security measure, the developer should expect that the full context-menu could be leaked anytime which could contain commands like (Download image, Reload, Save webpage), if this is a concern, the developer SHOULD NOT enable the default context-menu.
...
When it is set to `true` the [runtime methods](../reference/runtime/draganddrop.mdx) can be used. 
...
> **note**: Requires Windows 11 build 22621 or later.
...
> **note**: Minimum Windows Version: Windows 10/11 2009/21H2 Build 22000
