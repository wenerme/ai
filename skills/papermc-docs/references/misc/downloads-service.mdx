...
> **danger**: We emphatically **do not recommend** using unstable builds or auto-updaters within production environments.
...
> **note**: We require `jq` to be installed for the examples below. You can install it with `sudo apt-get install jq` on Debian/Ubuntu.
