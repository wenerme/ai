...
> **note**: The constructor takes your main plugin class as an argument in order to create the `Inventory`.
...
> **note**: We pass an instance of our plugin's main class as it's required by the constructor. If you've used the static method and removed the constructor
...
> **note**: You can store the created `MyInventory` instance, e.g. on a `Map<UUID, MyInventory>` for per-player use, or as a field to share the inventory between
