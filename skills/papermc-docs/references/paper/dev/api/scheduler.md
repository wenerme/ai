...
> **caution**: **Large portions of the Bukkit API are not safe to use from within asynchronous tasks**. If a method changes or
accesses the world state, it is not safe to be used from an asynchronous task.

> **note**: While the tasks are executed on separate threads, they are still started from the main thread
