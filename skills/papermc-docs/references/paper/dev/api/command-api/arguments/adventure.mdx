...
## Component argument
> **note**: This argument is very technical. Following the same format as the `/tellraw <player> <component>` command for its second argument, it expects the JSON
...
> **note**: Similar to the component argument, this argument is not really appropriate for general user input, as it also follows the JSON format for displaying components. Most users
...
> **caution**: By default, the consumer passed into `thenAccept` is not executed on the main thread, making it unsafe to use most of Paper API within it.
...
> **note**: A non-player sender is not capable of sending a signed message, which means that the resolved `CompletableFuture` will never be completed.
