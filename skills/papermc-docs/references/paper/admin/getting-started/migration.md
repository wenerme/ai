...
> **note**: Your new Paper server will still use [`bukkit.yml`](/paper/reference/bukkit-configuration)
...
> **note**: These steps assume a `level-name` (as set in `server.properties`) of `world`. If this is not the
