...
> **caution**: When listening to `PluginMessageEvent`, ensure the result is
...
> **caution**: On your backend server, only listen for plugin messages if you are sure only a trusted proxy can send them to your server.
...
> **tip**: This is only really useful for when you are making client-side mods. Otherwise, the player likely will just ignore the message.
