...
> **note**: Some platforms already provide MiniMessage natively. In this case you will not need to add MiniMessage as a dependency.
...
> **tip**: It's a good idea to initialize such a MiniMessage instance once, in a central location, and then use it for all your messages.
...
> **note**: The `Node` API in 4.10.0 is currently not very well developed -- most aspects are still internal. Additional information can be exposed as needed by tag developers.
