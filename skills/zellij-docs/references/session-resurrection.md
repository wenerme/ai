...
[zellij-session-manager-resurrection]
...
Sessions can also be resurrected and switched to from within a Zellij session using the `session-manager`. To do this, press `` to toggle the `EXITED` sessions and select one with `<ENTER>`.
