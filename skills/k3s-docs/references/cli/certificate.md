...
> **warning**: It is not recommended to share Root or Intermediate CAs across multiple clusters, or to use an existing private CA as your cluster CA.
...
> **warning**: You must not overwrite the currently in-use data in `/var/lib/rancher/k3s/server/tls`.  
