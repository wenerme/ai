...
> **note**: `--etcd-snapshot-retention` sets `--etcd-s3-retention`, if it is not explicitly set in the config.
...
> **note**: Pass only the `--etcd-s3` and `--etcd-s3-config-secret` flags to enable the Secret.  
...
> **warning**: 1. Node resources are also included in the etcd snapshot. If restoring to a new set of nodes, you will need to manually delete any old nodes that are no longer present in the cluster.
