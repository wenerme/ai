...
> **warning**: For best results, do NOT simply copy a prerendered `config.toml` into the template and make your desired changes. Use the base template, or provide a full template based on the k3s defaults linked above.
...
sudo docker run \
  ...
  server
```
> **note**: You must specify a valid K3s version as the tag; the `latest` tag is not maintained.  
