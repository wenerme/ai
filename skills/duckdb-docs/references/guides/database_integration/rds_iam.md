...
First, create a secret of type `rds`. It uses the [AWS credential chain](https://duckdb.org/docs/current/core_extensions/aws.html#credential_chain-provider) to mint the IAM token:
...
* [Postgres extension secrets — AWS RDS IAM Authentication](https://duckdb.org/docs/current/core_extensions/postgres/secrets.html#aws-rds-iam-authentication) — the full reference, including verifying the setup with `psql` first.
* [AWS extension](https://duckdb.org/docs/current/core_extensions/aws.html#credential_chain-provider) — the credential chain options available to the `rds` secret.
