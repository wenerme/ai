...
DuckDB is [thoroughly tested](https://duckdb.org/why_duckdb.html#thoroughly-tested) via an extensive test suite.
...
* **Internal errors:** an operation may result in an [`Internal Error`](https://duckdb.org/docs/current/dev/internal_errors.html), e.g.:
...
* **Out of memory errors:** A DuckDB crash can also be a symptom of the operating system killing the process.
  ...
  If your DuckDB session is killed by the OOM reaper, consult the [“OOM errors” page](https://duckdb.org/docs/current/guides/troubleshooting/oom_errors.html)

## Recovering Data

If your DuckDB session was writing to a persistent database file prior to crashing,
there might be a WAL ([write-ahead log](https://en.wikipedia.org/wiki/Write-ahead_logging)) file next to your database named `⟨database_filename⟩.wal`.
To recover data from the WAL file, simply start a new DuckDB session on the persistent database.
DuckDB will then replay the write-ahead log and perform a [checkpoint operation](https://duckdb.org/docs/current/sql/statements/checkpoint.html), restoring the database to the state before the crash.
...
you can try [building it from source](https://duckdb.org/docs/current/dev/building/overview.html).
...
To pinpoint the specific rules that caused the crash, you can try to [selectively disable optimizer rules](https://duckdb.org/docs/current/configuration/pragmas.html#selectively-disabling-optimizers). This way, your query can still benefit from the rest of the optimizer rules.
...
To understand whether this is the case, try reproducing the issue using plain SQL queries with the [DuckDB CLI client](https://duckdb.org/docs/current/clients/cli/overview.html).
