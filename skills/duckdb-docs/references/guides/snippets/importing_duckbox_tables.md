...
By default, the DuckDB [CLI client](https://duckdb.org/docs/current/clients/cli/overview.html) renders query results in the [duckbox format](https://duckdb.org/docs/current/clients/cli/output_formats.html),
which uses rich, ASCII-art inspired tables to show data.
These tables are often shared verbatim in other documents.
For example, take the table used to demonstrate [new CSV features in the DuckDB v1.2.0 release blog post](https://duckdb.org/2025/02/05/announcing-duckdb-120.html#csv-features.md):
...
we can turn the table into a `│`-separated file and read it with DuckDB's [CSV reader](https://duckdb.org/docs/current/data/csv/overview.html).
...
To parse duckbox tables with a single `read_csv` call – and without creating any temporary files –, we can use the [`shellfs` community extension](https://duckdb.org/community_extensions/extensions/shellfs.html):
...
We can also create a [table macro](https://duckdb.org/docs/current/sql/statements/create_macro.html#table-macros):
...
> Please consult the [Securing DuckDB page](https://duckdb.org/docs/current/operations_manual/securing_duckdb/overview.html) for more details.
...
* Only the data types [supported by the CSV sniffer](https://duckdb.org/docs/current/data/csv/auto_detection.html#type-detection) are parsed correctly. Values containing nested data will be parsed as a `VARCHAR`.
