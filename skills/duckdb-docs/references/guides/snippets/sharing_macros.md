...
DuckDB has a powerful [macro mechanism](https://duckdb.org/docs/current/sql/statements/create_macro.html) that allows creating shorthands for common tasks.
...
Make sure that the [`httpfs` extension](https://duckdb.org/docs/current/core_extensions/httpfs/overview.html) is installed:
...
It's also possible to share table macros. For example, we created the [`checksum` macro](https://duckdb.org/2024/10/11/duckdb-tricks-part-2.html#computing-checksums-for-columns) as follows:
...
To use it, make sure that the [`httpfs` extension](https://duckdb.org/docs/current/core_extensions/httpfs/overview.html) is installed:
