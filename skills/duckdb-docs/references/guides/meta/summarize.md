...
Below is an example of `SUMMARIZE` on the `lineitem` table of TPC-H `SF1` table, generated using the [`tpch` extension](https://duckdb.org/docs/current/core_extensions/tpch.html).
...
It is possible to summarize remote tables via the [`httpfs` extension](https://duckdb.org/docs/current/core_extensions/httpfs/overview.html) using the `SUMMARIZE TABLE` statement. For example:
