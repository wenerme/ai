...
SeaweedFS also supports Iceberg: table buckets store the table data as Parquet files, and the gateway's built-in Iceberg REST Catalog serves the table metadata. See the [SeaweedFS catalog example](https://duckdb.org/docs/current/core_extensions/iceberg/catalogs.html#seaweedfs) to query Iceberg tables through it.
