...
To load a Parquet file from S3, the [`httpfs` extension](https://duckdb.org/docs/current/core_extensions/httpfs/overview.html) is required. This can be installed using the `INSTALL` SQL command. This only needs to be run once.
...
DuckDB can also handle [Google Cloud Storage (GCS)](https://duckdb.org/docs/current/guides/network_cloud_storage/gcs_import.html) and [Cloudflare R2](https://duckdb.org/docs/current/guides/network_cloud_storage/cloudflare_r2_import.html) via the S3 API.
