...
DuckDB can read S3 Express One buckets using the [`httpfs` extension](https://duckdb.org/docs/current/core_extensions/httpfs/overview.html).

## Credentials and Configuration

The configuration of S3 Express One buckets is similar to [regular S3 buckets](https://duckdb.org/docs/current/guides/network_cloud_storage/s3_import.html) with one exception:
...
where the `⟨availability_zone⟩` (e.g., `use-az5`) can be obtained from the S3 Express One bucket's configuration page and the `⟨region⟩` is the AWS region (e.g., `us-east-1`).

For example, to allow DuckDB to use an S3 Express One bucket, configure the [Secrets manager](https://duckdb.org/docs/current/sql/statements/create_secret.html) as follows:
...
The “loading only” variant is running the load as part of an [`EXPLAIN ANALYZE`](https://duckdb.org/docs/current/guides/meta/explain_analyze.html) statement to measure the runtime without actually creating a local table, while the “creating local table” variant uses [`CREATE TABLE ... AS SELECT`](https://duckdb.org/docs/current/sql/statements/create_table.html#create-table--as-select-ctas) to create a persistent table on the local disk.
