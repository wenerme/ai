...
When combined with the [`COLUMNS` expression](https://duckdb.org/docs/current/sql/expressions/star.html#columns), we can write very generic SQL-only macros. For example, below is a custom version of `SUMMARIZE` that computes the `min` and `max` of every column in a table:
