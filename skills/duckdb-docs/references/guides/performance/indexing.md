...
DuckDB automatically creates [zonemaps](https://en.wikipedia.org/wiki/Block_Range_Index) (also known as min-max indexes) for the columns of all [general-purpose data types](https://duckdb.org/docs/current/sql/data_types/overview.html#general-purpose-data-types).
...
For an example, let’s repeat the [microbenchmark for timestamps](https://duckdb.org/docs/current/guides/performance/schema.html#microbenchmark-using-timestamps) with an ordered timestamp column using an ascending order vs. an unordered one.
...
First, such an index is created implicitly for columns with `PRIMARY KEY`, `FOREIGN KEY`, and `UNIQUE` [constraints](https://duckdb.org/docs/current/guides/performance/schema.html#constraints).
Second, explicitly running the [`CREATE INDEX`](https://duckdb.org/docs/current/sql/indexes.html) statement creates an ART index on the target column(s).
...
When in doubt, use [`EXPLAIN ANALYZE`](https://duckdb.org/docs/current/guides/meta/explain_analyze.html) to verify that your query plan uses the index scan.
...
> * If you define an ART index, do so after bulk loading the data to the table. Adding an index prior to loading, either explicitly or via primary/foreign keys, is [detrimental to load performance](https://duckdb.org/docs/current/guides/performance/schema.html#microbenchmark-the-effect-of-primary-keys).
