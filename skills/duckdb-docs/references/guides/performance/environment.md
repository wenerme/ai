...
> DuckDB can be compiled to other architecture such as [LoongArch](https://duckdb.org/_everywhere/morefine-m700s.html) and [RISC-V](https://duckdb.org/docs/current/dev/building/unofficial_and_unsupported_platforms.html#risc-v-architectures). However, there are no performance guarantees for these platforms.
...
If you are working in a memory-constrained environment, consider [limiting the number of threads](https://duckdb.org/docs/current/configuration/pragmas.html#threads), e.g., by issuing:
...
Read more in the [“How to Tune Workloads” page](https://duckdb.org/docs/current/guides/performance/how_to_tune_workloads.html#persistent-vs-in-memory-tables).
...
With these in mind, here are two common architectures and the related considerations when you are using DuckDB's [native database format](https://duckdb.org/docs/current/internals/storage.html):
...
If you have a many-core CPU running on a system where DuckDB ships with `jemalloc` as the default memory allocator, consider [enabling the allocator's background threads](https://duckdb.org/docs/current/internals/jemalloc.html#background-threads).
