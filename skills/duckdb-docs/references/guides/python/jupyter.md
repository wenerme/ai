...
> Warning [Variables](https://duckdb.org/docs/current/sql/statements/set_variable.html) are not recognized within a native DuckDB connection.
...
%load_ext sql
```
Either connect to a new [in-memory DuckDB](https://duckdb.org/docs/current/clients/python/dbapi.html#in-memory-connection), the [default connection](https://duckdb.org/docs/current/clients/python/dbapi.html#default-connection), or a file-backed database:
...
> The `%sql` command and `duckdb.sql` share the same [default connection](https://duckdb.org/docs/current/clients/python/dbapi.html) if you provide `duckdb:///:default:` as the SQLAlchemy connection string.
...
DuckDB's [httpfs extension](https://duckdb.org/docs/current/core_extensions/httpfs/overview.html) allows Parquet and CSV files to be queried remotely over http.
...
DuckDB can be used to process local [Parquet files](https://duckdb.org/docs/current/data/parquet/overview.html) as well, which may be desirable if querying the entire Parquet file, or running multiple queries that require large subsets of the file.
