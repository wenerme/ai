...
`PRAGMA` statements that assign a value to an option can also be issued using the [`SET` statement](https://duckdb.org/docs/current/sql/statements/set.html) and the value of an option can be retrieved using `SELECT current_setting(option_name)`.

For DuckDB's built in configuration options, see the [Configuration Reference](https://duckdb.org/docs/current/configuration/overview.html#configuration-reference).
DuckDB [extensions](https://duckdb.org/docs/current/extensions/overview.html) may register additional configuration options.
...
List all tables, with extra information, similarly to [`DESCRIBE`](https://duckdb.org/docs/current/guides/meta/describe.html):
...
| `compression`  | `VARCHAR` | Compression type used for this column – see the [“Lightweight Compression in DuckDB” blog post](https://duckdb.org/2022/10/28/lightweight-compression.html) |
| `stats`        | `VARCHAR` |                                                                                                                                                    |
| `has_updates`  | `BOOLEAN` |                                                                                                                                                    |
...
See [Storage](https://duckdb.org/docs/current/internals/storage.html) for more information.

#### Show Databases

The following statement is equivalent to the [`SHOW DATABASES` statement](https://duckdb.org/docs/current/sql/statements/attach.html):
...
> However, certain in-memory data structures such as [vectors](https://duckdb.org/docs/current/internals/vector.html) and query results are allocated outside of the buffer manager.
> Additionally, [aggregate functions](https://duckdb.org/docs/current/sql/functions/aggregates.html) with complex state (e.g., `list`, `mode`, `quantile`, `string_agg`, and `approx` functions) use memory outside of the buffer manager.
...
Prior to version 1.1.0, DuckDB's [replacement scan mechanism](https://duckdb.org/docs/current/clients/c/replacement_scans.html) in Python scanned the global Python namespace. To revert to this old behavior, use the following setting:
...
The format of this identifier matches the platform name as described in the [extension loading explainer](https://duckdb.org/docs/current/extensions/extension_distribution.html#platforms):
...
The output of [`EXPLAIN`](https://duckdb.org/docs/current/sql/statements/profiling.html) can be configured to show only the physical plan.
...
The profiling documentation contains an overview of the available [metrics](https://duckdb.org/docs/current/dev/profiling.html#metrics).
...
The available optimizations can be queried using the [`duckdb_optimizers()` table function](https://duckdb.org/docs/current/sql/meta/duckdb_table_functions.html#duckdb_optimizers).
...
The `create_fts_index` and `drop_fts_index` options are only available when the [`fts` extension](https://duckdb.org/docs/current/core_extensions/full_text_search.html) is loaded. Their usage is documented on the [Full-Text Search extension page](https://duckdb.org/docs/current/core_extensions/full_text_search.html).
...
When [`CHECKPOINT`](https://duckdb.org/docs/current/sql/statements/checkpoint.html) is called when no changes are made, force a checkpoint regardless:
...
It is also possible to set the block size on a per-file basis, see [`ATTACH`](https://duckdb.org/docs/current/sql/statements/attach.html) for details.
