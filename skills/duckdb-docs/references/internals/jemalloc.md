...
To disable `jemalloc`, [build DuckDB from source](https://duckdb.org/docs/current/dev/building/overview.html) and set the `SKIP_EXTENSIONS` flag as follows:
...
The macOS version of DuckDB does with `jemalloc` but can be [built from source](https://duckdb.org/docs/current/dev/building/macos.html) to include it:
