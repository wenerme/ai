...
[Pivoting](https://duckdb.org/docs/current/sql/statements/pivot.html) is implemented as a combination of SQL query re-writing and a dedicated `PhysicalPivot` operator for higher performance.
...
[`ENUM` types](https://duckdb.org/docs/current/sql/data_types/enum.html) are used to find the distinct values that should become columns.
