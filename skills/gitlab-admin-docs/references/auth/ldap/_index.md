...
| `user_filter`                   | No  | String                        | Filter LDAP users. Follows the format of [RFC 4515](https://www.rfc-editor.org/rfc/rfc4515.html). GitLab does not support `omniauth-ldap`'s custom filter syntax. Examples of the `user_filter` field syntax:- `'(employeeType=developer)'`- `'(&(objectclass=user)(\|(samaccountname=momo)(samaccountname=toto)))'` |
...
| `key`         | Client private key. | `'EXAMPLE_BEGIN_PRIVATE_KEY
