...
Deploy all GitLab components in Kubernetes using Cloud Native reference architectures with four standardized sizes and external PostgreSQL, Redis, and object storage.
...
Cloud Native architectures deploy GitLab components across Kubernetes and external services:
...
- **Supporting services** - Envoy Gateway, Toolbox, and monitoring components
...
- **Reduced operational overhead** - External services for PostgreSQL, Redis, and object storage eliminate database and cache maintenance
...
Before deploying a Cloud Native architecture, ensure you have:
...
| Supporting | Variable per service | 12 vCPU, 48 GB | 12 vCPU, 48 GB | GCP: 3 × n2-standard-4 (4 vCPU, 16 GB)AWS: 3 × c6i.xlarge (4 vCPU, 8 GB) |
...
- **Repository size:** Lightly used monorepos supported. Larger or heavily used monorepos may require a larger architecture size.
...
- **Repository size:** Moderately used monorepos supported. Larger or heavily used monorepos may require a larger architecture size.
...
- **Repository size:** Heavily used monorepos supported. Intensively used monorepos may require a larger architecture size.
...
| Supporting | Variable per service | 24 vCPU, 96 GB | 24 vCPU, 96 GB | GCP: 3 × n2-standard-8 (8 vCPU, 32 GB)AWS: 3 × c6i.2xlarge (8 vCPU, 16 GB) |
...
This section provides supplementary guidance for deploying and operating Cloud Native
architectures, including machine type selection, component-specific considerations, and scaling
strategies.
...
Gitaly in Kubernetes with the Cloud Native architectures uses StatefulSets with the following specifications:
...
- **PostgreSQL and Redis** - Upgrade to larger instance types through your service provider.
...
For detailed deployment steps, see [installing GitLab on Kubernetes](https://docs.gitlab.com/charts/installation/).
...
Key configuration areas for Cloud Native architectures:
