...
Configure GitLab Geo replication between two single-node sites for disaster recovery, supporting Linux package and Docker installations.
...
1. To set up PostgreSQL TLS verification on the secondary site, install `server.crt` for the `gitlab-psql` user:
...
1. Go to the primary node GitLab instance:
   ...
   1. Select **Add site**.
      ...
      [Form to add a new site with three input fields: Name, External URL, and Internal URL (optional).]
...
[The Geo Sites dashboard displaying the synchronization status.]
