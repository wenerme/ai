...
Use Geo for planned failover to migrate GitLab with minimal downtime by following preflight checks and sync steps to promote a secondary site without data loss.
...
1. In the left sidebar, select **Geo** > **Sites**.
   ...
   [Geo admin dashboard showing the synchronization status of a secondary site]
