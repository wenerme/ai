...
1. For the second **Location**:
   ...
   [Route53 traffic policy editor showing a geolocation rule with two locations - Default and Europe - each connected to endpoints with different IP addresses]

1. Select **Create traffic policy**.
1. Fill in **Policy record DNS name** with `gitlab`.

   [Web form for creating DNS policy records with fields for traffic policy, version, hosted zone, and DNS configuration settings]
