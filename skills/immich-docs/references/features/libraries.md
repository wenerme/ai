...
> **info**: Currently an external library can only belong to a single user which is selected when the library is initially created.
...
> **tip**: The `ro` flag at the end only gives read-only access to the volumes.
...
> **info**: _Remember to run `docker compose up -d` to register the changes. Make sure you can see the mounted path in the container._
