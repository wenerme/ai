...
In the [C++ API](https://duckdb.org/docs/current/clients/cpp.html), the Appender works as follows:
...
You can change this by explicitly using [transactions](https://duckdb.org/docs/current/sql/statements/transactions.html) and surrounding your batches of `AppendRow` calls by `BEGIN TRANSACTION` and `COMMIT` statements.
