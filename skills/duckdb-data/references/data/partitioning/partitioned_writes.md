...
When the `PARTITION_BY` clause is specified for the [`COPY` statement](https://duckdb.org/docs/current/sql/statements/copy.html), the files are written in a [Hive partitioned](https://duckdb.org/docs/current/data/partitioning/hive_partitioning.html) folder hierarchy. The target is the name of the root directory (in the example above: `orders`). The files are written in-order in the file hierarchy. Currently, one file is written per thread to each directory.
...
To handle slashes in column names, use Percent-Encoding implemented by the [`url_encode` function](https://duckdb.org/docs/current/sql/functions/text.html#url_encodestring).
