...
When using `read_csv`, the system tries to automatically infer how to read the CSV file using the [CSV sniffer](https://duckdb.org/2023/10/27/csv-sniffer.html).
...
The set of candidate types that should be considered by the CSV reader can be specified explicitly using the [`auto_type_candidates`](https://duckdb.org/docs/current/data/csv/overview.html#auto_type_candidates-details) option. `VARCHAR` as the fallback type will always be considered as a candidate type whether you specify it or not.
...
DuckDB supports the [ISO 8601 format](https://en.wikipedia.org/wiki/ISO_8601) by default for timestamps, dates and times. Unfortunately, not all dates and times are formatted using this standard. For that reason, the CSV reader also supports the `dateformat` and `timestampformat` options. Using this format the user can specify a [format string](https://duckdb.org/docs/current/sql/functions/dateformat.html) that specifies how the date or timestamp should be read.
