...
The [`COPY` statement](https://duckdb.org/docs/current/sql/statements/copy.html) copies data directly into a table. The CSV reader uses the schema of the table instead of auto-detecting types from the file. This speeds up the auto-detection, and prevents mistakes from being made during auto-detection.
...
To load data into _an existing table_ where the table has more columns than the CSV file, you can use the [`INSERT INTO ... BY NAME` clause](https://duckdb.org/docs/current/sql/statements/insert.html#insert-into--by-name):
...
If the [CSV sniffer](https://duckdb.org/2023/10/27/csv-sniffer.html) is not detecting the correct type, try increasing the sample size.
