...
[SDPose image multi-person detection workflow preview]
...
[SDPose video multi-person detection workflow preview]
...
[SDPose-OOD image to pose map workflow preview]
...
<div style={{display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: '1rem', alignItems: 'start'}}>
  [Input image]
  [SDPose-OOD image to pose map example output]
</div>
...
[SDPose-OOD video to pose map workflow preview]
