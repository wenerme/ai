...
### Pixal3D: Multi Views to 3D {#pixal3d-multi-views}

Generate a 3D model from a character turnaround sheet (front, side, and back views on one image). The multiview model uses the extra viewpoints to reconstruct geometry the single-image pipeline cannot see, which gives more accurate side and back profiles.

Use the `Character Turnaround Sheet Generator` template in the template library to produce the turnaround sheet with Nano Banana 2, or prepare your own multi-view image with the subject separated from the background.

[Pixal3D Multi Views workflow preview]

  
    Run this workflow instantly on Comfy Cloud
  
  
    Download JSON or search &quot;Pixal3D: Multi Views to 3D&quot; in Template Library
...
Upload this file to the `LoadImage` node:
    ...
    `LoadImage` node 364 · `character_turnaround_4view_sheet.png`
  

The workflow uses the models listed below. Download them and place them in the corresponding `models/` subdirectories:
