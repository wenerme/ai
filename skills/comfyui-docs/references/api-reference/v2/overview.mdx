...
The practical consequence: `Output.url` is not a shareable link. Put it in an `` that your users' browsers load and they get a `401`, because their browser does not carry your API key. The signed URL is the one you can hand out.
