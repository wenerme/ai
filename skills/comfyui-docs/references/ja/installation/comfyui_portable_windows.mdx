...
translationSourceHash: 62ce5da2
translationFrom: installation/comfyui_portable_windows.mdx
translationBlockHashes:
  "_intro": 9b2047f9
  "Download ComfyUI Portable": df3a115b
  "Additional ComfyUI Portable Instructions": 5df3867f
---

**ComfyUI Portable** は、ComfyUI の実行に必要な独立した **Python (python_embeded)** を統合した、スタンドアロンパッケージの完全な ComfyUI Windows バージョンです。解凍するだけで使用できます。このガイドセクションでは、ComfyUI Portable のインストール手順を説明します。
...
お使いの GPU タイプに合わせてダウンロードパッケージを選択してください。ComfyUI Portable はハードウェアごとに独立したパッケージを提供しています。
    ...
    `run_nvidia_gpu.bat` をダブルクリックして ComfyUI を起動します。
    ...
    ├── README_VERY_IMPORTANT.txt   // ComfyUI Portable の使用説明書（英語）
    ├── run_cpu.bat                 // ダブルクリックして ComfyUI を起動（CPU のみ）
    └── run_amd_gpu.bat             // ダブルクリックして ComfyUI を起動（AMD GPU）
    ```

    `run_amd_gpu.bat` をダブルクリックして ComfyUI を起動します。
    ...
    ├── README_VERY_IMPORTANT.txt   // ComfyUI Portable の使用説明書（英語）
    ├── run_cpu.bat                 // ダブルクリックして ComfyUI を起動（CPU のみ）
    └── run_intel_gpu.bat           // ダブルクリックして ComfyUI を起動（Intel GPU）
    ```

    `run_intel_gpu.bat` をダブルクリックして ComfyUI を起動します。
  

以下の画像に示すように、コマンドが実行されているのが確認できます。

[ComfyUI Portable コマンドプロンプト]
...
### 3. 追加 Python パッケージのインストール

ポータブル版には独立した Python 環境（`python_embeded`）が組み込まれています。追加の Python パッケージ（アテンション最適化、カスタムノード依存関係、その他のツールなど）をインストールするには、コマンドラインから組み込み Python の pip を使用します：

```bash
# まず ComfyUI ポータブルフォルダに移動
cd ComfyUI_windows_portable

# 組み込み Python を使用してパッケージをインストール
.\python_embeded\python.exe -m pip install <パッケージ名>
```

起動フラグが必要なパッケージをインストールした後、`.bat` ファイル（例：`run_nvidia_gpu.bat`）に対応するフラグを追加します：

```bat
.\python_embeded\python.exe -s ComfyUI\main.py --<起動フラグ> --windows-standalone-build
```

カスタムノードの依存関係インストールの詳細については、[カスタムノード依存関係ガイド](/ja/development/core-concepts/custom-nodes)を参照してください。
