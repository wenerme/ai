...
description: "このノードは、DA3GEOMETRY オブジェクトから深度マップを3Dポイントクラウドに変換します。"
mode: wide
---
...
**注意：**
- `batch_index` は入力ジオメトリのバッチサイズより小さい必要があります。それ以外の場合はエラーが発生します。
- フィルタリングですべてのピクセルが削除されると、エラーが発生します。`confidence_threshold` を下げるか `use_sky_mask` を無効にすることで回避できます。
- `confidence_threshold` は、ジオメトリに信頼度マップが含まれる場合にのみ適用されます（Small/Base モデル）。
- `use_sky_mask` は、ジオメトリにスカイマップが含まれる場合にのみ適用されます（Mono/Metric モデル）。
- `downsample` が 1 より大きい場合、カメラ内部パラメータもそれに応じてスケーリングされ、投影された点が正しく保たれます。

## 出力

| 出力名 | 説明 | データ型 |
|-------------|-------------|-----------|
| `point_cloud` | フィルタリングされた3Dポイント、オプションの色、およびオプションの信頼度値を含むポイントクラウド | DA3_POINT_CLOUD |
