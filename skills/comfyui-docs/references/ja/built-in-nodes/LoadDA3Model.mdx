...
description: "このノードは、Depth Anything 3 モデルをファイルから読み込み、深度推定タスクで使用できるように準備します。"
mode: wide
---
...
| `weight_dtype` | モデルの重みの数値精度（データ型）です。"default" はモデルの元の精度を使用し、"fp16" は16ビット浮動小数点数、"bf16" は bfloat16、"fp32" は32ビット浮動小数点数を使用します。（デフォルト: "default"） | STRING | いいえ | `"default"``"fp16"``"bf16"``"fp32"` |
