...
description: "VideoFrameSample ノードは、4 つの戦略のいずれかを使用して、ビデオから固定数のフレームを抽出します。"
mode: wide
---
...
| `戦略` | uniform：等間隔、head：最初のNフレーム、tail：最後のNフレーム、random：ランダムソート（デフォルト："uniform"）。 | COMBO | はい | "uniform""head""tail""random" |
...
- `strategy` が `"uniform"` で `num_frames` が 1 の場合、ビデオの中央フレームが選択されます。
- 認識されない `strategy` 値はエラーを発生させます。
