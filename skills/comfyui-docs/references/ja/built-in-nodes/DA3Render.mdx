...
| `output` | レンダリングする可視化の種類。- depth：正規化されたグレースケール深度画像。- depth_colored：Turbo カラーマップでマッピングされた深度。- sky_mask：天空確率 [0, 1]（Mono/Metric モデルのみ）。- confidence：正規化された深度信頼度（Small/Base モデルのみ）。 | COMBO | はい | "depth""depth_colored""sky_mask""confidence" |
...
| パラメータ | 説明 | データ型 | 必須 | 範囲 |
|-----------|-------------|-----------|----------|-------|
| `normalization` | 深度の正規化方法。- v2_style：知覚的にバランスの取れた結果のための平均/標準偏差正規化（デフォルト）。- min_max：最大コントラストのために深度範囲全体を [0, 1] に引き伸ばします。- raw：スケーリングなしで、Metric モデルのメートル単位を保持します。 | COMBO | はい | "v2_style""min_max""raw" |
| `apply_sky_clip` | 正規化前に空領域の深度を前景深度の99パーセンタイルにクリップします。da3_geometry 入力に sky キーが必要です（Mono/Metric モデルのみ）。デフォルト：False | BOOLEAN | はい | TrueFalse |

`output` が `"sky_mask"` に設定されている場合：

| パラメータ | 説明 | データ型 | 必須 | 範囲 |
|-----------|-------------|-----------|----------|-------|
| `colored` | 空マスクに Turbo カラーマップを適用します。デフォルト：False | BOOLEAN | はい | TrueFalse |

`output` が `"confidence"` に設定されている場合：

| パラメータ | 説明 | データ型 | 必須 | 範囲 |
|-----------|-------------|-----------|----------|-------|
| `colored` | 信頼度マップに Turbo カラーマップを適用します。デフォルト：False | BOOLEAN | はい | TrueFalse |
