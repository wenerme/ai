...
| `output` | ジオメトリデータからレンダリングする画像の種類。DirectXとOpenGLは法線マップの緑チャンネルの規則を制御します。DirectX：緑 = -Y下向き（Unreal）。OpenGL：緑 = +Y上向き（Blender、Substance、Unity、glTF）。（デフォルト："depth"） | COMBO | はい | `"depth"``"depth_colored"``"normal_opengl"``"normal_directx"``"mask"` |
