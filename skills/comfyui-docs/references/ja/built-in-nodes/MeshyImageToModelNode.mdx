...
| `should_remesh` | 生成されたメッシュを処理するかどうかを指定します。`"false"` に設定すると、未処理の三角形メッシュが返されます。 | DYNAMIC COMBO | はい | `"true"``"false"` |
| `topology` | リメッシュ後のモデルの目標ポリゴントポロジーです。この入力は、`should_remesh` が `"true"` に設定されている場合のみ使用可能です。 | COMBO | いいえ* | `"triangle"``"quad"` |
...
| `should_texture` | モデルにテクスチャを生成するかどうかを指定します。`"false"` に設定すると、テクスチャフェーズをスキップし、テクスチャのないメッシュが返されます。 | DYNAMIC COMBO | はい | `"true"``"false"` |
