...
| `texture_quality` | テクスチャ生成の品質レベル（デフォルト："standard"）。"detailed"オプションは0.20米ドル、"standard"は0.10米ドルです。 | COMBO | いいえ | "standard""detailed" |
| `texture_alignment` | テクスチャのアライメント方法（デフォルト："original_image"）。"original_image"はテクスチャを元の入力画像に合わせ、"geometry"は3Dジオメトリに合わせます。 | COMBO | いいえ | "original_image""geometry" |
