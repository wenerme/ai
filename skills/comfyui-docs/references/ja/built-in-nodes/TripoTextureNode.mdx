...
| `texture_quality` | COMBO | いいえ | "standard""detailed" | テクスチャ生成の品質レベル（デフォルト："standard"）。"detailed"オプションは0.20米ドル、"standard"は0.10米ドルです。 |
| `texture_alignment` | COMBO | いいえ | "original_image""geometry" | テクスチャのアライメント方法（デフォルト："original_image"）。"original_image"はテクスチャを元の入力画像に合わせ、"geometry"は3Dジオメトリに合わせます。 |
