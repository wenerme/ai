...
| `effect_scene` | COMBO | はい | `"chat"``"dance"``"hug"``"kill"``"kiss"``"pat"``"punch"``"shrug"``"slap"``"tickle"` | 動画生成に適用する特殊効果シーンの種類 |
| `model_name` | COMBO | いいえ | `"kling-v1"``"kling-v1-5"``"kling-v1-6"` | キャラクターエフェクトに使用するモデル（デフォルト："kling-v1"） |
| `mode` | COMBO | いいえ | `"std"``"pro"` | 動画生成モード（デフォルト："std"） |
| `duration` | COMBO | はい | `"5"``"10"` | 生成される動画の長さ（秒単位） |
