...
| `model_version` | 生成に使用するモデルバージョンです。 | COMBO | いいえ | `"v1.4"``"v3.0"``"v3.5"``"v3.6"` |
| `style` | Tripo では現在サポートされておらず、無視されます。古いワークフロー用に維持されています。（デフォルト: `"None"`） | COMBO | いいえ | `"None"``"realistic"``"cartoon"``"sculpture"``"low_poly"` |
| `texture` | テクスチャマップを生成します。オフにすると、ベアジオメトリのみを返し、`pbr` は無視されます。（デフォルト: True） | BOOLEAN | いいえ | TrueFalse |
| `pbr` | PBR マテリアルマップ（ベースカラー、メタリック、ラフネス、ノーマル）です。`texture` が必要です。（デフォルト: True） | BOOLEAN | いいえ | TrueFalse |
| `model_seed` | モデル生成用のランダムシードです。（デフォルト: 42） | INT | いいえ | 0～2147483647 |
| `orientation` | 生成されたモデルの向き設定です。（デフォルト: `"default"`） | COMBO | いいえ | `"default"``"front"``"back"``"left"``"right"``"top"``"bottom"` |
| `texture_seed` | テクスチャ生成用のランダムシードです。（デフォルト: 42） | INT | いいえ | 0～2147483647 |
| `texture_quality` | テクスチャ生成の品質レベルです。`detailed` = HD テクスチャ、`extreme` = 8K Ultra テクスチャ。（デフォルト: `"standard"`） | COMBO | いいえ | `"standard"``"detailed"``"extreme"` |
| `texture_alignment` | テクスチャマッピングの位置合わせ方法です。（デフォルト: `"original_image"`） | COMBO | いいえ | `"original_image"``"geometry"` |
...
| `quad` | クアッドメッシュを出力します。Tripo はクアッドメッシュを FBX として提供するため、結果は FBX 出力に渡され、GLB 出力は空のままです。（デフォルト: False） | BOOLEAN | いいえ | TrueFalse |
| `ジオメトリ品質` | ジオメトリ生成の品質レベルです。（デフォルト: `"standard"`） | COMBO | いいえ | `"standard"``"detailed"` |
| `smart_low_poly` | クリーンで手作り風のトポロジーを持つローポリメッシュです（500～20,000 フェイス、クアッドでは 500～10,000）。単純な被写体に最適で、複雑なものは失敗する場合があります。（デフォルト: False） | BOOLEAN | いいえ | TrueFalse |
| `auto_size` | テクスチャ付きモデルを実世界のメートル単位のサイズにスケーリングします。Tripo はサイズをモデルのシーン変換として保存し、モデルが変換、リギング、リターゲティングされるときにベイクします。テクスチャがない場合は無視されます。（デフォルト: True） | BOOLEAN | いいえ | TrueFalse |
