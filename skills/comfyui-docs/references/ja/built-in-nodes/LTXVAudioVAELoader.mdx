...
| パラメータ | 説明 | データ型 | 必須 | 範囲 |
| --- | --- | --- | --- | --- |
| `ckpt_name` | 読み込む Audio VAE チェックポイントです。ComfyUI の `checkpoints` ディレクトリにあるすべてのファイルから選択できるドロップダウンリストです。 | STRING | はい | `checkpoints` フォルダ内のすべてのファイル。*例：`"audio_vae.safetensors"`* |
