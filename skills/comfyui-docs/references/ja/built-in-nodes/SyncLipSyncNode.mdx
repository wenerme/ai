...
|---------------|-------------|-----------|----------|-------|
| `sync_mode` | 動画と音声の長さが一致しない場合の処理方法です。出力の長さも設定します（デフォルト："bounce"）。 | COMBO | はい | `"bounce"``"cut_off"``"loop"``"silence"``"remap"` |
| `speaker_selection` | 複数の人物が映っている場合に、どの顔にリップシンクを行うかを指定します（デフォルト："default"）。 | COMBO | はい | `"default"``"auto-detect"``"coordinates"` |
