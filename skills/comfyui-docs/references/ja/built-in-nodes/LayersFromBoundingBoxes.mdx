...
| `crop_to_content` | 各フレームを、存在する場合は metadata.content_rect にトリミングし、コンテンツをボックス位置に長方形オフセットを加えた場所に配置します。フレームがパディングされているバッチではオンにしたままにしてください。実際のコンテンツだけを本来の位置に保持します。（デフォルト: true） | BOOLEAN | いいえ | truefalse |
