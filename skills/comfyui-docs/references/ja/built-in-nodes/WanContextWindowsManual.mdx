...
| `コンテキストスケジュール` | コンテキストウィンドウのストライド。 | COMBO | はい | `"static_standard"``"uniform_standard"``"uniform_looped"``"batched"` |
...
| `融合方法` | コンテキストウィンドウをフュージョンする方法（デフォルト: "pyramid"）。 | COMBO | はい | `"pyramid"``"gaussian"``"average"``"overlap"` |
