...
| `sequential_image_generation` | グループ画像生成モード。"disabled"は単一の画像を生成します。"auto"はモデルが複数の関連画像（例：ストーリーシーン、キャラクターバリエーション）を生成するかどうかを決定します。デフォルト: "disabled"。 | STRING | いいえ | "disabled""auto" |
