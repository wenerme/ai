...
translationBlockHashes:
  "Common issues": 6d731b32
  "Installation issues": ad2335a7
  "Network issues": 17fc7007
  "Scanner troubleshooting": 63fe9c50
  "Security policy": b07e2c4f
  "Getting help": f37e18a2
---
...
git URL 経由でカスタムノードをインストールする際、またはナイトリー バージョンを使用する際に `This action is not allowed with this security level configuration` エラーが発生した場合は、`config.ini` ファイルでセキュリティレベルを調整する必要があります。
...
`<USER_DIRECTORY>` のデフォルトは `ComfyUI/user` です。詳細は [設定](/ja/manager/configuration) をご覧ください。
...
git URL 経由またはナイトリー バージョンでカスタムノードをインストールするには、`config.ini` で `security_level = weak` を設定してください。

## サポートを受ける

それでも問題が解決しない場合は：

1. [ComfyUI-Manager GitHub issues](https://github.com/Comfy-Org/ComfyUI-Manager/issues) で類似の問題を確認してください
