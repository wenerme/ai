...
クライアントを選択:
    ...
    ></video>
        ...
        [Claude Desktop — open Customize]
      
      
        **Connectors**（**2** と表示）をクリックします。

        [Claude Desktop — open Connectors]
    ...
    **comfy-cloud** プラグインをインストールします。MCP 接続とスラッシュコマンドを一度に追加します。プラグインは、Claude Code プラグインや Comfy のコミュニティエージェントスキルを提供する [Comfy Skills](https://github.com/Comfy-Org/comfy-skills/) リポジトリを通じて公開されています。
    ...
    ></video>
        ...
        [Codex add MCP server — step 3]
      
      
        1. 保存したサーバーエントリの **Authenticate**（**8** と表示）をクリックします。

        [Codex authenticate MCP server — step 4]
...
### はじめに
    ...
    MCP 互換クライアントならどれでも利用できます。

    **クラウド接続**にはリモート HTTP サポートが必要です。**Claude Code**、**Claude Desktop**、**Cursor**、**Codex**、**OpenClaw** には上記のファーストクラスセットアップ手順が用意されています。**Windsurf**、**Amp** などは OAuth または API キーを使用した同じ URL を利用します。

    **ローカル接続**では、クライアントがサブプロセスとしてローカル stdio サーバーを起動できる必要があります。これによりブラウザベースのクライアントは除外されます。[claude.ai](https://claude.ai) と ChatGPT はリモートコネクタのみを受け付けます。
  
  
    クラウド接続は `https://cloud.comfy.org/mcp` で動作します。

    ローカル接続には URL がありません。クライアントが `comfy-mcp` コマンドを直接起動し、stdio で通信します。
  
  
    はい。それが [ローカル Comfy MCP 接続](#local-comfy-mcp-connection) です。あなたのマシンにインストールされている ComfyUI を操作するため、エージェントは実際に所有しているモデル、LoRA、カスタムノードを認識し、あなたの GPU 上で実行します。
  
  
    はい。ローカルで ComfyUI を実行している場合におすすめです。ほとんどのクライアントは 2 つの MCP サーバーを問題なくホストでき、エージェントはそれらを区別して扱います。各接続はそれぞれ独自のワークフローを実行し、独自の結果を返します。

    ただし、2 つのサインインは**別々**です。どちらか一方でサインインしても、もう一方でもサインインされるわけではありません。同じ Comfy アカウントであっても同様です。
  
  
    エージェントに尋ねてください。エージェントは重い処理を開始する前に、あなたのハードウェアを読み取ります。

    **Mac** では、生成にクラウド接続を使用してください: 現在のオープンウェイトモデルは Apple GPU 上で実用的な速度で動作するには大きすぎます。**専用グラフィックスカードを搭載した PC** では、VRAM 24 GB 以上あれば動画を含むほとんどの処理が可能です。8〜24 GB では画像は問題ありませんが、動画は遅くなるか処理しきれません。8 GB 未満の場合はクラウドをご利用ください。
  
  
    クラウド接続は**パブリックベータ版**です。API、ツール、動作は開発中に変更される可能性があります。問題を報告するには [フィードバック](#feedback) をご覧ください。
  

### コストとアクセス

  
    探索はどちらの接続でも無料です。テンプレート、モデル、ノードの検索には Comfy アカウントのみが必要です。

    **クラウド接続**では、生成の実行には有効な Comfy Cloud サブスクリプションが必要です。新規ユーザーには 5 回の無料実行が提供されます。**ローカル接続**では、実行はあなたのハードウェア上で行われるため無料です。ただし 1 つ例外があります: パートナーモデルはパートナーのインフラストラクチャ上で実行され、クレジットを消費します。
  
  
    OAuth をサポートする対話型クライアント（Claude Code、Claude Desktop、Codex、OpenClaw など）では不要です。

    **Cursor** では MCP 設定に Comfy Cloud API キーが必要です。そこではまだ MCP OAuth は利用できません。ブラウザのないヘッドレス環境や CI セットアップでも同様に必要です。[クラウド接続のセットアップ](#set-up-the-cloud-connection) の **Cursor** タブと**その他のクライアント**タブを参照してください。
  

### 使い方

  
    MCP ツールを自分で呼び出す必要はありません。エージェントが依頼内容に基づいて適切なものを選択します。通常は、利用可能なものを**探索**し（`search_templates`、`search_models`、`search_nodes`）、生成を**実行**し、出力を**待機して取得**します。[エージェントができること](#what-your-agent-can-do) を参照してください。
  
  
    **クラウド接続**では、サーバーはあなたのマシンに書き込みません。`get_output` は一時的な署名付き URL と、シェルで実行するためのダウンロードコマンドを返します。[アップロードとダウンロード](#uploads-and-downloads) を参照してください。

    **ローカル接続**では、ComfyUI がワークスペースの `output/` ディレクトリに書き込み、`fetch_outputs(prompt_id, out_dir)` で完了したジョブのファイルを指定の場所にコピーします。
  
  
    元に戻す必要はありません。最初の接続に加えて 2 つ目の接続を追加してください。

    **ローカル → クラウド**（Cloud GPU やパートナーモデルが必要な場合）: エージェントにサインインを依頼し、クライアントに `https://cloud.comfy.org/mcp` を追加してください。

    **クラウド → ローカル**（自分のモデルとカスタムノードを使いたい場合）: ComfyUI とローカルサーバーをインストールし、クライアントをそのサーバーに向けてください。エージェントがこれらの作業のほとんどを行ってくれます。
  
  
    エージェントに尋ねるだけです。両方の接続を追加したら、ジョブをどこで実行するかを伝えます。「これは Comfy Cloud で実行して」「これはローカルでやって」と言えば、適切な接続を使います。切り替えるモードはなく、実行のたびに再設定する必要もありません。

    ワークフローがマシンに対して重すぎることが判明した場合、エージェントがそれを伝え、代わりに Comfy Cloud で実行するよう提案してくれます。また、1 つの接続しか設定されていない場合は、もう一方を追加するよう依頼してください。[クラウド接続のセットアップ](#set-up-the-cloud-connection) または [ローカル Comfy MCP 接続](#local-comfy-mcp-connection) を参照してください。
  
  
    **クラウド接続**では、何もする必要はありません。ホストされているため、常に最新バージョンが利用可能です。

    **ローカル接続**では、エージェントに対処を依頼してください。その後、**クライアントを再起動**するか新しいセッションを開始します。MCP サーバーはセッション開始時に読み込まれるため、実行中のものは再起動するまで古いバージョンを提供し続けます。
  

### トラブルシューティング

  
    いいえ。スラッシュコマンドは Claude Code プラグインに含まれています。Claude Desktop は同じ MCP サーバーに接続します。自然言語で依頼したりプロンプトピッカーを使えばツールは動作しますが、Claude Code プラグインやスラッシュコマンドはサポートされていません。
  
  
    `/comfy` や `/cloud` というコマンドはありません。コマンドは接続方法に応じて、次の 2 種類の接頭辞のいずれかで表示されます:
    ...
    いずれの場合も、自然言語で依頼するだけで使えます（「…の画像を生成」など）。MCP ツールはモデルが呼び出すもので、スラッシュコマンドは必要ありません。
  
  
    Claude Code では `/mcp` を実行し、**comfy-cloud** を選択して **Authenticate** を選びます。Claude Desktop では、**Customize → Connectors** からコネクタを再び開いてサインインをトリガーします。
