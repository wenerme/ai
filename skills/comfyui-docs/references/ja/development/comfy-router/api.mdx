...
このページの内容はすべて 1 行目、Comfy Router に関するものです。1 つのパートナーモデルを呼び出すのではなく ComfyUI のワークフローグラフ全体を実行したい場合は 2 行目です。同じパッケージに含まれており、[Comfy SDK](/ja/development/api-development/sdks) から始められます。
...
### タイムアウトと収集 {#timeouts-and-collection}
...
1 回の Router 呼び出しは、デフォルトで接続を 10 分間保持することがあります。クライアントのタイムアウトをその上限より上に設定すると、不透明なローカルアボートではなく、型付きの `504` とリクエスト ID を取得できます。アプリケーションがそれほど長く接続を保持できない場合は、[キュー配信](/ja/development/comfy-router/queue) が直ちに `request_id` を返し、後から結果を収集できます。
...
- [キュー配信](/ja/development/comfy-router/queue): 接続を保持せずにリクエストを送信し、ステータスを追跡して結果を収集またはキャンセルします。
