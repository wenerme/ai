...
同じボディを `POST https://api.comfy.org/v2/models/ideogram/ideogram-v4/requests` に送信します。Router は実行が受け付けられるとすぐに `201` と `request_id` を返し、結果は準備が整い次第、このプロセスまたは別のプロセスから収集できます。[キューの配信](/ja/development/comfy-router/queue)では、ステータス、キャンセル、収集について説明しています。
