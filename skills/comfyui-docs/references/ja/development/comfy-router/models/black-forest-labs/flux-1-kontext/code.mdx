...
description: "Comfy Router 経由で FLUX.1 Kontext Pro と Kontext Max を呼び出すための Python、TypeScript、cURL のスニペット、およびリクエストフィールドと結果の形状"
translationSourceHash: fc89b073
...
[Comfy ワークスペース](https://platform.comfy.org/profile/api-keys?onboarding=router)でキーを作成し、`COMFY_API_KEY` としてエクスポートします。Python と TypeScript のスニペットは Comfy SDK（`pip install comfy-sdk` と `npm install @comfyorg/sdk`）を使用します。cURL のスニペットは、生の HTTP で同じ呼び出しを行うものです。
