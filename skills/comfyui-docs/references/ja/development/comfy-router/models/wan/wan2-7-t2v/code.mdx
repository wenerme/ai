...
[Comfy ワークスペース](https://platform.comfy.org/profile/api-keys?onboarding=router)でキーを作成し、`COMFY_API_KEY` としてエクスポートします。Python と TypeScript のスニペットは Comfy SDK（`pip install comfy-sdk` および `npm install @comfyorg/sdk`）を使用しており、cURL スニペットは同じ呼び出しを生の HTTP で行うものです。
...
同じボディを `POST https://api.comfy.org/v2/models/wan/wan2.7-t2v/requests` に送信します。Router は実行が受け付けられるとすぐに `201` と `request_id` を返し、結果は準備ができ次第、このプロセスからでも別のプロセスからでも収集できます。[キュー配信](/ja/development/comfy-router/queue)では、ステータス、キャンセル、収集について説明します。
...
# 環境変数から COMFY_API_KEY を読み取ります。
# submit() の呼び出しごとに独自の Idempotency-Key が発行され、自動リトライで再利用されます。
async def main():
    async with AsyncComfy() as client:
...
# 3. 収集。モデル固有の出力とともに 200、まだ実行中の場合はステータスボディとともに 202 を返します。
...
<ParamField body="input.media" type="object[]">
  wan2.7、wan3.0、happyhorse-1.x モデル用のメディアアセットリスト。ビデオ生成のための参照素材（画像、オーディオ、ビデオ）
...
上限です。アセットが公開 URL の場合はそのバイト列が Comfy を経由しないため、
記載どおりに適用されます。インラインの data: URL は Comfy を経由するため、
転送上限にも直面します。Comfy Router の POST body は合計 100 MiB が上限で、
...
アセットはインラインでも余裕を持って収まります。つまり、1 つのアセットについては
パートナーのルールが効き、複数のアセットにまたがる場合は転送上限が効きます。
100 MiB の Router 上限は各要素ではなくリクエスト全体を制限するためです。
これらの上限に近いものは公開 URL として送信してください。
</ParamField>
...
<ParamField body="input.media[].url" type="string" required>
  メディアファイルの URL。公開 HTTP/HTTPS URL、OSS の一時 URL、または (上記 `media` の説明にあるモデルの項目が対応を示している場合、たとえば happyhorse-1.x の i2v と r2v の表記のように) インラインの `data:{MIME_type};base64,...` URL です。モデルごとのサイズとピクセルの下限、およびインラインペイロードを全体として制限する 100 MiB の Router リクエストボディ上限については、その説明を参照してください。
</ParamField>
...
このプロキシは、解像度も size も指定していないビデオ生成リクエストを拒否します。
...
Router が `GET /v2/models/wan/wan2.7-t2v/openapi.json` で提供するスキーマから生成されています。これは、リクエストがプロバイダーに到達する前に Router が呼び出しを検証する際に使用するのと同じドキュメントです。
