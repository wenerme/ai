...
[あなたの Comfy ワークスペース](https://platform.comfy.org/profile/api-keys?onboarding=router)でキーを作成し、`COMFY_API_KEY` としてエクスポートします。Python と TypeScript のスニペットは Comfy SDK（`pip install comfy-sdk`、`npm install @comfyorg/sdk`）を使用しています。cURL のスニペットは、同じ呼び出しを生の HTTP で行うものです。
...
同じボディを `POST https://api.comfy.org/v2/models/wan/happyhorse-1.1-i2v/requests` に送信します。Router は実行が受け付けられるとすぐに `201` と `request_id` を返し、結果は準備ができた時点で、このプロセスからでも別のプロセスからでも収集できます。[キュー配信](/ja/development/comfy-router/queue)では、ステータス、キャンセル、収集の流れを順に説明しています。
...
async def main():
    async with AsyncComfy() as client:
        ...
        print("request_id:", handle.request_id)  # モデル ID とあわせて、別のプロセスが必要とする情報はこれだけです
...
console.log("requestId:", handle.requestId); // モデル ID とあわせて、別のプロセスが必要とする情報はこれだけです
...
# 3. 収集。モデルのネイティブ出力を含む 200、まだ実行中であればステータスボディを含む 202。
...
転送上限にも達します。Comfy Router の POST ボディは合計 100 MiB が上限で、
超えると 413 が返ります。base64 はペイロードを約 4/3 に膨らませるため、およそ
75 MB を超える単一のインラインアセットは、ここに示すパートナーのルールに到達する
前に拒否されます。そのサイズであれば、パートナー自身の 20MB 上限にある単一の
アセットはインラインで問題なく収まります。したがって、1 つのアセットについては
パートナーのルールが適用され、複数のアセットにまたがる場合は転送上限が適用されます。
...
- wan3.0-video: ビデオ入力なしの場合は [2, 30] の整数。ビデオ入力がある場合は、
...
wan3.0-video の場合、adaptive（デフォルト）が入力メディアの比率と意図に基づいて
適切なアスペクト比を自動的に推奨します。
...
Router が `GET /v2/models/wan/happyhorse-1.1-i2v/openapi.json` で提供するスキーマから生成されています。これは、リクエストがプロバイダーに到達する前に呼び出しを検証するのと同じドキュメントです。
