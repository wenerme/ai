...
[あなたの Comfy ワークスペース](https://platform.comfy.org/profile/api-keys?onboarding=router)でキーを作成し、`COMFY_API_KEY` としてエクスポートします。Python と TypeScript のスニペットは Comfy SDK（`pip install comfy-sdk` および `npm install @comfyorg/sdk`）を使用しています。cURL のスニペットは、同じ呼び出しを生の HTTP で実行するものです。
...
同じリクエストボディを `POST https://api.comfy.org/v2/models/wan/wan2.7-videoedit/requests` に送信します。実行が受け付けられるとすぐに Router は `201` と `request_id` を返し、結果は準備ができた時点で、このプロセスからでも別のプロセスからでも収集できます。[キュー配信](/ja/development/comfy-router/queue)では、ステータス、キャンセル、収集の流れを説明しています。
...
# 環境変数から COMFY_API_KEY を読み取ります。
# submit() を呼び出すたびに独自の Idempotency-Key が生成され、自動再試行で再利用されます。
async def main():
    async with AsyncComfy() as client:
        ...
        print("request_id:", handle.request_id)  # モデル ID とあわせて、別のプロセスが必要とする情報のすべて
...
console.log("requestId:", handle.requestId); // モデル ID とあわせて、別のプロセスが必要とする情報のすべて
...
# 2. ステータスが COMPLETED になるまでポーリングし、各レスポンスが指定する Retry-After 秒の間待機します。
REQUEST_ID="<request_id from the 201 body>"
curl -i https://api.comfy.org/v2/models/wan/wan2.7-videoedit/requests/$REQUEST_ID/status \
  -H "X-API-Key: $COMFY_API_KEY"

# 3. 収集。モデル固有の出力とともに 200、まだ実行中であればステータスボディとともに 202 を返します。
...
<ParamField body="input.img_url" type="string">
  先頭フレーム画像のURLまたはBase64エンコードデータ。wan2.5-i2v-preview および wan2.6-i2v モデルでのみ必須です。happyhorse-1.x の i2v 表記は、ここでは先頭フレームを受け取りません。先頭フレームは type が `first_frame` の `media` 要素として渡します (下記 `media` を参照)。wan2.6-i2v で成功する `img_url` のボディは、happyhorse-1.0-i2v および happyhorse-1.1-i2v ではプロバイダーに拒否されます。画像形式はJPEG、JPG、PNG、BMP、WEBP。解像度は360～2000ピクセル。ファイルサイズは最大10MBです。
</ParamField>

<ParamField body="input.media" type="object[]">
  wan2.7、wan3.0、happyhorse-1.x モデル用のメディアアセットリスト。ビデオ生成に使用する参照素材（画像、オーディオ、ビデオ）を指定します。各要素には type と url フィールドが含まれます。
...
前に拒否されます。そのサイズでは、パートナー自身の 20MB 上限にある単一のアセットは
インラインでも問題なく収まるため、1 つのアセットについてはパートナーのルールが
適用されます。転送上限は複数のアセットにまたがって適用されます。100 MiB の
Router 上限は各要素ではなくリクエスト全体を制限するためです。これらの上限に近い
...
- wan2.6-r2v: 5 秒または 10 秒のみ（15 秒はサポートされません）
- wan2.7-i2v、wan2.7-t2v: [2, 15] の範囲の整数
- wan2.7-r2v、wan2.7-videoedit: [2, 10] の範囲の整数
- wan3.0-video: ビデオ入力がない場合は [2, 30] の範囲の整数。ビデオ入力がある場合は、
...
このプロキシは、解像度と size のどちらも指定していないビデオ生成リクエストを拒否します。
