...
同じリクエストボディを `POST https://api.comfy.org/v2/models/wan/wan2.7-videoedit/requests` に送信します。実行が受け付けられるとすぐに Router は `201` と `request_id` を返し、結果は準備ができた時点で、このプロセスからでも別のプロセスからでも収集できます。[キューイング配信](/ja/development/comfy-router/queue)では、ステータス、キャンセル、収集の流れを説明しています。
