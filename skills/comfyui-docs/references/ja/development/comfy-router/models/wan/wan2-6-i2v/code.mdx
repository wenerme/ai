...
[Comfy ワークスペース](https://platform.comfy.org/profile/api-keys?onboarding=router)でキーを作成し、`COMFY_API_KEY` としてエクスポートします。Python と TypeScript のスニペットは Comfy SDK（`pip install comfy-sdk` と `npm install @comfyorg/sdk`）を使用しており、cURL のスニペットは同じ呼び出しを生の HTTP で行うものです。
...
前に拒否されます。そのサイズであれば、パートナー自身の 20MB の上限にある単一の
アセットはインラインでも余裕をもって収まります。つまり、単一のアセットでは
...
- wan3.0-video: ビデオ入力なしの場合は [2, 30] の整数。ビデオ入力ありの場合は、入力ビデオの再生時間と出力ビデオの再生時間の合計が 30 秒を超えてはなりません。-1 を指定するとスマート再生時間モードが有効になり、モデルが適切な再生時間を選択します
...
wan3.0-video の場合、adaptive（デフォルト）が入力メディアの比率と意図に基づいて適切な比率を自動的に推奨します。
...
このプロキシは、解像度とサイズのいずれも指定されていないビデオ生成リクエストを拒否します。解像度ティアが課金レートを決定するためです。
