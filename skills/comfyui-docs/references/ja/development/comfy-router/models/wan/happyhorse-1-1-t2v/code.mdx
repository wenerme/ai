...
[Comfy ワークスペース](https://platform.comfy.org/profile/api-keys?onboarding=router)でキーを作成し、`COMFY_API_KEY` としてエクスポートします。Python と TypeScript のスニペットは Comfy SDK（`pip install comfy-sdk` と `npm install @comfyorg/sdk`）を使用しています。cURL のスニペットは、同じ呼び出しを raw HTTP で行うものです。
...
# COMFY_API_KEY を環境変数から読み取ります。
# 各 submit() 呼び出しは独自の Idempotency-Key を生成し、自動リトライのためにそれを再利用します。
async def main():
    async with AsyncComfy() as client:
...
# 3. 収集。完了時はモデルのネイティブ出力とともに 200、まだ実行中はステータスボディとともに 202 を返します。
...
- happyhorse-1.x-i2v: first_frame のみで、ちょうど 1 つ。少なくとも 300x300 ピクセル、
  ...
  これらの表記は img_url を一切受け付けません。先頭フレームはここに渡します。
...
上記のアセットごとの「最大 20MB」という数値は、最終的に受け取る画像に対する
パートナー側の上限であり、アセットが公開 URL の場合はそのまま適用されます。
そのバイト列が Comfy を経由しないためです。インラインの data: URL は Comfy を
経由するため、転送の上限にも掛かります。Comfy Router の POST ボディは合計で
100 MiB に制限され、それを超えると 413 が返されます。また base64 はペイロードを
約 4/3 に膨らませるため、およそ 75 MB を超える単一のインラインアセットは、
ここに示すパートナーのルールに到達する前に拒否されます。そのサイズであれば、
パートナー自身が定める 20MB の上限いっぱいの単一アセットでもインラインで余裕を
もって収まるため、アセット 1 つについてはパートナーのルールが効きます。転送の
上限は、それらが複数にわたる場合に効きます。100 MiB の Router の上限は各要素では
なくリクエスト全体を制限するためです。これらの上限に近いものは公開 URL として
送信してください。
</ParamField>
...
<ParamField body="input.media[].url" type="string" required>
  メディアファイルの URL。公開 HTTP/HTTPS URL、OSS の一時 URL、または (上記 `media` の説明にある該当モデルの項目が対応を認めている場合、たとえば happyhorse-1.x の i2v と r2v の表記のように) インラインの `data:{MIME_type};base64,...` URL です。モデルごとのサイズとピクセルの下限、およびインラインペイロード全体を制限する 100 MiB の Router リクエストボディ上限については、その説明を参照してください。
</ParamField>
...
- wan3.0-video: ビデオ入力なしの場合は [2, 30] の範囲の整数。ビデオ入力を使用する場合、
  入力ビデオの再生時間と出力ビデオの再生時間の合計が 30 秒を超えてはなりません。-1 を指定すると
...
`GET /v2/models/wan/happyhorse-1.1-t2v/openapi.json` で Router が提供するスキーマから生成されています。これは、リクエストがプロバイダーに到達する前に呼び出しの検証に使用されるものと同じドキュメントです。
