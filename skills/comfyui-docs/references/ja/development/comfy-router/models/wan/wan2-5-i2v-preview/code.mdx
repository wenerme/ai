...
上記のアセットごとの「最大 20MB」は、最終的に受け取る画像に対する PARTNER 側の
上限です。アセットが公開 URL の場合はそのまま適用されます。そのバイト列が
Comfy を経由しないためです。インラインの data: URL は Comfy を経由するため、
先に低いほうの転送上限に達します。Comfy Router の POST body は合計 10 MiB が上限で、
超えると 413 が返ります。base64 はペイロードを約 4/3 に膨らませるため、およそ
7.5 MiB を超える単一のインラインアセットは、ここに示すパートナーのルールに到達する
前に拒否されます。body が複数のインラインアセットを含む場合はさらに早くなります。
10 MiB は各要素ではなくリクエスト全体を制限するためです。これらの上限に近い
...
<ParamField body="input.media[].url" type="string" required>
  メディアファイルの URL。公開 HTTP/HTTPS URL、OSS の一時 URL、または (上記 `media` の説明にあるモデルの項目が対応すると述べている場合、たとえば happyhorse-1.x の i2v と r2v の表記のように) インラインの `data:{MIME_type};base64,...` URL です。モデルごとのサイズとピクセルの下限、およびパートナーのルールが適用される前にインラインペイロードを制限する 10 MiB の Router リクエストボディ上限については、その説明を参照してください。
</ParamField>
...
- wan2.6-r2v: 5 または 10 秒のみ（15秒には非対応）
...
<ParamField body="parameters.prompt_extend" type="boolean" default="true">
  プロンプトのインテリジェントリライトを有効にするかどうか。デフォルトは true です
</ParamField>
