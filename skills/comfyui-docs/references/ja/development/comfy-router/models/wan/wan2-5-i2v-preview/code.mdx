---
title: "Wan 2.5 I2V Preview を Comfy Router で使用する"
description: "Comfy Router 経由で HTTP を使用して wan/wan2.5-i2v-preview を呼び出すための Python、TypeScript、cURL スニペット、およびリクエストフィールドと結果の形状"
...
Wan 2.5 I2V Preview の API リファレンスです。Wan 2.5 の画像から動画への変換では、静止した最初のフレームを短いビデオに変換し、その動きとカメラの動きはプロンプトで記述します。

## クイックスタート

[Comfy ワークスペース](https://platform.comfy.org/profile/api-keys?onboarding=router)でキーを作成し、`COMFY_API_KEY` としてエクスポートします。Python と TypeScript のスニペットは Comfy SDK（`pip install comfy-sdk` および `npm install @comfyorg/sdk`）を使用しています。cURL のスニペットは、生の HTTP で同じ呼び出しを行います。

**モデル ID:** `wan/wan2.5-i2v-preview`

**エンドポイント:** `POST https://api.comfy.org/v2/models/wan/wan2.5-i2v-preview`
...
同じボディを `POST https://api.comfy.org/v2/models/wan/wan2.5-i2v-preview/requests` に送信します。Router は実行が受け付けられるとすぐに `201` と `request_id` を返し、結果は準備ができ次第、このプロセスからでも別のプロセスからでも収集できます。[キュー配信](/ja/development/comfy-router/queue)では、ステータス、キャンセル、収集について説明しています。
...
async def main():
    async with AsyncComfy() as client:
        ...
        print("request_id:", handle.request_id)  # モデル ID とこれだけで、別のプロセスに必要な情報は揃います

        # リクエストが完了するまでポーリングし、サーバーが指示する Retry-After の時間だけ待機します。
...
console.log("requestId:", handle.requestId); // モデル ID とこれだけで、別のプロセスに必要な情報は揃います
...
# 3. 収集。実行が完了していればモデル本来の出力とともに 200、まだ実行中ならステータスボディとともに 202 を返します。
...
- wan3.0-video: first_frame（最大1）、last_frame（最大1）、reference_image（最大10）、
  ...
  合計再生時間 \<= 15s）、file（最大1、link とは併用不可）、link（最大1、file とは
  併用不可）。reference_*/file/link タイプと first_frame/last_frame タイプは
  同一リクエスト内では排他的です。配列の順序が、プロンプト内でのアセットの参照順序
...
Comfy を経由しないためです。インラインの data: URL は Comfy を経由し、転送上限にも
直面します。Comfy Router の POST body は合計 100 MiB が上限で、超えると 413 が
返ります。base64 はペイロードを約 4/3 に膨らませるため、およそ 75 MB を超える
単一のインラインアセットは、ここに示すパートナーのルールに到達する前に拒否されます。
そのサイズであれば、パートナー自身の上限である 20MB ぎりぎりの単一アセットでも
インラインで余裕を持って収まります。つまり、アセット 1 つについてはパートナーの
ルールが支配的であり、転送上限のほうは複数のアセットにまたがる場合に効いてきます。
100 MiB の Router 上限は各要素ではなくリクエスト全体を制限するためです。これらの
上限に近いものは公開 URL として送信してください。
</ParamField>
...
<ParamField body="input.media[].url" type="string" required>
  メディアファイルの URL。公開 HTTP/HTTPS URL、OSS の一時 URL、または (上記 `media` の説明にあるモデルの項目が対応すると述べている場合、たとえば happyhorse-1.x の i2v と r2v の表記のように) インラインの `data:{MIME_type};base64,...` URL です。モデルごとのサイズとピクセルの下限、およびインラインペイロードを全体として制限する 100 MiB の Router リクエストボディ上限については、その説明を参照してください。
</ParamField>
...
<ParamField body="input.prompt" type="string">
  テキストプロンプト。中国語と英語に対応し、長さは800文字以内です
（wan3.0-video では最大20,000文字。制限を超えた内容は切り捨てられます）。
複数の参照ビデオを使う wan2.6-r2v では、「character1」「character2」などで参照ビデオの順に
被写体を参照できます。例: 「Character1 sings on the roadside, Character2 dances beside it」
wan3.0-video の参照モードでは、「Image 1」「Video 1」「Audio 1」などで、media 配列内の
対応する順序のメディアアセットを参照できます。
</ParamField>

<ParamField body="input.reference_video_urls" type="string[]">
  wan2.6-r2v モデル専用の参照ビデオURL。1～3個のビデオURLの配列です。
入力制限:
- 形式: mp4、mov
- 数量: 1～3本
- 単一ビデオの長さ: 2～30秒
- 単一ファイルサイズ: 最大30MB
- audio_url とは併用できません
参照再生時間: 単一ビデオは最大5秒、2本の場合はそれぞれ最大2.5秒、3本の場合はそれぞれ比例して短くなります。
...
<ParamField body="model" type="string">
  呼び出すモデルのID。このコンポーネントでは制約されません: Comfy Router が `POST /v2/models/wan/{model}` の `{model}` パスセグメントから設定するため、Router の呼び出し元はこれを省略します。`POST /proxy/wan/api/v1/services/aigc/video-generation/video-synthesis` への v1 直接呼び出しでは必須であり、受け付けられる表記の enum は、そのオペレーション自身のコンポーネントである `WanVideoGenerationRequest` にあります。
</ParamField>
...
wan2.7 モデルの場合、指定しなければ解像度ティアに基づいてデフォルトが決まります。
...
- wan3.0-video、wan3.0-video-prime: 480P、720P、1080P（アップストリームのデフォルトは 1080P）
...
スキーマから生成されています。Router は `GET /v2/models/wan/wan2.5-i2v-preview/openapi.json` でこのスキーマを提供しており、リクエストがプロバイダーに到達する前に呼び出しを検証する際にも同じドキュメントを使用します。
...
<ResponseField name="output.actual_prompt" type="string">
  インテリジェントリライト後の実際のプロンプト（ビデオタスク用）
</ResponseField>
...
<ResponseField name="message" type="string">
  失敗したリクエストの詳細情報。`output` の下ではなく、envelope の ROOT で報告されます（リクエストが成功した場合は返されません）。`output.message` にフォールバックする前に、こちらを確認してください。
</ResponseField>
...
ビデオ URL の有効期間は 24 時間です。ビデオを保持する必要がある場合は、速やかにダウンロードしてください。
