...
| モデルタイプ                  | 解像度 | VRAM 使用率 | 初回生成時間 | 2回目の生成時間 |
| --------------------------- | ------ | ----------- | ------------ | --------------- |
...
[Wan2.2 Fun Camera Control ワークフロー]

  
    Comfy Cloud で開く
  
  
    JSON をダウンロード、またはテンプレートライブラリで「Wan2.2 Fun Camera」を検索
...
**入力素材**
    ...
    動画生成の開始フレームです。この画像をダウンロードして使用するか、ご自身の画像に置き換えてください。
  

### 2. モデルリンク

以下のモデルは [Wan_2.2_ComfyUI_Repackaged](https://huggingface.co/Comfy-Org/Wan_2.2_ComfyUI_Repackaged) で入手できます。

**Diffusion モデル**

  
    Wan2.2 Fun Camera 用 High Noise 拡散モデル
  
  
    Wan2.2 Fun Camera 用 Low Noise 拡散モデル
  

**Wan2.2-Lightning LoRA（オプション：高速化用）**

  
    High Noise モデル用 4ステップ高速化 LoRA
  
  
    Low Noise モデル用 4ステップ高速化 LoRA
  

**VAE**

  Wan2.1 VAE（エンコード/デコード用）

**テキストエンコーダー**

  FP8 Scaled テキストエンコーダー
