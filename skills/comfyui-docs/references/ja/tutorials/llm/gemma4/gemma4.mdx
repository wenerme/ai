...
description: "ComfyUI で Google の軽量オープン LLM「Gemma 4」を実行し、テキスト生成、画像理解、動画分析、音声文字起こしを最大 256K コンテキストでサポートします。"
translationSourceHash: a0b55a32
translationFrom: tutorials/llm/gemma4/gemma4.mdx
translationBlockHashes:
  "_intro": 5d74d230
  "Available workflow": 850e970e
...
### Gemma 4：テキスト生成 {#llm_gemma4_text_gen}

テキストプロンプトと、オプションで画像、オーディオ、またはビデオを入力します。設定可能な推論、コーディング、多言語サポートを備えたテキスト出力を生成します。

[Gemma 4 テキスト生成ワークフローのプレビュー]
...
このワークフローは Gemma 4 のコアとなる**テキスト生成**機能を示しています。テキストプロンプトに加えて、オプションで画像、オーディオファイル、またはビデオを追加コンテキストとして受け付け、推論、コーディング、多言語プロンプトに対応した自然言語出力を生成します。
