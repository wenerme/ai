...
---
  ...
  **非推奨**: OpenAI が DALL·E 2 モデルの提供を終了したため、**OpenAI DALL·E 2** ノードは廃止され、ComfyUI から削除されました。代わりに **GPT Image** ノードを使用してください。このページは参考としてのみ残されています。

[OpenAI DALL·E 2 ノードのスクリーンショット]

OpenAI DALL·E 2 は、ComfyUI のパートナーノードシリーズの一つであり、OpenAI の **DALL·E 2** モデルを用いて画像を生成できます。

**OpenAI DALL·E 2** ノードは、OpenAI の画像生成 API を通じて画像を同期的に生成します。テキストプロンプトを受け取り、その説明に一致する画像を返します。
...
テキストプロンプトに基づいて入力画像のマスクされた領域を置き換えるInpainting：
...
対応するサンプルは非常にシンプルです。
[ComfyUI openai-dall-e-2 ワークフローの例]
