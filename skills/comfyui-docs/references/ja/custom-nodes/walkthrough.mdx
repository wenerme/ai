...
このウォークスルーを終えた後は、[バックエンドコード](./backend/server_overview) と [フロントエンドコード](./js/javascript_overview) の詳細に進んでください。

## 基本ノードの作成

### 前提条件

- 動作する ComfyUI の [インストール](/ja/installation/manual_install) 環境。開発には、ComfyUI を手動でインストールすることを推奨します。
- 動作する comfy-cli の [インストール](/ja/comfy-cli/getting-started) 環境。
...
カスタムノードの基本構造については、[こちら](/ja/custom-nodes/backend/server_overview) で詳しく説明されています。
...
ComfyUI がカスタムノードをどのように発見しロードするかについての詳しい説明は、[ノードライフサイクルドキュメント](/ja/custom-nodes/backend/lifecycle) を参照してください。
