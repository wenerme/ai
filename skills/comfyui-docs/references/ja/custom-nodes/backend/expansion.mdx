---
title: "ノード拡張"
description: "バックエンドのカスタムノードで新しいサブグラフを返すノード拡張をマスターしましょう。GraphBuilder の要件とキャッシュ手法を解説します。"
translationSourceHash: 02e00d1f
translationFrom: custom-nodes/backend/expansion.mdx
translationBlockHashes:
  "Node Expansion": 0cf67de0
  "See Also": b5ebf5ed
---
...
サブグラフ内のノードにリテラルではない入力（torch テンソルなど）を渡すことはできますが、これはサブグラフ*内部*のキャッシュを抑制する可能性があります。可能な場合は、ノード自体ではなくサブグラフオブジェクトへのリンクを渡すべきです。（これを容易に行うために、入力の[追加パラメータ](./datatypes#追加パラメータ)内で入力を `rawLink` として宣言できます。）
