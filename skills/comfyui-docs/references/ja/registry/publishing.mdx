...
comfy-cli はインストール済みですか？[まずはこちらを実行してください](/ja/comfy-cli/getting-started)。
...
作業ブランチの名前が `main` 以外の場合（例：`master`）、branches セクションに名前を追加してください。

`pyproject.toml` のバージョン番号を更新してプッシュしてください。レジストリに更新されたノードが表示されるはずです。

Github Action は、`pyproject.toml` ファイルを更新してプッシュするたびに自動実行されます。
