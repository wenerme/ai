...
translationFrom: registry/specifications.mdx, zh/registry/specifications.mdx
---
...
公開アーカイブからファイルを除外する（`includes` の逆）には、公開ページの [`.comfyignore`](/registry/publishing) を参照してください。
