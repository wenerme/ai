...
2. `도움말` 선택
3. `업데이트 확인` 클릭
[ComfyUI 데스크탑 업데이트 확인]

데스크탑 버전은 ComfyUI 핵심 코드와 의존성 업데이트를 포함한 모든 업데이트 과정을 자동으로 처리합니다
...
git checkout master
```
최신 기능과 보안 수정을 위해 정기적으로 업데이트하는 것이 좋으며, 안정적인 버전은 시스템 안정성을 위해 권장됩니다
