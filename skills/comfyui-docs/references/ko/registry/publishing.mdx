...
### 레지스트리 게시용 API 키 생성하기
  ...
  **중요:** 이 API 키는 **사용자 정의 노드를 레지스트리 및 ComfyUI-Manager에 게시하는 데만 사용됩니다**. 유료 파트너 노드를 워크플로우에 사용하려면 [파트너 노드 개요](/tutorials/partner-nodes/overview)를 참조하세요.
...
comfy-cli를 설치하셨나요? [먼저 설치하세요](/comfy-cli/getting-started).
...
이 파일을 리포지토리에 추가하세요. 더 자세한 정보는 [규격](/registry/specifications)을 확인하세요.
...
작업 중인 브랜치 이름이 `main`이 아닌 경우, 예를 들어 `master`라면 branches 섹션에 해당 이름을 추가하세요.

`pyproject.toml`의 버전 번호를 업데이트하여 푸시하세요. 레지스트리에 업데이트된 노드가 표시되는 것을 확인하실 수 있습니다.

GitHub Action은 `pyproject.toml` 파일을 업데이트할 때마다 자동으로 실행됩니다
