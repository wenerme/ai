...
**디퓨전 모델** : 버전 중 하나를 선택하세요:
    ...
    Wan2.1 I2V ATI 14B 확산 모델(fp8 정밀도). <code>ComfyUI/models/diffusion_models/</code>에 배치하세요.
  

**텍스트 인코더** : 버전 중 하나를 선택하세요:

  
    전체 정밀도 텍스트 인코더(품질이 높고 크기가 큽니다). <code>ComfyUI/models/text_encoders/</code>에 배치하세요.
  
  
    FP8 텍스트 인코더(VRAM 사용량이 적습니다). <code>ComfyUI/models/text_encoders/</code>에 배치하세요.
...
**CLIP Vision**
  ...
  참조 이미지 처리를 위한 CLIP Vision 모델. <code>ComfyUI/models/clip_vision/</code>에 배치하세요.
