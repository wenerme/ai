...
[SDPose-OOD 이미지를 포즈 맵으로 변환하는 워크플로 미리보기]
...
<div style={{display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: '1rem', alignItems: 'start'}}>
  [입력 이미지]
  [SDPose-OOD 이미지를 포즈 맵으로 변환한 출력 예시]
</div>
...
[SDPose-OOD 비디오-포즈 맵 변환 워크플로 미리보기]
