...
### TripoSplat: 이미지에서 가우시안 스플래트로 {#3d_triposplat_image_to_gaussian_splat}

단일 2D 이미지를 업로드하세요. 렌더링을 위한 밀도와 예산을 제어할 수 있는 고품질 3D 가우시안 스플래트 표현을 생성합니다.

[TripoSplat 워크플로우]

  
    Comfy Cloud에서 이 워크플로우를 바로 실행하세요
...
이 파일을 해당 `LoadImage` 노드에 업로드하세요:
    ...
    `LoadImage` 노드 99 · `white-hotel-on-rocky-island.png`
