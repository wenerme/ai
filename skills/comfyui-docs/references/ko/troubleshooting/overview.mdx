...
---
   ...
   우리는 많은 피드백 이슈를 받고 있으며, 제출된 대부분의 이슈가 커스텀 노드와 관련되어 있음을 발견했습니다. 따라서 오류 보고서를 제출하기 전에 [커스텀 노드 문제 해결 가이드](/troubleshooting/custom-node-issues)를 반드시 읽어보시기 바랍니다. 이렇게 하면 문제가 ComfyUI 핵심 문제로 인한 것이 아닌지 확인할 수 있습니다.
...
**빠른 해결 방법:**
1. **시스템 요구 사항 확인** - 시스템이 [최소 요구 사항](/installation/system_requirements)을 충족하는지 확인하세요.
...
2. **커스텀 노드 문제인지 확인** - [커스텀 노드 문제 해결 가이드](/troubleshooting/custom-node-issues)를 따르세요.
3. **모델 파일 확인** - 모델 설정은 [모델 문서](/development/core-concepts/models)를 참조하세요.
...
종합적인 데스크탑 설치 문제 해결은 [데스크탑 설치 가이드](/installation/desktop/windows)를 참조하세요.

- **지원되지 않는 장치**: ComfyUI 데스크탑 Windows는 CUDA 지원 NVIDIA GPU만 지원합니다. 다른 GPU는 [ComfyUI 포터블](/installation/comfyui_portable_windows) 또는 [수동 설치](/installation/manual_install)를 사용하세요.
- **설치 실패**: 설치 프로그램을 관리자 권한으로 실행하고, 최소 15GB 디스크 공간을 확보하세요.
- **유지관리 페이지**: 다운로드가 실패하면 [미러 설정](/installation/desktop/windows#mirror-settings)을 확인하세요.
...
아키텍처 불일치, 모델 누락, 로딩 오류 등 종합적인 모델 문제 해결은 전용 [모델 문제](/troubleshooting/model-issues) 페이지를 참조하세요.
...
**해결 방법:**
1. **API 키 유효성 확인** - [사용자 설정](/interface/user)에서 키를 확인하세요.
2. **계정 크레딧 확인** - 충분한 [API 크레딧](/interface/credits)이 있는지 확인하세요.
...
2. **기본 문제 해결 시도:**
   - [기본 워크플로우](/get_started/first_generation)로 테스트
