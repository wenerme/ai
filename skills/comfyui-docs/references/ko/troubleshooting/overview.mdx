...
translationSourceHash: f4943743
translationFrom: troubleshooting/overview.mdx
translationBlockHashes:
  "_intro": 5c70e303
  "Common Issues & Quick Fixes": a018a0e9
  "Installation-Specific Issues": a62b3b77
  "Model-Related Issues": 1fececea
  "Network & API Issues": 995f7fb7
  "Hardware-Specific Issues": fc108da5
  "Getting Help & Reporting Bugs": fcd87672
...
3. **모델 파일 확인** - 모델 설정은 [모델 문서](/ko/basic-concepts/models)를 참조하세요.
...
- **유지관리 페이지**: 다운로드가 실패하면 [미러 설정](/ko/installation/desktop/usage/settings#고급)을 확인하세요.
...
**Blackwell GPU(`sm_103`): 첫 번째 어텐션 레이어에서 생성이 실패함**

Blackwell 세대 NVIDIA GPU에서 PyTorch의 기본 스케일드 닷 프로덕트 어텐션(SDPA)이 cuDNN 백엔드를 선택할 수 있는데, 이 백엔드에는 이 아키텍처에 대한 유효한 실행 계획이 없습니다:

```
cuDNN Frontend error: No valid execution plans built
```

FLUX를 사용하면 UNet의 첫 번째 어텐션 블록에서 발생하므로 이미지가 전혀 생성되지 않습니다.

```bash
# SDPA와 cuDNN을 우회하는 수제 어텐션 구현을 사용합니다
python main.py --use-split-cross-attention

# Blackwell에는 CUDA 12.8 이상의 PyTorch 빌드도 필요합니다.
# cu121로 컴파일된 빌드는 이러한 카드에서 전혀 실행되지 않습니다
```

시작 전에 cuDNN SDPA 백엔드를 프로그래밍 방식으로 비활성화하는 방법(예: `sitecustomize.py`에서
`torch.backends.cuda.enable_cudnn_sdp(False)` 호출)은 작동하지 않습니다. 해당 플래그는 CUDA 초기화
과정에서 유지되지 않기 때문입니다. 실행 플래그가 확실한 해결책입니다.

*이 항목은 [PR #1491](https://github.com/Comfy-Org/docs/pull/1491)에서 추가되었습니다.*
...
Intel GPU에서 이 오류는 XPU 백엔드를 사용할 수 없어 ComfyUI가 CUDA를 사용하려고 했다는 의미입니다. 위의 NVIDIA CUDA 재설치 단계를 따르지 마세요. Intel GPU는 CUDA가 아니라 XPU를 사용합니다.

PyTorch가 Intel GPU를 인식하는지 확인하세요:

```bash
python -c "import torch; print('XPU available:', hasattr(torch, 'xpu') and torch.xpu.is_available())"
```

`True`가 출력되면 ComfyUI가 자동으로 XPU 백엔드를 사용합니다.

`False`가 출력되면:

1. 지원되는 Intel Arc GPU인지 확인하세요 (Arc A-Series, Arc B-Series, 또는 Arc Graphics가 탑재된 Core Ultra 프로세서).
2. [Intel GPU 드라이버](https://www.intel.com/content/www/us/en/developer/articles/tool/pytorch-prerequisites-for-intel-gpu.html)를 설치하거나 업데이트하세요.
3. 아래 옵션 중 하나로 XPU 지원 PyTorch를 설치하세요. CUDA wheel은 설치하지 마세요.
...
# Nightly XPU wheels (성능 개선이 포함될 수 있음)
...
#### 데스크탑 앱 문제
**보내는 곳:** [데스크탑 GitHub 이슈](https://github.com/Comfy-Org/Comfy-Desktop/issues)
