...
translationBlockHashes:
  "_intro": 74fe2b37
  "How to disable all custom nodes?": acf7da1a
  "What is Binary Search?": 7dd95c5a
  "Two Troubleshooting Methods": e27d6d29
  "Using Binary Search for Troubleshooting": 84acbb08
  "How to Fix the Issue": eb2a4de5
  "Reporting Issues": a0fe8b80
---
...
설정 메뉴에서 커스텀 노드를 비활성화한 상태로 Comfy 데스크탑을 시작하세요
[설정 메뉴 - 커스텀 노드 비활성화]
또는 서버를 수동으로 실행하세요:
```bash
...
- ❌ **문제 지속**: 커스텀 노드 문제가 아님 → [문제 보고](#문제-보고)
...
### 1. 커스텀 노드의 프론트엔드 확장 기능 문제 해결
   ...
   ComfyUI를 시작한 후 설정에서 `Extensions` 메뉴를 찾아 이미지에 표시된 단계를 따라 모든 서드파티 확장 기능을 비활성화하세요
      ...
      ComfyUI 프론트엔드에 진입할 수 없다면 프론트엔드 확장 기능 문제 해결 부분을 건너뛰고 [일반 커스텀 노드 문제 해결 방식](#2-일반-커스텀-노드-문제-해결)으로 넘어가세요
...
### 데스크탑 앱 문제
- **GitHub**: [Comfy 데스크탑 이슈](https://github.com/Comfy-Org/Comfy-Desktop/issues)
