...
이 페이지에서는 커스텀 노드를 만드는 과정을 단계별로 안내합니다.

예제에서는 이미지 배치를 받아서 그중 하나의 이미지를 반환합니다. 처음에는 평균적으로 가장 밝은 색상의 이미지를 반환하는 노드를 만든 다음, 선택 기준의 범위를 확장하고, 마지막으로 클라이언트 측 코드를 추가할 것입니다.

이 페이지에서는 Python이나 JavaScript에 대한 지식이 거의 필요하지 않습니다.

이 안내를 마친 후 [백엔드 코드](./backend/server_overview)와 [프론트엔드 코드](./js/javascript_overview)의 세부 정보를 살펴보세요.
...
커스텀 노드의 기본 구조는 [여기](/ko/custom-nodes/backend/server_overview)에서 자세히 설명되어 있습니다.

커스텀 노드는 Python 클래스를 사용해 정의하며, 여기에는 `CATEGORY`, 노드를 새 노드 메뉴 어디에 배치할지 지정하는 항목, `INPUT_TYPES`, 노드가 어떤 입력을 받을지 정의하는 클래스 메서드(자세한 내용은 [뒤에](/ko/custom-nodes/backend/server_overview#input_types) 참조), `RETURN_TYPES`, 노드가 어떤 출력을 내놓을지 정의하는 항목, 그리고 `FUNCTION`, 노드가 실행될 때 호출될 함수 이름이 포함되어야 합니다.
...
ComfyUI가 커스텀 노드를 어떻게 발견하고 로드하는지 자세한 설명은 [노드 라이프사이클 문서](/ko/custom-nodes/backend/lifecycle)를 참조하세요.
...
클라이언트에 Javascript를 추가하려면 커스텀 노드 디렉토리에 `web/js` 하위 디렉토리를 만들고, `__init__.py`의 끝을 수정해 Comfy에 이를 알리도록 `WEB_DIRECTORY`를 내보내세요:
...
Comfy 서버를 중지했다가 다시 시작하고 웹페이지를 새로고침한 뒤 워크플로를 실행하세요.

### 완성된 예제

완성된 예제는 [여기](https://gist.github.com/robinjhuang/fbf54b7715091c7b478724fc4dffbd03)에서 확인할 수 있습니다. 예제 워크플로 [JSON 파일](https://github.com/Comfy-Org/docs/blob/main/public/workflow.json)을 다운로드하거나 아래에서 볼 수 있습니다:
