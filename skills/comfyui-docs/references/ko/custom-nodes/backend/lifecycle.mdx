...
description: "ComfyUI가 커스텀 노드를 로드하는 과정을 설명합니다. __init__.py 임포트, NODE_CLASS_MAPPINGS, NODE_DISPLAY_NAME_MAPPINGS, 프론트엔드 JavaScript용 WEB_DIRECTORY를 다룹니다."
translationSourceHash: 63b407d9
...
이전 버전의 Comfy에서는 `__init__.py`를 통해 자바스크립트 파일을 메인 Comfy 웹 하위 디렉터리로 복사해야 했습니다. 아직도 그런 코드를 볼 수 있을 것입니다. 하지만 그럴 필요는 없습니다.