...
`collapse`는 이름이 잘못 지어졌습니다. 접힌 상태를 _전환_합니다.  
불린 매개변수를 받아 `node.collapsable === false`를 무시할 수 있습니다.
...
연결된 입력을 설명하는 배열의 `upstream_node_id`는 정수 대신 문자열로 표현됩니다.
...
`nodes.output`는 출력이 없는 노드에서는 존재하지 않으며, 빈 리스트가 아닙니다.
