...
description: "ImageRGBToYUV 노드는 RGB 컬러 이미지를 YUV 색 공간으로 변환합니다. 입력으로 RGB 이미지를 받아 Y(휘도), U(청색 투영), V(적색 투영)의 세 가지 개별 채널로 분리합니다. 각 출력 채널은 해당 YUV 구성 요소를 나타내는 별도의 그레이스케일 이미지로 반환됩니다."
mode: wide
---
