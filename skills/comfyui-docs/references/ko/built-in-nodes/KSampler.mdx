...
description: "KSampler는 다음과 같이 작동합니다: 특정 모델과 긍정 및 부정 조건을 기반으로 제공된 원본 잠재 이미지 정보를 수정합니다. 먼저 설정된 시드(seed) 및 디노이즈 강도(denoise strength)에 따라 원본 이미지 데이터에 노이즈를 추가한 후, 사전 설정된 모델(Model)과 긍정("
mode: wide
---
