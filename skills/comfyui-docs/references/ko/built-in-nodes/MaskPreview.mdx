...
description: "MaskPreview 노드는 마스크 데이터를 미리보기 이미지로 저장하여 ComfyUI 출력 디렉터리에 보관함으로써, 워크플로우 실행 중에 마스크 데이터를 시각적으로 확인할 수 있도록 합니다. 입력된 마스크를 이미지 표시에 적합한 형식으로 변환하고, 설정 가능한 파일 이름 접두사를 사용하여 저장합니다."
mode: wide
---
...
| 출력 이름 | 설명 | 데이터 타입 |
| --- | --- | --- |
| `mask` | UI에 표시할 미리보기 이미지 정보 및 메타데이터를 포함합니다. | DICT |
