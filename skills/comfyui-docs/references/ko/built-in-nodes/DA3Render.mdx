...
| `output` | 렌더링할 시각화 유형입니다.- depth: 정규화된 회색조 깊이 이미지.- depth_colored: Turbo 컬러맵으로 매핑된 깊이.- sky_mask: [0, 1] 범위의 하늘 확률(Mono/Metric 모델 전용).- confidence: 정규화된 깊이 신뢰도(Small/Base 모델 전용). | COMBO | 예 | "depth""depth_colored""sky_mask""confidence" |
...
| 매개변수 | 설명 | 데이터 타입 | 필수 | 범위 |
|-----------|-------------|-----------|----------|-------|
| `normalization` | 깊이 정규화 방법입니다.- v2_style: 지각적으로 균형 잡힌 결과를 위한 평균/표준편차 정규화(기본값).- min_max: 최대 대비를 위해 전체 깊이 범위를 [0, 1]로 늘립니다.- raw: 스케일링 없이 Metric 모델의 미터법 단위를 유지합니다. | COMBO | 예 | "v2_style""min_max""raw" |
| `apply_sky_clip` | 정규화 전에 하늘 영역 깊이를 전경 깊이의 99번째 백분위수로 클리핑합니다. da3_geometry 입력에 sky 키가 필요합니다(Mono/Metric 모델 전용). 기본값: False | BOOLEAN | 예 | TrueFalse |

`output`이 `"sky_mask"`로 설정된 경우:

| 매개변수 | 설명 | 데이터 타입 | 필수 | 범위 |
|-----------|-------------|-----------|----------|-------|
| `colored` | 하늘 마스크에 Turbo 컬러맵을 적용합니다. 기본값: False | BOOLEAN | 예 | TrueFalse |

`output`이 `"confidence"`로 설정된 경우:

| 매개변수 | 설명 | 데이터 타입 | 필수 | 범위 |
|-----------|-------------|-----------|----------|-------|
| `colored` | 신뢰도 맵에 Turbo 컬러맵을 적용합니다. 기본값: False | BOOLEAN | 예 | TrueFalse |
