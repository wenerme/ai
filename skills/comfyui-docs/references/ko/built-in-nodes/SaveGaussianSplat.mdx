...
description: "이 노드는 가우시안 스플랫 3D 파일을 출력 디렉토리에 저장합니다. 파일 저장 과정을 처리하고 3D 뷰포트를 위한 미리보기 데이터를 제공합니다."
mode: wide
---
...
|-----------|-------------|-----------|----------|-------|
| `model_3d` | 가우시안 스플랫 3D 파일입니다. | FILE3D | 예 | SplatAnyPLYSPLATSPZKSPLAT |
...
참고: `camera_info` 또는 `model_3d_info`가 제공되지 않으면 노드는 사용 가능할 때 `viewport_state`의 해당 값을 사용합니다.

## 출력

| 출력 이름 | 설명 | 데이터 타입 |
|-------------|-------------|-----------|
| `model_3d` | 출력 디렉토리에 저장된 가우시안 스플랫 3D 파일입니다. | FILE3D |
