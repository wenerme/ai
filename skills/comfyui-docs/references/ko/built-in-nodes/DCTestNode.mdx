...
| --- | --- | --- | --- | --- |
| `combo` | 어떤 입력 필드가 활성화되고 노드가 무엇을 출력할지 결정하는 주요 선택 항목입니다. | COMBO | 예 | `"option1"``"option2"``"option3"``"option4"` |
...
| `subcombo` | `combo`가 `"option4"`로 설정된 경우 나타나는 보조 선택 항목입니다. 중첩된 입력 필드 중 어떤 것이 활성화될지 결정합니다. | COMBO | 아니요 | `"opt1"``"opt2"` |
