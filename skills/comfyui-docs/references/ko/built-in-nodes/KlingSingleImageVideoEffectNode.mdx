...
| `효과 장면` | 비디오 생성에 적용할 특수 효과 장면의 유형입니다. 일부 효과는 가격이 다를 수 있습니다. | COMBO | 예 | `"dizzydizzy"``"bloombloom"``"neon"``"cartoon"``"sketch"``"oil"``"watercolor"``"3d"` |
| `모델 명` | 비디오 효과 생성에 사용할 특정 모델 버전입니다. | COMBO | 예 | `"kling-v1-5"``"kling-v1-6"` |
| `길이` | 생성된 비디오의 길이(초)입니다. | COMBO | 예 | `"5"``"10"` |
