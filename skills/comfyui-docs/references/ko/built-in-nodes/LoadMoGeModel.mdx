...
description: "파일에서 MoGe(Monocular Geometry) 모델을 로드하고 기하 추정 작업에 사용할 수 있도록 준비합니다. 이 노드는 geometryestimation 폴더에서 선택한 모델 파일을 읽고, 저장된 가중치로 MoGe 모델을 초기화합니다."
mode: wide
---
...
| 출력 이름 | 설명 | 데이터 타입 |
| --- | --- | --- |
| `MOGE_MODEL` | 로드된 MoGe 모델 인스턴스로, 기하 추정 워크플로에서 사용할 준비가 되어 있습니다. | MOGE_MODEL |
