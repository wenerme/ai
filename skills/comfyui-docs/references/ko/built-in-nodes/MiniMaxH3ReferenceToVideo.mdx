...
| `ref_image_size` | 참조 이미지 크기 조정입니다. `match`는 각 참조를 생성의 픽셀 영역에 맞게 조정합니다(축소만 하며 종횡비 유지). `max`는 최상의 아이덴티티 충실도를 위해 참조 파이프라인의 2048px 짧은 변을 사용합니다. 참조 토큰은 모든 샘플링 단계를 거치므로 `max`는 몇 배 더 느릴 수 있습니다(기본값: `match`). | COMBO | 예 | `"match"``"max"` |
