...
| `ref_image_size` | 참조 이미지 크기 조정 방식입니다. `match`는 각 참조 이미지의 종횡비를 유지하면서 생성 픽셀 영역에 맞춰 축소만 수행합니다. `max`는 참조 파이프라인의 2048px 짧은 변을 사용하여 최상의 정체성 충실도를 얻습니다. 참조 토큰은 모든 샘플링 단계에서 전달되므로 `max`는 몇 배 더 느릴 수 있습니다(기본값: `match`). | COMBO | 예 | `"match"``"max"` |
