...
description: "이 노드는 원본 이미지의 가로세로 비율을 유지하면서 짧은 쪽 가장자리가 지정된 길이와 일치하도록 이미지 크기를 조정합니다. 짧은 쪽의 목표 길이를 기준으로 새 치수를 계산하고 크기가 조정된 이미지를 반환합니다."
mode: wide
---
...
| `shorter_edge` | 짧은 쪽 가장자리의 목표 길이입니다. (기본값: 512) | INT | 예 | 1~8192 |
