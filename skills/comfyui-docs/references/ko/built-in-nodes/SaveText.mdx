...
| `format` | 텍스트를 저장할 파일 형식입니다 (기본값: "txt"). "json"을 선택하면 유효한 JSON 텍스트는 2칸 들여쓰기로 보기 좋게 출력되며, 유효하지 않은 JSON 및 다른 모든 형식은 원본 그대로 저장됩니다. 나열된 옵션만 지원됩니다. | COMBO | 아니요 | `"txt"``"csv"``"md"``"json"` |
