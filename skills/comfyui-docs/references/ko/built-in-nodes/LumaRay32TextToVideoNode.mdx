...
description: "이 노드는 Luma의 Ray 3.2 모델을 사용하여 텍스트 프롬프트로부터 비디오를 생성합니다. 프롬프트와 비디오 설정을 Luma API로 전송하고, 생성된 비디오와 생성 ID를 반환합니다."
mode: wide
---
...
| `화면비` | 생성된 비디오의 화면 비율입니다. | STRING | 예 | `"16:9"``"9:16"``"1:1"``"4:3"``"3:4"``"21:9"` |
| `해상도` | 비디오의 출력 해상도입니다(기본값: "720p"). | STRING | 예 | `"360p"``"540p"``"720p"``"1080p"` |
| `길이` | 생성된 비디오의 길이입니다. | STRING | 예 | `"5s"``"10s"` |
...
**참고:** `loop` 매개변수는 `duration`이 "5s"로 설정된 경우에만 활성화할 수 있습니다. `duration`을 "10s"로 설정하고 `loop`를 활성화하면 노드에서 오류가 반환됩니다.
