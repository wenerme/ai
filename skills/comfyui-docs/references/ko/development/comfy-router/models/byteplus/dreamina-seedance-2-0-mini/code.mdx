...
[Comfy 워크스페이스](https://platform.comfy.org/profile/api-keys?onboarding=router)에서 키를 생성하고 `COMFY_API_KEY`로 내보내세요. Python과 TypeScript 스니펫은 Comfy SDK(`pip install comfy-sdk`, `npm install @comfyorg/sdk`)를 사용하며, cURL 스니펫은 동일한 호출을 원시 HTTP로 실행한 것입니다.
...
동일한 본문을 `POST https://api.comfy.org/v2/models/byteplus/dreamina-seedance-2-0-mini/requests` 로 보냅니다. Router는 실행이 접수되는 즉시 `201` 과 `request_id` 를 응답하며, 결과는 준비가 되는 대로 이 프로세스나 다른 프로세스에서 수집할 수 있습니다. 상태, 취소, 수집 방법은 [대기 중 전송](/ko/development/comfy-router/queue) 에서 자세히 다룹니다.
...
<ParamField body="content[].audio_url" type="object">
  입력 오디오 객체. Seedance 2.5, 2.0 및 2.0 fast만 오디오 입력을 지원합니다. Seedance 2.0 및 2.0 fast는 오디오만 단독으로 사용할 수 없으며, 최소 1개의 이미지 또는 비디오를 포함해야 합니다. Seedance 2.5는 오디오 전용 입력을 지원합니다.
</ParamField>
...
최종 비디오는 Draft 작업의 프롬프트, 입력 에셋, 재생 시간, 비율, 시드, generate_audio 및 omni_reference_task_type을 재사용하므로 다시 전송하지 마세요. 해상도는 기본값이 1080p이며 1080p만 지원합니다.
</ParamField>

<ParamField body="content[].draft_task.id" type="string" required>
  `draft`를 true로 설정하여 Draft 비디오를 생성했을 때 반환된 작업 ID입니다.
</ParamField>
...
비디오의 경우: reference_video (Seedance 2.5, 2.0 및 2.0 fast만).
오디오의 경우: reference_audio (Seedance 2.5, 2.0 및 2.0 fast만).
...
텍스트 프롬프트 (필수): 중국어 및 영어 문자를 사용한 생성할 비디오에 대한 설명입니다.
...
BytePlus의 제약이 아닌 Comfy 측 가드레일입니다. BytePlus는 텍스트 길이 제한을
공개하지 않으며 테스트(2026-09-17)에서 40,000자를 허용했습니다.
바이트가 아닌 문자를 세므로 멀티바이트 프롬프트는 전송 시 이 크기의
몇 배가 될 수 있습니다. 이 값은 이 필드 하나만 제한하며 요청 전체를 제한하지는 않습니다:
`content`는 배열이고, 전체 문서를 제한하는 것은 요청당 본문 제한입니다. 이를 강제하는
표면은 Comfy Router (/v2/models/byteplus/\{model})이며, 직접 v1 /proxy 호출에서는
BytePlus 자체 검증기가 대신 응답합니다. 실제 트래픽보다 훨씬 높게 설정되어 실제
프롬프트를 판정하지 않으며, 호출자가 더 필요로 하면 값을 높이세요.
</ParamField>
...
반환된 작업 ID를 `draft_task` 콘텐츠 항목에 전달하여 최종 1080p 비디오를 렌더링하세요. Draft 작업 ID는 7일 동안 유효합니다.
</ParamField>

<ParamField body="duration" type="`-1` | object">
  초 단위의 비디오 재생 시간입니다. Seedance 2.5: [4,30] 또는 -1 (자동, 비디오 편집 작업은 -1만 지원). Seedance 2.0 및 2.0 fast: [4,15] 또는 -1 (자동). Seedance 1.5 pro: [4,12] 또는 -1. Seedance 1.0: [2,12].

  범위: `2` ~ `30`
</ParamField>

<ParamField body="execution_expires_after" type="integer">
  초 단위의 작업 타임아웃 임계값입니다. 기본값 172800 (48시간). 범위: [3600, 259200].
...
Seedance 2.5 첫 프레임 / 첫-마지막 프레임 생성: 출력은 첫 프레임의 화면 비율을 따르므로 `adaptive`만(또는 필드를 생략하는 경우만) 허용되며, 구체적인 비율을 지정하면 전달 전에 400으로 거부됩니다. Seedance 2.0은 이러한 모드에서 구체적인 비율을 허용합니다.
...
true: 생성된 비디오의 마지막 프레임 이미지를 반환합니다. 이 매개변수를 true로 설정한 뒤 비디오 생성 작업 정보 조회를 호출하면 마지막 프레임 이미지를 얻을 수 있습니다. 마지막 프레임 이미지는 PNG 형식이며 픽셀 너비와 높이가 생성된 비디오와 동일하고 워터마크가 포함되지 않습니다. 이 매개변수를 사용하면 여러 개의 연속된 비디오를 생성할 수 있습니다. 즉, 이전에 생성된 비디오의 마지막 프레임을 다음 비디오 작업의 첫 프레임으로 사용하여 여러 개의 연속된 비디오를 빠르게 생성할 수 있습니다.
false: 생성된 비디오의 마지막 프레임 이미지를 반환하지 않습니다.
</ParamField>

<ParamField body="seed" type="integer">
  무작위성을 제어하는 시드 정수입니다. 범위: [-1, 2^32-1]. -1은 무작위 시드를 사용합니다.

  범위: `-1` ~ `4294967295`
</ParamField>

<ParamField body="service_tier" type="string">
  처리 서비스 등급입니다. Seedance 2.5, 2.0 & 2.0 fast는 flex(오프라인 추론)를 지원하지 않습니다.
...
Router가 `GET /v2/models/byteplus/dreamina-seedance-2-0-mini/openapi.json`에서 제공하는 스키마를 기반으로 생성되었으며, 이는 요청이 공급자에게 도달하기 전에 호출을 검증할 때 사용하는 것과 동일한 문서입니다.

### 출력

<ResponseField name="content" type="object">
  비디오 생성 작업이 완료된 후의 출력으로, 출력 비디오의 다운로드 URL과 BytePlus가 반환하는 경우 마지막 프레임의 다운로드 URL을 포함합니다. `video_url`과 `last_frame_url`은 모두 Comfy 스토리지로 재호스팅(RE-HOSTED)되며, 여기의 다른 모든 필드는 BytePlus 자체 필드입니다. Null 허용: BytePlus는 작업 후 24시간 뒤에 URL을 지우므로, 그 이후에 폴링한 성공 문서는 `content`가 없거나 null일 수 있습니다.
</ResponseField>

<ResponseField name="content.last_frame_url" type="string">
  생성된 비디오의 마지막 프레임에 대한 다운로드 URL로, 요청에서 `return_last_frame`을 설정한 경우 반환됩니다. 이 URL로 이미지 형식을 추론하지 마세요. BytePlus는 요청 측에서 마지막 프레임을 PNG로 문서화하지만, Router는 전달받은 바이트를 그대로 재호스팅하고 업스트림 Content-Type 또는 콘텐츠 스니핑을 통해 형식을 판별하며, `image/jpeg`는 둘 다 실패했을 때의 최후 수단 폴백일 뿐입니다. Router는 마지막 프레임을 Comfy 스토리지로 재호스팅하고 이 필드를 다시 쓰므로, 일반적으로 최대 24시간 동안 유효한 Comfy 서명 URL입니다. 발급될 때 24시간으로 서명되고 23시간 메모에서 재생되므로, 나중에 폴링하면 남은 시간이 1시간도 안 되는 URL을 돌려받을 수 있습니다. 재호스팅을 수행할 수 없었던 경우에는 이 필드가 대신 BytePlus 자체 URL을 유지하며, BytePlus는 작업 후 24시간 뒤에 이를 지웁니다. 어느 쪽이든 링크는 만료되므로, URL을 저장하지 말고 프레임을 다운로드하세요.
</ResponseField>

<ResponseField name="content.output_format" type="string">
  생성된 비디오의 컨테이너 형식(mp4 또는 mov)으로, BytePlus가 이를 `content` 안에 중첩할 때 사용됩니다. Seedance 모델은 더 흔히 이를 `content`의 최상위 형제 필드로 반환하며(최상위 `output_format` 필드 참고), Router는 둘 중 존재하는 것을 읽습니다.
</ResponseField>

<ResponseField name="content.video_url" type="string">
  출력 비디오의 다운로드 URL입니다. Router는 비디오를 Comfy 스토리지로 재호스팅하고 이 필드를 다시 쓰므로, 일반적으로 최대 24시간 동안 유효한 Comfy 서명 URL입니다. 발급될 때 24시간으로 서명되고 23시간 메모에서 재생되므로, 나중에 폴링하면 남은 시간이 1시간도 안 되는 URL을 돌려받을 수 있습니다. 재호스팅을 수행할 수 없었던 경우에는 이 필드가 대신 BytePlus 자체 URL을 유지하며, BytePlus는 작업 후 24시간 뒤에 이를 지우고 일부 모델에서는 다운로드를 100회로 제한합니다. 어느 쪽이든 링크는 만료되므로, URL을 저장하지 말고 비디오를 다운로드하세요.
</ResponseField>
...
<ResponseField name="duration" type="number">
  생성된 비디오의 재생 시간(초)입니다. BytePlus가 이를 일관되게 처리하지 않기 때문에 정수가 아닌 숫자로 선언됩니다. 비디오 작업이 정수 초를 반환하는 것이 관찰된 반면, 인접한 BytePlus 인터페이스는 소수 재생 시간을 보고하므로, 클라이언트는 정수 값을 가정해서는 안 됩니다. BytePlus 자체 필드로, 성공한 비디오 작업에서 반환되며 변경 없이 전달됩니다.
</ResponseField>
...
<ResponseField name="output_format" type="string">
  생성된 비디오의 컨테이너 형식(mp4 또는 mov)으로, `content`의 형제 필드로 최상위에 반환됩니다. Seedance 비디오 작업 조회가 이를 반환하는 위치입니다. BytePlus 자체 필드로, 변경 없이 전달됩니다.
</ResponseField>
