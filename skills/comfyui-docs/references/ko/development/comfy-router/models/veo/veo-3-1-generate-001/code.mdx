---
title: "Comfy Router로 Veo 3.1 Generate 001 사용하기"
description: "Comfy Router를 통해 veo/veo-3.1-generate-001을 호출합니다: endpoint, 요청 형태, 그리고 Router가 반환하는 응답을 설명합니다."
translationSourceHash: 12d8dcbd
translationFrom: development/comfy-router/models/veo/veo-3-1-generate-001/code.mdx
translationBlockHashes:
  "_intro": 0925e6d2
  "Quick start": cd500ec3
  "Schema": bc219941
  "Examples": 88e27039
---

Veo가 제공하고 Comfy Router가 서비스하는 `veo/veo-3.1-generate-001`의 API 레퍼런스입니다.

## 빠른 시작

[Comfy 워크스페이스](https://platform.comfy.org/profile/api-keys?onboarding=router)에서 키를 생성하고 `COMFY_API_KEY`로 내보내세요. Python 및 TypeScript 스니펫은 Comfy SDK를 사용하며(`pip install comfy-sdk`, `npm install @comfyorg/sdk`), cURL 스니펫은 동일한 호출을 순수 HTTP로 수행합니다.

**모델 ID:** `veo/veo-3.1-generate-001`

**엔드포인트:** `POST https://api.comfy.org/v2/models/veo/veo-3.1-generate-001`
...
동일한 본문을 `POST https://api.comfy.org/v2/models/veo/veo-3.1-generate-001/requests`로 전송합니다. Router는 실행이 접수되는 즉시 `request_id`와 함께 `201`을 응답하며, 결과는 준비가 완료되면 이 프로세스든 다른 프로세스든 수집할 수 있습니다. [큐 전송](/ko/development/comfy-router/queue)에서 상태, 취소, 수집 과정을 차례로 안내합니다.
...
# 3. 수집합니다. 모델의 네이티브 출력과 함께 200, 아직 실행 중인 동안에는 상태 본문과 함께 202를 반환합니다.
...
<ParamField body="instances[].mask" type="object">
  비디오 편집을 위한 선택적 마스크. 입력 비디오에 적용됩니다.
</ParamField>
...
<ParamField body="instances[].referenceImages" type="object[]">
  비디오 생성을 유도하는 선택적 참조 이미지. 최대 3개의 에셋 이미지 또는 1개의 스타일 이미지를 지원합니다. Veo 3.1 모델(미리보기)에서 지원됩니다.
</ParamField>
...
<ParamField body="instances[].video" type="object">
  비디오 확장 또는 편집을 위한 선택적 입력 비디오. image 및 referenceImages와 호환되지 않습니다.
</ParamField>
...
<ParamField body="parameters.personGeneration" type="string">
  생성된 비디오의 인물을 제어합니다. 기본값: allow_adult

  가능한 값: `dont_allow`, `allow_adult`, `allowAll`
</ParamField>

<ParamField body="parameters.pubsubTopic" type="string">
  진행 상황 업데이트를 위한 Cloud Pub/Sub 토픽 (projects/\{project}/topics/\{topic})
</ParamField>
...
<ParamField body="parameters.seed" type="integer">
  결정론적 출력을 위한 랜덤 시드입니다. sampleCount > 1이면 비디오마다 다른 시드가 사용됩니다.
...
Router가 `GET /v2/models/veo/veo-3.1-generate-001/openapi.json`에서 제공하는 스키마를 기반으로 생성되었으며, 이는 요청이 공급자에게 도달하기 이전에 호출을 검증하는 데 사용하는 것과 동일한 문서입니다.
