---
title: "Comfy Router로 Meshy 6 사용하기"
description: "Comfy Router를 통해 meshy/meshy-6를 호출하는 방법: endpoint, 요청 형태, Router가 반환하는 응답."
translationSourceHash: 03010644
translationFrom: development/comfy-router/models/meshy/meshy-6/code.mdx
translationBlockHashes:
  "_intro": aa2fea0d
  "Quick start": 4cef8af8
  "Schema": 7b58e8aa
  "Examples": e2bc6209
---

Comfy Router가 Meshy에서 제공하는 `meshy/meshy-6`의 API 레퍼런스입니다.
...
**엔드포인트:** `POST https://api.comfy.org/v2/models/meshy/meshy-6`
...
동일한 본문을 `POST https://api.comfy.org/v2/models/meshy/meshy-6/requests` 로 보냅니다. Router는 실행이 접수되는 즉시 `201` 과 `request_id` 를 응답하며, 결과는 준비가 되는 대로 이 프로세스나 다른 프로세스에서 수집할 수 있습니다. 상태, 취소, 수집 방법은 [큐 전송](/ko/development/comfy-router/queue) 를 참고하세요.
...
# 3. 수집합니다. 200은 모델의 네이티브 출력과 함께, 202는 아직 실행 중일 때 상태 본문과 함께 반환됩니다.
...
<ParamField body="ai_model" type="string">
  실행할 Meshy 모델입니다. Comfy Router 경로에서는 경로(`/v2/models/meshy/{model}`)로 제공되며 생략하거나 null로 보낼 수 있습니다. Router는 검증 후 경로의 모델을 여기에 기록합니다. v1 /proxy 경로에서는 MeshyTextTo3DPreviewRequest를 참조하세요. 이 요청은 해당 값을 MeshyAiModel로 제한합니다.
</ParamField>

<ParamField body="art_style" type="string" default="&quot;realistic&quot;">
  원하는 오브젝트의 아트 스타일을 설명합니다.
...
<ParamField body="mode" type="string" required>
  미리보기 작업을 생성할 때는 이 필드를 "preview"로 설정해야 합니다.

  가능한 값: `preview`
</ParamField>

<ParamField body="moderation" type="boolean" default="false">
  참일 때 입력 콘텐츠에서 유해할 가능성이 있는 콘텐츠를 검사합니다.
</ParamField>
...
<ParamField body="prompt" type="string" required>
  3D 모델이 어떤 오브젝트인지 설명합니다. 최대 600자입니다.
</ParamField>

<ParamField body="should_remesh" type="boolean" default="true">
  리메시 단계를 활성화할지 여부를 제어합니다. false이면 최고 정밀도의 삼각형 메시를 반환합니다.
</ParamField>
...
<ParamField body="ai_model" type="string">
  실행할 Meshy 모델입니다. Comfy Router 경로에서는 경로(`/v2/models/meshy/{model}`)로 제공되며 생략하거나 null로 보낼 수 있습니다. Router는 검증 후 경로의 모델을 여기에 기록합니다. v1 /proxy 경로에서는 MeshyTextTo3DRefineRequest를 참조하세요. 이 요청은 해당 값을 MeshyAiModel로 제한합니다.
</ParamField>

<ParamField body="enable_pbr" type="boolean" default="false">
  기본 색상에 더해 PBR 맵(metallic, roughness, normal)을 생성합니다. Sculpture 스타일을 사용할 때는 enable_pbr을 false로 설정해야 합니다.
</ParamField>

<ParamField body="mode" type="string" required>
  refine 작업을 생성할 때는 이 필드를 "refine"으로 설정해야 합니다.
...
<ParamField body="preview_task_id" type="string" required>
  대응하는 미리보기 작업 id입니다. 지정한 미리보기 작업의 상태가 SUCCEEDED여야 합니다.
</ParamField>

<ParamField body="texture_image_url" type="string">
  텍스처링 과정을 안내할 2D 이미지를 제공합니다. .jpg, .jpeg, .png 형식 또는 base64로 인코딩된 데이터 URI를 지원합니다.
</ParamField>

<ParamField body="texture_prompt" type="string">
  텍스처링 과정을 안내할 추가 텍스트 프롬프트를 제공합니다. 최대 600자입니다.
</ParamField>
...
Router가 `GET /v2/models/meshy/meshy-6/openapi.json`에서 제공하는 스키마에서 생성되었으며, 이는 요청이 공급자에 도달하기 이전에 호출을 검증하는 데 사용하는 것과 동일한 문서입니다.
...
<ResponseField name="preceding_tasks" type="integer">
  앞선 작업의 개수입니다. status가 PENDING일 때만 의미가 있습니다.
</ResponseField>

<ResponseField name="progress" type="integer">
  작업의 진행률입니다. 시작되지 않은 경우 0이며, 성공하면 100입니다.
