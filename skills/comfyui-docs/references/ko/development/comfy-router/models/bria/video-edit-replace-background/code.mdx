...
동일한 본문을 `POST https://api.comfy.org/v2/models/bria/video-edit-replace-background/requests` 로 보냅니다. Router는 실행이 접수되는 즉시 `201` 과 `request_id` 를 응답하며, 결과는 준비가 되면 이 프로세스나 다른 프로세스에서 수집합니다. 상태, 취소, 수집 방법은 [큐 전송](/ko/development/comfy-router/queue) 에서 자세히 다룹니다.
...
print("requestId:", handle.requestId)  // 모델 ID와 함께라면 다른 프로세스에 필요한 것은 이것뿐입니다
...
# 3. 수집합니다. 완료되면 모델의 네이티브 출력과 함께 200을, 아직 실행 중이면 상태 본문과 함께 202를 반환합니다.
