...
[Comfy 워크스페이스](https://platform.comfy.org/profile/api-keys?onboarding=router)에서 키를 생성하고 `COMFY_API_KEY`로 내보내세요. Python과 TypeScript 스니펫은 Comfy SDK(`pip install comfy-sdk`, `npm install @comfyorg/sdk`)를 사용하며, cURL 스니펫은 raw HTTP를 통한 동일한 호출입니다.

**모델 ID:** `bria/image-edit-erase-by-text`

**엔드포인트:** `POST https://api.comfy.org/v2/models/bria/image-edit-erase-by-text`
...
동일한 본문을 `POST https://api.comfy.org/v2/models/bria/image-edit-erase-by-text/requests` 로 전송합니다. Router는 실행이 접수되는 즉시 `201`과 `request_id`를 응답하며, 결과는 준비가 되면 이 프로세스든 다른 프로세스든 수집할 수 있습니다. [대기열 기반 전달](/ko/development/comfy-router/queue)에서 상태, 취소, 수집 과정을 자세히 다룹니다.
