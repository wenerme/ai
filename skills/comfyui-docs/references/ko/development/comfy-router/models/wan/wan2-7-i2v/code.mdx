...
동일한 body를 `POST https://api.comfy.org/v2/models/wan/wan2.7-i2v/requests`로 전송합니다. Router는 실행이 접수되는 즉시 `request_id`와 함께 `201`을 응답하며, 결과는 준비가 되면 이 프로세스 또는 다른 프로세스에서 수집합니다. [대기열 전달](/ko/development/comfy-router/queue)에서 상태, 취소, 수집 과정을 자세히 다룹니다.
...
<ParamField body="input.img_url" type="string">
  첫 프레임 이미지 URL 또는 Base64로 인코딩된 데이터입니다. wan2.5-i2v-preview 및 wan2.6-i2v 모델에서만 필수입니다. happyhorse-1.x의 i2v 표기는 여기에서 첫 프레임을 받지 않습니다. 첫 프레임은 type이 `first_frame`인 `media` 요소로 전달합니다 (아래 `media` 참조). wan2.6-i2v에서 성공하는 `img_url` body는 happyhorse-1.0-i2v와 happyhorse-1.1-i2v에서는 프로바이더가 거부합니다. 이미지 형식: JPEG, JPG, PNG, BMP, WEBP. 해상도: 360-2000 픽셀. 파일 크기: 최대 10MB.
</ParamField>

<ParamField body="input.media" type="object[]">
  wan2.7, wan3.0 및 happyhorse-1.x 모델용 미디어 에셋 목록입니다. 비디오 생성을 위한 참조 자료(이미지, 오디오, 비디오)를
...
- wan3.0-video: first_frame (최대 1개), last_frame (최대 1개), reference_image (최대 10개),
  ...
  함께 사용할 수 없음). reference_*/file/link 유형과 first_frame/last_frame 유형은 동일한
  요청 내에서 상호 배타적입니다. 배열 순서는 프롬프트에서 에셋의 참조 순서(이미지 1, 비디오 1,
  오디오 1, ...)를 정의합니다.
...
위의 에셋별 "최대 20MB" 수치는 최종적으로 받는 이미지에 대한 PARTNER 측
상한이며, 에셋이 공개 URL일 때는 그대로 적용됩니다. 해당 바이트가
Comfy를 거치지 않기 때문입니다. 인라인 data: URL은 Comfy를 거치므로
더 낮은 전송 상한에 먼저 도달합니다. Comfy Router의 POST body는 총 10 MiB가
상한이며 이를 초과하면 413이 반환됩니다. base64는 페이로드를 약 4/3로
부풀리므로, 약 7.5 MiB를 넘는 단일 인라인 에셋은 여기에 설명된 파트너 규칙에
도달하기 전에 거부됩니다. body에 이러한 에셋이 여러 개 있으면 더욱 빨라집니다.
10 MiB는 각 요소가 아니라 전체 요청을 제한하기 때문입니다. 이 상한에 가까운
것은 공개 URL로 전송하세요.
</ParamField>
...
<ParamField body="input.media[].url" type="string" required>
  미디어 파일의 URL입니다. 공개 HTTP/HTTPS URL, OSS 임시 URL, 또는 (위 `media` 설명에서 해당 모델의 항목이 그렇다고 명시한 경우, 예를 들어 happyhorse-1.x의 i2v와 r2v 표기처럼) 인라인 `data:{MIME_type};base64,...` URL입니다. 모델별 크기와 픽셀 하한, 그리고 파트너 규칙이 적용되기 전에 인라인 페이로드를 제한하는 10 MiB Router 요청 body 상한에 대해서는 해당 설명을 참조하세요.
</ParamField>
...
- wan3.0-video: 비디오 입력이 없으면 [2, 30] 범위의 정수이고, 비디오 입력이 있으면 총
  입력 비디오 재생 시간 + 출력 비디오 재생 시간이 30초를 초과할 수 없습니다. -1은 모델이 적절한
  재생 시간을 선택하는 스마트 재생 시간 모드를 활성화합니다

  범위: `-1` ~ `30`
</ParamField>

<ParamField body="parameters.prompt_extend" type="boolean" default="true">
  프롬프트 지능형 재작성 활성화 여부입니다. 기본값은 참입니다
</ParamField>

<ParamField body="parameters.ratio" type="string">
  생성된 비디오의 화면 비율입니다. wan2.7 및 wan3.0 모델 전용입니다.
...
`GET /v2/models/wan/wan2.7-i2v/openapi.json`에서 Router가 제공하는 스키마에서 생성되었으며, 이는 요청이 공급자에 도달하기 전에 호출을 검증하는 데 사용하는 동일한 문서입니다.
### 출력
