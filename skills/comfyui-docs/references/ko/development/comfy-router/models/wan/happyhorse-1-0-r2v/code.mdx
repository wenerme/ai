...
동일한 본문을 `POST https://api.comfy.org/v2/models/wan/happyhorse-1.0-r2v/requests`로 전송합니다. Router는 실행이 접수되는 즉시 `request_id`와 함께 `201`을 응답하고, 결과는 준비되면 이 프로세스에서든 다른 프로세스에서든 수집됩니다. [대기열 전송](/ko/development/comfy-router/queue)에서 상태, 취소, 수집 과정을 자세히 안내합니다.
...
type이 `first_frame`인 `media` 요소로 받으며(아래 `media` 참조),
...
- wan3.0-video: first_frame (최대 1개), last_frame (최대 1개), reference_image (최대 10개),
  ...
  총 재생 시간 \<= 15초), file (최대 1개, link와 함께 사용할 수 없음), link (최대 1개,
  file과 함께 사용할 수 없음). reference_*/file/link 유형과 first_frame/last_frame 유형은
  동일한 요청 내에서 상호 배타적입니다. 배열 순서는 프롬프트에서 에셋의
  참조 순서를 정의합니다(Image 1, Video 1, Audio 1, ...).
...
위의 에셋별 "최대 20MB" 수치는 최종적으로 받게 되는 이미지에 대한 파트너 측
상한이며, 에셋이 공개 URL일 때는 명시된 그대로 적용됩니다. 해당 바이트가
Comfy를 거치지 않기 때문입니다. 인라인 data: URL은 Comfy를 거치므로
전송 상한도 함께 적용됩니다. Comfy Router의 POST body는 총 100 MiB가
상한이며 이를 초과하면 413이 반환됩니다. 그리고 base64는 페이로드를 약 4/3로
부풀리므로, 약 75 MB를 넘는 단일 인라인 에셋은 여기에 설명된 파트너 규칙에
도달하기도 전에 거부됩니다. 이 정도 크기에서는 파트너 자체의 20MB 상한에
해당하는 단일 에셋도 인라인으로 충분히 들어가므로, 에셋 하나에 대해서는
파트너 규칙이 구속력을 가집니다. 전송 상한은 여러 에셋에 걸쳐 구속력을
가집니다. 100 MiB Router 상한은 각 요소가 아니라 전체 요청을 제한하기
...
<ParamField body="input.media[].url" type="string" required>
  미디어 파일의 URL입니다. 공개 HTTP/HTTPS URL, OSS 임시 URL, 또는 (위 `media` 설명에서 해당 모델의 항목이 그렇다고 명시한 경우, 예를 들어 happyhorse-1.x의 i2v와 r2v 표기처럼) 인라인 `data:{MIME_type};base64,...` URL입니다. 모델별 크기와 픽셀 하한, 그리고 인라인 페이로드를 총량 기준으로 제한하는 100 MiB Router 요청 body 상한에 대해서는 해당 설명을 참조하세요.
</ParamField>
...
- origin: 입력 비디오의 원본 오디오를 강제로 보존합니다
...
- wan3.0-video: 비디오 입력이 없으면 [2, 30] 범위의 정수, 비디오 입력이 있으면
  입력 비디오 재생 시간 + 출력 비디오 재생 시간의 합이 30초를 초과할 수 없습니다. -1은
...
wan3.0-video의 경우 adaptive(기본값)가 입력 미디어의 비율과 의도에 따라
...
해상도 등급이 청구 요율을 결정하므로, 이 프록시는 resolution과
size를 모두 제공하지 않는 비디오 생성 요청을 거부합니다.
...
Router가 `GET /v2/models/wan/happyhorse-1.0-r2v/openapi.json`에서 제공하는 스키마에서 생성되었으며, 이는 요청이 공급자에 도달하기 전에 호출을 검증하는 데 사용하는 동일한 문서입니다.
...
비디오 URL은 24시간 동안 유효합니다. 비디오를 보관해야 한다면 지체 없이 다운로드하세요.
