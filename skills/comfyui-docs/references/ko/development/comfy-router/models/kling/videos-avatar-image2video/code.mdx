...
동일한 본문을 `POST https://api.comfy.org/v2/models/kling/videos-avatar-image2video/requests` 로 보냅니다. Router는 실행이 접수되는 즉시 `201` 과 `request_id` 를 응답하며, 결과는 준비가 되는 대로 이 프로세스나 다른 프로세스에서 수집할 수 있습니다. 상태, 취소, 수집 방법은 [대기 중 전달](/ko/development/comfy-router/queue) 를 참고하세요.
