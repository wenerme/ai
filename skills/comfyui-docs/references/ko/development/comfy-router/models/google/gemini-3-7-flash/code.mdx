---
title: "Gemini 3.7 Flash를 Comfy Router와 함께 사용하기"
description: "Comfy Router를 통해 vertexai/gemini-3.7-flash를 호출합니다: 엔드포인트, 요청 형태, 그리고 Router가 반환하는 응답."
...
Google이 제공하고 Comfy Router가 서비스하는 `vertexai/gemini-3.7-flash`의 API 레퍼런스입니다.

## 빠른 시작

[Comfy 워크스페이스](https://platform.comfy.org/profile/api-keys?onboarding=router)에서 키를 생성하고 `COMFY_API_KEY`로 내보내세요. Python과 TypeScript 스니펫은 Comfy SDK(`pip install comfy-sdk` 및 `npm install @comfyorg/sdk`)를 사용하며, cURL 스니펫은 동일한 호출을 순수 HTTP로 수행합니다.

**모델 ID:** `vertexai/gemini-3.7-flash`

**엔드포인트:** `POST https://api.comfy.org/v2/models/vertexai/gemini-3.7-flash`
...
# SDK는 idempotency 키를 자동으로 생성하고 자동 재시도에 재사용합니다.
...
동일한 본문을 `POST https://api.comfy.org/v2/models/vertexai/gemini-3.7-flash/requests` 로 보냅니다. Router는 실행이 접수되는 즉시 `201` 과 `request_id` 를 응답하며, 결과는 준비가 되는 대로 이 프로세스나 다른 프로세스에서 수집할 수 있습니다. 상태, 취소, 수집 방법은 [큐 전송](/ko/development/comfy-router/queue) 를 참고하세요.
...
async def main():
    async with AsyncComfy() as client:
        ...
        # 공급자가 반환하는 자체 페이로드로, models.run()이 반환하는 것과 동일한 값입니다.
        # 실패했거나 취소된 요청은 이 지점에서 타입이 지정된 Router 오류를 발생시킵니다.
...
// models.run()이 반환하는 것과 동일한 결과입니다. 실패했거나 취소된 요청은 이 지점에서 거부됩니다.
...
<ParamField body="contents[].parts[].fileData.mimeType" type="string">
  data 또는 fileUri 필드에 지정된 파일의 미디어 타입입니다. 사용 가능한 값은 다음과 같습니다. gemini-2.0-flash-lite 및 gemini-2.0-flash의 경우 오디오 파일의 최대 길이는 8.4시간이고, 비디오 파일(오디오 없음)의 최대 길이는 1시간입니다. 자세한 내용은 Gemini 오디오 및 비디오 요구 사항을 참조하세요. 텍스트 파일은 UTF-8로 인코딩되어야 합니다. 텍스트 파일의 콘텐츠는 토큰 한도에 포함됩니다. 이미지 해상도에는 제한이 없습니다.
...
<ParamField body="generationConfig" type="object">
  생성에 대한 샘플링, 길이 및 출력 설정입니다. 모든 필드는 선택 사항입니다. 아래에서 `default`를 선언한 필드는 생략 시 해당 값이 적용되고, 나머지는 모델 자체 동작을 따릅니다.
</ParamField>
...
<ParamField body="generationConfig.imageConfig.imageSize" type="string">
  선택 사항. 생성된 이미지의 크기를 지정합니다. 지원되는 값은 1K, 2K, 4K입니다. 지정하지 않으면 모델은 기본값인 1K를 사용합니다.
</ParamField>

<ParamField body="generationConfig.maxOutputTokens" type="integer">
  응답에서 생성할 수 있는 최대 토큰 수입니다. 토큰은 약 4자에 해당합니다. 100토큰은 대략 60~80단어에 해당합니다.
...
<ParamField body="generationConfig.seed" type="integer">
  시드를 특정 값으로 고정하면 모델은 반복 요청에 대해 동일한 응답을 제공하기 위해 최선을 다합니다. 결정적 출력은 보장되지 않습니다. 또한 동일한 시드 값을 사용하더라도 모델이나 temperature와 같은 파라미터 설정을 변경하면 응답이 달라질 수 있습니다. 기본적으로는 무작위 시드 값이 사용됩니다. 다음 모델에서 사용할 수 있습니다: gemini-2.5-flash, gemini-2.5-pro, gemini-2.5-flash-preview-04-1, gemini-2.5-pro-preview-05-0, gemini-2.0-flash-lite-00, gemini-2.0-flash-001
</ParamField>
...
<ParamField body="generationConfig.temperature" type="number" default="1">
  temperature는 응답 생성 중 샘플링에 사용되며, 이는 topP와 topK가 적용될 때 발생합니다. temperature는 토큰 선택의 무작위성 정도를 제어합니다. 낮은 temperature는 개방적이지 않거나 창의적이지 않은 응답이 필요한 프롬프트에 적합하고, 높은 temperature는 더 다양하거나 창의적인 결과를 만들 수 있습니다. temperature가 0이면 항상 확률이 가장 높은 토큰이 선택됩니다. 이 경우 주어진 프롬프트에 대한 응답은 대부분 결정적이지만, 약간의 변동은 여전히 가능합니다. 모델이 너무 일반적이거나 너무 짧은 응답을 반환하거나 대체 응답을 제공한다면 temperature를 높여 보세요.
...
<ParamField body="generationConfig.thinkingConfig" type="object">
  선택 사항입니다. 사고 기능에 대한 설정입니다. 사고는 모델이 복잡한 작업을 더 작은 단계로 나누어 더 높은 품질의 응답을 생성하는 과정입니다.
</ParamField>
...
<ParamField body="generationConfig.thinkingConfig.thinkingBudget" type="integer">
  선택 사항입니다. 모델의 사고 과정에 대한 토큰 예산입니다. 모델은 이 예산을 넘지 않도록 최선을 다합니다.
</ParamField>
...
<ParamField body="generationConfig.topK" type="integer" default="40">
  Top-K는 모델이 출력할 토큰을 선택하는 방식을 변경합니다. Top-K가 1이면 다음에 선택되는 토큰은 모델 어휘 전체에서 가장 확률이 높은 토큰입니다. Top-K가 3이면 temperature를 사용하여 확률이 가장 높은 3개의 토큰 중에서 다음 토큰이 선택됩니다.
...
Top-P는 모델이 출력할 토큰을 선택하는 방식을 변경합니다. 토큰은 확률이 가장 높은 것( top-K 참조)부터 시작하여 확률의 합이 top-P 값과 같아질 때까지 낮은 쪽으로 선택됩니다. 예를 들어 토큰 A, B, C의 확률이 각각 0.3, 0.2, 0.1이고 top-P 값이 0.5라면, 모델은 temperature를 사용하여 A 또는 B를 다음 토큰으로 선택하고 C는 후보에서 제외합니다.
...
<ParamField body="systemInstruction" type="object">
  모델이 더 나은 성능을 내도록 이끄는 지침입니다. 예를 들어 "가능한 한 간결하게 답하세요" 또는 "응답에 전문 용어를 사용하지 마세요"와 같습니다. 텍스트 문자열은 토큰 한도에 포함됩니다. systemInstruction의 역할 필드는 무시되며 모델의 성능에 영향을 주지 않습니다. 참고: parts에는 텍스트만 사용해야 하며, 각 part의 content는 별도의 단락에 있어야 합니다.
</ParamField>

<ParamField body="systemInstruction.parts" type="object[]" required>
  하나의 메시지를 구성하는 순서가 있는 part 목록입니다. part마다 서로 다른 IANA MIME 유형을 가질 수 있습니다. 최대 토큰 수나 이미지 개수와 같은 입력 제한에 대해서는 Google models 페이지의 모델 사양을 참조하세요.
</ParamField>
...
<ParamField body="tools" type="object[]">
  시스템이 모델의 지식과 범위를 벗어나 외부 시스템과 상호작용하여 하나 또는 일련의 작업을 수행할 수 있게 하는 코드 조각입니다. Function calling을 참조하세요.
</ParamField>
...
<ParamField body="videoMetadata" type="object">
  비디오 입력의 경우, 재생 시간(Duration) 형식으로 된 비디오의 시작 및 종료 오프셋입니다. 예를 들어 1:00에서 시작하는 10초 클립을 지정하려면 "startOffset": \{ "seconds": 60 } 및 "endOffset": \{ "seconds": 70 }으로 설정합니다. 메타데이터는 비디오 데이터가 inlineData 또는 fileData로 제공되는 동안에만 지정해야 합니다.
</ParamField>
...
<ParamField body="videoMetadata.endOffset.nanos" type="integer">
  나노초 해상도의 부호 있는 초 단위 소수 부분입니다. 소수 부분이 있는 음수 초 값이라도 nanos 값은 음수가 아니어야 합니다.

  범위: `0` ~ `999999999`
</ParamField>

<ParamField body="videoMetadata.endOffset.seconds" type="integer">
  시간 범위의 부호 있는 초입니다. -315,576,000,000부터 +315,576,000,000까지(포함)여야 합니다.
...
`GET /v2/models/vertexai/gemini-3.7-flash/openapi.json`에서 Router가 제공하는 스키마에서 생성되었으며, 이는 요청이 공급자에게 도달하기 전에 호출을 검증하는 데 사용하는 것과 동일한 문서입니다.
...
<ResponseField name="promptFeedback.safetyRatings[].probability" type="string">
  콘텐츠가 지정된 안전 카테고리를 위반할 확률입니다.가능한 값:  `NEGLIGIBLE`, `LOW`, `MEDIUM`, `HIGH`, `UNKNOWN`
</ResponseField>
...
<ResponseField name="usageMetadata.cachedContentTokenCount" type="integer">
  출력 전용. 입력에서 캐시된 부분(캐시된 콘텐츠)의 토큰 수입니다.
</ResponseField>
...
<ResponseField name="usageMetadata.promptTokenCount" type="integer">
  요청의 토큰 수입니다. cachedContent가 설정된 경우에도 이는 총 유효 프롬프트 크기이며, 캐시된 콘텐츠의 토큰 수를 포함합니다.
</ResponseField>

<ResponseField name="usageMetadata.promptTokensDetails" type="object[]">
  모달리티별 프롬프트 토큰의 분석입니다.
</ResponseField>
