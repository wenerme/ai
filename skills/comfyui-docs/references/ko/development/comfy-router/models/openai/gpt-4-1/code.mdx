---
title: "Comfy Router로 GPT 4.1 사용하기"
description: "Comfy Router를 통해 openai/gpt-4.1 호출하기: endpoint, 요청 형태, 그리고 Router가 반환하는 응답."
translationSourceHash: 442f4104
translationFrom: development/comfy-router/models/openai/gpt-4-1/code.mdx
translationBlockHashes:
  "_intro": 9b48bb1c
  "Quick start": 73cda8a0
  "Schema": b7d988d4
  "Examples": 67e6c8f1
---

`openai/gpt-4.1`에 대한 API 레퍼런스이며, Comfy Router가 OpenAI에서 제공합니다.
...
**모델 ID:** `openai/gpt-4.1`

**엔드포인트:** `POST https://api.comfy.org/v2/models/openai/gpt-4.1`
...
동일한 본문을 `POST https://api.comfy.org/v2/models/openai/gpt-4-1/requests` 로 보냅니다. Router는 실행이 접수되는 즉시 `201` 과 `request_id` 를 응답하며, 결과는 준비가 되는 대로 이 프로세스나 다른 프로세스에서 수집할 수 있습니다. 상태, 취소, 수집 방법은 [Queued delivery](/ko/development/comfy-router/queue) 를 참고하세요.
...
// Reads COMFY_API_KEY from the environment.
// Each submit() call mints its own Idempotency-Key and reuses it for automatic retries.
const handle = await comfy.models.submit("openai/gpt-4.1", {
  input: "Reply with the single word: ok",
  max_output_tokens: 1024,
});
...
curl https://api.comfy.org/v2/models/openai/gpt-4.1/requests \
  -H "X-API-Key: $COMFY_API_KEY" \
  -H "Idempotency-Key: $(uuidgen)" \
  -H "Content-Type: application/json" \
  -d "{\"input\": \"Reply with the single word: ok\", \"max_output_tokens\": 1024}"
...
curl -i https://api.comfy.org/v2/models/openai/gpt-4.1/requests/$REQUEST_ID/status \
  -H "X-API-Key: $COMFY_API_KEY"

# 3. Collect. 200 with the model's native output, 202 with the status body while it is still running.
curl https://api.comfy.org/v2/models/openai/gpt-4.1/requests/$REQUEST_ID \
  -H "X-API-Key: $COMFY_API_KEY"
```
...
<ParamField body="input" type="string | object[]" required>
  모델에 대한 텍스트, 이미지 또는 파일 입력으로, 응답을 생성하는 데 사용됩니다. Router가 제공할 수 없는 이 계약의 유일한 필드이며, 아래 `required`의 유일한 항목입니다.
</ParamField>
...
<ParamField body="max_output_tokens" type="integer">
  응답을 위해 생성되는 토큰 수의 상한으로, 표시되는 출력 토큰과 추론 토큰을 포함합니다. 추론 id에서는 이 상한이 숨겨진 추론 토큰과 공유되므로, 작은 값은 표시되는 텍스트가 나오기 전에 전체 예산을 소진할 수 있습니다. 이것이 추론 스모크 케이스가 1024를 보내는 반면 채팅 케이스는 16을 보내는 이유입니다.

  범위: `1` ~ `…`
</ParamField>

<ParamField body="model" type="string">
  OpenAI 모델 식별자입니다. Comfy Router에서는 이 필드가 선택 사항이며 Router가 `{model}` 경로 세그먼트에서 채웁니다. 명시적인 `null`도 같은 방식으로 교체됩니다. 경로와 일치하지 않는 값을 보내면 거부됩니다.
</ParamField>
...
<ParamField body="reasoning" type="object">
  추론 등급 전용입니다. 추론 모델을 위한 구성입니다. 예: `{"effort": "medium"}`. 변경 없이 전달됩니다. 허용되는 키는 OpenAI의 추론 가이드를 참조하십시오. 채팅 등급 id는 이를 무시합니다.
</ParamField>
...
<ParamField body="stream" type="boolean">
  이 필드를 보내는 호출자가 거부되지 않도록 선언되었지만, 이 표면에서는 아무 효과가 없습니다. Router가 디스패치 전에 이를 `false`로 확정합니다. 이는 `text/event-stream`을 중계하는 대신 공급자 응답을 캡처하기 때문입니다. openAiResponsesProxy의 ModifyResponse는 이를 디코딩할 수 없으므로, 스트리밍 생성은 OpenAI가 청구하고 아무도 계량하지 않게 됩니다. 스트림이 필요하면 `POST /proxy/openai/v1/responses`를 사용하십시오.
</ParamField>

<ParamField body="temperature" type="number">
  샘플링 temperature입니다. 채팅 등급 전용입니다: o 시리즈 추론 id(`o1`, `o1-pro`, `o3`, `o4-mini`)는 OpenAI에서 이 매개변수를 거부합니다. Router는 이들에 대해 이를 거부하지 않습니다. 두 등급이 하나의 스키마를 공유하는 이유에 대한 이 컴포넌트의 설명을 참조하십시오. 따라서 이를 보내는 추론 호출은 OpenAI 자체 오류로 응답됩니다.

  범위: `0` ~ `2`
</ParamField>

<ParamField body="text" type="object">
  출력 형식 구성입니다. 예: Structured Outputs용 `{"format": {"type": "json_schema", ...}}`. 변경 없이 전달됩니다.
</ParamField>

<ParamField body="tool_choice" type="string | object">
  모델이 사용할 도구를 선택하는 방식입니다. 문자열 모드 또는 도구를 지정하는 객체입니다.
</ParamField>

<ParamField body="tools" type="object[]">
  모델이 호출할 수 있는 도구 정의입니다. Router는 도구 분류를 좁히지 않습니다. 허용되는 형태는 OpenAI의 Responses API 참조를 확인하십시오.
</ParamField>

<ParamField body="top_p" type="number">
  Nucleus 샘플링 컷오프입니다. `temperature`와 동일한 조건으로 채팅 등급 전용입니다.

  범위: `0` ~ `1`
</ParamField>

<ParamField body="truncation" type="string">
  컨텍스트가 모델 창을 초과할 때의 잘림 전략입니다. 위의 세 가지 어휘와 달리 여기서는 enum이 실제로 강제됩니다. 이 두 값이 OpenAI가 문서화한 완전한 집합이며 늘어나지 않았기 때문입니다. 명시적인 `null`은 위 필드와 동일한 조건으로 여전히 허용됩니다.

  가능한 값: `auto`, `disabled`
</ParamField>

<ParamField body="usage" type="object">
  토큰 사용량 envelope입니다. v1 작업이 요청 본문에 이를 선언하기 때문에 이 계약에 존재합니다. OpenAI는 이를 응답에 채우므로 호출자가 보낼 이유가 없습니다.
</ParamField>

Router가 `GET /v2/models/openai/gpt-4.1/openapi.json`에서 제공하는 스키마에서 생성되었으며, 이는 요청이 공급자에 도달하기 전에 호출을 검증하는 데 사용하는 동일한 문서입니다.
### 출력

<ResponseField name="instructions" type="string">
  모델의 컨텍스트에서 첫 번째 항목으로 시스템(또는 개발자) 메시지를 삽입합니다.

`previous_response_id`와 함께 사용하는 경우, 이전 응답의 지침은 다음 응답으로 이어지지 않습니다. 따라서 새 응답에서 시스템(또는 개발자) 메시지를 간단히 교체할 수 있습니다.
</ResponseField>
...
<ResponseField name="truncation" type="string" default="&quot;disabled&quot;">
  모델 응답에 사용할 잘림 전략입니다.
- `auto`: 이 응답과 이전 응답의 컨텍스트가 모델의 컨텍스트 창 크기를 초과하면, 모델은 대화 중간의 입력 항목을 삭제하여 응답이 컨텍스트 창에 맞도록 잘라냅니다.
...
<ResponseField name="previous_response_id" type="string">
  모델에 대한 이전 응답의 고유 ID입니다. 이를 사용하여 다중 턴 대화를 생성할 수 있습니다. [대화 상태](https://platform.openai.com/docs/guides/conversation-state)에 대해 더 알아보세요.
</ResponseField>
...
[추론 모델](https://platform.openai.com/docs/guides/reasoning)의 추론 노력을 제한합니다. 현재 지원되는 값은 `low`, `medium`, `high`입니다. 추론 노력을 줄이면 응답이 더 빨라지고 응답에서 추론에 사용되는 토큰이 줄어들 수 있습니다.
...
모델이 수행한 추론의 요약입니다. 디버깅과 모델의 추론 과정을 이해하는 데 유용할 수 있습니다. `auto`, `concise`, `detailed` 중 하나입니다.
...
`{ "type": "json_schema" }`를 구성하면 Structured Outputs가 활성화되어 모델이 제공한 JSON 스키마와 일치하도록 보장합니다. 자세한 내용은 [Structured Outputs 가이드](https://platform.openai.com/docs/guides/structured-outputs)에서 확인하세요.

기본 형식은 추가 옵션이 없는 `{ "type": "text" }`입니다.

**gpt-4o 및 최신 모델에는 권장되지 않음:**

`{ "type": "json_object" }`로 설정하면 이전 JSON 모드가 활성화되어 모델이 생성하는 메시지가 유효한 JSON임을 보장합니다. 이를 지원하는 모델에서는 `json_schema`를 사용하는 것이 권장됩니다.
</ResponseField>

<ResponseField name="text.verbosity" type="string">
  모델 응답의 장황함을 제한합니다. `low`, `medium`, `high` 중 하나입니다.
</ResponseField>

<ResponseField name="tool_choice" type="`none`, `auto`, `required` | object">
  응답을 생성할 때 모델이 사용할 도구(또는 도구들)를 어떻게 선택해야 하는지 지정합니다. 모델이 호출할 수 있는 도구를 지정하는 방법은 `tools` 매개변수를 참조하세요.
</ResponseField>
...
<ResponseField name="completed_at" type="number">
  이 응답이 완료된 시점의 Unix 타임스탬프(초)입니다. 상태가 `completed`일 때만 표시됩니다.
</ResponseField>
...
<ResponseField name="frequency_penalty" type="number">
  지금까지의 텍스트에서 해당 토큰이 기존에 나타난 빈도를 기준으로 새 토큰에 페널티를 부여합니다.
</ResponseField>
...
<ResponseField name="output_text" type="string">
  SDK 전용 편의 속성으로, `output` 배열의 모든 `output_text` 항목에 있는 텍스트 출력을 집계한 값을 담고 있습니다(있는 경우).
...
<ResponseField name="presence_penalty" type="number">
  지금까지의 텍스트에 해당 토큰이 나타나는지 여부를 기준으로 새 토큰에 페널티를 부여합니다.
</ResponseField>

<ResponseField name="prompt_cache_key" type="string">
  OpenAI가 유사한 요청에 대한 응답을 캐시하여 캐시 적중률을 최적화하는 데 사용됩니다. `user` 필드를 대체합니다.
</ResponseField>
...
<ResponseField name="usage.input_tokens_details.cached_tokens" type="integer" required>
  캐시에서 검색된 토큰 수입니다.
[프롬프트 캐싱 자세히 보기](https://platform.openai.com/docs/guides/prompt-caching).
</ResponseField>
