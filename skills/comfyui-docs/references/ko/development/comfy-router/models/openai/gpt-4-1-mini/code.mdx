---
title: "Comfy Router에서 GPT 4.1 Mini 사용하기"
description: "Comfy Router를 통해 openai/gpt-4.1-mini 호출하기: endpoint, 요청 형태, 그리고 Router가 반환하는 응답."
translationSourceHash: cdb50b60
translationFrom: development/comfy-router/models/openai/gpt-4-1-mini/code.mdx
translationBlockHashes:
  "_intro": 22aeae46
  "Quick start": bd92ba2a
  "Schema": b894ba56
  "Examples": 67e6c8f1
---

`openai/gpt-4.1-mini`의 API 레퍼런스이며, OpenAI에서 제공하고 Comfy Router가 서빙합니다.
...
**모델 ID:** `openai/gpt-4.1-mini`

**엔드포인트:** `POST https://api.comfy.org/v2/models/openai/gpt-4.1-mini`
...
동일한 본문을 `POST https://api.comfy.org/v2/models/openai/gpt-4.1-mini/requests`로 보냅니다. Router는 실행이 접수되는 즉시 `201`과 `request_id`를 응답하며, 결과는 준비되는 대로 이 프로세스나 다른 프로세스에서 수집할 수 있습니다. 상태, 취소, 수집 방법은 [대기 중 전송](/ko/development/comfy-router/queue)를 참고하세요.
...
async def main():
    async with AsyncComfy() as client:
        ...
        # 공급자의 자체 payload이며, models.run()이 반환하는 것과 동일한 값입니다.
...
// 환경에서 COMFY_API_KEY를 읽습니다.
// 각 submit() 호출은 자체 Idempotency-Key를 발급하고 자동 재시도에 재사용합니다.
const handle = await comfy.models.submit("openai/gpt-4.1-mini", {
  input: "Reply with the single word: ok",
  max_output_tokens: 1024,
});
...
// models.run()이 반환하는 것과 동일한 결과입니다. 실패했거나 취소된 요청은 여기서 reject됩니다.
...
curl -i https://api.comfy.org/v2/models/openai/gpt-4.1-mini/requests/$REQUEST_ID/status \
  -H "X-API-Key: $COMFY_API_KEY"

# 3. 수집합니다. 모델의 네이티브 출력과 함께 200, 아직 실행 중일 때는 상태 본문과 함께 202를 반환합니다.
curl https://api.comfy.org/v2/models/openai/gpt-4.1-mini/requests/$REQUEST_ID \
  -H "X-API-Key: $COMFY_API_KEY"
```
...
<ParamField body="input" type="string | object[]" required>
  응답을 생성하는 데 사용되는 모델에 대한 텍스트, 이미지 또는 파일 입력입니다. 이 계약에서 Router가 제공할 수 없는 유일한 필드이며, 아래 `required`에 있는 유일한 항목입니다.
</ParamField>
...
<ParamField body="max_output_tokens" type="integer">
  표시되는 출력 토큰과 추론 토큰을 포함하여 응답을 위해 생성되는 토큰 수의 상한입니다. 추론 id에서는 이 상한을 숨김 추론 토큰과 공유하므로, 작은 값은 표시되는 텍스트가 나오기 전에 전체 예산을 소진할 수 있습니다. 이것이 reasoning 스모크 케이스가 1024를 보내고 chat 케이스가 16을 보내는 이유입니다.
...
<ParamField body="reasoning" type="object">
  REASONING TIER 전용. 추론 모델을 위한 구성으로, 예: `{"effort": "medium"}`. 변경 없이 전달됩니다. 허용되는 키는 OpenAI의 reasoning 가이드를 참조하세요. chat-tier id는 이를 무시합니다.
</ParamField>
...
<ParamField body="stream" type="boolean">
  이 필드를 보내는 호출자가 거부되지 않도록 선언되어 있지만, 이 표면에서는 아무 효과가 없습니다(INERT). Router는 디스패치 이전에 이를 `false`로 확정합니다. `text/event-stream`을 중계하는 대신 공급자 응답을 캡처하기 때문입니다. 이는 openAiResponsesProxy의 ModifyResponse가 디코딩할 수 없으므로, 스트리밍 생성은 OpenAI가 과금하고 아무도 미터링하지 않게 됩니다. 스트림이 필요하면 `POST /proxy/openai/v1/responses`를 사용하세요.
</ParamField>

<ParamField body="temperature" type="number">
  샘플링 temperature입니다. CHAT TIER 전용: o-시리즈 추론 id(`o1`, `o1-pro`, `o3`, `o4-mini`)는 OpenAI에서 이 매개변수를 거부합니다. Router는 이들을 위해 이 매개변수를 거부하지 않습니다(두 tier가 하나의 스키마를 공유하는 이유는 이 컴포넌트의 설명을 참조하세요). 따라서 이를 보내는 reasoning 호출은 OpenAI 자체의 오류로 응답됩니다.
...
<ParamField body="tool_choice" type="string | object">
  모델이 사용할 도구를 어떻게 선택해야 하는지입니다. 문자열 모드 또는 도구를 지정하는 객체입니다.
</ParamField>
...
<ParamField body="top_p" type="number">
  Nucleus 샘플링 컷오프입니다. `temperature`와 동일한 조건으로 CHAT TIER 전용입니다.

  범위: `0` ~ `1`
</ParamField>

<ParamField body="truncation" type="string">
  컨텍스트가 모델의 창을 초과할 때의 잘라내기 전략입니다. 위의 세 가지 어휘와 달리 여기서는 enum이 실제로 강제됩니다. 이 두 값이 OpenAI가 문서화한 완전한 집합이며 늘어나지 않았기 때문입니다. 명시적인 `null`은 위 필드들과 동일한 조건으로 여전히 허용됩니다.

  가능한 값: `auto`, `disabled`
</ParamField>

<ParamField body="usage" type="object">
  토큰 사용량 봉투입니다. v1 오퍼레이션이 요청 본문에 이를 선언하기 때문에 이 계약에 존재합니다. OpenAI는 이를 RESPONSE에 채우므로, 호출자가 보낼 이유가 없습니다.
</ParamField>

Router가 `GET /v2/models/openai/gpt-4.1-mini/openapi.json`에서 제공하는 스키마에서 생성되었으며, 이는 요청이 공급자에 도달하기 이전에 호출을 검증하는 동일한 문서입니다.
### 출력

<ResponseField name="instructions" type="string">
  모델 컨텍스트의 첫 번째 항목으로 시스템(또는 개발자) 메시지를 삽입합니다.

`previous_response_id`와 함께 사용하면 이전 응답의 instructions가 다음 응답으로 이어지지 않습니다. 따라서 새 응답에서 시스템(또는 개발자) 메시지를 간단히 교체할 수 있습니다.
</ResponseField>
...
<ResponseField name="truncation" type="string" default="&quot;disabled&quot;">
  모델 응답에 사용할 잘라내기(truncation) 전략입니다.
- `auto`: 이 응답과 이전 응답의 컨텍스트가 모델의 컨텍스트 창 크기를
  초과하면, 모델은 대화 중간의 입력 항목을 버려 컨텍스트 창에 맞게
  응답을 자릅니다.
- `disabled` (기본값): 모델 응답이 모델의 컨텍스트 창 크기를 초과하면
  요청이 400 오류로 실패합니다.
...
<ResponseField name="reasoning.context" type="string">
  이후 턴에서 어떤 추론 항목을 모델에 다시 제공할지 제어합니다. 예: `auto`, `current_turn`, `all_turns`.
</ResponseField>
...
추론 노력을 제한합니다. 현재 지원되는 값은 `low`, `medium`, `high`입니다.
...
이해하는 데 유용합니다. `auto`, `concise`, `detailed` 중 하나입니다.
...
모델이 생성한 메시지가 유효한 JSON임을 보장합니다. 이를 지원하는
...
<ResponseField name="moderation" type="object">
  검열된 완료가 요청된 경우, 응답 입력과 출력에 대한 검열 결과입니다.
</ResponseField>

<ResponseField name="object" type="string">
  이 리소스의 객체 유형으로, 항상 `response`로 설정됩니다.
...
- `output` 배열에 있는 항목의 길이와 순서는 모델의
  응답에 따라 달라집니다.
- `output` 배열의 첫 번째 항목에 접근하여 모델이 생성한
  콘텐츠를 담은 `assistant` 메시지라고 가정하기보다는,
...
<ResponseField name="top_logprobs" type="integer">
  각 토큰 위치에서 반환할, 각각 관련 로그 확률이 포함된 가장 가능성이 높은 토큰의 최대 수입니다.
</ResponseField>
...
<ResponseField name="user" type="string">
  최종 사용자를 위한 지원 중단된 식별자입니다. `safety_identifier`와 `prompt_cache_key`로 교체되었습니다.
</ResponseField>
