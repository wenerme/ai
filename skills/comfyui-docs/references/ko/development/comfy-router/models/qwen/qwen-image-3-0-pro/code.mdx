---
title: "Comfy Router로 Qwen Image 3.0 Pro 사용하기"
description: "Comfy Router를 통해 qwen/qwen-image-3.0-pro를 호출하는 방법: 엔드포인트, 요청 형태, Router가 반환하는 응답."
translationSourceHash: 8202984a
translationFrom: development/comfy-router/models/qwen/qwen-image-3-0-pro/code.mdx
translationBlockHashes:
  "_intro": b9aa2049
  "Quick start": 808373bb
  "Schema": 3517aa0a
  "Examples": a4380dd1
---

Qwen에서 제공하고 Comfy Router가 서비스하는 `qwen/qwen-image-3.0-pro`의 API 레퍼런스입니다.
...
**모델 ID:** `qwen/qwen-image-3.0-pro`

**엔드포인트:** `POST https://api.comfy.org/v2/models/qwen/qwen-image-3.0-pro`
...
동일한 본문을 `POST https://api.comfy.org/v2/models/qwen/qwen-image-3.0-pro/requests` 로 보냅니다. Router는 실행이 접수되는 즉시 `request_id` 와 함께 `201` 을 응답하며, 결과는 준비되는 대로 이 프로세스 또는 다른 프로세스에서 수집합니다. 상태, 취소, 수집 방법은 [대기열 전송](/ko/development/comfy-router/queue)에서 자세히 다룹니다.

```python Python
from comfy_sdk import Comfy

# 환경에서 COMFY_API_KEY 를 읽습니다.
...
# 3. 수집. 모델의 네이티브 출력과 함께 200, 아직 실행 중이라면 상태 본문과 함께 202 를 반환합니다.
...
<ParamField body="input.messages[].content" type="object[]" required>
  메시지 콘텐츠 배열. 텍스트 기반 이미지 생성에서는 텍스트 객체 하나를, 이미지 편집에서는 이미지 객체 1-3개와 텍스트 객체 하나를 포함합니다
</ParamField>

<ParamField body="input.messages[].content[].image" type="string">
  입력 이미지의 URL 또는 Base64로 인코딩된 데이터. 이미지 편집 시 1-3개의 이미지를 지원합니다
</ParamField>

<ParamField body="input.messages[].content[].text" type="string">
  생성하거나 편집할 이미지의 내용, 스타일, 구도를 설명하는 긍정 프롬프트
</ParamField>
...
<ParamField body="model" type="string">
  멀티모달 이미지 생성 및 편집을 위해 호출할 모델의 ID. 사용 가능한 값은 qwen-image-3.0-pro와 qwen-image-3.0입니다. 이 스키마의 `required` 목록에 없는 이유는 Comfy Router가 /v2/models/qwen/\{model}의 `{model}` 경로 세그먼트에서 이 값을 채우기 때문입니다. 따라서 Router 호출자는 이를 생략하며, /proxy/ 라우트에 대한 직접 v1 호출은 이를 반드시 제공해야 합니다.
</ParamField>
...
<ParamField body="parameters.size" type="string">
  출력 이미지 해상도로, 너비*높이 형식입니다(예: 1024*1024). API는 262144(512*512)에서 6553600(2560*2560) 사이의 픽셀 면적과 1:8에서 8:1 사이의 비율을 허용합니다. 지정하지 않으면 모델이 프롬프트를 기반으로 해상도를 자동으로 추천합니다
</ParamField>

<ParamField body="parameters.watermark" type="boolean" default="false">
  워터마크 추가 여부. 기본값은 false
</ParamField>

Router가 `GET /v2/models/qwen/qwen-image-3.0-pro/openapi.json`에서 제공하는 스키마를 기반으로 생성되었으며, 이는 요청이 공급자에 도달하기 전에 호출을 검증하는 데 사용하는 것과 동일한 문서입니다.
...
<ResponseField name="output.choices[].message.content[].image" type="string">
  생성된 이미지의 URL로 PNG 형식입니다. 링크는 24시간 동안 유효합니다
</ResponseField>

<ResponseField name="output.choices[].message.content[].text" type="string">
  이미지 대신 반환되는 텍스트 요소. 이 필드만 담은 요소는 어떤 자산도 생성하지 않았으므로, 호출자는 콘텐츠 요소의 존재 여부가 아니라 `image`를 기준으로 완료 여부를 판단합니다
</ResponseField>
...
<ResponseField name="usage.input_image_count" type="integer">
  요청의 입력 이미지 수. 텍스트 기반 이미지 생성의 경우 0을 반환합니다
</ResponseField>
