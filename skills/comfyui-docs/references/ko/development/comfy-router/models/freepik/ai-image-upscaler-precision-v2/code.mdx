...
# SDK가 자동으로 idempotency 키를 생성하고 자동 재시도 시 재사용합니다.
...
// 환경 변수에서 COMFY_API_KEY를 읽습니다.
// SDK는 호출마다 idempotency 키를 생성하고 자동 재시도 시 재사용합니다.
...
동일한 본문을 `POST https://api.comfy.org/v2/models/freepik/ai-image-upscaler-precision-v2/requests` 로 보냅니다. Router는 실행이 접수되는 즉시 `201` 과 `request_id` 를 응답하며, 결과는 준비가 되는 대로 이 프로세스나 다른 프로세스에서 수집할 수 있습니다. 상태, 취소, 수집 방법은 [큐 전송](/ko/development/comfy-router/queue) 를 참고하세요.
...
# 2. 상태가 COMPLETED가 될 때까지 폴링하며, 각 응답이 알려준 Retry-After 초만큼 대기합니다.
