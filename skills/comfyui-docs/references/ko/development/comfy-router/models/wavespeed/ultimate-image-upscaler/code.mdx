...
# SDK는 idempotency 키를 자동으로 생성하고 자동 재시도 시 재사용합니다.
...
// 환경 변수에서 COMFY_API_KEY를 읽습니다.
// SDK는 호출마다 idempotency 키를 발급하고 자동 재시도 시 재사용합니다.
...
동일한 본문을 `POST https://api.comfy.org/v2/models/wavespeed/ultimate-image-upscaler/requests`로 전송합니다. Router는 실행이 접수되는 즉시 `request_id`와 함께 `201`로 응답하며, 결과는 준비되면 이 프로세스 또는 다른 프로세스에서 수집됩니다. [대기열 전달](/ko/development/comfy-router/queue)에서 상태, 취소, 수집 과정을 차례로 살펴봅니다.
