---
title: "ComfyUI 서버 경로 (HTTP 및 WebSocket API)"
description: "ComfyUI Server API 경로. prompt, 큐, 히스토리, 모델, 사용자 데이터를 다루는 내장 GET / POST / WebSocket 엔드포인트와 커스텀 경로 추가 방법."
translationSourceHash: 2d869972
...
무엇을 하는지 잘 알고 있지 않다면, 클래스 내부에 `my_function`을 정의하지 마세요.  
`@routes.post` 데코레이터는 많은 작업을 수행합니다! 대신 위와 같이 함수를 정의한 후 클래스 메서드를 호출하세요.

변경하지 않는다면 `@routes.get`도 정의할 수 있습니다.
