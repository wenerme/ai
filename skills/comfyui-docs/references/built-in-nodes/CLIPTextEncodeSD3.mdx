...
| `empty_padding` | COMBO | Yes | `"none"``"empty_prompt"` | Controls how empty text inputs are handled. When set to "none", empty text inputs for `clip_g`, `clip_l`, or `t5xxl` will result in empty token lists instead of padding. This is an advanced parameter (default: "none"). |
