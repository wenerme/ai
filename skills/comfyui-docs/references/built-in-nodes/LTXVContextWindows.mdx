...
| `closed_loop` | Whether to close the context window loop; only applicable to looped schedules. (default: False) | BOOLEAN | No | TrueFalse |
...
| `freenoise` | Whether to apply FreeNoise noise shuffling, improves window blending. (default: True) | BOOLEAN | No | TrueFalse |
| `retain_first_frame` | Retain the first latent frame in every context window (may help retain initial reference). (default: False) | BOOLEAN | No | TrueFalse |
| `split_conds_to_windows` | Whether to split multiple conditionings (created by ConditionCombine) to each window based on region index. (default: False) | BOOLEAN | No | TrueFalse |
