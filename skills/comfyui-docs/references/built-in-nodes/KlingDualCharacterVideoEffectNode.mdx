...
| `effect_scene` | COMBO | Yes | `"chat"``"dance"``"hug"``"kill"``"kiss"``"pat"``"punch"``"shrug"``"slap"``"tickle"` | The type of special effect scene to apply to the video generation |
| `model_name` | COMBO | No | `"kling-v1"``"kling-v1-5"``"kling-v1-6"` | The model to use for character effects (default: "kling-v1") |
| `mode` | COMBO | No | `"std"``"pro"` | The video generation mode (default: "std") |
| `duration` | COMBO | Yes | `"5"``"10"` | The duration of the generated video in seconds |
