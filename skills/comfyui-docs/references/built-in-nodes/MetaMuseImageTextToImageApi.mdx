...
| `enable_web_search` | Lets the model search the web for facts and live information while planning the image. | BOOLEAN | No | TrueFalse (default: True) |
| `enable_image_search` | Lets the model search for reference images while planning the image. | BOOLEAN | No | TrueFalse (default: True) |
| `enable_shell` | Lets the model run code while planning, for precise layouts, charts and diagrams; when off, quantities and alignment are approximated. | BOOLEAN | No | TrueFalse (default: True) |
