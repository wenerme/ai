...
|-----------|-------------|-----------|----------|-------|
| `model` | The Gemini video model used to generate the video. | DYNAMIC_COMBO | Yes | "Omni Flash 1.1""Omni Flash" |
...
| `resolution` | Output resolution. (default: "720p") | COMBO | Yes | "360p""720p""1080p""4k" |
| `aspect_ratio` | Output aspect ratio: 16:9 (landscape) or 9:16 (portrait). The 'edit' and 'extend' tasks keep the aspect ratio of the input video instead. (default: "16:9") | COMBO | Yes | "16:9""9:16" |
| `task_type` | What to do with the prompt and the attached media. With 'auto' the model decides. 'text_to_video' generates from the prompt alone and rejects attached media. 'image_to_video' animates one image, or interpolates from a starting frame to an ending frame when two are attached. 'reference_to_video' treats the attached media as subject references. 'edit' rewrites exactly one attached video, and 'extend' appends new footage to it, so the output starts with the input video. (default: "auto") | COMBO | Yes | "auto""text_to_video""image_to_video""reference_to_video""edit""extend" |
...
| `aspect_ratio` | Output aspect ratio: 16:9 (landscape) or 9:16 (portrait). The 'edit' task keeps the aspect ratio of the input video instead. (default: "16:9") | COMBO | Yes | "16:9""9:16" |
| `task_type` | What to do with the prompt and the attached media. With 'auto' the model decides. 'text_to_video' generates from the prompt alone and rejects attached media. 'image_to_video' animates one image, or interpolates from a starting frame to an ending frame when two are attached. 'reference_to_video' treats the attached media as subject references. 'edit' rewrites exactly one attached video. (default: "auto") | COMBO | Yes | "auto""text_to_video""image_to_video""reference_to_video""edit" |
