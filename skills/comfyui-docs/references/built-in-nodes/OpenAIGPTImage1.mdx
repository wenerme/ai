...
Generates images synchronously via OpenAI's GPT Image endpoint. The node can create new images from text prompts or edit existing images when provided with an input image and optional mask. It supports the gpt-image-1, gpt-image-1.5, and gpt-image-2 models and is marked as deprecated.
...
- The `gpt-image-1` and `gpt-image-1.5` models only support the sizes `auto`, `1024x1024`, `1024x1536`, and `1536x1024`.
