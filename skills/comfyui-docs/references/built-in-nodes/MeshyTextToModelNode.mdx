...
| `style` | COMBO | Yes | `"realistic"``"sculpture"` | The artistic style for the generated 3D model. |
| `should_remesh` | DYNAMIC COMBO | Yes | `"true"``"false"` | Controls whether the generated mesh is processed. When set to "false", the node returns an unprocessed triangular mesh. Selecting "true" reveals additional parameters for topology and polycount. |
| `topology` | COMBO | No* | `"triangle"``"quad"` | The target polygon type for the remeshed model. This parameter is only available and required when `should_remesh` is set to "true". |
...
| `symmetry_mode` | COMBO | Yes | `"auto"``"on"``"off"` | Controls symmetry in the generated model. |
| `pose_mode` | COMBO | Yes | `""``"A-pose"``"T-pose"` | Specifies the pose mode for the generated model. An empty string means no specific pose is requested. |
