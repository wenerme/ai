...
|-------------|-----------|-------------|
| `positive` | CONDITIONING | Positive conditioning with video control data (vace_frames, vace_mask, vace_strength) applied |
| `negative` | CONDITIONING | Negative conditioning with video control data (vace_frames, vace_mask, vace_strength) applied |
| `latent` | LATENT | Empty latent tensor ready for video generation with shape [batch_size, 16, latent_length, height/8, width/8] |
