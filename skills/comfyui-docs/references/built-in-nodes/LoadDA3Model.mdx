...
| `weight_dtype` | The numerical precision (data type) for the model weights. "default" uses the model's original precision, "fp16" uses 16-bit floating point, "bf16" uses bfloat16, and "fp32" uses 32-bit floating point. (default: "default") | STRING | Yes | `"default"``"fp16"``"bf16"``"fp32"` |
