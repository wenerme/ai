...
description: "The Load Depth Anything 3 node loads a Depth Anything 3 model from a file, preparing it for depth estimation tasks."
mode: wide
---

The **Load Depth Anything 3** node loads a Depth Anything 3 model from a file, preparing it for depth estimation tasks. You can select the model file and optionally choose the numerical precision (data type) for the model's weights.
...
| `weight_dtype` | The numerical precision (data type) for the model weights. "default" uses the model's original precision, "fp16" uses 16-bit floating point, "bf16" uses bfloat16, and "fp32" uses 32-bit floating point. (default: "default") | STRING | No | `"default"``"fp16"``"bf16"``"fp32"` |
