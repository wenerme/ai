...
| `geometry_file_format` | The output file format for the 3D model geometry. | COMBO | Yes | `"glb"``"obj"``"stl"``"usdz"` |
| `texture_mode` | The texture generation mode. "Original" preserves input textures, "Clean" removes them, and "Style" applies a stylized texture. | COMBO | Yes | `"Original"``"Clean"``"Style"` |
