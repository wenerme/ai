...
| `mode` | COMBO | Yes | `"Fast"``"Regular"``"Extreme-High"` | The generation quality mode. Higher quality modes produce better results but cost more. |
| `material` | COMBO | Yes | `"PBR"``"Matte"` | The material type for the generated 3D model. |
| `geometry_file_format` | COMBO | Yes | `"glb"``"obj"``"stl"``"usdz"` | The output file format for the 3D model geometry. |
| `texture_mode` | COMBO | Yes | `"Original"``"Clean"``"Style"` | The texture generation mode. "Original" preserves input textures, "Clean" removes them, and "Style" applies a stylized texture. |
