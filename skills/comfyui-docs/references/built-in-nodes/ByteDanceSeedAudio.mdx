...
description: "Generate speech, music, sound effects and multi-speaker dialogue from a single prompt with ByteDance Seed Audio 1.0."
mode: wide
---
...
| `model` | Model version. `seed-audio-1.0-multilingual` supports 20 languages and per-sentence timing control via `[5.5s:8.0s]` timestamps. `seed-audio-1.0` supports English and Chinese only, without timing control. (default: "seed-audio-1.0-multilingual") | COMBO | No | `"seed-audio-1.0-multilingual"``"seed-audio-1.0"` |
