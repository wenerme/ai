...
| --- | --- | --- | --- | --- |
| `mode` | The audio input mode. `single_speaker` uses one audio input. `two_speakers` enables the additional audio input and masks listed in the Two Speakers Inputs section. | DYNAMIC_COMBO | Yes | `"single_speaker"``"two_speakers"` |
