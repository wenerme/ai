...
| `crop` | Crop region in pixels. Zero width/height keeps the full frame. The crop rectangle provides `x`, `y`, `width`, and `height` values, all with a default of 0. | VIDEO_EDIT | Yes | `x` ≥ 0`y` ≥ 0`width` ≥ 0`height` ≥ 0All values default to 0 |
