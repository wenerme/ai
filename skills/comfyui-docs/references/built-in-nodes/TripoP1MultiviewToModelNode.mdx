...
| `output_mode` | COMBO | Yes | `"geometry"``"textured"``"detailed"` | The output mode for the generated model. `"geometry"` produces a raw mesh, `"textured"` adds a standard texture, and `"detailed"` creates a high-detail textured model (default: `"textured"`). |
