...
description: "This node generates a video by interpolating between a provided start frame and an end frame, guided by a text prompt."
mode: wide
---
...
|-----------|-------------|-----------|----------|-------|
| `model` | The Vidu model to use for video generation. | COMBO | Yes | `"viduq2-pro-fast"``"viduq2-pro"``"viduq2-turbo"` |
...
| `resolution` | The output resolution of the generated video. | COMBO | No | `"720p"``"1080p"` |
| `movement_amplitude` | The movement amplitude of objects in the frame. | COMBO | No | `"auto"``"small"``"medium"``"large"` |
