...
| `moderation` | Moderation settings for the request. Selecting `"true"` reveals two additional boolean toggles; selecting `"false"` sends no moderation flags. Default: `"false"`. | DYNAMIC_COMBO | Yes | `"false"``"true"` |
...
| Parameter | Description | Data Type | Required | Range |
|-----------|-------------|-----------|----------|-------|
| `visual_input_moderation` | Enables visual content moderation on the input image. Default: false. | BOOLEAN | No | truefalse |
| `visual_output_moderation` | Enables visual content moderation on the output image. Default: false. | BOOLEAN | No | truefalse |
