...
| `device` | COMBO | Yes | `"default"``"cpu"``"gpu:0"``"gpu:1"``"gpu:2"``"gpu:3"``"gpu:4"``"gpu:5"``"gpu:6"``"gpu:7"` | The device to place the CLIP text encoder on. `"default"` restores the device assigned by the loader. `"cpu"` pins both the load and offload device to CPU. `"gpu:N"` pins the load device to the Nth available GPU (default: `"default"`). |
