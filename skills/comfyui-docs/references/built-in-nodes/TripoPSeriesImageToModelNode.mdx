...
| `model.texture_alignment` | Match the colors of the input image, or fit the textures to the generated geometry. Ignored when `model.texture` is `"none"` (default: `"original_image"`). Advanced setting. | COMBO | Yes | `"original_image"``"geometry"` |
| `model.orientation` | `"align_image"` rotates the model to the viewpoint of the input image. Ignored when `model.texture` is `"none"` (default: `"default"`). Advanced setting. | COMBO | Yes | `"default"``"align_image"` |
