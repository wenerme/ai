...
|-----------|-----------|----------|-------|-------------|
| `model` | COMBO | Yes | `"Seedance 2.0"``"Seedance 2.0 Fast"` | The model to use for video generation. Seedance 2.0 is for maximum quality, while Seedance 2.0 Fast is optimized for speed. Selecting a model will reveal additional inputs for `prompt`, `resolution`, `ratio`, `duration`, and `generate_audio`. |
