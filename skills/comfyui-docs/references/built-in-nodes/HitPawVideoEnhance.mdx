...
| --- | --- | --- | --- | --- |
| `model` | The AI model to use for video enhancement. Selecting a model reveals a nested `resolution` parameter. The available models and their supported resolutions vary. | DYNAMIC COMBO | Yes | `"Portrait Restore Model (1x)"``"Portrait Restore Model (2x)"``"General Restore Model (1x)"``"General Restore Model (2x)"``"General Restore Model (4x)"``"Ultra HD Model (2x)"``"Generative Model (1x)"` |
| `model.resolution` | The target resolution for the enhanced video. The `"8K"` option is only available for models other than `"Generative Model (1x)"`. | COMBO | Yes | For `"Generative Model (1x)"`: `"original"``"720p"``"1080p"``"2K/QHD"``"4K/UHD"`For all other models: `"original"``"720p"``"1080p"``"2K/QHD"``"4K/UHD"``"8K"` |
