...
description: "The Veo3FirstLastFrameNode uses Google's Veo 3 model to generate a video based on a text prompt, with a provided first and last frame that define the start and end of the video seq"
mode: wide
---
...
| `resolution` | The resolution of the output video. | COMBO | Yes | `"720p"``"1080p"``"4k"` |
| `aspect_ratio` | Aspect ratio of the output video (default: "16:9"). | COMBO | No | `"16:9"``"9:16"` |
...
| `model` | The specific Veo model to use for generation (default: "veo-3.1-generate"). | COMBO | No | `"veo-3.1-generate"``"veo-3.1-fast-generate"``"veo-3.1-lite"` |
