...
|-----------|-------------|-----------|----------|-------|
| `model` | Selects the Wan 3.0 model variant used for generation. | DYNAMIC_COMBO | Yes | `wan3.0-video``wan3.0-video-prime` |
...
| `ratio` | Aspect ratio of the output video. With "adaptive", the output dimensions are derived from the input media. | COMBO | Yes | "adaptive""16:9""9:16""1:1""4:3""3:4" |
| `duration` | Output duration in seconds. With "auto", the model chooses a duration that fits the prompt and reference media. The combined duration of reference videos and output must not exceed 30 seconds. | COMBO | Yes | "auto""2" through "30" (integer seconds) |
