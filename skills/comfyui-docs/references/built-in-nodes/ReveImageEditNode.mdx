...
| `model` | MODEL | Yes | `"reve-edit@20250915"``"reve-edit-fast@20251030"` | Model version to use for editing. |
| `model.aspect_ratio` | COMBO | No | `"auto"``"16:9"``"9:16"``"3:2"``"2:3"``"4:3"``"3:4"``"1:1"` | The aspect ratio for the edited image. When set to "auto", the aspect ratio is determined automatically. |
...
| `upscale` | COMBO | No | `"disabled"``"enabled"` | Controls whether to upscale the generated image. |
