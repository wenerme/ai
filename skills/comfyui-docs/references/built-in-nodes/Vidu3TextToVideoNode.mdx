...
|-----------|-----------|----------|-------|-------------|
| `model` | COMBO | Yes | `"viduq3-pro"``"viduq3-turbo"` | Model to use for video generation. Selecting a model reveals additional configuration parameters for aspect ratio, resolution, duration, and audio. |
| `model.aspect_ratio` | COMBO | Yes* | `"16:9"``"9:16"``"3:4"``"4:3"``"1:1"` | The aspect ratio of the output video. This parameter is revealed when a `model` is selected. |
| `model.resolution` | COMBO | Yes* | `"720p"``"1080p"` | Resolution of the output video. This parameter is revealed when a `model` is selected. |
