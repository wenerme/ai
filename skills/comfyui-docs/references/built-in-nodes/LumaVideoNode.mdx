...
| `model` | COMBO | Yes | `"ray_1_6"``"ray_2"` | The video generation model to use. |
| `aspect_ratio` | COMBO | Yes | `"16:9"``"9:16"``"1:1"``"4:3"``"3:4"``"21:9"``"9:21"` | The aspect ratio for the generated video (default: "16:9"). |
| `resolution` | COMBO | Yes | `"540p"``"720p"``"1080p"` | The output resolution for the video (default: "540p"). This parameter is ignored when using the `ray_1_6` model. |
| `duration` | COMBO | Yes | `"5s"``"9s"` | The duration of the generated video. This parameter is ignored when using the `ray_1_6` model. |
