...
| `model` | COMBO | Yes | `sd3.5-large``sd3.5-large-turbo``sd3.5-medium` | The Stable Diffusion 3.5 model to use for generation. |
| `aspect_ratio` | COMBO | Yes | `16:9``1:1``21:9``2:3``3:2``4:5``5:4``9:16``9:21` | Aspect ratio of generated image. (default: 1:1) |
| `style_preset` | COMBO | No | `3d-model``analog-film``anime``cinematic``comic-book``digital-art``enhance``fantasy-art``isometric``line-art``low-poly``modeling-compound``neon-punk``origami``photographic``pixel-art``tile-texture``None` | Optional desired style of generated image. Select "None" for no style preset. |
