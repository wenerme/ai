...
| `direction` | Forward continues after the prior clip; backward is prepended before it. Selecting "Forward (continue after)" also adds the `loop` option. | COMBO | Yes | "Forward (continue after)""Backward (lead-in before)" |
| `loop` | Loop the extended video seamlessly (forward extend only). Only available when `direction` is "Forward (continue after)". Default: False. | BOOLEAN | No | TrueFalse |
