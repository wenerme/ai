...
|-----------|-------------|-----------|----------|-------|
| `mode` | The loop iteration mode (default: "simple"). The selected mode determines which additional parameters are shown. | DYNAMIC_COMBO | Yes | `"simple"``"For"``"List"` |
| `cache_iterations` | Reuse unchanged iteration results from previous executions. Disable to execute every iteration again. Default: false. | BOOLEAN | Yes | truefalse |
