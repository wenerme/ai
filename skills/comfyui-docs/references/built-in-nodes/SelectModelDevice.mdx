...
| `device` | COMBO | Yes | `"default"``"cpu"``"gpu:0"``"gpu:1"``"gpu:2"``"gpu:3"``"gpu:4"``"gpu:5"``"gpu:6"``"gpu:7"` | The target device for the model. Options are dynamically generated based on available GPUs. (default: "default") |
