...
| `device` | The target device for the model. Options are dynamically generated based on the available devices on the current machine. (default: `"default"`) | COMBO | Yes | `"default"``"cpu"``"gpu:0"``"gpu:1"`... (one `"gpu:N"` entry per detected GPU) |
