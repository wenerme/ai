...
| `style_preset` | STRING | Yes | `"3d-model"``"analog-film"``"anime"``"cinematic"``"comic-book"``"digital-art"``"enhance"``"fantasy-art"``"isometric"``"line-art"``"low-poly"``"modeling-compound"``"neon-punk"``"origami"``"photographic"``"pixel-art"``"tile-texture"` | Optional desired style of generated image. (default: "None") |
