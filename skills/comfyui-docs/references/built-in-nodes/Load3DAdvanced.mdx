...
|-----------|-------------|-----------|----------|-------|
| `model_file` | The 3D model file to load. Select "none" to skip loading a model file. Files can also be uploaded through the widget. | COMBO | Yes | `"none"`Available 3D model files in the `input/3d` directory |
| `viewport_state` | The current viewport state containing camera and model information from the 3D viewer. | LOAD3D | Yes | - |
| `width` | Render width of the viewport in pixels (default: 1024). | INT | Yes | Min: 1Max: 4096Default: 1024Step: 1 |
| `height` | Render height of the viewport in pixels (default: 1024). | INT | Yes | Min: 1Max: 4096Default: 1024Step: 1 |
