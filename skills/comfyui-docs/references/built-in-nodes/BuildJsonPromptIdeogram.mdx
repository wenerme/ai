...
| `style` | The visual style category for the generated image. Select one of the options. (default: "none") | COMBO | Yes | `"none"``"photo"``"art_style"` |
...
**Note:** When `style` is set to "photo", the `photo` input becomes available and you should usually set `medium` to "photograph". When `style` is set to "art_style", the `art_style` input becomes available. The `background`, `aesthetics`, `lighting`, and `medium` parameters have default empty strings but are considered mandatory for reliable image generation; providing values is strongly recommended. The `high_level_description` value is only included in the output JSON when it is not blank. The `color_palette` values are only included in the output when a style other than "none" is selected and at least one color is provided.

## Outputs

| Output Name | Description | Data Type |
|-------------|-------------|-----------|
| `prompt` | A structured JSON dictionary containing the high-level description (when provided), the style description (if a style other than "none" is selected), and the compositional deconstruction with background and elements. | DICT |
