...
| `device` | Specifies the device to load the model onto. Use `"cpu"` to force loading onto the CPU. The default behavior (`"default"`) uses the system's automatic device placement (default: `"default"`). This is an advanced parameter. | COMBO | No | `"default"``"cpu"` |
