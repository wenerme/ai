...
|-----------|-------------|-----------|----------|-------|
| `source` | Segment an existing model, or generate a model from an image and segment it. The selected option determines which additional inputs appear. | DYNAMIC_COMBO | Yes | `"model"``"image"` |
...
| `granularity` | How finely the model is split into parts (default: "medium"). | COMBO | No | `"coarse"``"medium"``"fine"` |
