...
| `bit_depth` | Auto uses 8-bit for sRGB and 10-bit for HDR and HDR PQ. Explicit 8-bit and 10-bit choices are independent of colorspace. (default: "auto") | COMBO | No | `"auto"`810 |
| `color_space` | Colorspace of the input images. HDR selects BT.2020/HLG and HDR PQ selects BT.2020/PQ. (default: "sRGB") | COMBO | No | `"sRGB"``"HDR"``"HDR PQ"` |
| `codec` | Optionally encode the video immediately. None keeps the images in tensor form; Auto uses H.264. (default: "none") | COMBO | No | `"none"`Available video codec options from the video codec list (e.g. `"auto"` and other supported codecs) |
