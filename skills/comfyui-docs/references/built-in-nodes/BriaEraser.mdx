...
| `mask_type` | Selects how the mask was created. "manual" is for hand-drawn or brush masks; "automatic" is for masks produced by segmentation models such as SAM. | STRING | Yes | "manual""automatic" |
| `moderation` | Moderation settings. Set to "true" to enable content moderation on the input and/or output images. | STRING | Yes | "false""true" |
