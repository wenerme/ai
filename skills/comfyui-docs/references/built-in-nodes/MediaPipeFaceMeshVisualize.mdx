...
description: "Documentation for MediaPipeFaceMeshVisualize node."
mode: wide
---
...
| `connections` | Determines which parts of the face mesh to draw. `"all"` draws the full mesh (oval, eyes, brows, lips, irises, nose). `"fill"` draws a solid polygon of the face oval (silhouette mask). `"custom"` lets you toggle each feature individually. (default: `"all"`) | COMBO | Yes | `"all"``"fill"``"custom"` |
