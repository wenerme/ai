...
| `dim` | The dimension along which to cut the latent samples. "x" cuts along the last axis (typically width), "y" along the second-to-last axis (typically height), and "t" along the third-to-last axis (typically frames in video latents) | COMBO | Yes | "x""y""t" |
