...
| `interpolation` | The interpolation method used to transition between strength values (default: LINEAR) | COMBO | Yes | `LINEAR``EASE_IN``EASE_OUT``EASE_IN_OUT``EASE_OUT_IN``SINE``CUBIC``QUARTIC``QUINTIC``EXPO``CIRC``BACK``BOUNCE``ELASTIC` |
