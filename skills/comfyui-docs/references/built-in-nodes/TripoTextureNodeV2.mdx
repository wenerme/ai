...
| `pbr` | PBR material maps (base color, metallic, roughness, normal); off gives a plain color texture. (default: true) | BOOLEAN | No | truefalse |
| `texture_seed` | Seed used for texture generation. (default: 42) Advanced input. | INT | No | 0 to 2147483647 |
| `texture_quality` | Quality of the generated textures. `detailed` = HD textures, `extreme` = 8K Ultra textures. (default: "standard") Advanced input. | COMBO | No | "standard""detailed""extreme" |
| `texture_alignment` | How the textures are aligned to the model. (default: "original_image") Advanced input. | COMBO | No | "original_image""geometry" |
