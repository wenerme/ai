...
| `rig_type` | Skeleton type. "auto" runs Tripo's free rig check first and uses the recommended type. Default: "auto". | COMBO | No | "auto""biped""quadruped""hexapod""octopod""avian""serpentine""aquatic" |
| `spec` | Bone naming: Tripo native or Mixamo-compatible. Tripo cannot retarget its animation presets onto a v1.0 rig made with the mixamo spec; use tripo for Tripo: Retarget rigged model. Default: "tripo". | COMBO | No | "tripo""mixamo" |
