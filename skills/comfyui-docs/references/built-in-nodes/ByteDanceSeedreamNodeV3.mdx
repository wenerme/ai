...
| `model` | Selects the Seedream model to use. Each model exposes its own set of sub-parameters and limits below. | DYNAMIC_COMBO | Yes | "seedream 5.0 pro""seedream 5.0 lite""seedream-4-5-251128""seedream-4-0-250828" |

### Seedream 5.0 Pro Inputs (seedream 5.0 pro)

| Parameter | Description | Data Type | Required | Range |
|-----------|-------------|-----------|----------|-------|
| `size_preset` | Pick a recommended size. Select Custom to use the width and height below. Default: first recommended preset for this model. | COMBO | No | Model-specific recommended size presets"Custom" |
...
| `prompt_optimization` | Prompt-optimization mode when reference images are provided: 'standard' gives higher quality, 'fast' shorter generation time. Default: "standard". | COMBO | No | "standard""fast" |
