...
| `model` | COMBO | Yes | `"uni-1"``"uni-1-max"` | Model to use for generation. Selecting a model reveals additional settings for that model. |
...
| Parameter | Data Type | Required | Range | Description |
|-----------|-----------|----------|-------|-------------|
| `aspect_ratio` | COMBO | Yes | `"auto"``"3:1"``"2:1"``"16:9"``"3:2"``"1:1"``"2:3"``"9:16"``"1:2"``"1:3"` | Output image aspect ratio. `"auto"` lets the model pick based on the prompt. (default: `"auto"`) |
| `style` | COMBO | Yes | `"auto"``"manga"` | The visual style for the generated image. (default: `"auto"`) |
