...
| `output` | The type of image to render from the geometry data. DirectX vs OpenGL controls the normal-map green-channel convention. DirectX: green = -Y down (Unreal). OpenGL: green = +Y up (Blender, Substance, Unity, glTF). (default: "depth") | COMBO | Yes | `"depth"``"depth_colored"``"normal_opengl"``"normal_directx"``"mask"` |
