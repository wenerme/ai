...
| `tracks` | Optional motion tracking data containing object paths. Uses `track_path` and optionally `track_visibility`; if `track_visibility` is missing, all tracks are treated as visible. | TRACKS | No | - |
