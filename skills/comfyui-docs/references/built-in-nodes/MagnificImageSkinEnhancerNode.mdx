...
| `optimized_for` | COMBO | No | `"enhance_skin"``"improve_lighting"``"enhance_everything"``"transform_to_real"``"no_make_up"` | Enhancement optimization target. This input is only available when the `mode` is set to `"flexible"`. |
