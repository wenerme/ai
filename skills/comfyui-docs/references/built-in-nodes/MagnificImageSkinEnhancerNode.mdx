...
| `mode` | The processing mode to use. `"creative"` is for artistic enhancement, `"faithful"` for preserving the original appearance, and `"flexible"` for targeted optimization. | COMBO | Yes | `"creative"``"faithful"``"flexible"` |
...
| `optimized_for` | Enhancement optimization target. This input is only available when the `mode` is set to `"flexible"`. | COMBO | No | `"enhance_skin"``"improve_lighting"``"enhance_everything"``"transform_to_real"``"no_make_up"` |
