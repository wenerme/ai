...
| `aspect_ratio` | Output aspect ratio. The "auto" option is only allowed when at least one reference video is connected. | COMBO | Yes | "auto"plus the predefined PixVerse V6 aspect ratios |
| `quality` | Output resolution. Sets the long edge: 360p is 640px, 540p 1024px, 720p 1280px, 1080p 1920px. Default: "720p". | COMBO | Yes | "360p""540p""720p""1080p" |
...
| `generate_audio` | Generate a native audio track together with the video. Default: True. | BOOLEAN | Yes | TrueFalse |
...
| `style` | An optional visual style applied to the whole video. Default: "none". | COMBO | No | "none"plus the predefined PixVerse V6 styles |
