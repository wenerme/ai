...
| `aspect_ratio` | The desired aspect ratio (width to height) for the generated image. | COMBO | Yes | `"16:9"``"9:16"``"1:1"``"4:3"``"3:4"``"3:2"``"2:3"``"21:9"` |
| `series_amount` | Generate a series of images. This feature is not supported for the `kling-image-o1` model. (default: "disabled") | COMBO | Yes | `"disabled"``"2"``"3"``"4"``"5"``"6"``"7"``"8"``"9"` |
