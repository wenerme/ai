...
| `model` | The specific Gemini model to use for generation. The "Nano Banana 2 (Gemini 3.1 Flash Image)" option maps to the `gemini-3.1-flash-image` model internally, and "gemini-3-pro-image-preview" maps to `gemini-3-pro-image`. | COMBO | Yes | `"gemini-3-pro-image-preview"``"Nano Banana 2 (Gemini 3.1 Flash Image)"` |
...
| `response_modalities` | Choose 'IMAGE' for image-only output, or 'IMAGE+TEXT' to return both the generated image and a text response. | COMBO | Yes | `"IMAGE+TEXT"``"IMAGE"` |
