...
| `context_schedule` | COMBO | Yes | `"static_standard"``"uniform_standard"``"uniform_looped"``"batched"` | The stride of the context window. |
...
| `fuse_method` | COMBO | Yes | `"pyramid"``"gaussian"``"average"``"overlap"` | The method to use to fuse the context windows (default: "pyramid"). |
