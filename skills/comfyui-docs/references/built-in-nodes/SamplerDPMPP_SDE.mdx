...
| `noise_device` | Selects the device where noise calculations are performed. When set to "cpu", the `dpmpp_sde` sampler is created; when set to "gpu", the `dpmpp_sde_gpu` sampler is created (default: "gpu") | COMBO | Yes | "gpu""cpu" |
