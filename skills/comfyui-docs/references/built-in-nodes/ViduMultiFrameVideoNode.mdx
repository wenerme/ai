...
|-----------|-------------|-----------|----------|-------|
| `model` | The Vidu model to use for video generation. | COMBO | Yes | "viduq2-pro""viduq2-turbo" |
...
| `resolution` | The resolution of the output video. | COMBO | Yes | "720p""1080p" |
| `frames` | Number of keyframe transitions (2-9). Selecting a value dynamically reveals the required inputs for each frame. | DYNAMIC_COMBO | Yes | "2""3""4""5""6""7""8""9" |
...
- The number of frames can be between 2 and 9, so the possible frame indices run from 1 to 9.
