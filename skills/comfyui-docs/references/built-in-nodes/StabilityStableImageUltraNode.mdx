...
| `aspect_ratio` | COMBO | Yes | `"1:1"``"16:9"``"21:9"``"2:3"``"3:2"``"4:5"``"5:4"``"9:16"``"9:21"` | Aspect ratio of generated image (default: "1:1"). |
| `style_preset` | COMBO | No | `"3d-model"``"analog-film"``"anime"``"cinematic"``"comic-book"``"digital-art"``"enhance"``"fantasy-art"``"isometric"``"line-art"``"low-poly"``"modeling-compound"``"neon-punk"``"origami"``"photographic"``"pixel-art"``"tile-texture"` | Optional desired style of generated image. Select "None" to not apply any style preset. |
