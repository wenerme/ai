...
| `generate_audio` | When enabled, the node generates audio for the video. Note: `"kling-3.0-turbo"` always generates native audio, so this toggle is ignored for that model. Default is True. | BOOLEAN | Yes | TrueFalse |
