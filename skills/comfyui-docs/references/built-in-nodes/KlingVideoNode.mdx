...
|-----------|-------------|-----------|----------|-------|
| `multi_shot` | Generate a series of video segments with individual prompts and durations. When set to a storyboard option, additional inputs for each storyboard's prompt and duration appear. | DYNAMIC_COMBO | Yes | `"disabled"``"1 storyboard"``"2 storyboards"``"3 storyboards"``"4 storyboards"``"5 storyboards"``"6 storyboards"` |
| `generate_audio` | When enabled, the node generates audio for the video. Note: `"kling-3.0-turbo"` always generates native audio, so the audio toggle is ignored for that model. Default is True. | BOOLEAN | Yes | TrueFalse |
| `model` | Model and generation settings. Selecting a model reveals its resolution and aspect ratio sub-parameters. | DYNAMIC_COMBO | Yes | `"kling-v3"``"kling-3.0-turbo"` |
...
| Parameter | Description | Data Type | Required | Range |
|-----------|-------------|-----------|----------|-------|
| `model.resolution` | The resolution for the generated video. Default is `"1080p"`. | COMBO | Yes | `"4k"``"1080p"``"720p"` |
| `model.aspect_ratio` | The aspect ratio for the generated video. Ignored in image-to-video mode. | COMBO | Yes | `"16:9"``"9:16"``"1:1"` |

### kling-3.0-turbo Inputs

| Parameter | Description | Data Type | Required | Range |
|-----------|-------------|-----------|----------|-------|
| `model.resolution` | The resolution for the generated video. Default is `"720p"`. | COMBO | Yes | `"1080p"``"720p"` |
| `model.aspect_ratio` | The aspect ratio for the generated video. Ignored in image-to-video mode. | COMBO | Yes | `"16:9"``"9:16"``"1:1"` |
