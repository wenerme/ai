...
| `storyboards` | Generate a series of video segments with individual prompts and durations. Only supported for `kling-v3-omni`. | DYNAMIC_COMBO | No | `"disabled"``"1 storyboard"``"2 storyboards"``"3 storyboards"``"4 storyboards"``"5 storyboards"``"6 storyboards"` |
