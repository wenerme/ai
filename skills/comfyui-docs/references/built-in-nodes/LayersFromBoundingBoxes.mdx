...
| `crop_to_content` | Crop each frame to metadata.content_rect where present and place the content at the box position plus the rect offset. Leave on for batches whose frames are padded — it keeps only the real content at its true spot. (default: true) | BOOLEAN | No | truefalse |
