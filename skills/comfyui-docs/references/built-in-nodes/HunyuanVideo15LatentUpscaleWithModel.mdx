...
| `upscale_method` | The interpolation algorithm used for the initial upscaling step (default: `"bilinear"`). | COMBO | Yes | `"nearest-exact"``"bilinear"``"area"``"bicubic"``"bislerp"` |
...
| `crop` | Determines how the upscaled latent is cropped to fit the target dimensions. | COMBO | Yes | `"disabled"``"center"` |
