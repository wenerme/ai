...
| `alpha_mode` | COMBO | Yes | `"select"``"fill"``"custom"` | How to handle the alpha matte. "select" uses a keyframe to select the subject, "fill" replaces the entire image without a separate matte, "custom" uses a user-provided mask. |
| `max_resolution` | COMBO | Yes | `"1080p"``"720p"` | The maximum resolution for the output image. Higher resolution costs more credits. |
