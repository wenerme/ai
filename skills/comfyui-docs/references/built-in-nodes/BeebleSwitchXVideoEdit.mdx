...
| `alpha_mode` | COMBO | Yes | `"fill"``"select"``"custom"` | The alpha matte mode. "fill" mode has no separate matte and fills the entire frame. "select" mode uses a single keyframe image to define the area to edit. "custom" mode uses a full alpha video to define the area to edit frame by frame. |
| `max_resolution` | COMBO | Yes | `"720p"``"1080p"` | The maximum resolution for the output video (default: "1080p"). |
