...
| `dim` | COMBO | Yes | `"x"``"-x"``"y"``"-y"``"t"``"-t"` | The dimension along which to concatenate the latent samples. Positive values concatenate samples1 before samples2, negative values concatenate samples2 before samples1 |
