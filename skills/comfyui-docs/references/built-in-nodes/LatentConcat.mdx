...
description: "The LatentConcat node combines two latent samples by joining them together along a chosen dimension."
mode: wide
---
...
| `dim` | The dimension along which to concatenate the latent samples. Positive values (x, y, t) place samples1 before samples2 in the result. Negative values (-x, -y, -t) place samples2 before samples1. The dimension mapping is: x = width, y = height, t = time/frames | COMBO | Yes | `"x"``"-x"``"y"``"-y"``"t"``"-t"` |

**Note:** The second latent sample (`samples2`) is automatically repeated as needed to match the batch size of the first latent sample (`samples1`) before concatenation.
