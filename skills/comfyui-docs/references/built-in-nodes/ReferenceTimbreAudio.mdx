...
description: "This node sets a reference audio timbre for use in the \"ace step 1.5\" process."
mode: wide
---

This node sets a reference audio timbre for use in the "ace step 1.5" process. It works by taking a conditioning input and optionally a latent representation of audio, then attaches that latent data to the conditioning for use by subsequent nodes in the workflow. This node is currently marked as experimental.
...
| `latent` | An optional latent representation of the reference audio. When provided, its samples are added (appended) to the conditioning so they can be used as reference audio timbre latents. | LATENT | No |  |
