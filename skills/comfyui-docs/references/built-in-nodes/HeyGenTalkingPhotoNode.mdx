...
| `speech` | Drive the avatar with a text script (HeyGen text-to-speech) or your own audio. | DYNAMIC_COMBO | Yes | `"script"``"audio"` |
| `resolution` | Output video resolution (default: `"1080p"`). | COMBO | No | `"720p"``"1080p"` |
| `aspect_ratio` | Output aspect ratio. `"auto"` follows the input image (default: `"auto"`). | COMBO | No | `"auto"``"16:9"``"9:16"``"1:1"``"4:5"``"5:4"` |
| `expressiveness` | How expressive the animated face and gestures are (default: `"low"`). | COMBO | No | `"low"``"medium"``"high"` |
