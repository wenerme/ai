...
|-----------|-------------|-----------|----------|-------|
| `placement` | "spread across the clip" lets FLUX 3 place the images (one opens the clip, two become its start and end); "at times" pins every image to a second you choose. | DYNAMIC_COMBO | Yes | `"spread across the clip"` (default)`"at times"` |
...
| `aspect_ratio` | Output aspect ratio. "auto" picks one from the prompt and inputs. | COMBO | Yes | `"auto"` (default)other available aspect ratios |
| `duration` | Clip length in seconds. "auto" fits the length to the content. | COMBO | Yes | `"auto"` (default)other available durations |
| `resolution` | Output resolution. | COMBO | Yes | `"720p"` (default)`"1080p"` |
...
### spread across the clip Inputs

This placement option has no additional parameters.
...
| Parameter | Description | Data Type | Required | Range |
|-----------|-------------|-----------|----------|-------|
| `keyframes` | Growable slot: connect 1 to 10 keyframe images in playback order, e.g. `image_1`, `image_2`, and so on. Each image becomes a frame of the clip. Minimum 256x256 pixels each; aspect ratio cannot be more extreme than 64:1. | IMAGE | Yes | 1 to 10 images |
