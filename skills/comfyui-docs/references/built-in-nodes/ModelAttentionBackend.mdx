...
| `attention` | The attention backend to use (default: "pytorch attention"). If the selected backend is unavailable, PyTorch attention is used as a fallback. | STRING | Yes | "pytorch attention""comfy kitchen attention" |
