...
|-----------|-----------|----------|-------|-------------|
| `model` | STRING | Yes | `"generative_portrait"``"generative"` | The enhancement model to use. The `generative_portrait` model is optimized for portraits, while `generative` is a general-purpose model. |
| `image` | IMAGE | Yes | - | The input image to be enhanced. |
| `upscale_factor` | INT | Yes | `1``2``4` | The factor by which to upscale the image's dimensions. A factor of 1 means no upscaling, 2 doubles the dimensions, and 4 quadruples them. |
