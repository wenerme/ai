...
| `loop` | Make the video loop seamlessly. Not available when an `end_frame` is set. | BOOLEAN | Yes | TrueFalse |
...
**Note:** At least one of `start_frame` or `end_frame` must be provided. If both are provided, the generated video transitions from the start frame to the end frame. The `loop` option cannot be enabled when an `end_frame` is set.
