...
description: "The WanCameraImageToVideo node prepares conditioning and latent data for video generation from images."
mode: wide
---

The WanCameraImageToVideo node prepares conditioning and latent data for video generation from images. It takes positive and negative conditioning prompts, along with an optional starting image and optional camera controls, and outputs modified conditioning plus an empty latent tensor ready for a video model to fill in.
...
**Note:** When `start_image` is provided, only the first `length` frames of the input image are used to initialize the video sequence, and the node applies a mask to blend these starting frames with generated content. The `camera_conditions` and `clip_vision_output` parameters are optional, but when provided, they modify the conditioning for both positive and negative prompts.
