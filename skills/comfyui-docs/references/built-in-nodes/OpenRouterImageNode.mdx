...
|-----------|-------------|-----------|----------|-------|
| `model` | The OpenRouter image model used to generate the image. Selecting a model reveals the model-specific options listed below. | DYNAMIC_COMBO | Yes | `microsoft/mai-image-2.6``microsoft/mai-image-2.6-flash` |
...
| `aspect_ratio` | Aspect ratio of the generated image, also applied when reference images are connected. Default: `"1:1"`. `"auto"` lets the model choose the ratio for text to image (rendered at the 1.5K size) and keeps the aspect ratio of the first reference image when editing. | COMBO | Yes | `"1:1"``"16:9"``"9:16"``"3:2"``"2:3"``"4:3"``"3:4"``"auto"` |
| `resolution` | Output size tier. 1K is about 1 megapixel (1:1 is 1024x1024, 16:9 is 1360x768); 1.5K is about 2.3 megapixels (1:1 is 1536x1536, 16:9 is 2048x1152). Default: `"1K"`. Ignored when `aspect_ratio` is `"auto"`. | COMBO | Yes | `"1K"``"1.5K"` |
