...
This node generates a video using the MiniMax H3 model, using reference images, videos, and audio to guide the result. Reference media are referred to in the prompt by their connection order, for example "Image 1", "Image 2", "Video 1", or "Audio 1".
...
| `prompt` | Text description of the video to generate. Reference media can be referred to by their order, for example "Image 1", "Image 2", "Video 1", or "Audio 1". | STRING | Yes | Min length: 1 character (after trimming whitespace) |
...
| `watermark` | Whether to add an AIGC watermark to the video (default: false). | BOOLEAN | No | truefalse |
