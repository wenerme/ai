...
| `format` | The output container. Auto uses MP4 for Auto/H.264 and WebM for AV1. MP4, MKV, and WebM select a specific container. Selecting a format also determines which codec options are available (default: `auto`). | DYNAMIC_COMBO | Yes | `"auto"``"mp4"``"mkv"``"webm"` |
| `codec` | The output video codec. Auto preserves a compatible source stream. H.264 and AV1 re-encoding support SDR, HDR (HLG), and HDR PQ. This selector is nested under the chosen format (default: `auto`). | DYNAMIC_COMBO | No | `"auto"``"h264"``"av1"` (not available with the `webm` format) |
...
| Parameter | Description | Data Type | Required | Range |
| --- | --- | --- | --- | --- |
| `encoding` | Automatic preserves compatible H.264 streams. Re-encode applies custom encoding options. | DYNAMIC_COMBO | No | `"auto"``"re-encode"` |
...
| Parameter | Description | Data Type | Required | Range |
| --- | --- | --- | --- | --- |
| `encoding` | Automatic preserves compatible AV1 streams. Re-encode applies custom encoding options. | DYNAMIC_COMBO | No | `"auto"``"re-encode"` |
