...
| `position` | How to place this image on the output video's timeline. Choose between absolute timing (seconds from start) or fractional timing (percentage of video duration). Selecting an option reveals the corresponding sub-parameter. | COMBO | Yes | `Absolute (seconds)``Fraction (0.0 to 1.0)` |
