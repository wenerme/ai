...
| `size` | Output resolution level. "auto" follows the input image size (clamped to the 1K-2K range). Default: "auto". | COMBO | Yes | "auto""1K""1.5K""2K" |
| `seed` | Seed to use for generation. Default: 0. | INT | Yes | 0 to 2147483647 |
| `prompt_optimization` | Prompt-optimization mode: "standard" gives higher quality, "fast" shorter generation time. Default: "standard". | COMBO | No | "standard""fast" |
