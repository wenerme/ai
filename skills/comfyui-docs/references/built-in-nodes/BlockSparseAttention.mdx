...
| `sink_conditioning` | MiniMax-H3 only. `exact_kv`: every query attends the packed text/audio/reference rows exactly (about 3% cost). `exact_kv_and_rows`: additionally runs the target-audio query rows dense (keeps generated audio intact). `off` disables this behavior. Default: "exact_kv_and_rows". Advanced input. | COMBO | No | `"exact_kv"``"exact_kv_and_rows"``"off"` |
