...
| Parameter | Description | Data Type | Required | Range |
|-----------|-------------|-----------|----------|-------|
| `images` | Growable slot: connect 1 or more reference images to edit. The first slot is `image`, additional slots are `image_1`, `image_2`, etc. The maximum number of images depends on the selected model. | IMAGE | Yes | 1 image for `grok-imagine-image-pro`1 to 3 images for `grok-imagine-image-2.0`, `grok-imagine-image-quality`, and `grok-imagine-image` |
