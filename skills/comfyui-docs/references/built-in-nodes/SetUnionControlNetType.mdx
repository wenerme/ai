...
| `type` | The control network type to apply. Use "auto" for automatic type detection or select a specific control network type from the available options (default: "auto") | COMBO | Yes | `"auto"``"openpose"``"depth"``"hed/pidi/softedge"``"canny"``"scribble"``"seg"``"tile"``"inpaint"``"lineart"``"blur"``"mlsd"``"normalbae"``"mask"` |
