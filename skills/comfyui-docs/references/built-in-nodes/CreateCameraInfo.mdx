...
|-----------|-------------|-----------|----------|-------|
| `mode` | How to define the camera: orbit angles, an explicit position, or a position + quaternion. | COMBO | Yes | `"orbit"``"look_at"``"quaternion"` |
...
| `camera_type` | Projection used by Render Splat: perspective (foreshortening) or orthographic (parallel). (default: "perspective") | COMBO | No | `"perspective"``"orthographic"` |
