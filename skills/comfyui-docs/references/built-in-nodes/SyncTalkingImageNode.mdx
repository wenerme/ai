...
description: "Animate a still portrait into a talking video driven by speech audio, using sync.so's sync-3 model."
mode: wide
---
...
| `speaker_selection` | Which face to animate when several people are visible. `default`: let the model decide. `coordinates`: target the face at pixel (`speaker_x`, `speaker_y`) in the image. Auto-detection is not supported for images. (default: "default") | COMBO | No | `"default"``"coordinates"` |
...
| `auto_downscale` | Automatically downscale the image if it exceeds the 4K (4096x2160) input limit; speaker coordinates are scaled to match. When disabled, an oversized image raises an error instead. (default: True) | BOOLEAN | No | TrueFalse |
