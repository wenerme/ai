...
Router has not published an authored input schema for this model yet: `GET /v2/models/qwen/qwen-image-3.0-pro/openapi.json` returns an open object with `x-comfy-input-schema-authored: false`. Router forwards the body to Qwen unchanged, so Qwen's own API documentation is authoritative for the request fields, and nothing is validated server side.
