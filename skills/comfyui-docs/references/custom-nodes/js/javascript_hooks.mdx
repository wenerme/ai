---
title: "ComfyUI JavaScript Hooks"
description: "Reference for ComfyUI frontend JavaScript hooks: app.registerExtension, nodeCreated, beforeRegisterNodeDef, and the callbacks that customize UI behavior."
---
...
Events during workflow execution are handled by 
the `apiUpdateHandlers` 
...
This method is called, on each registered extension, for _every_ node type, not just the ones added by that extension.
...
Since other extensions may also modify nodes, aim to write code that makes as few assumptions as possible.
...
With great power comes great responsibility. Hijacking core behavior makes it more likely your nodes
will be incompatible with other custom nodes, or future Comfy updates
