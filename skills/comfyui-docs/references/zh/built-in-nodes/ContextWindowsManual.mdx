...
| `context_schedule` | COMBO | 否 | `STATIC_STANDARD``UNIFORM_STANDARD``UNIFORM_LOOPED``BATCHED` | 上下文窗口的步进调度模式。 |
| `context_stride` | INT | 否 | 1+ | 上下文窗口的步长；仅适用于均匀调度模式（默认值：1）。 |
| `closed_loop` | BOOLEAN | 否 | - | 是否闭合上下文窗口循环；仅适用于循环调度模式（默认值：False）。 |
| `fuse_method` | COMBO | 否 | `PYRAMID``LIST_STATIC` | 用于融合上下文窗口的方法（默认值：PYRAMID）。 |
