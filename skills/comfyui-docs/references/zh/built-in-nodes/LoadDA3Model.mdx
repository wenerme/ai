...
description: "此节点从文件加载 Depth Anything 3 模型，为深度估计任务做好准备。"
mode: wide
---
...
| `weight_dtype` | 模型权重的数值精度（数据类型）。"default" 使用模型的原始精度，"fp16" 使用 16 位浮点数，"bf16" 使用 bfloat16，"fp32" 使用 32 位浮点数。（默认值："default"） | STRING | 否 | `"default"``"fp16"``"bf16"``"fp32"` |
