...
| `output` | 从几何数据渲染的图像类型。DirectX 与 OpenGL 控制法线贴图的绿色通道约定。DirectX：绿色 = -Y 向下（Unreal）。OpenGL：绿色 = +Y 向上（Blender、Substance、Unity、glTF）。（默认值："depth"） | COMBO | 是 | `"depth"``"depth_colored"``"normal_opengl"``"normal_directx"``"mask"` |
