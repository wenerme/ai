...
description: "WaveSpeed 图像放大节点使用外部 AI 服务来提高图像的分辨率和质量。"
mode: wide
---
...
| --- | --- | --- | --- | --- |
| `model` | 用于放大的 AI 模型。"SeedVR2" 和 "Ultimate" 提供不同的质量和价格等级。 | STRING | 是 | `"SeedVR2"``"Ultimate"` |
| `image` | 待放大的输入图像。 | IMAGE | 是 |  |
| `target_resolution` | 放大后图像的期望输出分辨率。 | STRING | 是 | `"2K"``"4K"``"8K"` |
