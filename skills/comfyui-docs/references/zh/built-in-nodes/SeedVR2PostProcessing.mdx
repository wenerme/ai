...
| `color_correction_method` | 将生成图像颜色匹配到原始图像的方法。lab：在 CIELAB 色彩空间中传递颜色，保留细节（最忠实）。wavelet：传递低频颜色，保留放大后的高频细节。adain：按通道匹配均值/标准差（最快，全局色调）。none：跳过色彩传递（仅几何对齐）。（默认值："lab"） | COMBO | 是 | `"lab"``"wavelet"``"adain"``"none"` |
