...
| `分辨率` | 编辑后视频的输出分辨率。 | COMBO | 是 | `"360p"``"540p"``"720p"``"1080p"` |
| `强度` | 保留源内容与重新想象源内容之间的强度。"auto" 让 Ray 3.2 自动选择；adhere_* 最大程度保留原内容，flex_* 较为平衡，reimagine_* 变化最大。（默认值："auto"） | COMBO | 是 | `"auto"``"adhere_1"``"adhere_2"``"adhere_3"``"flex_1"``"flex_2"``"flex_3"``"reimagine_1"``"reimagine_2"``"reimagine_3"` |
| `种子` | 用于结果可复现的随机种子。 | INT | 是 | - |

| `分辨率` | 编辑后视频的输出分辨率。（默认值："720p"） | COMBO | 是 | `"360p"``"540p"``"720p"``"1080p"` |
