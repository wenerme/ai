...
| 参数 | 描述 | 数据类型 | 必需 | 范围 |
|-----------|-------------|-----------|----------|-------|
| `输出模式` | 控制生成的模型仅包含几何体还是还包含颜色/PBR 纹理。"Geometry only" 返回无纹理网格。"Textured" 添加颜色/PBR 贴图并显示下方的纹理选项。 | DYNAMIC_COMBO | 是 | `"Geometry only"``"Textured"` |
...
| `texture_quality` | 纹理质量预设。detailed = HD 纹理，extreme = 8K 超高清纹理。默认："standard"。 | COMBO | 是 | `"standard"``"detailed"``"extreme"` |
