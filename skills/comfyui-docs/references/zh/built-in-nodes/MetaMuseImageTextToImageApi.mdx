...
| `enable_web_search` | 允许模型在规划图像时搜索网络，获取事实和实时信息。 | BOOLEAN | 否 | TrueFalse (default: True) |
| `enable_image_search` | 允许模型在规划图像时搜索参考图像。 | BOOLEAN | 否 | TrueFalse (default: True) |
| `enable_shell` | 允许模型在规划过程中运行代码，以实现精确的布局、图表和示意图；关闭时，数量和排列将采用近似值。 | BOOLEAN | 否 | TrueFalse (default: True) |
