...
| `audio_setting` | 'auto'：模型根据提示词决定是否重新生成音频。'origin'：保留输入视频的原始音频。(默认: "auto") | COMBO | 是 | `"auto"``"origin"` |
...
| `ratio` | 宽高比。如果未更改，则近似输入视频的宽高比。 | COMBO | 是 | `"16:9"``"9:16"``"1:1"``"4:3"``"3:4"` |
| `duration` | 输出时长（秒）。'auto' 与输入视频时长匹配。具体值将从视频开头截取。(默认: "auto") | COMBO | 是 | `"auto"``"2"``"3"``"4"``"5"``"6"``"7"``"8"``"9"``"10"` |
