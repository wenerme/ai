...
| 参数 | 数据类型 | 是否必填 | 范围 | 描述 |
|-----------|-----------|----------|-------|-------------|
| `模型` | COMBO | 是 | `"Seedance 2.0"``"Seedance 2.0 Fast"` | 用于视频生成的模型。Seedance 2.0 提供最高质量，而 Seedance 2.0 Fast 则针对速度进行了优化。选择模型后，将显示 `prompt`、`resolution`、`ratio`、`duration` 和 `generate_audio` 的额外输入项。 |
