...
| 参数 | 描述 | 数据类型 | 是否必需 | 范围 |
| --- | --- | --- | --- | --- |
| `model` | 用于生成的模型。选择模型会显示该模型的附加设置。 | DYNAMIC_COMBO | 是 | `"uni-1"``"uni-1-max"` |
...
| 参数 | 描述 | 数据类型 | 是否必需 | 范围 |
| --- | --- | --- | --- | --- |
| `aspect_ratio` | 输出图像的宽高比。`"auto"` 让模型根据提示词自行选择。（默认：`"auto"`） | COMBO | 是 | `"auto"``"3:1"``"2:1"``"16:9"``"3:2"``"1:1"``"2:3"``"9:16"``"1:2"``"1:3"` |
| `style` | 风格预设。`"auto"` 根据提示词自动选择；`"manga"` 应用漫画/动漫美学，并要求使用纵向宽高比（2:3、9:16、1:2、1:3）。（默认：`"auto"`） | COMBO | 是 | `"auto"``"manga"` |
