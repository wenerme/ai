...
| `model` | 用于生成的模型。选择模型后会显示该模型的附加设置。 | COMBO | 是 | `"uni-1"``"uni-1-max"` |
...
| 参数 | 描述 | 数据类型 | 是否必填 | 取值范围 |
| --- | --- | --- | --- | --- |
| `aspect_ratio` | 输出图像的宽高比。`"auto"` 让模型根据提示自动选择。（默认值：`"auto"`） | COMBO | 是 | `"auto"``"3:1"``"2:1"``"16:9"``"3:2"``"1:1"``"2:3"``"9:16"``"1:2"``"1:3"` |
| `style` | 生成图像的视觉风格。（默认值：`"auto"`） | COMBO | 是 | `"auto"``"manga"` |
