...
| 参数 | 描述 | 数据类型 | 必填 | 范围 |
| --- | --- | --- | --- | --- |
| `模型` | 用于视频生成的 OpenAI Sora 模型（默认值："sora-2"） | COMBO | 是 | "sora-2""sora-2-pro" |
| `提示词` | 引导文本；如果存在输入图像，则可为空（默认值：空） | STRING | 是 | - |
| `尺寸` | 生成视频的分辨率（默认值："1280x720"） | COMBO | 是 | "720x1280""1280x720""1024x1792""1792x1024" |
| `时长` | 生成视频的时长，单位为秒（默认值：8） | COMBO | 是 | 4812 |
