...
| `scale_factor` | COMBO | 是 | `"2x"``"4x"``"8x"``"16x"` | 图像尺寸的放大倍数。 |
| `optimized_for` | COMBO | 是 | `"standard"``"soft_portraits"``"hard_portraits"``"art_n_illustration"``"videogame_assets"``"nature_n_landscapes"``"films_n_photography"``"3d_renders"``"science_fiction_n_horror"` | 为增强过程优化的风格或内容类型。 |
...
| `engine` | COMBO | 是 | `"automatic"``"magnific_illusio"``"magnific_sharpy"``"magnific_sparkle"` | 用于处理的特定 AI 引擎。 |
