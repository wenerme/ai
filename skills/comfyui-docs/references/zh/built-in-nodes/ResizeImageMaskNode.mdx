...
| `调整类型` | COMBO | 是 | `SCALE_BY``SCALE_DIMENSIONS``SCALE_LONGER_DIMENSION``SCALE_SHORTER_DIMENSION``SCALE_WIDTH``SCALE_HEIGHT``SCALE_TOTAL_PIXELS``MATCH_SIZE` | 用于确定新尺寸的方法。所需参数会根据所选类型而变化。 |
...
| `crop` | COMBO | 否 | `"disabled"``"center"` | 当尺寸与宽高比不匹配时应用的裁剪方法。仅当 `调整类型` 为 `SCALE_DIMENSIONS` 或 `MATCH_SIZE` 时可用（默认值："center"）。 |
...
| `缩放方法` | COMBO | 是 | `"nearest-exact"``"bilinear"``"area"``"bicubic"``"lanczos"` | 用于缩放的插值算法（默认值："area"）。 |
