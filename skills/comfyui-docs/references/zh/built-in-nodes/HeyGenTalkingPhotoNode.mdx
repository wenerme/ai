...
| `speech` | 通过文本脚本（HeyGen 文本转语音）或您自己的音频驱动虚拟形象。 | COMBO | 是 | `"script"``"audio"` |
...
| `aspect_ratio` | 输出宽高比。'auto' 跟随输入图像（默认值："auto"）。 | COMBO | 否 | `"auto"``"16:9"``"9:16"``"1:1"``"4:5"``"5:4"` |
| `expressiveness` | 动画面部和手势的表现力程度（默认值："low"）。 | COMBO | 否 | `"low"``"medium"``"high"` |
