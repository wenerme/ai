...
| `语音` | 使用文本脚本（HeyGen 文本转语音）或您自己的音频来驱动虚拟形象。 | DYNAMIC_COMBO | 是 | `"script"``"audio"` |
| `分辨率` | 输出视频分辨率（默认：`"1080p"`）。 | COMBO | 否 | `"720p"``"1080p"` |
| `宽高比` | 输出宽高比。`"auto"` 跟随输入图像（默认：`"auto"`）。 | COMBO | 否 | `"auto"``"16:9"``"9:16"``"1:1"``"4:5"``"5:4"` |
| `表现力` | 动画面部和手势的表现力程度（默认：`"low"`）。 | COMBO | 否 | `"low"``"medium"``"high"` |
