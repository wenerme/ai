...
| `类型` | 所加载模型的架构类型。这决定了使用哪个具体的编码器变体。默认值为 `"stable_diffusion"`。 | STRING | 是 | `"stable_diffusion"``"stable_cascade"``"sd3"``"stable_audio"``"mochi"``"ltxv"``"pixart"``"cosmos"``"lumina2"``"wan"``"hidream"``"chroma"``"ace"``"omnigen2"``"qwen_image"``"hunyuan_image"``"flux2"``"ovis"``"longcat_image"``"cogvideox"` |
| `设备` | 加载模型的目标设备。`"default"` 在可用时使用 GPU，而 `"cpu"` 强制使用 CPU 加载。这是一个高级选项（默认值：`"default"`）。 | STRING | 否 | `"default"``"cpu"` |
