...
| `设备` | VAE 的目标设备。`"default"` 恢复加载器分配的设备。`"gpu:N"` 将 VAE 固定到第 N 个可用的 GPU。不支持 CPU 作为选择，如果提供则会被忽略。（默认值：`"default"`） | COMBO | 是 | `"default"``"gpu:0"``"gpu:1"``"gpu:2"``"gpu:3"``"gpu:4"``"gpu:5"``"gpu:6"``"gpu:7"` |
