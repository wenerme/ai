...
|---------------|-------------|-----------|----------|-------|
| `sync_mode` | 处理视频与音频时长不匹配的方式；同时决定输出长度（默认值："bounce"）。 | COMBO | 是 | `"bounce"``"cut_off"``"loop"``"silence"``"remap"` |
| `speaker_selection` | 当画面中出现多人时，选择要唇形同步的面部（默认值："default"）。 | COMBO | 是 | `"default"``"auto-detect"``"coordinates"` |
