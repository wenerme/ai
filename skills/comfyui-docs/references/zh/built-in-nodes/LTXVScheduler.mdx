...
| `拉伸` | BOOLEAN | 是 | True/False | 将 sigma 值拉伸至 [terminal, 1] 范围内（默认值：True） |
| `终值` | FLOAT | 是 | 0.0-0.99 | 拉伸后 sigma 值的终止值（默认值：0.1） |
| `Latent` | LATENT | 否 | - | 可选的潜变量输入，用于计算 sigma 调整所需的 token 数量 |
