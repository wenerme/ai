...
description: "使用 sync.so 的 sync-3 模型，将静态人像动画化为由语音音频驱动的说话视频。"
mode: wide
---
...
| `speaker_selection` | 当画面中出现多人时，选择对哪张面部进行动画化。`default`：让模型自行决定。`coordinates`：定位图片中像素坐标 (speaker_x, speaker_y) 处的面部。图片不支持自动检测。（默认值："default"） | COMBO | 否 | `"default"``"coordinates"` |
...
| `auto_downscale` | 若图片超过 4K（4096x2160）输入限制，自动降低分辨率；说话者坐标会相应缩放。禁用时，超尺寸图片将引发错误。（默认值：True） | BOOLEAN | 否 | TrueFalse |
