...
| `mode` | COMBO | 是 | `"creative"``"faithful"``"flexible"` | 使用的处理模式。`"creative"` 用于艺术增强，`"faithful"` 用于保留原始外观，`"flexible"` 用于针对性优化。 |
| `skin_detail` | INT | 否 | 0 到 100 | 皮肤细节增强级别。此输入仅在 `mode` 设置为 `"faithful"` 时可用且必需（默认值：80）。 |
| `optimized_for` | COMBO | 否 | `"enhance_skin"``"improve_lighting"``"enhance_everything"``"transform_to_real"``"no_make_up"` | 增强优化目标。此输入仅在 `mode` 设置为 `"flexible"` 时可用且必需。 |
