...
| `model` | 用于编辑的模型版本。 | DYNAMIC_COMBO | 是 | `"reve-edit@20250915"``"reve-edit-fast@20251030"` |
| `upscale` | 放大生成的图像。可能会增加额外费用。（默认值："disabled"） | DYNAMIC_COMBO | 否 | `"disabled"``"enabled"` |
| `remove_background` | 从生成的图像中移除背景。可能会增加额外费用。（默认值：False） | BOOLEAN | 否 | `true``false` |
...
| 参数 | 描述 | 数据类型 | 必填 | 范围 |
|-----------|-------------|-----------|----------|-------|
| `aspect_ratio` | 输出图像的宽高比。设置为 "auto" 时，宽高比将自动确定。 | COMBO | 否 | `"auto"``"16:9"``"9:16"``"3:2"``"2:3"``"4:3"``"3:4"``"1:1"` |
