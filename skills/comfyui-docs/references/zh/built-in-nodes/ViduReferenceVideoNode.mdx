...
| `aspect_ratio` | COMBO | 否 | `"16:9"``"9:16"``"1:1"``"4:3"``"3:4"``"21:9"``"9:21"` | 输出视频的宽高比（默认值："16:9"） |
| `resolution` | COMBO | 否 | `"480p"``"720p"``"1080p"``"1440p"``"2160p"` | 支持的值可能因模型和时长而异（默认值："1080p"） |
| `movement_amplitude` | COMBO | 否 | `"auto"``"low"``"medium"``"high"` | 画面中物体的运动幅度（默认值："auto"） |
