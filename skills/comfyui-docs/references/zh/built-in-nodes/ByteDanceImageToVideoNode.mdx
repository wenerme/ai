...
|-----------|-------------|-----------|----------|-------|
| `模型` | 用于视频生成的 ByteDance 模型（默认：`"seedance-1-0-pro-fast-251015"`）。 | COMBO | 是 | `"seedance-1-5-pro-251215"``"seedance-1-0-pro-250528"``"seedance-1-0-pro-fast-251015"` |
...
| `摄像头已固定` | 指定是否固定摄像机。平台会在你的提示词后附加一条固定摄像机的指令，但不保证实际效果（默认：False）。 | BOOLEAN | 否 | `False``True` |
| `水印` | 是否在视频中添加“AI 生成”水印（默认：False）。 | BOOLEAN | 否 | `False``True` |
| `generate_audio` | 对于除 `seedance-1-5-pro-251215` 之外的任何模型，此参数都会被忽略（默认：False）。 | BOOLEAN | 否 | `False``True` |
