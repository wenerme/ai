...
| `style` | 风格化输出偏好。 | COMBO | 是 | `"standard"``"darker_but_realistic"``"clean"``"smooth"``"brighter"``"contrasted_n_hdr"``"just_composition"` |
...
| `advanced_settings` | 高级照明控制的微调选项。当设置为 `"enabled"` 时，将显示更多参数。 | DYNAMICCOMBO | 是 | `"disabled"``"enabled"` |
