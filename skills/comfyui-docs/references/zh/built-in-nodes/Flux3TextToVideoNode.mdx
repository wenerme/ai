...
description: "使用 FLUX 3 根据文本提示生成带有同步音频的视频。"
mode: wide
---
...
| `resolution` | 输出分辨率。（默认："720p"） | STRING | Yes | `"720p"``"1080p"` |
| `generate_audio` | 生成同步音频（环境音、语音、效果）。关闭则生成不带音轨的视频。（默认：True） | BOOLEAN | Yes | TrueFalse |
