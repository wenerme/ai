...
| `bit_depth` | 自动模式对 sRGB 使用 8 位，对 HDR 使用 10 位。显式选择 8 位或 10 位时与色彩空间无关。（默认值："auto"） | COMBO | 否 | `"auto"`810 |
| `color_space` | 输入图像的色彩空间。HDR 使用 BT.2020/HLG，HDR PQ 使用 BT.2020/PQ。（默认值："sRGB"） | COMBO | 否 | `"sRGB"``"HDR"``"HDR PQ"` |
