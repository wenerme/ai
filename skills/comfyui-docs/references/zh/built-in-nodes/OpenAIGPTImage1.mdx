...
| `质量` | COMBO | 否 | "low""medium""high" | 图像质量，影响成本和生成时间（默认："low"） |
| `背景` | COMBO | 否 | "auto""opaque""transparent" | 返回带或不带背景的图像（默认："auto"） |
| `尺寸` | COMBO | 否 | "auto""1024x1024""1024x1536""1536x1024""2048x2048""2048x1152""1152x2048""3840x2160""2160x3840""Custom" | 图像尺寸。选择"Custom"以使用自定义宽度和高度（仅限 GPT Image 2）（默认："auto"） |
| `数量` | INT | 否 | 1 到 8 | 生成图像的数量（默认：1） |
| `参考图像` | IMAGE | 否 | - | 用于图像编辑的可选参考图像 |
| `遮罩` | MASK | 否 | - | 用于修复的可选遮罩（白色区域将被替换） |
| `模型` | COMBO | 否 | "gpt-image-1""gpt-image-1.5""gpt-image-2" | 使用的 GPT 图像模型（默认："gpt-image-2"） |
