...
| `格式` | 3D 文件的输出格式。ply：标准 3D 高斯泼溅格式，包含完整球谐函数。ksplat：mkkellogg SplatBuffer（第 0 级，未压缩），仅包含基础颜色。spz：Niantic gzip 压缩格式（体积缩小约 10 倍），仅包含基础颜色（默认值："ply"） | COMBO | 是 | `"ply"``"ksplat"``"spz"` |
