...
| `flavor` | 风格迁移的风格类型。 | COMBO | 否 | "faithful""gen_z""psychedelia""detaily""clear""donotstyle""donotstyle_sharp" |
| `engine` | 处理引擎选择。 | COMBO | 否 | "balanced""definio""illusio""3d_cartoon""colorful_anime""caricature""real""super_real""softy" |
| `portrait_mode` | 启用肖像模式以进行面部增强。 | COMBO | 否 | "disabled""enabled" |
| `portrait_style` | 应用于肖像图像的视觉风格。此输入仅在 `portrait_mode` 设置为 "enabled" 时可用。 | COMBO | 否 | "standard""pop""super_pop" |
| `portrait_beautifier` | 肖像的面部美化强度。此输入仅在 `portrait_mode` 设置为 "enabled" 时可用。 | COMBO | 否 | "none""beautify_face""beautify_face_max" |
