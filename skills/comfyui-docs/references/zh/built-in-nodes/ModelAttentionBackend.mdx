...
| `attention` | 要应用的稠密注意力后端。Comfy Kitchen attention 使用量化 INT8 注意力，并且仅在 Nvidia 和 AMD GPU 上可用。默认值："pytorch attention"。如果所选后端不可用，则使用 PyTorch attention 作为回退。 | COMBO | 是 | "pytorch attention""comfy kitchen attention" |
