...
| `should_remesh` | COMBO | 是 | `"true"``"false"` | 确定是否对生成的网格进行处理。当设置为 `"false"` 时，节点将返回未经处理的三角网格。 |
| `topology` | COMBO | 否 | `"triangle"``"quad"` | 重新网格化输出的目标多边形类型。此参数仅在 `should_remesh` 设置为 `"true"` 时可用且必需。 |
...
| `should_texture` | COMBO | 是 | `"true"``"false"` | 确定是否生成纹理。将其设置为 `"false"` 将跳过纹理阶段，并返回一个没有纹理的网格。 |
