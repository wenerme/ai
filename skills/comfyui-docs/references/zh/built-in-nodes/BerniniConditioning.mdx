...
description: "BerniniConditioning 节点为 Wan2.2-A14B 模型准备视频和图像条件数据。"
mode: wide
---

BerniniConditioning 节点为 Wan2.2-A14B 模型准备视频和图像条件数据。它使用提供的 VAE 对源视频、参考视频和参考图像进行编码，然后将它们作为上下文令牌附加到条件数据中。任务将根据连接的输入自动推断。
...
| `参考视频` | 要插入到源视频中的视频（ads2v）。按长度裁剪并保持宽高比缩放（长边限制为 ref_max_size）。 | IMAGE | 否 | - |
| `参考图像` | 作为上下文令牌注入的参考图像（r2v, rv2v）。每张图像按其原始宽高比独立编码（长边限制为 ref_max_size）。最多可提供 8 张图像。 | IMAGE | 否 | 0 至 8 张图像 |
...
上下文流按固定顺序附加：`source_video`（源 ID 1），然后是 `reference_video`（源 ID 2），再是每张参考图像（源 ID 3、4...）。包含多张图像的参考图像输入会为每张图像贡献一个流。
