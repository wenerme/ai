...
| `模式` | COMBO | 是 | `"Fast"``"Regular"``"Extreme-High"` | 生成质量模式。更高质量的模式可产生更好的结果，但成本更高。 |
| `材质` | COMBO | 是 | `"PBR"``"Matte"` | 生成 3D 模型的材质类型。 |
| `几何文件格式` | COMBO | 是 | `"glb"``"obj"``"stl"``"usdz"` | 3D 模型几何体的输出文件格式。 |
| `贴图模式` | COMBO | 是 | `"Original"``"Clean"``"Style"` | 纹理生成模式。“Original”保留输入纹理，“Clean”移除纹理，“Style”应用风格化纹理。 |
