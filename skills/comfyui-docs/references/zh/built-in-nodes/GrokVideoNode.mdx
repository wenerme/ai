...
| 参数 | 描述 | 数据类型 | 是否必需 | 范围 |
|-----------|-------------|-----------|----------|-------|
| `model` | 用于视频生成的模型。 | COMBO | 是 | `"grok-imagine-video"``"grok-imagine-video-1.5"` |
| `prompt` | 所需视频的文本描述。当提供输入图像并使用 `grok-imagine-video-1.5` 时，此参数为可选项。 | STRING | 是 | - |
| `resolution` | 输出视频的分辨率。1080p 仅适用于 `grok-imagine-video-1.5`。 | COMBO | 是 | `"480p"``"720p"``"1080p"` |
| `aspect_ratio` | 输出视频的宽高比。 | COMBO | 是 | `"auto"``"16:9"``"4:3"``"3:2"``"1:1"``"2:3"``"3:4"``"9:16"` |
