...
| `desired_increase` | 分辨率倍数。输出每边必须不超过 8192 像素。 | COMBO | 是 | "2""4" |
| `auto_downscale` | 当输出将超过限制时，自动降低倍数；如果这仍然不够，则缩小输入图像。（默认：False） | BOOLEAN | 是 | TrueFalse |
| `内容审核` | 审核设置。设置为 "true" 时，启用 `visual_input_moderation` 和 `visual_output_moderation` 子选项，二者默认均为 False。 | DYNAMIC_COMBO | 是 | "false""true" |
