...
| `desired_increase` | 分辨率倍率。输出图像的每一边必须在 8192 像素以内。 | COMBO | 是 | "2""4" |
| `auto_downscale` | 当输出将超过限制时，自动降低倍率；如果仍然不够，则对输入图像进行降采样。（默认值：False） | BOOLEAN | 是 | TrueFalse |
| `moderation` | 审核设置。设置为“true”时，启用 `visual_input_moderation` 和 `visual_output_moderation` 子选项，两者均默认为 False。 | COMBO | 是 | "false""true" |
