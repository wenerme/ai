...
description: "此节点通过反投影深度图并对生成的点云进行三角剖分，将 DA3GEOMETRY 数据包转换为 3D 网格。"
mode: wide
---
...
| `batch_index` | 要转换的批次中的图像索引。每张图像的顶点数量不同，因此批次无法堆叠（默认值：0）。必须小于输入几何体的批次大小，否则将引发错误 | INT | 是 | 0 至 4096 |
...
深度值非有限、为零或为负的像素始终会被排除在网格之外。如果生成的网格为空，将引发错误；错误信息建议提高 `discontinuity_threshold`、降低 `confidence_threshold` 或禁用 `use_sky_mask`。
