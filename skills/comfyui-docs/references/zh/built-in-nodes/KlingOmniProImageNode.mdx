...
| --- | --- | --- | --- | --- |
| `模型` | 用于图像生成的特定 Kling AI 模型。 | COMBO | 是 | `"kling-v3-omni"``"kling-image-o1"` |
| `提示词` | 描述图像内容的文本提示。可包含正面和负面描述。文本长度必须在 1 到 2500 个字符之间。 | STRING | 是 | - |
| `分辨率` | 生成图像的目标分辨率。注意：`kling-image-o1` 模型不支持 4K 分辨率。 | COMBO | 是 | `"1K"``"2K"``"4K"` |
| `宽高比` | 生成图像的期望宽高比（宽度与高度之比）。 | COMBO | 是 | `"16:9"``"9:16"``"1:1"``"4:3"``"3:4"``"3:2"``"2:3"``"21:9"` |
| `系列数量` | 生成一系列图像。`kling-image-o1` 模型不支持此功能。（默认值："disabled"） | COMBO | 是 | `"disabled"``"2"``"3"``"4"``"5"``"6"``"7"``"8"``"9"` |
