...
| `image_type` | COMBO | 是 | KlingImageGenImageReferenceType 中的选项（从源代码提取） | 图像参考类型选择 |
| `image_fidelity` | FLOAT | 是 | 0.0 - 1.0 | 用户上传图像的参考强度（默认值：0.5） |
| `human_fidelity` | FLOAT | 是 | 0.0 - 1.0 | 主体参考相似度（默认值：0.45） |
| `model_name` | COMBO | 是 | "kling-v1"（及 KlingImageGenModelName 中的其他选项） | 图像生成的模型选择（默认值："kling-v1"） |
| `aspect_ratio` | COMBO | 是 | "16:9"（及 KlingImageGenAspectRatio 中的其他选项） | 生成图像的宽高比（默认值："16:9"） |
