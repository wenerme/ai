...
| `图像类型` | 图像参考类型选择（高级）。当提供参考图像时必填。 | COMBO | 是 | `"subject_reference"``"style_reference"` |
| `图像参考强度` | 用户上传图像的参考强度（默认值：0.5，高级） | FLOAT | 是 | 0.0 - 1.0 |
| `主体参考强度` | 主体参考相似度（默认值：0.45，高级） | FLOAT | 是 | 0.0 - 1.0 |
| `模型` | 图像生成模型选择（默认值："kling-v3"） | COMBO | 是 | `"kling-v3"``"kling-v2"``"kling-v1-5"` |
| `宽高比` | 生成图像的宽高比（默认值："16:9"） | COMBO | 是 | `"16:9"``"9:16"``"1:1"``"4:3"``"3:4"``"21:9"``"9:21"` |
