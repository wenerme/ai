...
description: "此节点从您的 ComfyUI 输入目录加载 3D 模型文件，并提供模型数据以及相机和视口信息。"
mode: wide
---
...
| 参数 | 描述 | 数据类型 | 是否必需 | 范围 |
|-----------|-------------|-----------|----------|-------|
| `model_file` | 要加载的 3D 模型文件。选择 "none" 可跳过加载模型。 | STRING | 是 | `"none"`input/3d 目录中可用的 3D 文件列表 |
| `viewport_state` | 当前视口状态，包含来自 3D 查看器的相机和模型信息。 | LOAD3D | 是 | - |
| `width` | 输出图像的宽度（像素），默认值：1024 | INT | 是 | 最小值：1最大值：4096步长：1 |
| `height` | 输出图像的高度（像素），默认值：1024 | INT | 是 | 最小值：1最大值：4096步长：1 |
