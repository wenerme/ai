...
| `model` | 用于生成的模型版本和宽高比。第一个选项选择模型，后续选项定义图像的宽高比。 | COMBO | 是 | `"reve-create@20250915"``"3:2"``"16:9"``"9:16"``"2:3"``"4:3"``"3:4"``"1:1"` |
| `upscale` | 启用或禁用放大后期处理步骤。启用时，您还必须选择放大倍数。 | COMBO | 否 | `"disabled"``"enabled"` |
| `upscale_factor` | 增加图像分辨率的倍数。此参数仅在 `upscale` 设置为 `"enabled"` 时生效。 | COMBO | 否 | `2``3``4` |
