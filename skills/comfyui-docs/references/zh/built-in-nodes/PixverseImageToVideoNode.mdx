...
| `质量` | 视频质量设置（默认值：res_540p） | COMBO | 是 | `res_540p``res_1080p` |
| `时长（秒）` | 生成视频的时长（秒） | COMBO | 是 | `dur_2``dur_5``dur_10` |
| `运动模式` | 应用于视频生成的运动风格 | COMBO | 是 | `normal``fast``slow``zoom_in``zoom_out``pan_left``pan_right``pan_up``pan_down``tilt_up``tilt_down``roll_clockwise``roll_counterclockwise` |
