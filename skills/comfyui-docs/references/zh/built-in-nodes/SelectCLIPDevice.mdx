...
| `device` | 放置 CLIP 文本编码器的设备。`"default"` 恢复由加载器分配的设备。`"cpu"` 将加载设备和卸载设备都固定到 CPU。`"gpu:N"` 将加载设备固定到第 N 个可用的 GPU（默认值：`"default"`）。 | COMBO | 是 | `"default"``"cpu"``"gpu:0"``"gpu:1"``"gpu:2"``"gpu:3"``"gpu:4"``"gpu:5"``"gpu:6"``"gpu:7"` |
