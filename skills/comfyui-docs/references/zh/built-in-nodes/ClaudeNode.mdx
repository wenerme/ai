...
| 参数 | 描述 | 数据类型 | 必填 | 范围 |
|-----------|-------------|-----------|----------|-------|
| `model` | 用于生成响应的 Claude 模型。选择模型后，下方会显示该模型专属的设置。 | DYNAMIC_COMBO | 是 | `"Opus 5"``"Opus 4.8"``"Fable 5"``"Sonnet 5"``"Opus 4.7"``"Opus 4.6"``"Sonnet 4.6"``"Sonnet 4.5"``"Haiku 4.5"` |
...
| `reasoning_effort` | 扩展思考强度。该模型始终启用推理。（默认值："high"） | COMBO | 是 | `"low"``"medium"``"high"` |
...
|-----------|-------------|-----------|----------|-------|
| `max_tokens` | 要生成的最大 token 数（启用时包含推理 token）。（默认值：32768） | INT | 是 | 4096 到 64000 |
| `reasoning_effort` | 扩展思考强度。`"off"` 表示禁用推理。（默认值："off"） | COMBO | 是 | `"off"``"low"``"medium"``"high"` |
