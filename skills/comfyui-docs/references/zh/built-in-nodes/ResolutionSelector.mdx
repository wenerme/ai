...
| 参数 | 描述 | 数据类型 | 必需 | 范围 |
|-----------|-------------|-----------|----------|-------|
| `宽高比` | 输出尺寸的宽高比（默认：`"1:1 (Square)"`）。 | COMBO | 是 | `"1:1 (Square)"``"2:3 (Portrait Photo)"``"3:2 (Photo)"``"3:4 (Portrait Standard)"``"4:3 (Standard)"``"9:16 (Portrait Widescreen)"``"16:9 (Widescreen)"``"21:9 (Ultrawide)"` |
