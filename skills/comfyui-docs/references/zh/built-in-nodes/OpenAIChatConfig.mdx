...
| 参数 | 数据类型 | 必填 | 取值范围 | 描述 |
|------|-----------|----------|-------|-------------|
| `truncation` | COMBO | 是 | `"auto"``"disabled"` | 模型响应使用的截断策略。auto：如果本次响应及之前对话的上下文超出模型的上下文窗口大小，模型将通过丢弃对话中间部分的输入项来截断响应以适应上下文窗口。disabled：如果模型响应将超出模型的上下文窗口大小，请求将失败并返回 400 错误（默认值："auto"） |
