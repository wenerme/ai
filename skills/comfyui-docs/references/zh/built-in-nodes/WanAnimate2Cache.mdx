...
| `device` | 缓存的存放位置。cpu（内存）是安全选择；在常规尺寸下，缓存与模型无法同时放入显存。gpu（显存）如果放得下则可能更快。（默认值："cpu"） | STRING | 是 | "cpu""gpu" |
| `dtype` | 存储精度。default 以模型的计算 dtype 存储激活值。int8 将缓存减半，int4 减至四分之一，convrot 用于保持精度。（默认值："default"） | STRING | 是 | "default""int8""int4" |
