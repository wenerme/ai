...
| 参数 | 数据类型 | 必填 | 取值范围 | 描述 |
| :--- | :--- | :--- | :--- | :--- |
| `model` | COMBO | 是 | `"viduq2-pro"``"viduq2-turbo"` | 用于视频生成的 Vidu 模型。 |
| `start_image` | IMAGE | 是 | - | 起始帧图像。宽高比必须在 1:4 到 4:1 之间。 |
| `seed` | INT | 否 | 0 到 2147483647 | 用于随机数生成的种子值，以确保结果可复现（默认值：1）。 |
| `resolution` | COMBO | 是 | `"720p"``"1080p"` | 输出视频的分辨率。 |
| `frames` | DYNAMICCOMBO | 是 | `"2"``"3"``"4"``"5"``"6"``"7"``"8"``"9"` | 关键帧过渡的数量（2-9）。选择一个值会动态显示每个帧所需的输入参数。 |
