...
| `apply_text_normalization` | 文本规范化模式。"auto"让系统决定，"on"始终应用规范化，"off"跳过规范化。 | COMBO | 否 | `"auto"``"on"``"off"` |
| `model` | 用于文本转语音的模型。选择模型后会显示其特定参数。 | DYNAMICCOMBO | 否 | `"eleven_multilingual_v2"``"eleven_v3"` |
