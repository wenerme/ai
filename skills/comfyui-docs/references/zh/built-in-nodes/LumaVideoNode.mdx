...
| `模型` | 要使用的视频生成模型。 | COMBO | 是 | `"ray_1_6"``"ray_2"` |
| `宽高比` | 生成视频的宽高比（默认："16:9"）。 | COMBO | 是 | `"16:9"``"9:16"``"1:1"``"4:3"``"3:4"``"21:9"``"9:21"` |
| `分辨率` | 视频的输出分辨率（默认："540p"）。使用 `ray_1_6` 模型时忽略此参数。 | COMBO | 是 | `"540p"``"720p"``"1080p"` |
| `时长` | 生成视频的时长。使用 `ray_1_6` 模型时忽略此参数。 | COMBO | 是 | `"5s"``"9s"` |
