...
LTX-2.5 内置定制的 Gemma 4 12B 文本编码器，面对复杂提示词时仍能保留多个主体、动作、光影与相机方向；同时还配备了可选的提示词增强器，可将简短提示词扩展为详细指令。自动时长功能会在扩散开始之前，根据所描述的动作预测片段长度。蒸馏模型提供更快的生成速度，输出支持原生 4K HDR、最高 50 FPS，并为专业后期精修 pipeline 提供 RAW 工作流。
...
- **提示词增强器**（可选）：一个轻量级模型，可将简短提示词扩展为丰富的电影级指令
...
#### 模型下载
    ...
    可选：仅在启用提示词增强器时需要。放入 <code>ComfyUI/models/text_encoders/</code>
...
│   │   └── gemma4_e2b_it_int8_convrot.safetensors（可选）
│   ├── 📂 vae/
...
3. **使用提示词增强器（可选）**：工作流默认关闭该功能。在 LTX-2.5 节点上启用 `prompt_enhance`，即可将简短提示词扩展为详细的电影级指令。它需要单独的增强器模型（约 5 GB），并会增加约 1-2 分钟的生成时间
...
FLF2V 工作流使用与文生视频相同的模型集，但不包含空间放大模型。
    ...
    可选：仅在启用提示词增强器时需要。放到 <code>ComfyUI/models/text_encoders/</code>
