...
description: "在 ComfyUI 中使用微软开源的 3.8B 文生图模型 Lens：双流 MMDiT 骨干网络、GPT-OSS-20B 文本特征，以及 FLUX.2 语义 VAE。"
translationSourceHash: 7385b700
translationFrom: tutorials/image/lens/lens.mdx
translationBlockHashes:
  "_intro": 48b7db5c
  "Lens text-to-image workflow": 35c2bf24
...
标准版和 Turbo 版均使用 **Subgraph** 节点管理文生图生成流程。你可以打开子图查看或自定义内部节点。

  本工作流使用 Subgraph 节点实现模块化处理。查看 Subgraph 文档了解如何自定义和扩展工作流。

### Lens：文生图 {#image_lens_t2i}

输入文本提示并选择分辨率和宽高比。使用高效的 Lens 文生图模型生成高质量图像。
...
![Lens 文生图示例输出](https://raw.githubusercontent.com/Comfy-Org/workflow_templates/main/output/image_lens_t2i.png)
...
2. 前往 **模板** 并搜索 **Lens**
3. 选择 **Lens** 工作流
4. 下载缺失的模型（见 [模型下载](#模型下载)），输入提示词，然后点击 **队列运行**

### Lens Turbo：文生图 {#image_lens_turbo_t2i}

输入文本提示，选择分辨率、宽高比和推理步数。使用 Lens 文生图模型生成高质量图像。
...
![Lens Turbo 文生图示例输出](https://raw.githubusercontent.com/Comfy-Org/workflow_templates/main/output/image_lens_turbo_t2i.png)

#### 开始使用

1. 更新 ComfyUI 到最新版本
   
2. 前往 **模板** 并搜索 **Lens Turbo**
3. 选择 **Lens Turbo** 工作流
4. 下载缺失的模型（见 [模型下载](#模型下载)），输入提示词，然后点击 **队列运行**
