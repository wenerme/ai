...
目前 ComfyUI 已原生集成了对应的 Rodin 模型生成 API，现在你可以在 ComfyUI 中便捷地使用相关节点来进行模型生成。

## Rodin模型生成的优势

- **单视图模型生成**：根据单个图像输入生成3D模型
- **多视图模型生成**：根据最多5个图像输入生成3D模型，例如正面、背面和左侧视图
- **不同的细节级别**：支持以可调节的细节生成模型，包括不同的多边形数量，以获得更平滑、更精细的模型
