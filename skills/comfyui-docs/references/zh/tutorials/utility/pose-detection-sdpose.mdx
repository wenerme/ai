...
### SDPose：图像多人检测 {#utility_sdpose_multi_person}
...
### SDPose：视频多人检测 {#utility_sdpose_multi_person_video}
...
### SDPose-OOD：图像到姿态图 {#utility_sdpose_ood_image_to_pose}

上传图片以提取姿态关键点，并使用 SDPose-OOD 模型生成对应的姿态图。

[SDPose-OOD 图像到姿态图工作流预览]
...
<div style={{display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: '1rem', alignItems: 'start'}}>
  [输入图像]
  [SDPose-OOD 图像到姿态图示例输出]
</div>

### SDPose-OOD：视频到姿态图 {#utility_sdpose_ood_video_to_pose_map}

上传视频以提取姿态关键点并生成姿态图。该工作流支持多人检测，并使用增强型 SDPose 模型进行精准的全身特征提取。

[SDPose-OOD 视频到姿态图工作流预览]
