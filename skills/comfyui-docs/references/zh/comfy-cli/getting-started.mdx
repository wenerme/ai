...
translationBlockHashes:
  ...
  "Run Workflows {#comfy run}": 653c16e1
...
```bash
comfy templates ls --type image --tag "Text to Image"   # 浏览
comfy templates show <name>                              # 完整元数据
comfy templates fetch <name> --out my.json               # 拉取工作流 JSON
```
...
```bash
comfy nodes search "checkpoint"              # 模糊搜索
comfy nodes show KSampler                    # 完整 schema：输入、输出、默认值
comfy nodes ls --produces IMAGE --limit 10   # 按输出类型筛选
comfy nodes ls --api-only                    # 仅 partner-API 节点
```

**模型：**

```bash
comfy models list-folders                    # 每个模型文件夹
comfy models search --text "wan2.2" --type lora
comfy models show wan2.2_vae.safetensors     # 完整元数据
```
