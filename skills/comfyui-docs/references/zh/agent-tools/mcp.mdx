...
### 入门
    ...
    **本地连接**需要一个能够将本地 stdio 服务器作为子进程启动的客户端。这排除了基于浏览器的客户端。[claude.ai](https://claude.ai) 和 ChatGPT 仅接受远程连接器。
    ...
    可以。这就是[本地 Comfy MCP 连接](#local-comfy-mcp-connection)。它会驱动你自己机器上安装的 ComfyUI，因此你的智能体可以看到你实际拥有的模型、LoRA 和自定义节点，并在你的 GPU 上运行。
    ...
    云端连接目前处于**公开测试版**阶段。在我们迭代期间，API、工具和行为可能会发生变化。本地连接适用于本地 ComfyUI 安装。如需反馈问题，请参阅[反馈](#feedback)。
...
### 费用与访问
    ...
    **Cursor** 需要在您的 MCP 配置中提供 Comfy Cloud API 密钥；该客户端目前尚不支持 MCP OAuth。无浏览器的无头设置和 CI 设置也需要一个。请参阅 [设置云端连接](#set-up-the-cloud-connection) 下的 **Cursor** 和 **其他客户端** 选项卡。
  

### 使用

  
    你无需自己调用 MCP 工具：你的智能体会根据你的请求来选择它们。通常，它会**发现**可用的工具（`search_templates`、`search_models`、`search_nodes`），**运行**生成任务，然后**等待并获取**输出。参见[你的智能体可以做什么](#what-your-agent-can-do)。
  
  
    在**云端连接**下，服务器绝不会在你的机器上写入任何内容：`get_output` 会返回一个临时签名 URL 和一条立即可运行的下载指令，供你的智能体在你的 shell 中执行。参见[上传与下载](#uploads-and-downloads)。
    ...
    如果某个工作流对你的机器来说负担过重，你的智能体会告诉你，并建议改为在 Comfy Cloud 上运行。如果只设置了一种连接，请让它添加另一种连接：参见[设置云端连接](#set-up-the-cloud-connection)或[本地 Comfy MCP 连接](#local-comfy-mcp-connection)。
