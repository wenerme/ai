...
将相同的请求体发送到 `POST https://api.comfy.org/v2/models/byteplus/dreamina-seedance-2-0-fast-260128/requests`。运行被受理后，Router 会立即返回 `201` 和 `request_id`；结果就绪后，可以从本进程或其他进程收集。状态、取消与收集的细节见 [队列投递](/zh/development/comfy-router/queue)。
...
最终视频会复用 Draft 任务的提示词、输入资产、时长、比例、种子、generate_audio 和 omni_reference_task_type；请勿再次提交这些内容。分辨率默认为且仅支持 1080p。
</ParamField>
...
Comfy 侧的护栏限制，而非 BytePlus 的约束。BytePlus 未公布
文本长度限制，且在测试（2026-09-17）中接受了 40,000 个字符。
它按字符计数，而非字节，因此多字节提示词在传输时可能是
该大小的数倍。它约束的是这一个字段，而不是请求本身：
`content` 是一个数组，整个文档受每个请求的请求体限制约束。
Comfy Router（/v2/models/byteplus/\{model}）是执行该限制的接口；
在直接调用 v1 /proxy 时，则由 BytePlus 自己的校验器来响应。
其设置远高于真实流量，因此绝不会对真实提示词做出裁决。如果调用方
需要更多，可以调高它。
</ParamField>
...
将返回的任务 ID 传入 `draft_task` 内容项以渲染最终的 1080p 视频。Draft 任务 ID 的有效期为 7 天。
</ParamField>
...
<ParamField body="ratio" type="string">
  已生成视频的宽高比例。Seedance 2.0 与 2.0 fast、1.5 pro 默认值：adaptive。
Seedance 2.5 首帧 / 首尾帧生成：输出会沿用首帧的宽高比例，因此只接受 `adaptive`（或省略该字段）；若指定具体比例，会在派发前以 400 拒绝。Seedance 2.0 在这些模式下可以接受具体比例。
...
<ParamField body="return_last_frame" type="boolean" default="false">
  是否返回已生成视频的尾帧图像。
true：返回已生成视频的尾帧图像。将该参数设为 true 之后，可通过调用查询视频生成任务信息来获取尾帧图像。尾帧图像为 PNG 格式，其像素宽度和高度与已生成视频一致，且不含水印。使用该参数可以生成多个连续的视频：将前一个已生成视频的尾帧作为下一个视频任务的首帧，从而快速生成多个连续视频。
false：不返回已生成视频的尾帧图像。
</ParamField>
...
根据 Router 在 `GET /v2/models/byteplus/dreamina-seedance-2-0-fast-260128/openapi.json` 提供的 schema 生成，该文档与请求到达提供商之前用于校验调用的文档相同。

### 输出

<ResponseField name="content" type="object">
  视频生成任务完成后得到的输出，其中包含输出视频的下载 URL，以及在 BytePlus 返回时其最后一帧的下载 URL。`video_url` 和 `last_frame_url` 都会被重新托管到 Comfy 存储上；这里其他所有字段都是 BytePlus 自有的。可为空：BytePlus 会在任务完成 24 小时后清除这些 URL，此后轮询到的 succeeded 文档可能缺失 `content` 或其为 null。
</ResponseField>

<ResponseField name="content.last_frame_url" type="string">
  已生成视频最后一帧的下载 URL，当请求设置了 `return_last_frame` 时返回。不要根据该 URL 推断图像格式：BytePlus 在请求侧将最后一帧记录为 PNG，Router 会重新托管它收到的任何字节，并根据上游的 Content-Type 或内容嗅探来判定其类型，只有在两者都失败时才以 `image/jpeg` 作为最后兜底。Router 会将最后一帧重新托管到 Comfy 存储并重写该字段，因此它通常是 Comfy 签名的 URL，有效期最长 24 小时：签发时签名有效期为 24 小时，并从 23 小时的备忘缓存中重放，所以稍后的一次轮询可能返回仅剩一小时有效期的 URL。当无法执行重新托管时，该字段会保留 BytePlus 自己的 URL，而 BytePlus 会在任务完成 24 小时后将其清除。无论哪种情况，链接都会过期，因此请下载该帧，而不要只保存 URL。
</ResponseField>

<ResponseField name="content.output_format" type="string">
  已生成视频的容器格式（mp4 或 mov），在 BytePlus 将其嵌套在 `content` 内时出现。Seedance 模型更常将其作为 `content` 的顶层同级字段返回，参见顶层的 `output_format` 字段，Router 会读取两者中存在的那个。
</ResponseField>

<ResponseField name="content.video_url" type="string">
  输出视频的下载 URL。Router 会将视频重新托管到 Comfy 存储并重写该字段，因此它通常是 Comfy 签名的 URL，有效期最长 24 小时：签发时签名有效期为 24 小时，并从 23 小时的备忘缓存中重放，所以稍后的一次轮询可能返回仅剩一小时有效期的 URL。当无法执行重新托管时，该字段会保留 BytePlus 自己的 URL，而 BytePlus 会在任务完成 24 小时后将其清除，并且在某些模型上限制为 100 次下载。无论哪种情况，链接都会过期，因此请下载视频，而不要只保存 URL。
</ResponseField>
...
<ResponseField name="duration" type="number">
  已生成视频的时长，单位为秒。声明为 number 而不是 integer，是因为 BytePlus 在这一点上并不一致：视频任务曾观察到返回整数秒，而同类的 BytePlus 接口会报告小数时长，因此客户端不能假定它是整数值。这是 BytePlus 自有字段，在成功的视频任务中返回并原样转发。
</ResponseField>

<ResponseField name="error" type="object">
  错误信息。任务成功时返回 null。任务失败时返回错误信息。
</ResponseField>

<ResponseField name="error.code" type="string">
  上游 ModelArk 错误码。SensitiveContentDetected、InputTextSensitiveContentDetected、InputImageSensitiveContentDetected、InputVideoSensitiveContentDetected、InputAudioSensitiveContentDetected、OutputTextSensitiveContentDetected、OutputImageSensitiveContentDetected、OutputVideoSensitiveContentDetected 和 OutputAudioSensitiveContentDetected 表示内容策略拒绝。同一族系可以带以点分隔的原因，例如 InputImageSensitiveContentDetected.PrivacyInformation、OutputVideoSensitiveContentDetected.PolicyViolation 或 OutputImageSensitiveContentDetected.DeepFake。这是一个开放字符串，而非枚举：其他错误码描述验证和提供商故障。Router 会在 HTTP 400 错误信封和 HTTP 200 失败任务响应中识别策略族系，而不会覆盖传输层故障。
</ResponseField>
...
<ResponseField name="output_format" type="string">
  已生成视频的容器格式（mp4 或 mov），作为 `content` 的同级字段在顶层返回：Seedance 视频任务查询就是在这里返回它的。这是 BytePlus 自有字段，原样转发。
</ResponseField>

<ResponseField name="resolution" type="string">
  已生成视频的分辨率，例如 `1080p`。这是 BytePlus 自有字段，在成功的视频任务中返回并原样转发。
</ResponseField>

<ResponseField name="seed" type="integer">
  任务实际使用的生成种子。这是 BytePlus 自有字段，在成功的视频任务中返回并原样转发。
...
<ResponseField name="usage.total_tokens" type="integer">
  对于视频生成模型，输入 token 数量不予计算，默认为 0。因此 total_tokens = completion_tokens。
</ResponseField>
