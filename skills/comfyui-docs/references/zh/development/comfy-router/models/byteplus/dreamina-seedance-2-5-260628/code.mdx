...
translationSourceHash: f333bd86
...
Base64：格式 data:audio/\<format>;base64,\<content>
Asset ID：格式 asset://\<ASSET_ID>
</ParamField>
...
<ParamField body="return_last_frame" type="boolean" default="false">
  是否返回已生成视频的最后一帧图像。是：返回已生成视频的最后一帧图像。将此参数设置为是后，可通过调用“查询视频生成任务信息”来获取最后一帧图像。最后一帧图像为 PNG 格式，其像素宽度和高度与已生成视频一致，且不包含水印。使用此参数可生成多个连续视频：将前一个已生成视频的最后一帧用作下一个视频任务的首帧，从而快速生成多个连续视频。
否：不返回已生成视频的最后一帧图像。
</ParamField>

<ParamField body="seed" type="integer">
  用于控制随机性的种子整数。取值范围：[-1, 2^32-1]。取 -1 表示使用随机种子。
...
<ResponseField name="content" type="object">
  视频生成任务完成后得到的输出，其中包含输出视频的下载 URL，以及在 BytePlus 返回时其末帧的下载 URL。`video_url` 和 `last_frame_url` 都会被重新托管到 Comfy 存储上；此处其他所有字段均为 BytePlus 自身的字段。可为空。BytePlus 会在任务完成 24 小时后清除这些 URL，因此在此之后轮询一个已成功的文档时，`content` 可能缺失或为 null。
</ResponseField>

<ResponseField name="content.last_frame_url" type="string">
  已生成视频末帧的下载 URL，当请求设置了 `return_last_frame` 时返回。不要根据此 URL 推断图像格式：BytePlus 在请求侧将末帧记录为 PNG，Router 会重新托管它收到的任意字节，并根据上游的 Content-Type 或内容嗅探来确定其类型，而 `image/jpeg` 仅在两者都失败时作为最后的兜底。Router 会将末帧重新托管到 Comfy 存储上并重写此字段，因此它通常是 Comfy 签名的 URL，有效期最长 24 小时：签发时按 24 小时签名，并从 23 小时的备忘中重放，因此稍后轮询可能返回一个仅剩一小时有效期的 URL。当无法执行重新托管时，此字段会保留 BytePlus 自身的 URL，而 BytePlus 会在任务完成 24 小时后清除它。无论哪种情况，链接都会过期，因此请下载该帧，而不要存储这个 URL。
</ResponseField>
...
<ResponseField name="content.video_url" type="string">
  输出视频的下载 URL。Router 会将视频重新托管到 Comfy 存储上并重写此字段，因此它通常是 Comfy 签名的 URL，有效期最长 24 小时：签发时按 24 小时签名，并从 23 小时的备忘中重放，因此稍后轮询可能返回一个仅剩一小时有效期的 URL。当无法执行重新托管时，此字段会保留 BytePlus 自身的 URL，而 BytePlus 会在任务完成 24 小时后清除它，并且某些模型上限制为 100 次下载。无论哪种情况，链接都会过期，因此请下载该视频，而不要存储这个 URL。
</ResponseField>
...
<ResponseField name="duration" type="number">
  已生成视频的时长，单位为秒。声明为 number 而非 integer，是因为 BytePlus 在这方面并不一致：视频任务曾被观察到返回整数秒，而同类的其他 BytePlus 接口会报告带小数的时长，因此客户端不能假定其为整数值。这是 BytePlus 自身的字段，会在成功的视频任务中返回并原样转发。
</ResponseField>
...
<ResponseField name="output_format" type="string">
  已生成视频的容器格式（mp4 或 mov），作为 `content` 的同级字段在顶层返回，这正是 Seedance 视频任务查询返回它的位置。这是 BytePlus 自身的字段，会原样转发。
</ResponseField>

<ResponseField name="resolution" type="string">
  已生成视频的分辨率，例如 `1080p`。这是 BytePlus 自身的字段，会在成功的视频任务中返回并原样转发。
</ResponseField>

<ResponseField name="seed" type="integer">
  该任务实际使用的生成种子。这是 BytePlus 自身的字段，会在成功的视频任务中返回并原样转发。
