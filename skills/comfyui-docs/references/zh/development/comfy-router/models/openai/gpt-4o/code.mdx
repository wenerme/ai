---
title: "将 GPT 4o 与 Comfy Router 配合使用"
description: "通过 Comfy Router 调用 openai/gpt-4o：endpoint、请求形状以及 Router 返回的响应。"
translationSourceHash: 1249eb47
translationFrom: development/comfy-router/models/openai/gpt-4o/code.mdx
translationBlockHashes:
  "_intro": d4463f47
  "Quick start": 73c84a23
  "Schema": a6d45cf8
  "Examples": 67e6c8f1
---

`openai/gpt-4o` 的 API 参考文档，由 Comfy Router 从 OpenAI 提供。

## 快速开始

在[你的 Comfy 工作区](https://platform.comfy.org/profile/api-keys?onboarding=router)中创建一个密钥，并将其导出为 `COMFY_API_KEY`。Python、TypeScript 和 Swift 代码片段使用 Comfy SDK（`pip install comfy-sdk`、`npm install @comfyorg/sdk`，以及 [`ComfySwiftSDK`](https://github.com/Comfy-Org/comfy-swift-sdk) Swift 包）；cURL 代码片段则通过原始 HTTP 发起同样的调用。

**Model ID:** `openai/gpt-4o`

**Endpoint:** `POST https://api.comfy.org/v2/models/openai/gpt-4o`
...
将相同的请求体发送到 `POST https://api.comfy.org/v2/models/openai/gpt-4o/requests`。运行被受理后，Router 会立即返回 `201` 和 `request_id`；结果就绪后，可以从本进程或其他进程收集。状态、取消与收集的细节见 [队列投递](/zh/development/comfy-router/queue)。
...
# 2. 轮询直到状态为 COMPLETED，每个响应中指定的 Retry-After 秒数都要等待。
REQUEST_ID="<request_id from the 201 body>"
curl -i https://api.comfy.org/v2/models/openai/gpt-4o/requests/$REQUEST_ID/status \
  -H "X-API-Key: $COMFY_API_KEY"

# 3. 收集结果。返回 200 及模型的原生输出；仍在运行时返回 202 及状态正文。
...
<ParamField body="instructions" type="string">
  在模型上下文的最前面插入一条 system（或 developer）消息。
</ParamField>

<ParamField body="max_output_tokens" type="integer">
  为响应生成的 token 数量上限，包含可见输出 token 和推理 token。在推理类 id 上，这一上限与隐藏的推理 token 共享，因此较小的值可能在产生任何可见文本之前就耗尽全部预算：这正是推理冒烟用例发送 1024、而聊天用例发送 16 的原因。

  范围：`1` 到 `…`
</ParamField>

<ParamField body="model" type="string">
  OpenAI 模型标识符。在 Comfy Router 上该字段为可选，Router 会从 `{model}` 路径段填充它；显式的 `null` 也会以同样的方式被替换。发送与路径不一致的值会被拒绝。
</ParamField>
...
<ParamField body="stream" type="boolean">
  声明此字段是为了让发送它的调用方不被拒绝，但它在此接口上是无效的：Router 会在派发之前将其固定为 `false`，因为它捕获的是提供商的响应，而不是转发 `text/event-stream`。openAiResponsesProxy 的 ModifyResponse 无法解码该流，因此流式生成会被 OpenAI 计费，却无人计量。如果你需要流式传输，请使用 `POST /proxy/openai/v1/responses`。
</ParamField>

<ParamField body="temperature" type="number">
  采样温度。仅限聊天层级：o 系列推理 id（`o1`、`o1-pro`、`o3`、`o4-mini`）在 OpenAI 处会拒绝该参数。Router 不会替它们拒绝，参见本组件关于两个层级为何共用同一 Schema 的说明。因此，发送该参数的推理调用会收到 OpenAI 自身的错误。
...
<ParamField body="truncation" type="string">
  当上下文超出模型窗口时采用的截断策略。与上方三个词表不同，这里的枚举是强制执行的，因为这两个值是 OpenAI 文档中的完整集合，且至今没有扩充。显式的 `null` 仍被接受，条件与其上方的字段相同。

  可能的值：`auto`、`disabled`
</ParamField>

<ParamField body="usage" type="object">
  token 用量封装。该契约中存在此字段，是因为 v1 操作在请求体上声明了它；OpenAI 是在响应中填充它，因此调用方没有理由发送它。
</ParamField>

根据 Router 在 `GET /v2/models/openai/gpt-4o/openapi.json` 提供的 Schema 生成，这也是它在请求到达提供商之前用于校验调用的同一份文档。
### 输出

<ResponseField name="instructions" type="string">
  将系统（或开发者）消息作为第一个条目插入模型的上下文中。

当与 `previous_response_id` 一起使用时，上一个响应中的指令不会延续到下一个响应。这样可以轻松地在新的响应中替换系统（或开发者）消息。
</ResponseField>
...
<ResponseField name="truncation" type="string" default="&quot;disabled&quot;">
  用于模型响应的截断策略。
- `auto`：如果此响应及之前的响应的上下文超过模型的上下文窗口大小，模型将通过丢弃对话中间的输入条目来截断响应，以适应上下文窗口。
...
<ResponseField name="previous_response_id" type="string">
  模型的上一个响应的唯一 ID。使用它可创建多轮对话。了解更多关于
...
限制[推理模型](https://platform.openai.com/docs/guides/reasoning)的推理投入程度。目前支持的值为 `low`、`medium` 和 `high`。降低推理投入程度可以加快响应速度，并减少响应中用于推理的 token 数量。
...
配置 `{ "type": "json_schema" }` 会启用结构化输出（Structured Outputs），确保模型与您提供的 JSON schema 匹配。请在
...
设置为 `{ "type": "json_object" }` 会启用较旧的 JSON 模式，该模式可确保模型生成的消息是有效的 JSON。对于支持 `json_schema` 的模型，优先使用它。
</ResponseField>
...
<ResponseField name="tool_choice" type="`none`, `auto`, `required` | object">
  模型在生成响应时应如何选择要使用哪个（或哪些）工具。请参阅 `tools` 参数，了解如何指定模型可以调用的工具。
</ResponseField>
...
<ResponseField name="completed_at" type="number">
  此 Response 完成时的 Unix 时间戳（以秒为单位）。仅在状态为 `completed` 时才会出现。
</ResponseField>
...
<ResponseField name="frequency_penalty" type="number">
  根据 token 在当前文本中已有的出现频率对新 token 进行惩罚。
</ResponseField>
...
- 与其访问 `output` 数组中的第一项并假设它是包含模型生成内容的 `assistant` 消息，
  不如在 SDK 支持的情况下考虑使用 `output_text` 属性。
</ResponseField>

<ResponseField name="output_text" type="string">
  仅 SDK 提供的便捷属性，包含 `output` 数组中所有 `output_text` 项的
聚合文本输出（如果有）。Python 和 JavaScript SDK
支持。
</ResponseField>
