...
将相同的请求体发送到 `POST https://api.comfy.org/v2/models/openai/gpt-5.6-sol/requests`。运行一经受理，Router 就会返回 `201` 和一个 `request_id`；结果就绪后，可以从本进程或其他进程收集。[队列投递](/zh/development/comfy-router/queue) 会详细说明状态、取消与收集。
