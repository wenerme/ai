...
将相同的请求体发送到 `POST https://api.comfy.org/v2/models/openai/gpt-5.6-luna/requests`。运行一经受理，Router 就会立即返回 `201` 和 `request_id`；结果就绪后即可收集，无论是从本进程还是其他进程。[队列投递](/zh/development/comfy-router/queue)会逐步讲解状态、取消与收集。
...
async def main():
    async with AsyncComfy() as client:
        ...
        # 轮询直到请求完成，并等待服务器指定的 Retry-After 时间。
...
# 2. 轮询直到状态为 COMPLETED，每次响应都要等待其指定的 Retry-After 秒数。
