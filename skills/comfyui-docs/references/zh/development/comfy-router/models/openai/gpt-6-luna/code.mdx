---
title: "将 GPT 6 Luna 与 Comfy Router 配合使用"
description: "通过 Comfy Router 调用 openai/gpt-6-luna：端点、请求结构以及 Router 返回的响应。"
...
`openai/gpt-6-luna` 的 API 参考，由 Comfy Router 从 OpenAI 提供。

## 快速开始

在[你的 Comfy 工作区](https://platform.comfy.org/profile/api-keys?onboarding=router)中创建密钥，并将其导出为 `COMFY_API_KEY`。Python 和 TypeScript 片段使用 Comfy SDK（`pip install comfy-sdk` 和 `npm install @comfyorg/sdk`）；cURL 片段就是通过原始 HTTP 进行的同一调用。

**模型 ID：** `openai/gpt-6-luna`

**端点：** `POST https://api.comfy.org/v2/models/openai/gpt-6-luna`
...
相同的请求体，发送到 `POST https://api.comfy.org/v2/models/openai/gpt-6-luna/requests`。一旦运行被接纳，Router 就会返回 `201` 以及一个 `request_id`；结果准备就绪后，无论从当前进程还是另一个进程都可以收集它。[排队投递](/zh/development/comfy-router/queue) 详细介绍了状态、取消和收集。
...
async def main():
    async with AsyncComfy() as client:
        ...
        print("request_id:", handle.request_id)  # 搭配模型 ID，就是另一个进程所需要的全部信息
...
console.log("requestId:", handle.requestId); // 搭配模型 ID，就是另一个进程所需要的全部信息
...
# 3. 收集。返回 200 时是模型的原生输出，仍在运行时返回 202 和状态正文。
...
<ParamField body="input" type="string | object[]" required>
  提供给模型的文本、图像或文件输入，用于生成响应。这是本契约中 Router 无法自行提供的唯一字段，也是下方 `required` 里唯一的条目。
</ParamField>
...
<ParamField body="max_output_tokens" type="integer">
  一次响应所生成 token 数量的上限，包括可见的输出 token 与推理 token。对于 reasoning id，该上限与隐藏的推理 token 共享，因此较小的取值可能在产生任何可见文本之前就耗尽整个预算，这也是 reasoning 冒烟用例发送 1024、而 chat 用例发送 16 的原因。

  范围：`1` 到 `…`
</ParamField>

<ParamField body="model" type="string">
  OpenAI 模型标识符。在 Comfy Router 上该字段为可选，Router 会从 `{model}` 路径段填充它；显式的 `null` 也以相同方式被替换。发送与路径不一致的取值会被拒绝。
</ParamField>
...
<ParamField body="store" type="boolean">
  OpenAI 是否存储已生成的响应以便日后检索。
</ParamField>

<ParamField body="stream" type="boolean">
  声明该字段是为了让发送它的调用方不被拒绝，但它在此接口上不起作用：Router 会在派发之前将其置为 `false`，因为它捕获的是提供商响应，而不是转发 `text/event-stream`，而 openAiResponsesProxy 的 ModifyResponse 无法解码该流，因此流式生成会被 OpenAI 计费却不会被任何人计量。如果你需要流式传输，请使用 `POST /proxy/openai/v1/responses`。
</ParamField>

<ParamField body="temperature" type="number">
  采样温度。仅限 CHAT 层级：o 系列推理 id（`o1`、`o1-pro`、`o3`、`o4-mini`）在 OpenAI 处会拒绝该参数。Router 不会替它们拒绝该参数，见本组件关于两个层级为何共用一个模式的说明，因此发送该参数的推理调用会由 OpenAI 自己的错误来作答。
...
<ParamField body="tool_choice" type="string | object">
  模型应如何选择要使用的工具。可以是字符串模式，也可以是一个指明某个工具的对象。
</ParamField>
...
<ParamField body="truncation" type="string">
  上下文超出模型窗口时的截断策略。与上面的三种词汇表不同，这里的枚举是被强制执行的，因为这两个取值就是 OpenAI 文档记载的完整集合，且至今没有增加。显式的 `null` 仍然被接受，条件与其上方的字段相同。

  可选值：`auto`、`disabled`
</ParamField>

<ParamField body="usage" type="object">
  Token 用量封装。该契约中出现此字段是因为 v1 操作在请求体上声明了它；OpenAI 会在响应中填充它，因此调用方没有理由发送它。
</ParamField>

由 Router 在 `GET /v2/models/openai/gpt-6-luna/openapi.json` 提供的模式生成，这也是它在请求到达提供商之前用于校验调用的同一份文档。

### 输出

<ResponseField name="instructions" type="string">
  以系统（或开发者）消息的形式插入到模型上下文的第一项。

与 `previous_response_id` 一起使用时，上一个响应的 instructions 不会延续到下一个响应。因此在新的响应中替换系统（或开发者）消息会非常简单。
</ResponseField>

<ResponseField name="max_output_tokens" type="integer">
  单个响应可生成的 token 数量上限，包括可见输出 token 和[推理 token](https://platform.openai.com/docs/guides/reasoning)。
</ResponseField>
...
<ResponseField name="truncation" type="string" default="&quot;disabled&quot;">
  用于模型响应的截断策略。
- `auto`：如果本次响应及之前响应的上下文超过模型的上下文窗口大小，模型将通过丢弃对话中间的输入项来截断响应，以适应上下文窗口。
...
<ResponseField name="previous_response_id" type="string">
  模型上一个响应的唯一 ID。可用它创建多轮对话。了解有关[对话状态](https://platform.openai.com/docs/guides/conversation-state)的更多信息。
</ResponseField>
...
约束[推理模型](https://platform.openai.com/docs/guides/reasoning)的推理努力程度。目前支持的值为 `low`、`medium` 和 `high`。降低推理努力程度可以加快响应速度，并减少响应中用于推理的 token 数量。
...
配置 `{ "type": "json_schema" }` 可启用结构化输出，确保模型与您提供的 JSON schema 匹配。请参阅[结构化输出指南](https://platform.openai.com/docs/guides/structured-outputs)了解更多信息。

默认格式为 `{ "type": "text" }`，且不附加其他选项。

**不建议用于 gpt-4o 及更新的模型：**

设置为 `{ "type": "json_object" }` 会启用较旧的 JSON 模式，确保模型生成的消息是有效的 JSON。对于支持 `json_schema` 的模型，建议优先使用它。
</ResponseField>
...
<ResponseField name="completed_at" type="number">
  此响应完成时的 Unix 时间戳（以秒为单位）。仅在状态为 `completed` 时提供。
</ResponseField>
...
- 与其访问 `output` 数组中的第一项并
  ...
  您可以考虑使用 SDK 中支持该属性的 `output_text` 属性。
</ResponseField>

<ResponseField name="output_text" type="string">
  仅 SDK 提供的便捷属性，包含来自 `output` 数组中所有
`output_text` 项的聚合文本输出（如果存在）。
Python 和 JavaScript SDK 均支持。
</ResponseField>
...
<ResponseField name="top_logprobs" type="integer">
  在每个 token 位置返回的最可能 token 的最大数量，每个 token 都带有相关的对数概率。
</ResponseField>
