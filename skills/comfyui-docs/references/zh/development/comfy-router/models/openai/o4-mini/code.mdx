...
将相同的请求体发送到 `POST https://api.comfy.org/v2/models/openai/o4-mini/requests`。运行一旦被受理，Router 就会返回 `201` 和 `request_id`；结果就绪后，可从本进程或其他进程收集。[队列投递](/zh/development/comfy-router/queue) 介绍了状态、取消与收集。
...
async def main():
    async with AsyncComfy() as client:
        ...
        print("request_id:", handle.request_id)  # 配合模型 ID，就是另一个进程所需的全部
...
console.log("requestId:", handle.requestId); // 配合模型 ID，就是另一个进程所需的全部
...
```bash cURL
# 1. 提交。Router 返回 201，并给出 request_id、status_url、response_url 和 cancel_url。
...
# 3. 收集。仍在运行时返回带状态正文的 202，完成后返回模型原生输出的 200。
