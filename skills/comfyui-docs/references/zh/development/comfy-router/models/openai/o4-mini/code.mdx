---
title: "搭配 Comfy Router 使用 o4-mini"
description: "通过 Comfy Router 调用 openai/o4-mini：端点、请求结构以及 Router 返回的响应。"
translationSourceHash: a7838b75
translationFrom: development/comfy-router/models/openai/o4-mini/code.mdx
translationBlockHashes:
  "_intro": 03d40f5d
  "Quick start": 571d5ca9
  "Schema": 14c35480
  "Examples": 67e6c8f1
---

`openai/o4-mini` 的 API 参考，由 Comfy Router 从 OpenAI 提供。
...
**模型 ID：** `openai/o4-mini`

**端点：** `POST https://api.comfy.org/v2/models/openai/o4-mini`
...
将相同的请求体发送到 `POST https://api.comfy.org/v2/models/openai/o4-mini/requests`。运行被受理后，Router 会立即返回 `201` 和 `request_id`；结果就绪后，可以从本进程或其他进程收集。状态、取消与收集的细节见 [队列投递](/zh/development/comfy-router/queue)。
...
with Comfy() as client:
    ...
    print("request_id:", handle.request_id)  # 有了模型 ID，另一个进程所需的一切
...
// 从环境变量读取 COMFY_API_KEY。
// 每次 submit() 调用都会生成自己的 Idempotency-Key，并在自动重试时复用它。
const handle = await comfy.models.submit("openai/o4-mini", {
  input: "Reply with the single word: ok",
  max_output_tokens: 1024,
});
console.log("requestId:", handle.requestId); // 有了模型 ID，另一个进程所需的一切
...
print("requestId:", handle.requestId)  // 有了模型 ID，另一个进程所需的一切
...
```bash cURL
# 1. 提交。Router 返回 201，并在响应体中给出 request_id、status_url、response_url 和 cancel_url。
...
curl -i https://api.comfy.org/v2/models/openai/o4-mini/requests/$REQUEST_ID/status \
  -H "X-API-Key: $COMFY_API_KEY"

# 3. 收集。仍在运行时返回带状态正文的 202，完成后返回模型的原生输出和 200。
curl https://api.comfy.org/v2/models/openai/o4-mini/requests/$REQUEST_ID \
  -H "X-API-Key: $COMFY_API_KEY"
```
...
<ParamField body="input" type="string | object[]" required>
  提供给模型的文本、图像或文件输入，用于生成响应。这是本契约中 Router 唯一无法提供的字段，也是下方 `required` 中唯一的条目。
</ParamField>

<ParamField body="instructions" type="string">
  在模型的上下文中插入一条系统（或开发者）消息，作为第一项内容。
</ParamField>

<ParamField body="max_output_tokens" type="integer">
  为响应生成的 token 数量上限，包括可见的输出 token 和推理 token。在推理 id 上，该上限与隐藏的推理 token 共享，因此一个较小的值可能在产生任何可见文本之前就耗尽整个预算：这正是推理冒烟用例发送 1024，而聊天用例只发送 16 的原因。

  范围：`1` 至 `…`
</ParamField>

<ParamField body="model" type="string">
  OpenAI 模型标识符。在 Comfy Router 上该字段为可选，Router 会从 `{model}` 路径段填充它；显式的 `null` 也会以相同方式被替换。发送与路径不一致的值会被拒绝。
</ParamField>
...
<ParamField body="stream" type="boolean">
  声明该字段是为了让发送它的调用方不会被拒绝，但它在此接口上是无效的：Router 会在分发之前将其确定为 `false`，因为它捕获的是提供商的响应，而不是转发 `text/event-stream`；openAiResponsesProxy 的 ModifyResponse 无法解码后者，因此流式生成会被 OpenAI 计费，却没有任何人对其进行计量。如果需要流式传输，请使用 `POST /proxy/openai/v1/responses`。
</ParamField>

<ParamField body="temperature" type="number">
  采样温度。仅限聊天层级：o 系列推理 id（`o1`、`o1-pro`、`o3`、`o4-mini`）在 OpenAI 处会拒绝此参数。Router 不会代它们拒绝该参数（关于两个层级为何共享同一个 schema，请参阅本组件的说明），因此发送该参数的推理调用会由 OpenAI 自身的错误来应答。
...
<ParamField body="tools" type="object[]">
  模型可以调用的工具定义。Router 不会缩小工具的类别范围；可接受的形状请参阅 OpenAI 的 Responses API 参考。
</ParamField>

<ParamField body="top_p" type="number">
  核采样截止值。仅限聊天层级，条件与 `temperature` 相同。

  范围：`0` 至 `1`
</ParamField>

<ParamField body="truncation" type="string">
  当上下文超出模型窗口时采用的截断策略。与上面三组词表不同，这里的枚举值确实会被强制校验，因为这两个值是 OpenAI 文档中记载的完整集合，且至今没有增加。显式的 `null` 仍然被接受，条件与它上面的字段相同。

  可能的值：`auto`、`disabled`
</ParamField>

<ParamField body="usage" type="object">
  token 用量封装。它出现在本契约中，是因为 v1 操作在请求体上声明了它；OpenAI 会在响应中填充它，因此调用方没有理由发送它。
</ParamField>

由 Router 在 `GET /v2/models/openai/o4-mini/openapi.json` 提供的 schema 生成，该文档也正是请求到达提供商之前 Router 用来校验调用的同一份文档。
### 输出

<ResponseField name="instructions" type="string">
  将系统（或开发者）消息插入到模型上下文的第一个位置。
...
<ResponseField name="max_output_tokens" type="integer">
  一次响应可生成的 token 数上限，包括可见的输出 token 和[推理 token](https://platform.openai.com/docs/guides/reasoning)。
</ResponseField>
...
<ResponseField name="truncation" type="string" default="&quot;disabled&quot;">
  用于模型响应的截断策略。
- `auto`：如果本次响应及此前响应的上下文超出模型的上下文窗口大小，模型将通过丢弃对话中间的输入项来截断响应，以适应上下文窗口。
- `disabled`（默认）：如果模型响应将超出模型的上下文窗口大小，请求会失败并返回 400 错误。
...
限制[推理模型](https://platform.openai.com/docs/guides/reasoning)的推理投入程度。目前支持的值为 `low`、`medium` 和 `high`。降低推理投入程度可以让响应更快，并减少响应中用于推理的 token 数量。
...
配置 `{ "type": "json_schema" }` 会启用结构化输出，可确保模型符合你提供的 JSON schema。请参阅[结构化输出指南](https://platform.openai.com/docs/guides/structured-outputs)了解更多。

默认格式为 `{ "type": "text" }`，不带任何额外选项。

**不推荐用于 gpt-4o 及更新的模型：**

设置为 `{ "type": "json_object" }` 会启用较旧的 JSON 模式，可确保模型生成的消息是有效的 JSON。对于支持 `json_schema` 的模型，推荐使用它。
</ResponseField>

<ResponseField name="text.verbosity" type="string">
  限制模型响应的详尽程度。取值为 `low`、`medium` 或 `high` 之一。
</ResponseField>

<ResponseField name="tool_choice" type="`none`, `auto`, `required` | object">
  模型在生成响应时应如何选择要使用的一个或多个工具。关于如何指定模型可以调用的工具，请参阅 `tools` 参数。
</ResponseField>
...
<ResponseField name="completed_at" type="number">
  此 Response 完成时的 Unix 时间戳（以秒为单位）。仅在状态为 `completed` 时出现。
</ResponseField>
...
<ResponseField name="frequency_penalty" type="number">
  根据新 token 在截至目前文本中已有的出现频率对其进行惩罚。
</ResponseField>
...
- `output` 数组中各项的长度和顺序取决于
  模型的响应。
- 与其访问 `output` 数组中的第一项，并假定它是包含
  模型生成内容的 `assistant` 消息，不如考虑使用 SDK 中
  受支持的 `output_text` 属性。
</ResponseField>

<ResponseField name="output_text" type="string">
  仅 SDK 提供的便捷属性，包含 `output` 数组中所有 `output_text` 项的聚合文本输出（如有）。
...
<ResponseField name="presence_penalty" type="number">
  根据新 token 是否已在截至目前文本中出现过对其进行惩罚。
</ResponseField>

<ResponseField name="prompt_cache_key" type="string">
  OpenAI 用于缓存相似请求的响应以优化缓存命中率。会替代 `user` 字段。
</ResponseField>
...
<ResponseField name="top_logprobs" type="integer">
  在每个 token 位置返回的最可能 token 的最大数量，每个都带有相关的 log 概率。
</ResponseField>
...
<ResponseField name="usage.input_tokens_details.cached_tokens" type="integer" required>
  从缓存中检索到的 token 数量。
[关于提示缓存的更多说明](https://platform.openai.com/docs/guides/prompt-caching)。
</ResponseField>
