---
title: "使用 Comfy Router 调用 Veo 3.1 Lite Generate 001"
description: "通过 Comfy Router 调用 veo/veo-3.1-lite-generate-001：端点、请求结构以及 Router 返回的响应。"
...
`veo/veo-3.1-lite-generate-001` 的 API 参考文档，由 Comfy Router 从 Veo 提供。

## 快速开始

在[你的 Comfy 工作区](https://platform.comfy.org/profile/api-keys?onboarding=router)中创建一个密钥，并将其导出为 `COMFY_API_KEY`。Python、TypeScript 和 Swift 代码片段使用 Comfy SDK（`pip install comfy-sdk`、`npm install @comfyorg/sdk`，以及 [`ComfySwiftSDK`](https://github.com/Comfy-Org/comfy-swift-sdk) Swift 包）；cURL 代码片段是通过原始 HTTP 发起的同一个调用。

**模型 ID：** `veo/veo-3.1-lite-generate-001`

**端点：** `POST https://api.comfy.org/v2/models/veo/veo-3.1-lite-generate-001`
...
同一请求体，发送到 `POST https://api.comfy.org/v2/models/veo/veo-3.1-lite-generate-001/requests`。运行一经接纳，Router 就会返回 `201` 以及一个 `request_id`，结果准备就绪后即可收集，可以由当前进程收集，也可以由另一个进程收集。[队列投递](/zh/development/comfy-router/queue)介绍了状态查询、取消和收集的完整流程。
...
<ParamField body="instances[].referenceImages" type="object[]">
  用于引导视频生成的可选参考图像。最多支持 3 张资产图像或 1 张风格图像。受 Veo 3.1 模型(预览)支持。
</ParamField>
...
<ParamField body="parameters.durationSeconds" type="number">
  已生成视频的目标时长(秒)。Veo 2：5-8。Veo 3/3.1：4、6 或 8。默认值：8
</ParamField>

<ParamField body="parameters.enhancePrompt" type="boolean">
  自动优化提示词以获得更高质量。默认为 是。
</ParamField>
...
<ParamField body="parameters.generateAudio" type="boolean">
  是否在生成视频的同时生成音频。默认为 是。受 Veo 3.0+ 模型支持。
</ParamField>
...
<ParamField body="parameters.resolution" type="string">
  输出视频分辨率。受 Veo 3.0+ 模型支持。默认值：720p
...
<ParamField body="parameters.seed" type="integer">
  用于确定性输出的随机种子。如果 sampleCount > 1，则为每个视频使用不同的种子。
...
根据 Router 在 `GET /v2/models/veo/veo-3.1-lite-generate-001/openapi.json` 提供的 schema 生成，该文档与请求到达提供商之前用于校验调用的文档相同。
### 输出
