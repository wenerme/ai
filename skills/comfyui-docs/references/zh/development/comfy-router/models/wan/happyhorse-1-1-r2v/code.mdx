---
title: "通过 Comfy Router 使用 HappyHorse 1.1 R2V"
description: "通过 Comfy Router 以 HTTP 调用 wan/happyhorse-1.1-r2v 的 Python、TypeScript 和 cURL 代码片段，以及请求字段和结果形状"
translationSourceHash: 5a900877
translationFrom: development/comfy-router/models/wan/happyhorse-1-1-r2v/code.mdx
translationBlockHashes:
  "_intro": 9cfab6c4
  "Quick start": cca21857
  "Schema": f61bc915
  "Examples": a91cb6b1
---
...
同样的请求体，发送到 `POST https://api.comfy.org/v2/models/wan/happyhorse-1.1-r2v/requests`。只要运行被受理，Router 就会立即返回 `201` 和 `request_id`，结果就绪后即可从当前进程或其他进程收集。[队列式交付](/zh/development/comfy-router/queue) 介绍了状态查询、取消和结果收集的完整流程。
...
<ParamField body="input.audio_url" type="string">
  音频文件下载 URL。支持格式：mp3 和 wav。不能与 reference_video_urls 同时使用。
</ParamField>

<ParamField body="input.img_url" type="string">
  首帧图片 URL 或 Base64 编码数据。仅 wan2.5-i2v-preview 与 wan2.6-i2v 模型需要该参数。happyhorse-1.x 的 i2v 写法不在此处传入首帧：它们把首帧作为 `media` 中 type 为 `first_frame` 的元素传入（见下方 `media` 说明），一个在 wan2.6-i2v 上能成功的 `img_url` 请求体，会被提供方在 happyhorse-1.0-i2v 与 happyhorse-1.1-i2v 上拒绝。图片格式：JPEG、JPG、PNG、BMP、WEBP。分辨率：360-2000 像素。文件大小：最大 10MB。
</ParamField>
...
对于带多个参考视频的 wan2.6-r2v，请按参考视频的顺序使用 'character1'、'character2' 等来指代
主体。例如：“Character1 在路边唱歌，Character2 在旁边跳舞”
...
<ParamField body="model" type="string">
  要调用的模型 ID。在此组件上不受约束：Comfy Router 会从 `POST /v2/models/wan/{model}` 的 `{model}` 路径段中填充它，因此 Router 调用方可以省略它。直接对 `POST /proxy/wan/api/v1/services/aigc/video-generation/video-synthesis` 发起 v1 调用时则必须提供它，可接受拼写的枚举定义在该操作自身的组件 `WanVideoGenerationRequest` 上。
</ParamField>
...
<ParamField body="parameters.ratio" type="string">
  生成视频的宽高比例。仅适用于 wan2.7 和 wan3.0 模型。
对于 wan2.7 模型，如果未提供，则根据分辨率档位取默认值。
...
由 Router 在 `GET /v2/models/wan/happyhorse-1.1-r2v/openapi.json` 提供的 schema 生成，该文档与它在请求到达提供商之前用于校验调用的文档完全相同。
### 输出
...
<ResponseField name="code" type="string">
  失败请求的错误码，报告在信封的根层级而非 `output` 下（请求成功时不返回）。
</ResponseField>

<ResponseField name="message" type="string">
  失败请求的详细信息，报告在信封的根层级而非 `output` 下（请求成功时不返回）。在回退到 `output.message` 之前请先阅读此项。
</ResponseField>
