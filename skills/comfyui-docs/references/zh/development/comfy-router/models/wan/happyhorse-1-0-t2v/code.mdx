...
在 [你的 Comfy 工作区](https://platform.comfy.org/profile/api-keys?onboarding=router) 中创建密钥，并将其导出为 `COMFY_API_KEY`。Python、TypeScript 和 Swift 代码片段使用 Comfy SDK（`pip install comfy-sdk`、`npm install @comfyorg/sdk`，以及 [`ComfySwiftSDK`](https://github.com/Comfy-Org/comfy-swift-sdk) Swift 包）；cURL 代码片段则是通过原始 HTTP 执行同一调用。
...
相同的请求体，发送到 `POST https://api.comfy.org/v2/models/wan/happyhorse-1.0-t2v/requests`。运行一经接纳，Router 就会返回 `201` 和 `request_id`；结果就绪后即可从当前进程或其他进程收集。[队列投递](/zh/development/comfy-router/queue) 会逐步介绍状态、取消与收集。
...
<ParamField body="input.img_url" type="string">
  首帧图像网址或 Base64 编码数据。
仅 wan2.5-i2v-preview 和 wan2.6-i2v 模型需要。happyhorse-1.x 的 i2v 拼写
不在此处传入首帧：它们将其作为类型为 `first_frame` 的 `media` 元素传入
（见下方 `media`），而在 wan2.6-i2v 上能够成功的 `img_url` 请求体，
...
- wan3.0-video: first_frame（最多 1 个）、last_frame（最多 1 个）、reference_image（最多 10 个）、
  ...
  与 file 一起使用）。在同一请求内，reference_*/file/link 类型与 first_frame/last_frame 类型
  互斥。数组顺序定义了提示中资产的引用
...
- happyhorse-1.x-i2v: 仅 first_frame，且恰好 1 个。至少 300x300 像素，
  ...
  这些拼写不接受 img_url；首帧在此处传入。
- happyhorse-1.x-r2v: 仅 reference_image，1 到 9 个。最短边至少
  400 像素，最大 20MB，公开 URL 或 data: URL。reference_video 不是此操作的
  输入类型。
- happyhorse-1.x-video-edit: video
上面每个资产的“最大 20MB”数字是合作伙伴对其最终获得图像的容量上限；当资产是
公开 URL 时，它们按字面适用，因为这些字节从不经过 Comfy。内联 data: URL
确实会经过 Comfy，也会遇到传输上限：Comfy Router 的 POST 请求体总上限为
100 MiB，超过即返回 413；base64 会使负载膨胀约 4/3，因此单个内联资产超过
约 75 MB 就会在触及此处任何合作伙伴规则之前被拒绝。在该规模下，单个资产即便
达到合作伙伴自身的 20MB 上限也能轻松以内联方式容纳，因此对单个资产而言起约束
作用的是合作伙伴规则；而传输上限则在有多个资产时起约束作用，因为 100 MiB 的
Router 上限约束的是整个请求，而不是每个元素。任何接近这些上限的内容，请以
公开 URL 形式发送。
</ParamField>
...
<ParamField body="input.media[].url" type="string" required>
  媒体文件的 URL：公开的 HTTP/HTTPS URL、OSS 临时 URL，或者（在上方 `media` 描述中该模型的条目允许时，happyhorse-1.x 的 i2v 和 r2v 拼写就是如此）内联的 `data:{MIME_type};base64,...` URL。有关各模型的大小和像素下限，以及约束内联负载总量的 100 MiB Router 请求体上限，请参阅该描述。
</ParamField>
...
例如："Character1 在路边唱歌，Character2 在旁边跳舞"
对于 wan3.0-video 参考模式，请使用 'Image 1'、'Video 1'、'Audio 1' 等来按 media 数组中
对应的顺序指代媒体资产。
</ParamField>

<ParamField body="input.reference_video_urls" type="string[]">
  仅用于 wan2.6-r2v 模型的参考视频 URL。包含 1-3 个视频 URL 的数组。
输入限制：
- 格式：mp4、mov
- 数量：1-3 个视频
- 单个视频长度：2-30 秒
- 单个文件大小：最大 30MB
- 不能与 audio_url 一起使用
参考时长：单个视频最大 5s，两个视频每个最大 2.5s，三个视频按比例更少。
计费：按实际使用的参考时长计费。
</ParamField>
...
<ParamField body="model" type="string">
  要调用的模型 ID。此组件上不作约束：Comfy Router 会从 `POST /v2/models/wan/{model}` 的 `{model}` 路径段填充它，因此 Router 调用方会省略它。直接向 `POST /proxy/wan/api/v1/services/aigc/video-generation/video-synthesis` 发起 v1 调用则必须提供它，可接受拼写的枚举位于该操作自己的组件 `WanVideoGenerationRequest` 上。
</ParamField>
...
- wan3.0-video：无视频输入时为 [2, 30] 范围内的整数；有视频输入时，输入视频
  总时长加输出视频时长不得超过 30 秒；-1 启用智能时长模式，由模型选取合适的时长
...
根据 Router 在 `GET /v2/models/wan/happyhorse-1.0-t2v/openapi.json` 提供的 schema 生成，该文档与 Router 在请求到达提供商之前用于校验调用的文档相同。
...
<ResponseField name="code" type="string">
  失败请求的错误码，报告在响应包的根层级而非 `output` 下（请求成功时不返回）。
</ResponseField>

<ResponseField name="message" type="string">
  失败请求的详细信息，报告在响应包的根层级而非 `output` 下（请求成功时不返回）。在回退到 `output.message` 之前请先阅读此项。
</ResponseField>
