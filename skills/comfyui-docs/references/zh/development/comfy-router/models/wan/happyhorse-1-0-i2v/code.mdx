...
在[你的 Comfy 工作区](https://platform.comfy.org/profile/api-keys?onboarding=router)中创建密钥，并将其导出为 `COMFY_API_KEY`。Python、TypeScript 和 Swift 代码片段使用 Comfy SDK（`pip install comfy-sdk`、`npm install @comfyorg/sdk`，以及 [`ComfySwiftSDK`](https://github.com/Comfy-Org/comfy-swift-sdk) Swift 包）；cURL 代码片段则是通过原始 HTTP 进行的同一调用。
...
同一个请求体，发送到 `POST https://api.comfy.org/v2/models/wan/happyhorse-1.0-i2v/requests`。只要运行被接纳，Router 就会立即返回 `201` 和一个 `request_id`，结果准备就绪后可从当前进程或其他进程收集。[队列投递](/zh/development/comfy-router/queue) 介绍了状态、取消和收集的完整流程。
...
<ParamField body="input.audio_url" type="string">
  音频文件下载链接。支持的格式：mp3 和 wav。不能与 reference_video_urls 一起使用。
</ParamField>

<ParamField body="input.img_url" type="string">
  首帧图像网址或 Base64 编码数据。
仅 wan2.5-i2v-preview 和 wan2.6-i2v 模型需要此项。happyhorse-1.x 的 i2v 写法
并不在此处传入首帧：它们把首帧作为 `media` 中类型为 `first_frame` 的元素传入
（见下文 `media`），而在 wan2.6-i2v 上能够成功的 `img_url` 请求体，在
...
- happyhorse-1.x-r2v：仅 reference_image，1 到 9 个。最短边至少
  400 像素，最大 20MB，可以是公开 URL 或 data: URL。reference_video 不是该操作的
  输入类型。
- happyhorse-1.x-video-edit: video
上文按资产标注的“最大 20MB”是合作伙伴对其最终得到的图像所设的上限；当资产是
公开 URL 时，这些数字按其字面值生效，因为这些字节从不经过 Comfy。内联的
data: URL 则确实会经过 Comfy，并且还要受传输上限的约束：Comfy Router 的 POST
请求体总量上限为 100 MiB，超过即返回 413；而 base64 会使负载膨胀约 4/3，因此
单个超过约 75 MB 的内联资产会在触及此处任何合作伙伴规则之前就被拒绝。在该
尺寸下，单个处于合作伙伴自身 20MB 上限的资产可以轻松内联传输，因此对于单个
资产而言，起约束作用的是合作伙伴规则；传输上限则会在多个资产叠加时起作用，
因为 100 MiB 的 Router 上限约束的是整个请求，而不是每个元素。凡是接近这些
上限的内容，请以公开 URL 发送。
</ParamField>
...
<ParamField body="input.media[].url" type="string" required>
  媒体文件的 URL：公开的 HTTP/HTTPS URL、OSS 临时 URL，或者在上文 `media` 描述中该模型的条目如此说明时（例如 happyhorse-1.x 的 i2v 和 r2v 写法），使用内联的 `data:{MIME_type};base64,...` URL。各模型的尺寸与像素下限，以及约束内联负载总量的 100 MiB Router 请求体上限，请参阅该描述。
</ParamField>
...
对于包含多个参考视频的 wan2.6-r2v，请按照参考视频的顺序使用 'character1'、'character2' 等来指代主体。
示例：“Character1 在路边唱歌，Character2 在旁边跳舞”
对于 wan3.0-video 的参考模式，请使用 'Image 1'、'Video 1'、'Audio 1' 等来指代 media 数组中
对应顺序的媒体资产。
</ParamField>
...
- 单个文件大小：最大 30MB
- 不能与 audio_url 一起使用
参考时长：单个视频最多 5 秒，两个视频各最多 2.5 秒，三个视频按比例更短。
计费：按实际使用的参考时长计费。
</ParamField>
...
<ParamField body="model" type="string">
  要调用的模型 ID。本组件不对此做约束：Comfy Router 会从 `POST /v2/models/wan/{model}` 的 `{model}` 路径段填充它，因此 Router 调用方可以省略它。直接以 v1 方式调用 `POST /proxy/wan/api/v1/services/aigc/video-generation/video-synthesis` 时必须提供它，可接受的拼写枚举位于该操作自身的组件 `WanVideoGenerationRequest` 上。
</ParamField>
...
对于 wan3.0-video，adaptive（默认值）会根据输入媒体的
比例和意图自动推荐合适的宽高比。
...
本页由 Router 在 `GET /v2/models/wan/happyhorse-1.0-i2v/openapi.json` 提供的 schema 生成，该文档与请求到达提供商之前用于校验调用的文档相同。
...
<ResponseField name="code" type="string">
  失败请求的错误代码，在信封的根层级报告，而不是在 `output` 下（请求成功时不返回）。
</ResponseField>

<ResponseField name="message" type="string">
  失败请求的详细信息，在信封的根层级报告，而不是在 `output` 下（请求成功时不返回）。在回退到 `output.message` 之前请先阅读此项。
</ResponseField>
