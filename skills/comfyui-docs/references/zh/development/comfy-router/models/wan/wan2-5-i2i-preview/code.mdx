...
在[你的 Comfy 工作区](https://platform.comfy.org/profile/api-keys?onboarding=router)中创建一个密钥，并将其导出为 `COMFY_API_KEY`。Python 和 TypeScript 片段使用 Comfy SDK（`pip install comfy-sdk` 和 `npm install @comfyorg/sdk`）；cURL 片段则是通过原生 HTTP 执行的同一调用。
...
将相同的请求体发送到 `POST https://api.comfy.org/v2/models/wan/wan2.5-i2i-preview/requests`。运行被受理后，Router 会立即返回 `201` 和 `request_id`；结果就绪后，可从本进程或其他进程收集。[队列投递](/zh/development/comfy-router/queue) 介绍了状态、取消与收集的完整流程。
...
# 3. 收集。完成后返回 200 及模型的原始输出；仍在运行时返回 202 及状态响应体。
