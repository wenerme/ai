...
在[你的 Comfy 工作区](https://platform.comfy.org/profile/api-keys?onboarding=router)中创建一个密钥，并将其导出为 `COMFY_API_KEY`。Python 和 TypeScript 代码片段使用 Comfy SDK（`pip install comfy-sdk` 和 `npm install @comfyorg/sdk`）；cURL 代码片段是通过原生 HTTP 进行的同一调用。
...
async def main():
    async with AsyncComfy() as client:
        ...
        # 轮询直到请求完成，等待时长以服务器返回的 Retry-After 为准。
...
<ParamField body="input.img_url" type="string">
  首帧图片网址或 Base64 编码数据。
仅 wan2.5-i2v-preview 和 wan2.6-i2v 模型需要。happyhorse-1.x 的
i2v 写法不在此处传入首帧：它们将首帧作为类型为 `first_frame` 的
`media` 元素传入（见下方 `media`），而在 wan2.6-i2v 上能够成功的
`img_url` 请求体，在 happyhorse-1.0-i2v 和 happyhorse-1.1-i2v 上会被
提供商拒绝。
图片格式：JPEG、JPG、PNG、BMP、WEBP。分辨率：360-2000 像素。
文件大小：最大 10MB。
</ParamField>

<ParamField body="input.media" type="object[]">
  wan2.7、wan3.0 和 happyhorse-1.x 模型的媒体资产列表。指定用于视频生成的
...
- wan3.0-video: first_frame（最多 1 个）、last_frame（最多 1 个）、reference_image（最多 10 个）、
  ...
  与 file 一起使用）。在同一次请求中，reference_*/file/link 类型与 first_frame/last_frame 类型
  互斥。数组顺序定义了提示词中资产的引用
  顺序（Image 1、Video 1、Audio 1……）。
- happyhorse-1.x-i2v: 仅 first_frame，且恰好 1 个。至少 300x300 像素，
  JPEG/JPG/PNG/WEBP，最大 20MB，可以是公共网址或 data:\{MIME_type};base64,... 网址。
  这些写法不使用 img_url；首帧放在这里。
- happyhorse-1.x-r2v: 仅 reference_image，1 到 9 个。最短边至少
  400 像素，最大 20MB，可以是公共网址或 data: 网址。reference_video 不是此操作的
  输入类型。
- happyhorse-1.x-video-edit: video
上面每个资产"最大 20MB"是合作伙伴对其最终获得的图像的上限，当资产是
公共网址时按字面适用，因为这些字节从不经过 Comfy。而内联的 data: 网址
确实会经过 Comfy，并同样受传输上限约束：Comfy Router 的 POST 请求体
总计上限为 100 MiB，超过则返回 413，而 base64 会使载荷膨胀
约 4/3，因此单个内联资产只要超过约 75 MB，就会在
触达这里的任何合作伙伴规则之前被拒绝。在该大小下，单个资产即使达到
合作伙伴自身的 20MB 上限也能轻松内联，因此对于单个资产而言，起约束作用的
是合作伙伴规则；而传输上限则在有多个资产时起约束作用，因为
100 MiB 的 Router 上限约束的是整个请求，而非每个元素。接近这些
上限的任何内容都应作为公共网址发送。
</ParamField>
...
<ParamField body="input.media[].url" type="string" required>
  媒体文件的网址：公共 HTTP/HTTPS 网址、OSS 临时网址，或者在
  上方 `media` 描述中该模型条目如此说明的情况下（happyhorse-1.x 的 i2v 和 r2v 写法即是如此），
  内联的 `data:{MIME_type};base64,...` 网址。关于各模型的大小与像素下限，以及
  约束内联载荷总量的 100 MiB Router 请求体上限，请参阅该描述。
</ParamField>
...
对于带多个参考视频的 wan2.6-r2v，使用 'character1'、'character2' 等按参考视频的
顺序指代主体。例如："Character1 在路边唱歌，Character2 在旁边跳舞"
对于 wan3.0-video 参考模式，使用 'Image 1'、'Video 1'、'Audio 1' 等按 media 数组中
对应的顺序指代媒体资产。
</ParamField>

<ParamField body="input.reference_video_urls" type="string[]">
  仅 wan2.6-r2v 模型使用的参考视频网址。1-3 个视频网址的数组。
输入限制：
- 格式：mp4、mov
- 数量：1-3 个视频
- 单个视频时长：2-30 秒
- 单个文件大小：最大 30MB
- 不能与 audio_url 一起使用
参考时长：单个视频最多 5 秒，两个视频每个最多 2.5 秒，三个视频按比例更短。
计费：基于实际使用的参考时长。
</ParamField>

<ParamField body="input.template" type="string">
  视频效果模板名称。可选。目前支持：squish、flying、carousel。使用时将忽略 prompt 参数。
</ParamField>

<ParamField body="model" type="string">
  要调用的模型 ID。此组件不对其进行约束：Comfy Router 会从 `POST /v2/models/wan/{model}` 的 `{model}` 路径段填充它，因此 Router 调用方可以省略。直接对 `POST /proxy/wan/api/v1/services/aigc/video-generation/video-synthesis` 发起 v1 调用时必须提供它，可接受的写法枚举位于该操作自身的组件 `WanVideoGenerationRequest` 上。
</ParamField>
...
- wan3.0-video：无视频输入时为 [2, 30] 区间内的整数；有视频输入时，输入视频
...
对于 wan3.0-video，自适应（默认值）会根据输入媒体的比例和意图
自动推荐合适的宽高比。
...
本文档由 Router 在 `GET /v2/models/wan/happyhorse-1.1-t2v/openapi.json` 提供的 schema 生成，也是请求到达提供商之前用于校验调用的同一份文档。
...
<ResponseField name="usage.duration" type="number">
  生成视频的时长，单位为秒（I2V 和 wan3.0-video 任务）
</ResponseField>
...
<ResponseField name="code" type="string">
  请求失败时的错误码，在响应封装结构的根层级返回，而不是在 `output` 下（请求成功时不返回）。
</ResponseField>

<ResponseField name="message" type="string">
  请求失败的详细信息，在响应封装结构的根层级返回，而不是在 `output` 下（请求成功时不返回）。在回退到 `output.message` 之前请先读取此项。
</ResponseField>
