---
title: "使用 Comfy Router 调用 Wan 2.5 T2V Preview"
description: "通过 Comfy Router 以 HTTP 方式调用 wan/wan2.5-t2v-preview 的 Python、TypeScript 和 cURL 代码片段，以及请求字段和返回结果结构"
translationSourceHash: d0050c3b
translationFrom: development/comfy-router/models/wan/wan2-5-t2v-preview/code.mdx
translationBlockHashes:
  "_intro": 09319905
  "Quick start": 02a1b906
  "Schema": 6031e990
  "Examples": 12f77722
---

Wan 2.5 T2V Preview 的 API 参考。Wan 2.5 文生视频仅凭提示词即可生成一段短视频，无需参考图像或视频。

## 快速开始

在[您的 Comfy 工作区](https://platform.comfy.org/profile/api-keys?onboarding=router)中创建密钥，并将其导出为 `COMFY_API_KEY`。Python、TypeScript 和 Swift 代码片段使用 Comfy SDK（`pip install comfy-sdk`、`npm install @comfyorg/sdk`，以及 [`ComfySwiftSDK`](https://github.com/Comfy-Org/comfy-swift-sdk) Swift 包）；cURL 代码片段则是通过原始 HTTP 进行的相同调用。

**模型 ID：** `wan/wan2.5-t2v-preview`

**端点：** `POST https://api.comfy.org/v2/models/wan/wan2.5-t2v-preview`
...
const result = await comfy.models.run<Result>("wan/wan2.5-t2v-preview", {
  input: {
    prompt: "a single red maple leaf falling onto still water, slow motion, shallow depth of field",
  },
  parameters: {
...
相同的请求体，发送到 `POST https://api.comfy.org/v2/models/wan/wan2.5-t2v-preview/requests`。运行一经接纳，Router 便立即返回 `201` 和 `request_id`，结果就绪后即可收集，无论来自当前进程还是另一个进程。[队列投递](/zh/development/comfy-router/queue) 会详细介绍状态、取消与收集的流程。
...
with Comfy() as client:
    ...
    # 轮询直到请求完成，每次等待服务器给出的 Retry-After 时长。
...
// 每次 submit() 调用都会生成自己的 Idempotency-Key，并在自动重试时复用它。
type Result = { output: { video_url: string } };
const handle = await comfy.models.submit<Result>("wan/wan2.5-t2v-preview", {
  input: {
    prompt: "a single red maple leaf falling onto still water, slow motion, shallow depth of field",
  },
  parameters: {
...
curl https://api.comfy.org/v2/models/wan/wan2.5-t2v-preview/requests \
  -H "X-API-Key: $COMFY_API_KEY" \
  -H "Idempotency-Key: $(uuidgen)" \
  -H "Content-Type: application/json" \
  -d "{\"input\": {\"prompt\":\"a single red maple leaf falling onto still water, slow motion, shallow depth of field\"}, \"parameters\": {\"resolution\":\"720P\",\"duration\":5}}"

# 2. 轮询直到状态为 COMPLETED，每次等待响应中给出的 Retry-After 秒数。
REQUEST_ID="<request_id from the 201 body>"
curl -i https://api.comfy.org/v2/models/wan/wan2.5-t2v-preview/requests/$REQUEST_ID/status \
  -H "X-API-Key: $COMFY_API_KEY"

# 3. 收集。仍在运行时返回 202 和状态响应体，完成后返回 200 和模型的原始输出。
curl https://api.comfy.org/v2/models/wan/wan2.5-t2v-preview/requests/$REQUEST_ID \
  -H "X-API-Key: $COMFY_API_KEY"
```
...
仅 wan2.5-i2v-preview 和 wan2.6-i2v 模型需要。happyhorse-1.x 的 i2v 写法不在此处传入首帧：它们将首帧作为类型为 `first_frame` 的 `media` 元素传入（见下文 `media`），而在 wan2.6-i2v 上能够成功的 `img_url` 请求体，会被提供商在 happyhorse-1.0-i2v 和 happyhorse-1.1-i2v 上拒绝。
...
- wan3.0-video: first_frame（最多 1 个）、last_frame（最多 1 个）、reference_image（最多 10 个）、
  ...
  在同一请求内互斥。数组顺序定义了提示中资产的引用顺序（Image 1、Video 1、Audio 1、...）。
- happyhorse-1.x-i2v: 仅 first_frame，且恰好 1 个。至少 300x300 像素，
  JPEG/JPG/PNG/WEBP，最大 20MB，公开 URL 或 data:\{MIME_type};base64,... URL。
  这些写法不接受 img_url；首帧从这里传入。
- happyhorse-1.x-r2v: 仅 reference_image，1 到 9 个。最短边至少
  400 像素，最大 20MB，公开 URL 或 data: URL。reference_video 不是该操作的
  输入类型。
- happyhorse-1.x-video-edit: video
以上各资产的“最大 20MB”数值是合作伙伴对其最终拿到的图像所设的上限，
当资产为公开 URL 时按字面适用，因为这些字节从不经过 Comfy。内联的
data: URL 确实会经过 Comfy，并且也会遇到传输上限：Comfy Router 的 POST 请求体
总计上限为 100 MiB，超出即返回 413，而 base64 会使负载膨胀约 4/3，
因此单个内联资产超过约 75 MB 时，尚未触达这里的任何合作伙伴规则就会被拒绝。
在该体积下，单个资产按合作伙伴自身 20MB 的上限可以轻松地以内联方式容纳，
所以对单个资产起约束作用的是合作伙伴规则；传输上限则在多个资产时起约束作用，
因为 100 MiB 的 Router 上限约束的是整个请求，而非每个元素。接近这些上限的
任何内容都应以公开 URL 发送。
</ParamField>
...
<ParamField body="input.media[].url" type="string" required>
  媒体文件的 URL：公开 HTTP/HTTPS URL、OSS 临时 URL，或（在该模型于上述 `media` 描述中的条目允许时，正如 happyhorse-1.x 的 i2v 和 r2v 写法那样）内联的 `data:{MIME_type};base64,...` URL。有关各模型的大小与像素下限，以及以总量约束内联负载的 100 MiB Router 请求体上限，请参阅该描述。
</ParamField>
...
对于带有多个参考视频的 wan2.6-r2v，请使用 'character1'、'character2' 等，按参考视频的顺序引用主体。示例：“Character1 在路边唱歌，Character2 在旁边跳舞”
对于 wan3.0-video 参考模式，请使用 'Image 1'、'Video 1'、'Audio 1' 等，按 media 数组中的相应顺序引用媒体资产。
</ParamField>
...
<ParamField body="model" type="string">
  要调用的模型 ID。该组件上不做约束：Comfy Router 会从 `POST /v2/models/wan/{model}` 的 `{model}` 路径段中填充它，因此 Router 调用方会省略它。直接以 v1 调用 `POST /proxy/wan/api/v1/services/aigc/video-generation/video-synthesis` 时必须提供它，可接受写法的枚举位于该操作自身的组件 `WanVideoGenerationRequest` 上。
</ParamField>
...
- wan3.0-video：无视频输入时为 [2, 30] 范围内的整数；有视频输入时，
  输入视频时长与输出视频时长之和不得超过 30 秒；-1 启用智能时长模式，
  由模型选择合适的时长
...
对于 wan2.7 模型，若未提供，则根据分辨率档位采用默认值。
对于 wan3.0-video，adaptive（默认）会根据输入媒体的比例和意图自动推荐合适的宽高比。
...
根据 Router 在 `GET /v2/models/wan/wan2.5-t2v-preview/openapi.json` 提供的 schema 生成，该 schema 也是请求到达提供商之前 Router 用于校验调用的同一份文档。
...
<ResponseField name="code" type="string">
  请求失败时的错误码，报告在响应外层结构的根层级，而非 `output` 下（请求成功时不返回）。
</ResponseField>

<ResponseField name="message" type="string">
  请求失败的详细信息，报告在响应外层结构的根层级，而非 `output` 下（请求成功时不返回）。在回退到 `output.message` 之前请先读取此项。
</ResponseField>
...
视频 URL 的有效期为 24 小时。如需保留，请及时下载视频。
