---
title: "通过 Comfy Router 使用 Wan 2.6 R2V"
description: "通过 Comfy Router 以 HTTP 方式调用 wan/wan2.6-r2v 的 Python、TypeScript 和 cURL 代码片段，以及请求字段和结果结构"
translationSourceHash: 8c87fa98
translationFrom: development/comfy-router/models/wan/wan2-6-r2v/code.mdx
translationBlockHashes:
  "_intro": e8706b25
  "Quick start": 3d59d4a1
  "Schema": 9c478b60
  "Examples": dd88ebee
---

Wan 2.6 R2V 的 API 参考。Wan 2.6 参考生视频可将一到三个参考视频中的主体带入新的场景，并在提示词中以 character1、character2 等加以指代。

## 快速开始

在[你的 Comfy 工作区](https://platform.comfy.org/profile/api-keys?onboarding=router)中创建一个密钥，并将其导出为 `COMFY_API_KEY`。Python、TypeScript 和 Swift 代码片段使用 Comfy SDK（`pip install comfy-sdk`、`npm install @comfyorg/sdk`，以及 [`ComfySwiftSDK`](https://github.com/Comfy-Org/comfy-swift-sdk) Swift 包）；cURL 代码片段则是通过原生 HTTP 发起的同一次调用。

**模型 ID：** `wan/wan2.6-r2v`

**端点：** `POST https://api.comfy.org/v2/models/wan/wan2.6-r2v`
...
const result = await comfy.models.run<Result>("wan/wan2.6-r2v", {
  input: {
    prompt: "character1 walks in from the left, turns to the camera and waves, plain orange backdrop",
    reference_video_urls: ["https://raw.githubusercontent.com/Comfy-Org/workflow_templates/main/input/street_dancer.mp4"],
  },
...
同一请求体，发送至 `POST https://api.comfy.org/v2/models/wan/wan2.6-r2v/requests`。只要运行被接受，Router 就会返回 `201` 和一个 `request_id`，结果就绪后即可收集，无论从当前进程还是其他进程。 [队列投递](/zh/development/comfy-router/queue) 介绍了状态查询、取消和结果收集。
...
with Comfy() as client:
    ...
    print("request_id:", handle.request_id)  # 加上模型 ID，就是另一个进程所需的全部内容

    # 轮询直到请求完成，每轮按服务器指定的 Retry-After 等待。
...
// 每次 submit() 调用都会生成自己的 Idempotency-Key，并在自动重试时复用它。
type Result = { output: { video_url: string } };
const handle = await comfy.models.submit<Result>("wan/wan2.6-r2v", {
  input: {
    prompt: "character1 walks in from the left, turns to the camera and waves, plain orange backdrop",
    reference_video_urls: ["https://raw.githubusercontent.com/Comfy-Org/workflow_templates/main/input/street_dancer.mp4"],
  },
...
console.log("requestId:", handle.requestId); // 加上模型 ID，就是另一个进程所需的全部内容
...
print("requestId:", handle.requestId)  // 加上模型 ID，就是另一个进程所需的全部内容
...
curl https://api.comfy.org/v2/models/wan/wan2.6-r2v/requests \
  -H "X-API-Key: $COMFY_API_KEY" \
  -H "Idempotency-Key: $(uuidgen)" \
  -H "Content-Type: application/json" \
  -d "{\"input\": {\"prompt\":\"character1 walks in from the left, turns to the camera and waves, plain orange backdrop\",\"reference_video_urls\":[\"https://raw.githubusercontent.com/Comfy-Org/workflow_templates/main/input/street_dancer.mp4\"]}, \"parameters\": {\"resolution\":\"720P\",\"duration\":5}}"

# 2. 轮询直到 status 为 COMPLETED，每轮按响应中指定的 Retry-After 秒数等待。
REQUEST_ID="<request_id from the 201 body>"
curl -i https://api.comfy.org/v2/models/wan/wan2.6-r2v/requests/$REQUEST_ID/status \
  -H "X-API-Key: $COMFY_API_KEY"

# 3. 收集结果。返回 200 及模型的原生输出；仍在运行时返回 202 及状态响应体。
curl https://api.comfy.org/v2/models/wan/wan2.6-r2v/requests/$REQUEST_ID \
  -H "X-API-Key: $COMFY_API_KEY"
```
...
拼写不在此处传入首帧：它们将首帧作为 `media` 中类型为 `first_frame` 的元素传入
（参见下文 `media`），而在 wan2.6-i2v 上能够成功的 `img_url` 请求体，
...
参考素材（图像、音频、视频）。每个元素包含一个 type 字段和一个
url 字段。
支持的 type 取值因模型而异：
...
- wan3.0-video: first_frame（最多 1 个）、last_frame（最多 1 个）、reference_image（最多 10 个）、
  ...
  与 file 同时使用）。在同一请求中，reference_*/file/link 类型与 first_frame/last_frame
  类型互斥。数组顺序决定提示词中资产的引用
...
上述每项资产"最大 20MB"的数值是合作伙伴对其最终接收图像设定的容量上限，
当资产是公开网址时按字面适用，因为这些字节从不会经过 Comfy 传输。内联
data: 网址则会经过 Comfy 传输，因此还要受传输层上限约束：Comfy Router 的 POST 请求体
总上限为 100 MiB，超出会返回 413；而 base64 会使载荷膨胀约 4/3，因此
单个内联资产超过约 75 MB 时，在触达此处任何合作伙伴规则之前就会被拒绝。
在该大小下，单个资产即使达到合作伙伴自身 20MB 的上限也能轻松内联容纳，
因此对于单个资产而言，起约束作用的是合作伙伴规则；而传输层上限则在多个
资产同时出现时起作用，因为 100 MiB 的 Router 上限约束的是整个请求，而非每个
元素。任何接近这些上限的内容，请以公开网址发送。
</ParamField>
...
<ParamField body="input.media[].url" type="string" required>
  媒体文件的网址：公开的 HTTP/HTTPS 网址、OSS 临时网址，或者（当上文中 `media` 描述里对应模型的条目如此说明时，例如 happyhorse-1.x 的 i2v 和 r2v 拼写）内联的 `data:{MIME_type};base64,...` 网址。有关各模型的尺寸与像素下限，以及 100 MiB 的 Router 请求体总上限对内联载荷的聚合约束，请参见该描述。
</ParamField>
...
对于带有多个参考视频的 wan2.6-r2v，可使用 'character1'、'character2' 等按参考视频的顺序指代
主体。例如："Character1 sings on the roadside, Character2 dances beside it"
对于 wan3.0-video 的参考模式，可使用 'Image 1'、'Video 1'、'Audio 1' 等按 media 数组中的对应顺序
指代媒体资产。
</ParamField>
...
- 单个文件大小：最大 30MB
- 不能与 audio_url 同时使用
参考时长：单个视频最长 5s，两个视频每个最长 2.5s，三个视频则相应更短。
计费：按实际使用的参考时长计费。
</ParamField>
...
<ParamField body="model" type="string">
  要调用的模型 ID。此组件不做约束：Comfy Router 会从 `POST /v2/models/wan/{model}` 的 `{model}` 路径段填充它，因此 Router 调用方会省略该字段。直接以 v1 调用 `POST /proxy/wan/api/v1/services/aigc/video-generation/video-synthesis` 时必须提供它，其接受的拼写枚举位于该操作自身的组件 `WanVideoGenerationRequest` 中。
</ParamField>
...
- wan2.7-i2v、wan2.7-t2v：取值范围为 [2, 15] 的整数
- wan2.7-r2v、wan2.7-videoedit：取值范围为 [2, 10] 的整数
- wan3.0-video：无视频输入时为 [2, 30] 的整数；有视频输入时，输入视频时长与输出视频时长之和
  不得超过 30 秒；-1 启用智能时长模式，由模型自行选择合适的时长
...
对于 wan2.7 模型，如果未提供该参数，则根据分辨率档位设置默认值。
...
根据 schema 生成，Router 在 `GET /v2/models/wan/wan2.6-r2v/openapi.json` 提供该 schema，它也是请求到达提供商之前用于校验调用的同一份文档。
...
<ResponseField name="code" type="string">
  失败请求的错误代码，报告在信封的根层级，而不是 `output` 之下（请求成功时不返回）。
</ResponseField>

<ResponseField name="message" type="string">
  失败请求的详细信息，报告在信封的根层级，而不是 `output` 之下（请求成功时不返回）。在回退到 `output.message` 之前请先读取此项。
</ResponseField>
