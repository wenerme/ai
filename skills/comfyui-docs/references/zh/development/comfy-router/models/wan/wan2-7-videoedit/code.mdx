---
title: "搭配 Comfy Router 使用 Wan 2.7 Video Edit"
description: "用于通过 Comfy Router 以 HTTP 方式调用 wan/wan2.7-videoedit 的 Python、TypeScript 和 cURL 代码片段，以及请求字段和结果结构"
translationSourceHash: 9c2145f8
translationFrom: development/comfy-router/models/wan/wan2-7-videoedit/code.mdx
translationBlockHashes:
  "_intro": 47b7d5e3
  "Quick start": defbf2cb
  "Schema": 46b86f43
  "Examples": 622d1416
---

Wan 2.7 视频编辑的 API 参考。Wan 2.7 视频编辑可根据文本指令重写已有的视频片段，并可选地由参考图像进行引导。

## 快速开始

在[你的 Comfy 工作区](https://platform.comfy.org/profile/api-keys)中创建密钥，并将其导出为 `COMFY_API_KEY`。Python 和 TypeScript 代码片段使用 Comfy SDK（`pip install comfy-sdk`、`npm install @comfyorg/sdk`）；cURL 代码片段则是通过原生 HTTP 发起的同一个调用。

**模型 ID：** `wan/wan2.7-videoedit`

**端点：** `POST https://api.comfy.org/v2/models/wan/wan2.7-videoedit`
...
const result = await comfy.models.run<Result>("wan/wan2.7-videoedit", {
  input: {
    prompt: "restyle the clip as a hand-drawn ink sketch, keep the dancer's movement unchanged",
    media: [
      {
...
同一个请求体,发送到 `POST https://api.comfy.org/v2/models/wan/wan2.7-videoedit/requests`。只要运行被接纳,Router 就会返回 `201` 和 `request_id`,结果准备就绪后,即可从当前进程或另一个进程进行收集。[排队交付](/zh/development/comfy-router/queue) 完整介绍了状态、取消与收集的流程。
...
- wan3.0-video：first_frame（最大值 1）、last_frame（最大值 1）、reference_image（最大值 10）、
  ...
  与 file 同时使用）。在同一次请求中，reference_*/file/link 类型与 first_frame/last_frame
  类型互斥。数组顺序决定提示词中资产的引用顺序（Image 1、Video 1、Audio 1……）。
...
对于带多个参考视频的 wan2.6-r2v，请按照参考视频的顺序使用 'character1'、'character2' 等来引用主体。
示例："Character1 在路边唱歌，Character2 在旁边跳舞"
对于 wan3.0-video 参考模式，请使用 'Image 1'、'Video 1'、'Audio 1' 等，按照 media 数组中的
对应顺序引用媒体资产。
</ParamField>

<ParamField body="input.reference_video_urls" type="string[]">
  仅限 wan2.6-r2v 模型的参考视频 URL。由 1-3 个视频 URL 组成的数组。
输入限制：
- 格式：mp4、mov
- 数量：1-3 个视频
- 单个视频长度：2-30 秒
- 单个文件大小：最大 30MB
- 不能与 audio_url 同时使用
参考时长：单个视频最长 5s，两个视频各最长 2.5s，三个视频按比例相应减少。
计费：按实际使用的参考时长计费。
</ParamField>
...
<ParamField body="model" type="string">
  要调用的模型 ID。此组件不作约束：Comfy Router 会从 `POST /v2/models/wan/{model}` 的 `{model}` 路径段中填入该值，因此 Router 调用方可以省略。直接向 `POST /proxy/wan/api/v1/services/aigc/video-generation/video-synthesis` 发起的 v1 调用则必须提供，可接受的拼写枚举位于该操作自身的组件 `WanVideoGenerationRequest` 上。
</ParamField>
...
- wan3.0-video：无视频输入时为 [2, 30] 区间内的整数；有视频输入时，输入视频总时长
  加上输出视频时长不得超过 30 秒；-1 启用智能时长模式，由模型选择合适的时长

  范围：`-1` 到 `30`
</ParamField>

<ParamField body="parameters.prompt_extend" type="boolean" default="true">
  是否启用提示词智能改写。默认是 true
</ParamField>
...
自动推荐合适的宽高比。
  ...
  可能取值：`adaptive`、`16:9`、`9:16`、`1:1`、`4:3`、`3:4`
</ParamField>
...
根据 Router 在 `GET /v2/models/wan/wan2.7-videoedit/openapi.json` 提供的 schema 生成，该文档也是它在请求到达提供商之前用于校验调用的同一份文档。
### 输出
...
<ResponseField name="output.video_url" type="string">
  已完成的视频生成任务的视频 URL。连线有效期为 24 小时
</ResponseField>
...
<ResponseField name="code" type="string">
  请求失败时的错误码，在响应信封（envelope）的根层级而非 `output` 下上报（请求成功时不返回）。
</ResponseField>

<ResponseField name="message" type="string">
  请求失败的详细信息，在响应信封（envelope）的根层级而非 `output` 下上报（请求成功时不返回）。在回退到 `output.message` 之前请先阅读此项。
</ResponseField>
