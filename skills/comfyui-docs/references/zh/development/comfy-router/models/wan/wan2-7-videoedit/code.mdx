...
在[你的 Comfy 工作区](https://platform.comfy.org/profile/api-keys?onboarding=router)中创建密钥，并将其导出为 `COMFY_API_KEY`。Python、TypeScript 和 Swift 代码片段使用 Comfy SDK（`pip install comfy-sdk`、`npm install @comfyorg/sdk`，以及 [`ComfySwiftSDK`](https://github.com/Comfy-Org/comfy-swift-sdk) Swift 包）；cURL 代码片段则是通过原生 HTTP 发起的同一个调用。
...
同一个请求体,发送到 `POST https://api.comfy.org/v2/models/wan/wan2.7-videoedit/requests`。只要运行被接纳,Router 就会返回 `201` 和 `request_id`,结果准备就绪后,即可从当前进程或另一个进程进行收集。[队列投递](/zh/development/comfy-router/queue) 完整介绍了状态、取消与收集的流程。
...
# 3. 收集。返回 200 及模型的原生输出,或在其仍在运行时返回 202 及状态响应体。
...
<ParamField body="input.img_url" type="string">
  首帧图像网址或 Base64 编码数据。
仅 wan2.5-i2v-preview 和 wan2.6-i2v 模型必填。happyhorse-1.x 的 i2v 拼写不在此处接收首帧：
它们把首帧作为类型为 `first_frame` 的 `media` 元素传入（见下方 `media`），
而在 wan2.6-i2v 上能成功的 `img_url` 请求体，会在 happyhorse-1.0-i2v 和
happyhorse-1.1-i2v 上被提供商拒绝。
...
上述每项资产“最大 20MB”的数值是合作伙伴对其最终得到的图像所设的封顶值，当资产是公开网址时
按字面适用，因为这些字节从不经过 Comfy。内联的 data: 网址确实会经过
Comfy，并且同样受制于传输封顶：Comfy Router 的 POST 请求体
总计上限为 100 MiB，超过则返回 413，而 base64 会使负载膨胀
约 4/3，因此单个内联资产若超过约 75 MB，会在触达此处任何合作伙伴规则
之前就被拒绝。在这个大小下，单个资产处于合作伙伴自身 20MB
封顶之内可以轻松以内联方式容纳，因此对单个资产而言起约束作用的是合作伙伴规则；而传输封顶
约束的是多个资产，因为 100 MiB 的 Router 上限约束的是整个请求，
而非每个元素。任何接近这些上限的内容都应作为公开网址发送。
</ParamField>
...
<ParamField body="input.media[].url" type="string" required>
  媒体文件的网址：公开 HTTP/HTTPS 网址、OSS 临时网址，或者（在上文 `media` 描述中该模型的条目
  如此说明时，例如 happyhorse-1.x 的 i2v 和 r2v 拼写那样）内联的 `data:{MIME_type};base64,...` 网址。请参阅该描述以了解各模型的
  大小和像素下限，以及约束内联负载总量的 100 MiB Router 请求体上限。
</ParamField>
...
对于带多个参考视频的 wan2.6-r2v，按参考视频的顺序使用 'character1'、'character2' 等来指代主体。
...
<ParamField body="model" type="string">
  要调用的模型 ID。在此组件上不受约束：Comfy Router 会从 `POST /v2/models/wan/{model}` 的 `{model}` 路径段填充它，因此 Router 调用方会省略它。直接对 `POST /proxy/wan/api/v1/services/aigc/video-generation/video-synthesis` 发起 v1 调用时必须提供它，可接受的拼写枚举位于该操作自己的组件 `WanVideoGenerationRequest` 上。
</ParamField>
...
- wan3.0-video：无视频输入时为 [2, 30] 范围内的整数；有视频输入时，
  输入视频总时长与输出视频时长之和不得超过 30 秒；-1 启用智能时长模式，
  由模型选择合适的时长
...
wan2.5 T2V：480P（480*832、832*480、624*624）、720P、1080P 尺寸
wan2.6 T2V/R2V（不支持 480P）：
...
由 Router 在 `GET /v2/models/wan/wan2.7-videoedit/openapi.json` 提供的 schema 生成，与请求到达提供商之前校验调用时所用的文档相同。
...
<ResponseField name="code" type="string">
  失败请求的错误码，报告在响应封装的 ROOT 层级，而不是在 `output` 下（如果请求成功则不返回）。
</ResponseField>

<ResponseField name="message" type="string">
  失败请求的详细信息，报告在响应封装的 ROOT 层级，而不是在 `output` 下（如果请求成功则不返回）。在回退到 `output.message` 之前，请先查看此内容。
</ResponseField>
