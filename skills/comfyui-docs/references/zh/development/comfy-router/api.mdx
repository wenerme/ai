...
本页内容全部属于第一行，也就是 Comfy Router。如果你想运行完整的 ComfyUI 工作流图，而不是调用单个合作伙伴模型，那就是第二行。它位于同一个包中，入口在 [Comfy SDKs](/zh/development/api-development/sdks)。
...
一次路由器调用默认最多可保持连接 10 分钟。请将客户端超时设置为高于该上限，这样你拿到的是带类型的 `504` 和请求 ID，而不是一个含义不明的本地中止。如果您的应用无法保持这么久的连接，[队列投递](/zh/development/comfy-router/queue) 会立即返回 `request_id`，让您可以稍后收集结果。
...
- [队列投递](/zh/development/comfy-router/queue)：无需保持连接即可提交请求、跟踪状态、收集结果或取消。
