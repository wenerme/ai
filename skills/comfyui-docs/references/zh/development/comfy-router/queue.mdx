...
**同步路由有 10 分钟的上限。** 这个上限正是本页存在的原因：Router 的默认截止时间为 10 分钟（可按部署配置），同步调用一旦达到该时限就会被切断，返回 `504` / `deadline_exceeded`，不论提供商是否仍在工作。如果一次生成的运行时间可能超过这个上限，或者调用方无法将连接保持打开十分钟，就必须使用队列。关于截止时间会告诉你什么、不会告诉你关于计费的什么，请参阅[调用会在服务器截止时间被切断](/zh/development/comfy-router/limitations#调用会在服务器截止时间被中断)。
...
| 应答 | `200`，返回模型的原生输出 | `201`，返回一个 `request_id` 和三个 URL |
| 结果 | 在响应中 | 稍后收集，输出逐字节完全相同 |
| 时间限制 | 受 Router 的 [10 分钟截止时间](/zh/development/comfy-router/limitations#调用会在服务器截止时间被中断)约束，之后返回 `504` / `deadline_exceeded` | 运行本身没有限制。完成的结果会[保留 24 小时](#幂等性与计费) |
...
- **`subscribe(model, body, ...)`** 通过一次调用完成提交、轮询和收集，并带有进度回调。
- **`handle(model, request_id)`** 根据这两个 ID 在另一个进程中重建句柄，而无需发起任何调用。

两个 ID 在所有地方都需要，因为二者共同寻址该请求：路由为 `/v2/models/{provider}/{model}/requests/{request_id}`。

  这里的 `events()` **不是** Comfy Cloud 客户端的 `job.events()`。这里的是轮询状态路由，并为 Router 请求产生队列观测结果（状态和队列位置）。Comfy Cloud 的那个则是 ComfyUI 工作流任务的实时 SSE 流，携带进度、预览和输出。`subscribe()` 又是另一种东西：它根本不是迭代器，而是把提交、轮询和收集合并到一次调用中。请参阅 [`events()` 不是 `subscribe()`](/zh/development/api-development/sdks#events-不是-subscribe)。
...
这会将[快速开始](/zh/development/comfy-router/quickstart) 所发送的同一请求加入队列，并收集生成的图像。请先将你的密钥导出为 `COMFY_API_KEY`。
...
# 1. 提交。Router 返回 201，包含 request_id、status_url、response_url 和 cancel_url。
curl "$BASE/requests" \
...
# 4. 取消尚未完成的请求。这只是一次请求，而不是保证。
curl -X PUT "$BASE/requests/$REQUEST_ID/cancel" -H "X-API-Key: $COMFY_API_KEY"
```

每个[模型页面](/zh/development/comfy-router/models) 都会在 **排队并稍后收集** 一节下，为其自身的模型提供这种形态，与同步示例并列。

### 一次调用中跟踪进度并收集结果

当你确实想要等待、同时也想展示进度时，`subscribe` 会将提交、轮询和收集合并为一次调用：
...
超时是客户端的约束，在服务端没有含义。超时之后，`subscribe` 会在抛出之前尽力执行一次取消。取消只对尚未开始运行的请求生效：已经在合作方那里进行中的一次生成会照常完成并计费，无论是否有人收集它。当请求的生命周期需要长于调用者时，请使用 `submit`。

### 从另一个进程收集结果

请把 `request_id` 与模型 ID 一起保存。重建句柄需要这两者，而在你使用它之前不会发起任何调用。
...
`status()` 只进行一次轮询并返回当前状态。`cancel()` 请求服务器停止一个尚未完成的请求。这是一次请求，而不是保证：已经在合作方那里发出的运行可能依然会完成，而随后一次 `status()` 才是真实情况。
...
`AsyncComfy` 对应了每一个名称、参数和参数顺序。没有 `submit_async`，原因与没有 `run_async` 相同。
...
已完成但未成功的请求会以 `COMPLETED` 并携带 `error_type` 的形式上报。`get()` 和 `subscribe()` 会把它转换为该类别的类型化 Router 错误：Python 中是 `comfy_sdk.router_exceptions` 里的类，TypeScript 中是 `routerErrors.*`。事件迭代器在该情况下不会抛出，因为它只是队列进度的视图：携带 `error_type` 的完成会作为最后一次观测产出，而由 `get()` 负责收集。提交时的 `403` `not_enabled` 会以 `NotEnabled` 到达，并且是终止性的，因此 SDK 不会重试它。
