...
| 方法                        | 优点                                             | 缺点                                                   | 
| -------------------------- | ------------------------------------------------ | ----------------------------------------------------- |
| **ComfyUI Manager** (推荐) | 1. 自动化安装2. 依赖处理3. 图形界面         | 不在 registry 中注册的节点无法通过 Manager 直接搜索到       |
| **Git 克隆**               | 可以安装不在 registry 中注册的节点                    | 1. 需要Git知识2. 手动处理依赖3. 存在安装风险      |
| **代码仓库 ZIP 下载**        | 1. 无需Git2. 手动处理一切                     | 1. 需要手动处理依赖2. 无版本控制3. 存在安装风险    |
