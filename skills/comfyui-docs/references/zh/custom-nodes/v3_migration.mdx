...
translationSourceHash: ef191ae4
translationFrom: custom-nodes/v3_migration.mdx
translationBlockHashes:
  "Overview": 109e87b3
  "Core Concepts": 85282582
  "Migration Steps": 7840c755
---

## 概述

ComfyUI V3 架构引入了一种更有条理的节点定义方式，今后节点功能的扩展也只会添加到 V3 架构中。你可以参考本指南，将现有的 V1 节点迁移到新的 V3 架构。
...
def validate_inputs(cls, **kwargs) -> bool | str:
    # 有效时返回 True，否则返回错误字符串
    if error_condition:
        return "Error message"
    return True
```

#### 惰性求值 (V1 → V3) {#lazy-evaluation-v1-v3}

函数 ```check_lazy_status``` 改为类方法，其他部分保持不变。
...
数据类 ```Schema``` 定义了 V3 节点的所有属性。以下是所有可用字段的完整参考：
...
| `lazy` | `bool` | `None` | 标记输入为惰性求值（参见[惰性求值](#lazy-evaluation-v1-v3)）。 |
...
隐藏输入提供对执行上下文的访问，例如提示词元数据、节点 ID 和其他内部值。它们不会显示在 UI 中。

在 V1 中，隐藏输入通过 `INPUT_TYPES` 中的 `"hidden"` 键声明。在 V3 中，它们通过 Schema 的 `hidden` 参数声明，其值通过 `cls.hidden` 访问。
...
| 隐藏枚举 | 描述 |
|-------------|-------------|
| `io.Hidden.unique_id` | 节点的全局唯一标识符，与客户端侧的 id 一致。 |
| `io.Hidden.prompt` | 客户端发送的完整提示词。 |
| `io.Hidden.extra_pnginfo` | 复制到已保存 `.png` 文件元数据中的字典。 |
| `io.Hidden.dynprompt` | 可能在执行过程中发生变化的 `DynamicPrompt` 实例。 |
| `io.Hidden.auth_token_comfy_org` | 在前端登录 ComfyOrg 账户后获取的令牌。 |
| `io.Hidden.api_key_comfy_org` | ComfyOrg 生成的 API 密钥，可跳过前端登录。 |

某些隐藏值会根据 Schema 标志自动添加。输出节点（`is_output_node=True`）会自动接收 `prompt` 和 `extra_pnginfo`。API 节点（`is_api_node=True`）会自动接收身份验证令牌。

### UI 辅助工具

V3 在 `ui` 模块中提供了内置的 UI 辅助工具，用于处理预览和保存文件等常见模式。通过 `ui` 参数将它们传递给 `io.NodeOutput`。

#### 预览辅助工具

预览辅助工具保存临时文件并返回用于节点内显示的 UI 数据。
...
保存辅助工具提供将文件保存到输出目录并嵌入相应元数据的方法。它们通常用于输出节点。
...
return io.NodeOutput(
    ui=ui.ImageSaveHelper.get_save_images_ui(
        ...
        cls=cls,  # 传入隐藏的 prompt/extra_pnginfo 用于元数据
    )
)
...
向保存/预览辅助工具传递 `cls=cls` 可以让它们自动将工作流元数据（prompt、extra_pnginfo）嵌入到保存的文件中。请确保在 Schema 的 hidden 列表中包含 `io.Hidden.prompt` 和 `io.Hidden.extra_pnginfo`，或者设置 `is_output_node=True`（会自动添加它们）。

#### 返回原始 UI 字典

如果需要返回没有对应辅助工具的 UI 数据，可以直接传递字典：
...
用于产生副作用（如保存文件）的节点。与 V1 相同，将节点标记为输出节点后，节点的上下文菜单中会显示 `run` 播放按钮，允许部分执行图。
...
可以通过类定义或 ```Custom``` 辅助函数创建自定义输入/输出类型。
...
# 带小部件回退的多类型输入（未连接任何内容时显示 String 小部件）
io.MultiType.Input(
...
V3 引入了根据用户交互改变可用输入的动态输入类型。这些功能在 V1 中没有对应物。
...
def execute(cls, images: io.Autogrow.Type) -> io.NodeOutput:
    # 'images' 是将输入名称映射到其值的字典
    image_list = list(images.values())
    return io.NodeOutput(batch(image_list))
```

**TemplateNames** 生成具有指定名称的输入：
...
Autogrow 可以与 MatchType 结合使用，创建类型匹配的输入列表：
...
def execute(cls, image, resize_type: dict) -> io.NodeOutput:
    # resize_type 是一个字典，包含所选选项的键及其输入
...
V3 支持异步的 `execute` 方法。这对执行 I/O 操作、API 调用或其他异步工作的节点很有用。只需将 `execute` 声明为 `async`：
...
在节点的 `execute` 方法内报告执行进度。进度条显示在 ComfyUI 界面中。这取代了 V1 中使用 `comfy.utils.PROGRESS_BAR_HOOK` 的模式。
...
`set_progress` 的 `preview_image` 参数可以接受 PIL 图像、`ImageInput` 张量或 `None`。在 `execute` 内部调用时，`node_id` 会从执行上下文中自动确定。

#### 节点替换

节点替换允许将旧节点/已弃用节点映射到新节点，使现有工作流自动升级。在扩展的 `on_load` 方法中使用 `ComfyAPI` 注册替换。
...
# 返回多个输出值（顺序与 Schema 中 outputs 列表一致）
