...
X.509 workload identity federation is available in beta for the OpenAI API;
  Codex does not support it. If X.509 doesn't appear as a provider type, contact
  your system administrator. For Codex, use an OIDC token or SPIFFE JWT-SVID and
  follow the [Codex workload identity
  guide](https://developers.openai.com/codex/enterprise/workload-identity).
