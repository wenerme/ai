---
title: "Amazon Timestream query editor | Grafana Plugins documentation"
...
| **Wait for all queries** | When enabled, the plugin fetches all paginated result pages before returning data. Enable this for [alerting queries](/docs/plugins/grafana-timestream-datasource/latest/alerting/). |
...
For the full list of global variable formats, refer to [Global variables](/docs/grafana/latest/dashboards/variables/add-template-variables/#global-variables).
...
- **Transformations:** Apply [transformations](/docs/grafana/latest/panels-visualizations/query-transform-data/transform-data/) to reshape a single query’s results for different visualizations.

### Enable query caching

In Grafana Enterprise and Grafana Cloud, enable [query caching](/docs/grafana/latest/administration/data-source-management/#query-caching) to avoid re-running identical queries within a configurable time window.
