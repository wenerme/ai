---
title: "Troubleshoot Amazon Managed Service for Prometheus issues | Grafana Plugins documentation"
...
This document provides solutions to common issues you may encounter when configuring or using the Amazon Managed Service for Prometheus data source. For configuration instructions, refer to [Configure the Amazon Managed Service for Prometheus data source](/docs/plugins/grafana-amazonprometheus-datasource/latest/configure/).
...
4. Confirm your Grafana version meets the plugin’s minimum requirement. For the supported versions, refer to [Requirements](/docs/plugins/grafana-amazonprometheus-datasource/latest/#requirements).
5. If the errors persist, restart Grafana and clear your browser cache.

For install, upgrade, and catalog issues, refer to [Troubleshoot installation issues](/docs/plugins/grafana-amazonprometheus-datasource/latest/install/#troubleshoot-installation-issues).
...
If the SigV4 option is missing from the authentication drop-down in Grafana Cloud, contact [Grafana Support](/help/) to enable SigV4 authentication for your instance. After Support enables it and the instance restarts, the **SigV4 auth** option appears in the authentication drop-down.
...
2. Verify the **External ID** in the data source configuration matches the `sts:ExternalId` condition in the trust policy.
3. Verify the **Assume Role ARN** in the data source matches the role you configured.
...
2. Verify STS is enabled for the region. If you use regional STS endpoints, confirm the region is activated in your AWS account.
3. Verify the **Assume Role ARN** is correct and the role exists in the target account.
4. Verify the base identity has `sts:AssumeRole` permission for the target role.
...
2. Verify the **Default Region** matches the workspace region, because SigV4 signs requests per region.
3. Verify the **Service** field is set to `aps` unless AWS instructs you to use a different value.
4. Verify the Grafana server clock is accurate, because a large clock skew invalidates signatures.
...
2. Confirm the endpoint policy allows the calling principal. For Grafana Cloud, allow the Grafana Labs account `008923505280` or the role it assumes.
3. Confirm the endpoint policy allows the required `aps` actions, such as `aps:QueryMetrics` and the label and series actions.
4. Confirm the endpoint policy `Resource` includes your workspace ARN.
5. Confirm the endpoint’s security group allows inbound HTTPS on port 443 from the source network.
6. Confirm DNS resolves the workspace endpoint to the private endpoint addresses.
...
2. Increase the query step or **Min step** to request fewer data points.
3. Enable query caching in Grafana, available in Grafana Enterprise and Grafana Cloud.
4. Request a quota increase from AWS for your workspace.
...
| Cause                         | Solution                                                                                                                                                                                                |
|-------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Rule management not enabled   | Enable **Manage alerts via Alerting UI** on the data source [configuration page](/docs/plugins/grafana-amazonprometheus-datasource/latest/configure/).                                                  |
...
| Platform-side issue           | If your configuration and permissions are correct, this error can be a Grafana Cloud platform issue, such as a data proxy problem on a specific cluster. Contact [Grafana Support](/help/) to escalate. |
...
3. Look for entries from the `grafana-amazonprometheus-datasource` plugin that include request and response details.
...
2. Review the [plugin GitHub issues](https://github.com/grafana/grafana-amazonprometheus-datasource/issues) for known bugs.
3. Consult the [Amazon Managed Service for Prometheus documentation](https://docs.aws.amazon.com/prometheus/) for service-specific guidance.
