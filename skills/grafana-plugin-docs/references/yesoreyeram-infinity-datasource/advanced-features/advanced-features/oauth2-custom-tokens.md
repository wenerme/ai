---
title: "OAuth2 token customization | Grafana Plugins documentation"
...
http [Copy code to clipboard] Copy
...
3. Select your OAuth2 type (for example, **Client Credentials**).
4. Fill in the required OAuth2 fields (Client ID, Client Secret, Token URL).
5. Expand **Customize OAuth2 token (Advanced)**.
6. In the **Custom Token Header** field, enter your custom header name.
7. In the **Custom Token Template** field, enter your custom token format using placeholders.
8. To add token request headers, use the **Token request headers** fields to enter key-value pairs.
9. Click **Save &amp; test**.
