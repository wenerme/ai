---
title: "AWS IoT SiteWise template variables | Grafana Plugins documentation"
...
For general information about Grafana variables, refer to [Templates and variables](/docs/grafana/latest/dashboards/variables/).
...
- Reviewed the [AWS IoT SiteWise query editor](/docs/plugins/grafana-iot-sitewise-datasource/latest/query-editor/), because query variables reuse the same visual builder.
...
4. Select the **AWS IoT SiteWise** data source.
5. Select a query type and complete the query fields.
6. Click **Run query** to preview the values, then click **Apply**.
...
2. Create a **List assets** variable named `asset`. Set the query **Filter** to **All** and the **Model ID** to `${model}`.
3. In your panels, reference `${asset}` in the **Asset** field.
