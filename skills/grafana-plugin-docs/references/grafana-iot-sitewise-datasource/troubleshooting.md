---
title: "Troubleshoot AWS IoT SiteWise data source issues | Grafana Plugins documentation"
...
This document provides solutions to common issues you might encounter when you configure or use the AWS IoT SiteWise data source. For configuration instructions, refer to [Configure the AWS IoT SiteWise data source](/docs/plugins/grafana-iot-sitewise-datasource/latest/configure/).
...
2. Search for **AWS IoT SiteWise** and select the plugin.
3. Read the installed version on the plugin page. The page also lists the available versions and the Grafana version each one supports.
...
2. If the plugin is too new for your Grafana version, downgrade the plugin to a compatible version, or upgrade Grafana.
3. If the plugin is too old for your Grafana version, update the plugin to the latest compatible version.
4. Clear the browser cache and reload after you change versions.
...
> Note
>
> To reduce the risk of an upgrade breaking a dashboard, test changes in a separate Grafana instance, such as a local OSS instance, before you rely on them in Grafana Cloud. If a Grafana Cloud upgrade breaks the plugin and you can’t resolve it, contact [Grafana Support](/help/).
...
2. To use the SQL editor, grant `iotsitewise:ExecuteQuery`.
3. For cross-account access, verify the assume role Amazon Resource Name and external ID.
...
3. If you set a custom endpoint, verify that the URL is correct and reachable.
...
2. Increase the aggregate resolution to reduce the number of API calls.
3. Keep the **Client cache** option enabled to reuse results for relative time ranges.
4. Request a quota increase from AWS.
...
2. Verify that the **Default Region** in the data source configuration matches where your assets are stored.
3. Confirm that the assets and properties you query exist in the selected Region.
...
2. Run the query again to fetch fresh results.
3. Re-enable **Client cache** if you want to continue reusing results for relative time ranges.
...
2. Where possible, query by asset and property instead of by alias, so the plugin resolves a consistent asset property name.
3. Add a **Rename by regex** or **Organize fields** transformation to set a stable display name.
...
2. To confirm a fallback occurred, enable debug logging and look for log entries about the raw property alias fallback. Plugin version 2.6.0 added debug logging for this fallback.
...
2. Use a query type that returns variable options: **List asset models**, **List assets**, or **List associated assets**.
3. For the **List assets** query, verify that the **Model ID** is set when you use the **All** filter.
4. Verify that the identity has permission to list the requested resources.
...
**Solution:** Build the alert rule query so it returns numeric time series data, such as a numeric asset property with the **Time series** format. Refer to [Query requirements for alerting](/docs/plugins/grafana-iot-sitewise-datasource/latest/alerting/#query-requirements-for-alerting).
...
3. Look for AWS IoT SiteWise entries that include request and response details.
...
2. Review the [AWS IoT SiteWise data source plugin GitHub issues](https://github.com/grafana/iot-sitewise-datasource/issues) for known bugs.
3. Consult the [AWS IoT SiteWise documentation](https://docs.aws.amazon.com/iot-sitewise/latest/userguide/what-is-sitewise.html) for service-specific guidance.
