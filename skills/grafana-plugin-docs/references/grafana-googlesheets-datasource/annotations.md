...
3. Click **Add annotation query** (or **+ New query** if you already have one).
4. Enter a **Name** for the annotation (for example, “Deployments”, “Incidents”).
5. Select your **Google Sheets** data source.
6. In the query editor, set **Spreadsheet ID** and **Range** to the sheet and range that contain your annotation data (for example, `Sheet1!A1:C100`). The first row should be the header row with column names.
7. Enable **Use Time Filter** so only rows within the dashboard time range are included.
8. Optionally adjust **Cache Time**.
9. Click **Apply** to save, then **Save dashboard**.
...
For details, refer to [Annotate visualizations](/docs/grafana/latest/dashboards/build-dashboards/annotate-visualizations/).
