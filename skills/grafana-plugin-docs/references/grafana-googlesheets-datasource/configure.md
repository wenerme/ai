---
title: "Configure the Google Sheets data source | Grafana Plugins documentation"
...
- **Plugin installed:** The Google Sheets data source plugin. For instructions, refer to [Install a plugin](/docs/grafana/latest/administration/plugin-management/#install-a-plugin).
...
2. [Google Sheets API](https://console.cloud.google.com/apis/library/sheets.googleapis.com?q=sheet) enabled for the project.
3. [Google Drive API](https://console.cloud.google.com/apis/library/drive.googleapis.com?q=drive) enabled for the project.
4. Service account created in the project.
5. JSON key file downloaded for the service account.
6. JSON key file uploaded to the data source configuration in Grafana.
7. Spreadsheets shared with the service account email (`client_email` from the JSON key file).
8. **Save &amp; test** shows **Success**.
...
1. Enable the APIs in your Google Cloud project.
   ...
   2. Open the [Google Drive API page](https://console.cloud.google.com/apis/library/drive.googleapis.com?q=drive) and click enable.
2. Open the [Credentials](https://console.cloud.google.com/apis/credentials) page in the Google Cloud Console.
...
2. Verify that the GCE virtual machine instance is running as the service account that you created. For more information, refer to [setting up an instance to run as a service account](https://cloud.google.com/compute/docs/access/create-enable-service-accounts-for-instances#using).
...
Click **Save &amp; test** to verify the connection. A successful connection shows the message **Success**. If the test fails, refer to [Troubleshooting](/docs/plugins/grafana-googlesheets-datasource/latest/troubleshooting/).

## Provision the data source

You can define the Google Sheets data source with [Grafana provisioning](/docs/grafana/latest/administration/provisioning/#data-sources) (YAML) or with the [Grafana Terraform provider](https://registry.terraform.io/providers/grafana/grafana/latest/docs/resources/data_source).
