---
title: "Google Sheets query editor | Grafana Plugins documentation"
...
- Ensure you have [configured the Google Sheets data source](/docs/plugins/grafana-googlesheets-datasource/latest/configure/) and that **Save &amp; test** shows **Success**.
...
1. Select the **Google Sheets** data source for the panel (or open [Explore](/docs/grafana/latest/explore/) and select it there).
2. In **Spreadsheet ID**, enter the spreadsheet ID or URL, or choose a spreadsheet from the list if you use JWT authentication.
3. Optionally, enter a **Range** in A1 notation (for example, `Sheet1!A2:E`). Leave it empty to use the entire first sheet.
4. Set **Cache Time** if you want something other than the default (5 minutes). Use `0s` to disable caching.
5. Turn **Use Time Filter** on if your data has a time column and you want to filter by the dashboard time range.
6. Run the query. The panel refreshes with the data from your sheet.
...
> Tip
>
> If you’ve configured a default spreadsheet ID in the data source settings, it will be automatically pre-filled when you create a new query. Refer to [Configure the data source](/docs/plugins/grafana-googlesheets-datasource/latest/configure/#default-spreadsheet-id) for more information.
...
- Enter a spreadsheet URL including a range.
  ...
  2. Select the cells that you want to include.
  3. Right-click one of the cells and choose **Get link to this range**. The link is copied to your clipboard.
...
- Select a spreadsheet from the drop-down menu.
  ...
  To configure a service account with JWT authentication, refer to [Authenticate with a service account JWT](/docs/plugins/grafana-googlesheets-datasource/latest/configure/#authenticate-with-a-service-account-jwt).
...
Use a Google Sheet as an annotation source to overlay events on panels. Refer to [Annotations](/docs/plugins/grafana-googlesheets-datasource/latest/annotations/) for sheet layout (time, text, tags), steps to add an annotation query, and **Use Time Filter**.

**Same spreadsheet, different panels**

Use one **Spreadsheet ID** across multiple panels and set a different **Range** in each (for example, `Sales!A1:E100`, `Inventory!A1:C50`). To save time, set a default spreadsheet in the [data source configuration](/docs/plugins/grafana-googlesheets-datasource/latest/configure/#default-spreadsheet-id) so new queries are pre-filled.

**Dynamic spreadsheet or range with template variables**

Use [template variables](/docs/plugins/grafana-googlesheets-datasource/latest/template-variables/) in **Spreadsheet ID** or **Range** (for example, `$spreadsheet` or `Sheet1!A1:$region`) so users can switch sheets or ranges from the dashboard. The query runs with the selected variable values.
...
You can use the [SQL expression](/docs/grafana/latest/panels-visualizations/query-transform-data/expression-queries/) transformation to filter or aggregate Google Sheets data. However, there are limitations to be aware of:
...
- For complex transformations, consider using the built-in [transformations](/docs/grafana/latest/panels-visualizations/query-transform-data/transform-data/) (such as **Convert field type**, **Filter by value**, or **Group by**) instead of SQL expressions, as these handle Google Sheets data more reliably.

## Next steps

- [Use template variables](/docs/plugins/grafana-googlesheets-datasource/latest/template-variables/) to make dashboards dynamic.
- [Quick start: create a sample dashboard](/docs/plugins/grafana-googlesheets-datasource/latest/#quick-start-create-a-sample-dashboard) to try the data source with sample data.
- [Configure the data source](/docs/plugins/grafana-googlesheets-datasource/latest/configure/) to change authentication or default spreadsheet.
