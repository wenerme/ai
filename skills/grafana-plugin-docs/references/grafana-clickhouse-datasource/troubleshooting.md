---
title: "Troubleshoot ClickHouse data source issues | Grafana Plugins documentation"
...
3. **Check ClickHouse user permissions.** The connection test may pass, but queries can still fail if the user lacks permission to modify settings such as `max_execution_time`. Refer to [Required SETTINGS permissions](/docs/plugins/grafana-clickhouse-datasource/latest/configure/#required-settings-permissions).
...
1. [Open a support ticket](/profile/org#support) and request that your instance be moved to the **Steady** release channel.
...
3. Set the **Port** to a TLS-enabled port (8443 for HTTP, 9440 for Native).
4. Configure a valid CA certificate if the ClickHouse server uses a self-signed certificate.
...
1. Open the data source configuration in Grafana.
2. Disable **Skip TLS Verify**.
3. If the ClickHouse server uses a self-signed certificate, provide the CA certificate in the TLS settings instead of skipping verification.
...
1. Open the data source configuration in Grafana.
2. In the **Database credentials** section, enable **Allow service account fallback**.
3. Ensure the configured username and password have the permissions needed by your alert queries.
...
**Error message:** “DB::Exception: Cannot modify ‘max\_execution\_time’: Setting is locked (in readonly mode)”
...
For more details on configuring permissions, refer to [ClickHouse user and permissions](/docs/plugins/grafana-clickhouse-datasource/latest/configure/#clickhouse-user-and-permissions).
...
- “$\_\_timeFilter\_ms expected 1 argument(s), received \[n]”
...
- “$\_\_timeInterval\_ms expected 1 argument(s), received \[n]”
...
2. **Manual filter placement (v4.12.0+):** If you have a complex query where automatic filter injection fails, use the `$__adHocFilters('table_name')` macro to explicitly specify where filters are applied. Place it in the `SETTINGS` clause. For details and examples, refer to [Apply ad hoc filters manually](/docs/plugins/grafana-clickhouse-datasource/latest/template-variables/#apply-ad-hoc-filters-manually-with-__adhocfilters).
...
1. Add a [Convert field type](/docs/grafana/latest/panels-visualizations/query-transform-data/transform-data/#convert-field-type) transformation to the panel.
