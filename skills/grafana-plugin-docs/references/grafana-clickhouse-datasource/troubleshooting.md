...
[Documentation](/docs/)[breadcrumb arrow] [Plugins](/docs/plugins/)[breadcrumb arrow] [ClickHouse data source](/docs/plugins/grafana-clickhouse-datasource/latest/)[breadcrumb arrow] Troubleshooting
...
3. **Check ClickHouse user permissions.** The connection test may pass, but queries can still fail if the user lacks permission to modify settings such as `max_execution_time`. See [Required SETTINGS permissions](/docs/plugins/grafana-clickhouse-datasource/latest/configure/#required-settings-permissions).
...
- “$\_\_timeFilter\_ms: expected 1 argument, received \[n]”
...
- “$\_\_timeInterval\_ms: expected 1 argument, received \[n]”
