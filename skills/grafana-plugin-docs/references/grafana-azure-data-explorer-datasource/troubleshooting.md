---
title: "Troubleshoot Azure Data Explorer data source issues | Grafana Plugins documentation"
...
This document provides solutions to common issues you might encounter when configuring or using the Azure Data Explorer data source. For configuration instructions, refer to [Configure the Azure Data Explorer data source](/docs/plugins/grafana-azure-data-explorer-datasource/latest/configure/).
...
2. Verify the **Default cluster URL** is correct and reachable from the Grafana server.
3. Confirm the selected authentication method is fully configured, and that the identity has viewer access to the database. Refer to [Authentication errors](#authentication-errors).
4. Click **Save &amp; test** and note the exact message. Use it to find the matching section in this guide.
5. Review the Grafana server logs and the browser developer console for plugin errors. Refer to [Enable debug logging](#enable-debug-logging).
6. Confirm the problem isn’t on the Azure side, such as a paused cluster, changed permissions, or a networking change.
...
2. Clear your browser cache and reload. Stale frontend assets left over from an upgrade can cause loading failures and freezes.
3. Review the Grafana server logs for plugin load errors, and the browser developer console for frontend errors.
4. If the problem started immediately after a plugin update, roll back to the last version that worked, then upgrade again once a fixed release is available. For install and rollback commands, refer to [Upgrade the plugin](/docs/plugins/grafana-azure-data-explorer-datasource/latest/install/#upgrade-the-plugin).
...
2. Upgrade to the latest patch release for your Grafana minor version, which often contains the fix.
3. Verify the plugin version is compatible with your Grafana version. The plugin requires Grafana 11.6.11 or later.
...
2. Verify the managed identity still has viewer access to the database.
3. If the failure began right after an upgrade, treat it as a version regression. Refer to [Authentication regressions after a Grafana upgrade](#authentication-regressions-after-a-grafana-upgrade).
...
2. Confirm the **Azure Data Explorer** `user_impersonation` API permission is granted and **Admin consent** is enabled.
3. Confirm the `[auth.azuread]` `scopes` setting contains `openid email profile` and that ID tokens are enabled.
4. Don’t use OBO authentication for alerting. Alert rules stop working after the rule’s creator signs out.
...
3. If you enforce trusted endpoints, verify the cluster URL matches an allowed endpoint. Refer to [Enforce trusted endpoints](/docs/plugins/grafana-azure-data-explorer-datasource/latest/configure/#enforce-trusted-endpoints).
4. For private networks in Grafana Cloud, configure [Private data source connect](/docs/grafana-cloud/connect-externally-hosted/private-data-source-connect/).
...
The data source connected to your cluster, but it couldn’t query Azure Resource Graph to enumerate clusters. Azure Resource Graph requires the identity to have Reader access at the subscription level. Grant the identity the `Reader` role, or enter the cluster URL manually in the query header. For role assignment steps, refer to [Configure the Azure Data Explorer data source](/docs/plugins/grafana-azure-data-explorer-datasource/latest/configure/#app-registration).
...
2. Verify the PDC network path can reach the cluster URL on outbound HTTPS port 443.
3. Confirm the data source is assigned to the correct PDC connection in its settings.
4. To isolate whether an issue is specific to PDC, temporarily test with a direct connection if your network allows it.

For setup details, refer to [Private data source connect](/docs/plugins/grafana-azure-data-explorer-datasource/latest/configure/#private-data-source-connect).
...
2. Confirm the cluster and its permissions haven’t changed on the Azure side.
3. If failures coincide with a Grafana stack redeployment or restart, retest after the deployment stabilizes. Transient errors during a redeployment usually resolve on their own.
4. Review the Grafana server logs to correlate failures with cluster-side or network events.
...
3. Increase the **Query timeout** value in the data source **Additional settings**.
4. Avoid resource-intensive `mv-expand` operations on large dynamic columns.
...
1. Enable [fallback service credentials](/docs/plugins/grafana-azure-data-explorer-datasource/latest/configure/#fallback-service-credentials).
2. Enable the `idForwarding` feature toggle so Grafana can distinguish background requests from user requests.
3. Confirm the fallback identity has viewer access to the database.
...
2. Verify you selected the correct **Query Type** and completed the required fields, such as **Cluster**, **Database**, or **Table**.
3. For a **Kusto Query** variable, verify the query uses `project` to return a single column of string values.
4. Verify the identity has permission to list the requested resources.
...
2. Use only letters, numbers, dashes (`-`), and underscores (`_`) in the UID.
3. If you don’t need a fixed UID, omit the field and let Grafana generate one.
...
3. Look for Azure Data Explorer entries that include request and response details.
...
2. Review the [`azure-data-explorer-datasource` GitHub issues](https://github.com/grafana/azure-data-explorer-datasource/issues) for known bugs.
3. Consult the [Azure Data Explorer documentation](https://learn.microsoft.com/en-us/azure/data-explorer/) for service-specific guidance.
4. Contact Grafana Support if you’re an Enterprise, Cloud Pro, or Cloud Advanced customer.
5. When reporting issues, include the Grafana version, plugin version, error messages (with sensitive information redacted), steps to reproduce, and relevant configuration (with credentials redacted).
