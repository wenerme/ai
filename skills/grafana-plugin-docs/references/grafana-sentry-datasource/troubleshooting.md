---
title: "Troubleshoot Sentry data source issues | Grafana Plugins documentation"
...
This document provides solutions to common issues you may encounter when configuring or using the Sentry data source. For configuration instructions, refer to [Configure the Sentry data source](/docs/plugins/grafana-sentry-datasource/latest/configure/).
...
2. Confirm the Sentry organization has at least one project.
3. Check that the **Sentry Org** slug matches the organization where projects exist.
...
| Network isolation                | For Grafana Cloud accessing a private Sentry instance, configure [Private data source connect](/docs/grafana-cloud/connect-externally-hosted/private-data-source-connect/). |
...
2. Add project and environment filters to reduce the result set.
3. Reduce the **Limit** value in the query editor.
4. For Events Stats or Spans Stats queries, reduce the number of **Group** fields.
...
2. If you’re using a query type added in a newer plugin version (for example, Spans requires v2.2.0+, Metrics requires v1.8.0+), update the plugin to the required version.
...
1. Verify the **Sentry Org** field is set in the data source configuration. Refer to [Configure the Sentry data source](/docs/plugins/grafana-sentry-datasource/latest/configure/).
2. Check that the slug doesn’t contain extra spaces or the full URL — only enter the slug portion.
...
2. Verify your Sentry plan includes access to the spans/tracing API.
...
2. Test the same query directly in Sentry’s Discover view to verify the syntax is valid.
3. For Events or Spans queries, verify the **Fields** values are valid Sentry field names.
...
2. Add a more specific **Query** filter (for example, `is:unresolved level:error`) to reduce the number of matching issues.
3. Filter by **Projects** or **Environments** to narrow the scope.
...
| Authentication token expired | Generate a new token and update the data source configuration. Refer to [Configure the Sentry data source](/docs/plugins/grafana-sentry-datasource/latest/configure/). |
| Query returns no data        | Verify the query returns data by running it in [Explore](/docs/grafana/latest/explore/) before creating an alert rule.                                                 |
...
2. Check the **Threshold** expression operator and value. Preview the alert rule to see the actual values being compared.
3. For Issues queries, verify the **Limit** is set high enough to capture all matching issues, since the **Count** expression only counts returned rows.
...
2. For **Environments** variables, ensure the selected project IDs are valid and the projects have environments configured in Sentry.
3. For **Projects** variables filtered by team, verify the team slug is correct.
4. Check that the authentication token has **Read** permissions for the resources the variable queries.
...
2. For **Projects** variables, filter by team slug to reduce the number of results.
...
2. Use larger time intervals in Stats queries to reduce the number of API calls.
3. Reduce the number of panels querying the same Sentry data source on a single dashboard.
...
3. Look for entries containing `sentry` for request and response details.
...
2. Review the [Sentry data source GitHub issues](https://github.com/grafana/sentry-datasource/issues) for known bugs.
3. Consult the [Sentry documentation](https://docs.sentry.io/) for Sentry-specific guidance.
4. Contact Grafana Support if you’re a Cloud Pro, Cloud Contracted, or Enterprise user.
