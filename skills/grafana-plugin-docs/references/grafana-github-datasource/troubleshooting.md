---
title: "Troubleshoot GitHub data source issues | Grafana Plugins documentation"
...
This document provides solutions to common issues you may encounter when configuring or using the GitHub data source. For configuration instructions, refer to [Configure the GitHub data source](/docs/plugins/grafana-github-datasource/latest/configure/).
...
| Cause                                                 | Solution                                                                                                                                      |
|-------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------|
| Missing or incorrect token scopes                     | Verify the token includes all [required scopes](/docs/plugins/grafana-github-datasource/latest/configure/#personal-access-token-permissions). |
...
| Insufficient app permissions                 | Verify the app has the [required permissions](/docs/plugins/grafana-github-datasource/latest/configure/#github-app-permissions).                                  |
...
2. For GitHub Apps, verify the app is installed on the organization and has the required repository permissions.
3. Generate a new token or update the app permissions and reconfigure the data source.
...
2. Ensure the URL doesn’t include a trailing path like `/api/v3` – the plugin adds this automatically.
3. Verify the GitHub Enterprise Server instance is accessible from the Grafana server.
...
2. Check that firewall rules allow outbound HTTPS (port 443) traffic to the GitHub endpoint.
3. Verify proxy settings if your Grafana server uses a proxy for outbound requests.
4. For Grafana Cloud, configure [Private data source connect](/docs/grafana-cloud/connect-externally-hosted/private-data-source-connect/) if your GitHub Enterprise Server is in a private network.
...
2. Use more specific queries to reduce the number of API calls.
3. If using a personal access token, consider switching to a GitHub App, which has higher rate limits.
4. For Grafana Enterprise or Grafana Cloud, enable [query caching](/docs/grafana/latest/administration/data-source-management/#query-and-resource-caching) for additional caching control beyond the built-in cache.
...
1. Verify the token or GitHub App has the [code scanning permissions](/docs/plugins/grafana-github-datasource/latest/configure/#code-scanning-permissions): `code scanning alerts: read-only` and `security_events: read-only`.
2. Verify code scanning is enabled on the repository.
3. If querying at the organization level, leave the **Repository** field empty.
...
4. Look for entries containing `github` for request and response details.
...
2. Review the [GitHub data source plugin issues](https://github.com/grafana/github-datasource/issues) for known bugs.
3. Consult the [GitHub API documentation](https://docs.github.com/en/rest) for service-specific guidance.
4. Contact [Grafana Support](/support/) if you’re a Grafana Cloud Pro, Cloud Advanced, or Enterprise customer.
