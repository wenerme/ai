---
title: "AWS IoT TwinMaker pre-built dashboards | Grafana Plugins documentation"
...
2. Click **Import** next to the dashboard template you want to load.
3. Click the dashboard name to navigate to the imported dashboard.
4. Edit the dashboard as desired, then click **Save As** to save your own copy.
...
3. Select an alarm to populate the history panel. The history panel shows an error until the `sel_entity` and `sel_comp` variables have values. Set them by either:
