...
- Switch between LIVE and ON\_DEMAND mode by clicking the “Live” button
...
- ON\_DEMAND: video will be requested from Amazon Kinesis Video Streams with the time range set on the dashboard
...
![](https://github.com/grafana/grafana-iot-twinmaker-app/blob/main/docs/DatasourceConfig.png)

01. Go to Configuration → Datasources and click “Add data source”
02. Find the AWS IoT TwinMaker Datasource under the “Industrial &amp; IoT” section
03. On the datasource settings page choose your [authentication provider](/docs/grafana/next/datasources/aws-cloudwatch/aws-authentication/)
...
04. You need to set an “Assume Role ARN” for any environment Grafana is running in. This is an [IAM role](https://docs.aws.amazon.com/iot-twinmaker/latest/guide/dashboard-IAM-role.html) you create for your TwinMaker workspace.
...
05. (Optional) Set an External ID if your role is assumed from a separate account
06. (Optional) AWS developers may use a custom endpoint for testing

    a. Note: You cannot currently set an “Assume Role ARN” and a custom endpoint together
07. Select your region
08. Click “Save &amp; test” to load your workspaces

    a. The error “Missing WorkspaceID configuration” is expected to appear
09. Select your TwinMaker workspace ID. Any query that uses this datasource instance will have access to resources within the workspace.
10. Click “Save &amp; test”

## AWS IoT TwinMaker Dashboards

![](https://github.com/grafana/grafana-iot-twinmaker-app/blob/main/docs/DashboardTab.png)
...
![](https://github.com/grafana/grafana-iot-twinmaker-app/blob/main/docs/AlarmDashboard.png)
