---
title: "Troubleshoot OpenSearch data source issues | Grafana Plugins documentation"
...
This document provides solutions to common issues you might encounter when configuring or using the OpenSearch data source. For configuration instructions, refer to [Configure the OpenSearch data source](/docs/plugins/grafana-opensearch-datasource/latest/configure/).
...
| Missing IAM permissions   | Ensure the IAM role or user has `es:ESHttpGet` and `es:ESHttpPost` permissions on the domain. Refer to [IAM policies for OpenSearch Service](/docs/plugins/grafana-opensearch-datasource/latest/configure/). |
...
2. Check that the Grafana server can reach the OpenSearch instance over the network.
3. Verify firewall rules allow outbound traffic on the configured port.
...
2. For testing, enable **Skip TLS Verify** in the data source settings (not recommended for production).
3. Ensure the certificate isn’t expired.
4. Verify the certificate’s Subject Alternative Name (SAN) matches the hostname in the URL.
...
2. If auto-detection fails, verify the URL is correct and the OpenSearch instance is reachable.
3. Ensure the credentials have permissions to query the root endpoint.
...
This error occurs when the plugin detects an Elasticsearch instance with version 7.11 or later. Install and use the [Elasticsearch data source](/docs/grafana/latest/datasources/elasticsearch/) instead of the OpenSearch data source.
...
2. Ensure indices exist that match the pattern for the most recent six-hour window.
3. If indices only exist for older data, use a static index name instead of a time-based pattern.
...
2. Ensure the credentials have permissions to call the `_field_caps` API on the target index.
3. Check that the OpenSearch instance is reachable.
...
2. If using an index pattern with a time-based suffix, verify the **Pattern** setting matches your naming convention.
3. Check that the index hasn’t been deleted or rolled over.
4. For time-based patterns, ensure data exists within the dashboard’s time range.
...
2. If using provisioned dashboards, ensure the `queryType` field in the query JSON is set to `"lucene"` or `"ppl"`.
...
1. Verify PPL is [enabled in the data source settings](/docs/plugins/grafana-opensearch-datasource/latest/configure/).
2. Check the PPL syntax against the [OpenSearch PPL documentation](https://opensearch.org/docs/latest/search-plugins/sql/ppl/index/).
3. Verify the index name in the `source` command matches an existing index.
4. For OpenSearch Serverless, ensure PPL is supported for your collection type.
...
2. Verify the timestamp column has a type of `timestamp`, `datetime`, or `date` in the OpenSearch index mapping.
3. Verify the value column contains only numeric data.
4. Test the query in OpenSearch Dashboards or the PPL CLI to inspect the returned schema.
...
2. Verify trace data in the target index follows the expected [Data Prepper](https://opensearch.org/docs/latest/data-prepper/) schema.
3. Check that `startTime` and event `time` fields are in a parsable format.
...
3. Increase the **Max concurrent shard requests** setting if queries involve many shards.
4. Break complex aggregation queries into smaller parts.
...
2. Check that the JSON query syntax is valid, for example `{"find": "terms", "field": "@hostname"}`.
3. Ensure the field name in the query exists in the index and has the correct type.
4. For cascading variables, verify that parent variables have valid selections.
5. Check that the credentials have permissions to query the target index.
...
2. If using `orderBy`, set it to `"term"`, `"key"`, or `"doc_count"`.
3. If using `order`, set it to `"asc"` or `"desc"`.
...
2. Add a `size` property to limit the number of returned values, for example `{"find": "terms", "field": "@hostname", "size": 100}`.
3. Narrow the dashboard time range to reduce the data scanned by the terms aggregation.
...
1. Verify the data source connection is working using **Save &amp; test**.
2. Check that the Lucene query in the annotation configuration is syntactically valid.
3. Ensure the **Time** field is mapped to a valid date field in your index (default: `@timestamp`).
4. Verify the credentials have read access to the target index.
...
2. Ensure the PPL query returns exactly two columns: a timestamp field and a numeric value field.
3. Use `stats` with a time aggregation to structure the output correctly, for example:
...
3. Look for OpenSearch-specific entries that include request and response details.
...
2. Review the [OpenSearch plugin GitHub issues](https://github.com/grafana/opensearch-datasource/issues) for known bugs.
3. Consult the [OpenSearch documentation](https://opensearch.org/docs/latest/) for service-specific guidance.
4. Contact Grafana Support if you have a Grafana Cloud Pro or Advanced plan, or a Grafana Enterprise license.
