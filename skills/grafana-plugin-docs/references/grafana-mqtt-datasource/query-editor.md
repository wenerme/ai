---
title: "MQTT query editor | Grafana Plugins documentation"
...
2. In the **Topic** field, enter the MQTT topic you want to subscribe to (for example, `home/bedroom/temperature`).
3. The panel begins streaming data as soon as the topic is set.
...
2. Click the **Transform data** tab.
3. Select **Extract fields**.
4. Choose the JSON field you want to extract values from.

For more information, refer to [Transform data](/docs/grafana/latest/panels-visualizations/query-transform-data/transform-data/).
...
1. Open the panel editor.
2. Click the **Transform data** tab.
3. Select **Convert field type**.
4. Choose the field that contains the timestamp.
5. Set the target type to **Time**.

For more information, refer to [Convert field type](/docs/grafana/latest/panels-visualizations/query-transform-data/transform-data/#convert-field-type).

## Streaming behavior

The MQTT data source uses the [Grafana Live](/docs/grafana/latest/setup-grafana/set-up-grafana-live/) streaming API to push data to panels in real time.

The data flow works as follows:

1. When a panel with an MQTT query is opened, Grafana subscribes to the specified MQTT topic on the broker.
2. Incoming messages accumulate in a buffer on the backend.
3. At each query interval, the buffered messages are converted into a data frame and pushed to the panel.
4. When the panel is closed or the query is removed, Grafana unsubscribes from the topic.
