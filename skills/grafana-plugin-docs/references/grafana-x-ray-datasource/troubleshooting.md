---
title: "Troubleshoot AWS Application Signals data source issues | Grafana Plugins documentation"
...
| Cause                                                       | Solution                                                                                                                                                                                                                                                           |
|-------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| IAM identity is missing required actions                    | Apply the [IAM policy](/docs/plugins/grafana-x-ray-datasource/latest/configure/#iam-policy). Include `xray:*` read actions, `application-signals:List*`, `ec2:DescribeRegions`, and the OAM actions for cross-account.                                             |
...
| Assume Role trust policy doesn’t allow the Grafana identity | Update the target role’s trust policy to allow `sts:AssumeRole` from your Grafana IAM identity. For Grafana Cloud, refer to the [Grafana Assume Role documentation](/docs/grafana-cloud/connect-externally-hosted/data-source-management/aws-iam-role/).           |
...
2. For **AWS SDK Default**, confirm the environment exposes credentials (environment variables, EC2 role, or container role).
3. For **EC2 IAM role**, confirm a role is attached to the EC2 instance and the instance metadata service is reachable.
4. Restart `grafana-server` after changing environment variables or credentials files.
...
2. For Grafana Assume Role: confirm the target role’s maximum session duration is long enough for your usage pattern.
3. For Credentials file auth: regenerate the credentials and update `~/.aws/credentials`.
...
2. Confirm the **Default region** is set to a region where your resources exist.
3. If you use a custom **Endpoint**, confirm the URL is correct and reachable from Grafana.
4. For private AWS networks or VPC endpoints in Grafana Cloud, configure [Private Data source Connect](/docs/grafana-cloud/connect-externally-hosted/private-data-source-connect/) and select a PDC network in the data source settings.
...
2. If you set a custom **Endpoint**, make sure it corresponds to the same region as **Default region**.
3. Clear any custom endpoint you no longer need. A stale endpoint silently routes requests to the wrong service.
...
| IAM identity lacks read access on the target resource | Confirm the IAM identity has `xray:BatchGetTraces` and `xray:GetTraceSummaries`. See the [IAM policy](/docs/plugins/grafana-x-ray-datasource/latest/configure/#iam-policy). |
...
2. Confirm the trace exists in the **Default region** of the data source, or select the correct region in the query header.
3. X-Ray retains traces for 30 days by default. Older traces are no longer retrievable.
4. If the trace uses the W3C format, the plugin automatically converts it. If it still can’t be found, retry with the X-Ray trace ID format (`1-xxxxxxxx-xxxxxxxxxxxxxxxxxxxxxxxx`).
...
2. If you use a template variable for service, confirm it’s populated (for example, depending on **AccountId**) before the dependent query runs.
...
2. Update the plugin to the latest version. Navigate to **Plugins and data** &gt; **Plugins**.
...
2. Add a filter expression or select a group to make the result set more specific.
3. Split the query across multiple panels or time ranges if you need a full audit.
...
2. Set the **State** filter to **All** to see both active and closed insights.
3. Confirm the IAM identity has `xray:GetInsightSummaries` and `xray:GetInsight`.
4. Insights are only generated for groups that contain enough trace volume to detect anomalies. Verify in the AWS console that the selected group has active insights.
...
2. If cross-account observability is configured, check the **AccountId** multi-select. Leaving it empty returns only the data source’s own account; selecting linked accounts widens the map.
3. Use a [Node graph](/docs/grafana/latest/panels-visualizations/visualizations/node-graph/) panel — the service map data isn’t designed for table or time-series visualizations.
4. Confirm the IAM identity has `xray:GetServiceGraph`.
...
Refer to [Template variables — The Service variable’s value is a JSON blob](/docs/plugins/grafana-x-ray-datasource/latest/template-variables/#the-service-variables-value-is-a-json-blob).

## Alerting errors

These issues surface when building or evaluating [Grafana alert rules](/docs/plugins/grafana-x-ray-datasource/latest/alerting/) on AWS Application Signals or X-Ray data.
...
Rewrite the query as a **Trace Statistics** query with the same filter expression. Refer to the [alerting-compatible query types table](/docs/plugins/grafana-x-ray-datasource/latest/alerting/#alerting-compatible-query-types).
...
2. Use a single column per alert rule (for example, only **Fault Count**) so the series set is deterministic.
3. Set **Resolution** to `300s` for low-traffic services so each bucket has enough data to produce a stable series.
...
2. Consolidate multiple per-service alerts into one alert with a broader filter expression and label-based routing.
3. Request a [quota increase](https://docs.aws.amazon.com/general/latest/gr/xray.html) from AWS if you run many concurrent alert rules.
...
For production SLO alerting, prefer native CloudWatch alarms on the SLO metrics Application Signals publishes. Refer to [Alerting on Application Signals SLOs](/docs/plugins/grafana-x-ray-datasource/latest/alerting/#alerting-on-application-signals-slos).
...
| Service variable value is JSON             | If another plugin or panel expects a plain service name, switch to `${service:text}`. Refer to [Template variables — The Service variable’s value is a JSON blob](/docs/plugins/grafana-x-ray-datasource/latest/template-variables/#the-service-variables-value-is-a-json-blob). |
...
2. Narrow the scope of variable queries (for example, restrict **Services** to a single account).
3. Enable query caching in Grafana Cloud or Grafana Enterprise.
...
2. Increase **Resolution** on Trace statistics queries (for example, from **60s** to **300s**) to reduce the number of API calls.
3. Combine multiple narrow panels into a single broader query where possible.
4. Enable [query caching](/docs/grafana/latest/administration/data-source-management/#query-caching) in Grafana Cloud or Grafana Enterprise.
5. Request a quota increase from AWS if you have a high-traffic monitoring account.
...
2. Add a filter expression to reduce the trace population.
3. Use an X-Ray **Group** with a pre-defined filter expression for commonly inspected slices.
...
4. Look for entries that include request and response details for X-Ray or Application Signals.
...
2. Review or open an issue on the [AWS Application Signals plugin GitHub repository](https://github.com/grafana/x-ray-datasource/issues).
3. Consult the [AWS X-Ray documentation](https://docs.aws.amazon.com/xray/latest/devguide/aws-xray.html) and [AWS Application Signals documentation](https://docs.aws.amazon.com/AmazonCloudWatch/latest/monitoring/CloudWatch-Application-Monitoring-Sections.html) for service-specific guidance.
4. Contact [Grafana Support](/support/) if you’re a Grafana Cloud or Grafana Enterprise customer.
