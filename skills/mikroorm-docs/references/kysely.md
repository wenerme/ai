...
### Generating Entity Exports with the CLI
...
The command reads entity paths from your ORM config (`entitiesTs` or `entities`), discovers all entity classes and schemas, and generates a file like this:
...
|------|------|-------------|
| `-p, --path` | `string[]` | Glob patterns for entity source files |
| `-o, --out` | `string` | Output file path (default: next to ORM config) |
| `-d, --dump` | `boolean` | Print to stdout instead of writing |

If `--path` is not provided, the command extracts folder/glob paths from your config's `entitiesTs` (preferred) or `entities` array. After switching your config to use the generated barrel file (which contains entity references, not paths), you'll need to pass `--path` explicitly to re-run the command:

```bash
npx mikro-orm discovery:export --path './src/entities/*.ts'
```
