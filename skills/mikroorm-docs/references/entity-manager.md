...
Values generated by property `onCreate` hooks (i.e. when no value was provided for such property) are used only for the insert part of the query — they are automatically excluded from the merge clause, so they never overwrite the values of an already existing row.
