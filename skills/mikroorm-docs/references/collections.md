...
Use `loadCount()` to get the number of items from the database without loading the entities. The result is cached — pass `{ refresh: true }` to force a reload, or pass a `where` clause to count a subset (uncached). For an uncached variant, use `count()` directly. When using [dataloaders](./dataloaders.md), pass `{ dataloader: true }` to batch multiple count queries:
