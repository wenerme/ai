...
1. Go to the pipeline, job, [environment](../environments/deployments.md#configure-manual-deployments),
   or deployment view.
1. Next to the manual job, select **Run** ().
...
- Select the **name** of the manual job in the pipeline view, not **Run** ().
...
- With the same variables:
  - From the job details page, select **Retry** ().
...
To stop the active timer of a delayed job, select **Unschedule** ().
This job can no longer be scheduled to run automatically. You can, however, execute the job manually.

To start a delayed job manually, select **Unschedule** () to stop the delay timer and then select **Run** ().
