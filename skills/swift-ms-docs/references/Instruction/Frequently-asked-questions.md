...
Streaming loading `--streaming True`, loading while training, needs to set max_steps. For detailed instructions, please search for this parameter in [Command Line Parameter Document](https://swift.readthedocs.io/en/latest/Instruction/Command-line-parameters.html#id4). 
Note:
...
- For the multi-label classification data format, see [Custom Dataset](https://swift.readthedocs.io/en/latest/Customization/Custom-dataset.html). 
...
```
all-router is not a wildcard matching module name, but a special keyword that tells the framework to "include routers in the trainable scope." 
