...
**Browser binding session and DevTools methods**

- Browser Run browser bindings now support typed methods for [acquiring and connecting to sessions](https://developers.cloudflare.com/browser-run/reference/browser-binding-api/), launching a browser with [outbound Worker routing](https://developers.cloudflare.com/browser-run/features/outbound-workers/), creating Live View URLs, and managing session lifecycle.
- Added a nested `devtools` binding with methods for browser version and protocol information, listing and creating targets, and activating or closing targets. See the [Browser binding API reference](https://developers.cloudflare.com/browser-run/reference/browser-binding-api/) for the full API.
