...
| Maximum Durable Object classes (per account) | 500 (Workers Paid) / 100 (Free) <sup>[1](#user-content-fn-1)</sup> |
| Storage per account | Unlimited (Workers Paid) / 5GB (Free) <sup>[2](#user-content-fn-2)</sup> |
| Storage per class | Unlimited <sup>[3](#user-content-fn-3)</sup> |
| Storage per Durable Object | 10 GB <sup>[3](#user-content-fn-3)</sup> |
...
| CPU per request | 30 seconds (default) / configurable to 5 minutes of [active CPU time](https://developers.cloudflare.com/workers/platform/limits/#cpu-time) <sup>[4](#user-content-fn-4)</sup> |
...
| --- | --- |
| Number of Objects | Unlimited (within an account or of a given class) |
| Maximum Durable Object classes (per account) | 500 (Workers Paid) / 100 (Free) <sup>[5](#user-content-fn-5)</sup> |
| Storage per account | 50 GB (can be raised by contacting Cloudflare) <sup>[6](#user-content-fn-6)</sup> |
| Storage per class | Unlimited |
| Storage per Durable Object | Unlimited |
| Key size | 2 KiB (2048 bytes) |
| Value size | 128 KiB (131072 bytes) |
| WebSocket message size | 32 MiB (only for received messages) |
| CPU per request | 30s (including WebSocket messages) <sup>[7](#user-content-fn-7)</sup> |
| Simultaneous outgoing connections/request | 6 (same as [Workers](https://developers.cloudflare.com/workers/platform/limits/#simultaneous-open-connections)) |
...
3. Accounts on the Workers Free plan are limited to 5 GB total Durable Objects storage. [↩](#user-content-fnref-3) [↩<sup>2</sup>](#user-content-fnref-3-2)
