...
* [  wrangler.jsonc ](#tab-panel-8529)
* [  wrangler.toml ](#tab-panel-8530)
...
export default {  async fetch(request, env, ctx) {    const projectId = getProjectIdFromRequest(request);
    const worker = env.LOADER.get(projectId, async () => {      const serverCode = await loadServerCode(projectId);
      return {        mainModule: "index.js",        modules: {          "index.js": { js: serverCode },        },        compatibilityDate: "2026-06-24",        env: {          ASSETS: ctx.exports.AssetBinding({            props: { projectId },          }),        },      };    });
