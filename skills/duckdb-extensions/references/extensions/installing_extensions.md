...
API to install and load extensions. For example, the [Python client](https://duckdb.org/docs/current/clients/python/overview.html#loading-and-installing-extensions), has dedicated `install_extension(name: str)` and `load_extension(name: str)` methods. For more details on a specific client API, refer
to the [Client API documentation](https://duckdb.org/docs/current/clients/overview.html)
...
* Extensions cannot be reloaded. If you [update extensions](https://duckdb.org/docs/current/sql/statements/update_extensions.html), restart the DuckDB process to use newer extensions.
