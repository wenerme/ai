...
* the extension is available, but in a different repository, try for example `INSTALL ⟨name⟩ FROM core;` or `INSTALL ⟨name⟩ FROM community;` or `INSTALL ⟨name⟩ FROM core_nightly;` (see the [Installing Extensions page](https://duckdb.org/docs/current/extensions/installing_extensions.html#extension-repositories)).
