...
The list includes most, but not all, available core extensions. For the full list, we maintain a [list of core extensions](https://duckdb.org/docs/current/core_extensions/overview.html).
...
For example, to use the built-in [`json` extension](https://duckdb.org/docs/current/data/json/overview.html) to read a JSON file:
...
To make the DuckDB distribution lightweight, only a few essential extensions are built-in, varying slightly per distribution. Which extension is built-in on which platform is documented in the [list of core extensions](https://duckdb.org/docs/current/core_extensions/overview.html#default-extensions).
...
> Extension installation and loading are subject to a few [limitations](https://duckdb.org/docs/current/extensions/installing_extensions.html#limitations).
...
to install and load the [`spatial` extension](https://duckdb.org/docs/current/core_extensions/spatial/overview.html), run:
...
This repository can either be an alias, such as [`community`](https://duckdb.org/community_extensions/), or it can be a direct URL, provided as a single-quoted string.
...
DuckDB will automatically install and load the [`httpfs`](https://duckdb.org/docs/current/core_extensions/httpfs/overview.html) extension. No explicit `INSTALL` or `LOAD` statements are required.
...
To see which extensions can be autoloaded, check the [core extensions list](https://duckdb.org/docs/current/core_extensions/overview.html).

### Community Extensions

DuckDB supports installing third-party [community extensions](https://duckdb.org/community_extensions/). For example, you can install the [`tarfs` community extension](https://duckdb.org/community_extensions/extensions/tarfs.html) via:

```sql
INSTALL tarfs FROM community;
```

Community extensions are contributed by community members but they are built, [signed](https://duckdb.org/docs/current/extensions/extension_distribution.html#signed-extensions), and distributed in a centralized repository.
...
For more details on extension versions, refer to the [Extension Versioning page](https://duckdb.org/docs/current/extensions/versioning_of_extensions.html).
...
See the [installation instructions](https://duckdb.org/docs/current/extensions/installing_extensions.html) and the [advanced installation methods page](https://duckdb.org/docs/current/extensions/advanced_installation_methods.html).
