...
Just like [DuckDB itself](https://duckdb.org/release_calendar.html), DuckDB extensions have their own version number. To ensure consistent semantics
of these version numbers across the various extensions, DuckDB's [Core Extensions](https://duckdb.org/docs/current/core_extensions/overview.html) use
...
To achieve this, Core Extensions tend to regularly push builds to the [`core_nightly` repository](https://duckdb.org/docs/current/extensions/installing_extensions.html#extension-repositories). Let's look at an example:
...
* core out-of tree extensions tend to live in repositories named `github.com/duckdb/duckdb-⟨extension_name⟩` but the name may vary. See the [full list](https://duckdb.org/docs/current/core_extensions/overview.html) of core extensions for details.
