...
For platforms where packages for certain extensions are not available, users can build them from source and [install the resulting binaries manually](https://duckdb.org/docs/current/extensions/advanced_installation_methods.html#installing-an-extension-from-an-explicit-path).
...
* `windows_amd64_mingw`
* `wasm_eh` and `wasm_mvp` (see [DuckDB-Wasm's extensions](https://duckdb.org/docs/current/clients/wasm/extensions.html))
...
which itself is supported through an extension ([`httpfs`](https://duckdb.org/docs/current/core_extensions/httpfs/overview.html)).

### Unsigned Extensions

> Warning Only load unsigned extensions from sources you trust.
> Avoid loading unsigned extensions over HTTP.
> Consult the [Securing DuckDB page](https://duckdb.org/docs/current/operations_manual/securing_duckdb/securing_extensions.html) for guidelines on how to set up DuckDB in a secure manner.
...
To load unsigned extensions using the [CLI client](https://duckdb.org/docs/current/clients/cli/overview.html), pass the `-unsigned` flag to it on startup:
...
For client APIs, the `allow_unsigned_extensions` database configuration options needs to be set, see the respective [Client API docs](https://duckdb.org/docs/current/clients/overview.html).
For example, for the Python client, see the [Loading and Installing Extensions section in the Python API documentation](https://duckdb.org/docs/current/clients/python/overview.html#loading-and-installing-extensions).
...
This structure is described in the [“Downloading Extensions Directly from S3” section](https://duckdb.org/docs/current/extensions/advanced_installation_methods.html#downloading-extensions-directly-from-s3), and is the same
...
If the custom repository is served over HTTPS or S3, the [`httpfs` extension](https://duckdb.org/docs/current/core_extensions/httpfs/overview.html) is required. DuckDB will attempt to [autoload](https://duckdb.org/docs/current/extensions/overview.html#autoloading-extensions)
