...
The `iceberg` extension supports writing to Iceberg tables that are managed by an [Iceberg REST Catalog](https://duckdb.org/docs/current/core_extensions/iceberg/iceberg_rest_catalogs.html). All write operations go through the attached catalog and are committed as new Iceberg snapshots.

> Writing requires an attached catalog. The path-based `iceberg_scan` interface described in the [overview](https://duckdb.org/docs/current/core_extensions/iceberg/overview.html) is read-only. To write, first [attach an Iceberg REST catalog](https://duckdb.org/docs/current/core_extensions/iceberg/iceberg_rest_catalogs.html).
...
See the [Functions and Settings Reference](https://duckdb.org/docs/current/core_extensions/iceberg/reference.html#table-and-schema-property-functions) for the equivalent schema (namespace) property functions.
...
`MERGE INTO` performs an upsert against a source relation. The join key is given with a second `USING` clause (see the [`MERGE INTO` statement](https://duckdb.org/docs/current/sql/statements/merge_into.html)):
...
Because the full DDL and DML set is supported, [`COPY FROM DATABASE`](https://duckdb.org/docs/current/sql/statements/copy.html#copy-from-database--to) can perform deep copies between Iceberg and DuckDB storage in either direction:
...
To copy an Iceberg catalog into [DuckLake](https://duckdb.org/docs/current/core_extensions/ducklake.html), see [Interoperability with DuckLake](https://duckdb.org/docs/current/core_extensions/iceberg/overview.html#interoperability-with-ducklake).
