...
This page lists the options of the `iceberg` extension: the parameters accepted by the [Iceberg functions](https://duckdb.org/docs/current/core_extensions/iceberg/iceberg_functions.html), the options of the `ATTACH` and `CREATE SECRET` statements used to [connect to a catalog](https://duckdb.org/docs/current/core_extensions/iceberg/catalogs.html), and the global settings.
...
| `REMOVE_FILES_ON_DELETE`             | `BOOLEAN`  | `true`               | If disabled, files (data and metadata) created during the transaction will not be cleaned up on ROLLBACK or retries of the COMMIT. |
...
* **Reading and metadata**: `SELECT`, time travel with the `AT` clause, and the `iceberg_metadata`, `iceberg_snapshots`, and statistics functions. See the [Functions and Settings Reference](https://duckdb.org/docs/current/core_extensions/iceberg/reference.html).
* **Writing**: `CREATE`/`DROP SCHEMA` and `TABLE`, partitioning, `INSERT`, `UPDATE`, `DELETE`, `MERGE INTO`, `ALTER TABLE`, table properties, and `COPY FROM DATABASE`. See [Writing to Iceberg](https://duckdb.org/docs/current/core_extensions/iceberg/writing.html).
