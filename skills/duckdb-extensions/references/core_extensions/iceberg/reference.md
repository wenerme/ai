...
This page is a reference for all functions and settings provided by the [`iceberg` extension](https://duckdb.org/docs/current/core_extensions/iceberg/overview.html). For task-oriented documentation, see the [Overview](https://duckdb.org/docs/current/core_extensions/iceberg/overview.html) (reading), [Writing to Iceberg](https://duckdb.org/docs/current/core_extensions/iceberg/writing.html), and [Iceberg REST Catalogs](https://duckdb.org/docs/current/core_extensions/iceberg/iceberg_rest_catalogs.html) pages.

Functions that take a table accept either a path to a table's metadata (e.g., `'data/iceberg/lineitem_iceberg'`) or, when a [catalog is attached](https://duckdb.org/docs/current/core_extensions/iceberg/iceberg_rest_catalogs.html), a fully qualified table name (e.g., `my_catalog.default.my_table`).
...
See [Selecting Metadata Versions](https://duckdb.org/docs/current/core_extensions/iceberg/overview.html#selecting-metadata-versions) for details.
...
These functions read and modify [Iceberg table properties](https://iceberg.apache.org/spec/#table-metadata-fields) and Iceberg schema (namespace) properties. They require an attached catalog. See [Writing to Iceberg](https://duckdb.org/docs/current/core_extensions/iceberg/writing.html#table-properties).
...
| Function | Description |
| --- | --- |
| `iceberg_to_ducklake(iceberg_catalog, ducklake_catalog, skip_tables := [...])` | Performs a metadata-only copy of an attached Iceberg catalog into a [DuckLake](https://duckdb.org/docs/current/core_extensions/ducklake.html) catalog. The optional `skip_tables` parameter (`VARCHAR[]`) excludes tables. |

See [Interoperability with DuckLake](https://duckdb.org/docs/current/core_extensions/iceberg/overview.html#interoperability-with-ducklake).

## Partition Transform Functions

These scalar functions implement the [Iceberg partition transforms](https://iceberg.apache.org/spec/#partition-transforms). They are most often used in a `PARTITIONED BY` clause (see [Partitioning](https://duckdb.org/docs/current/core_extensions/iceberg/writing.html#partitioning)), but can also be called directly.
