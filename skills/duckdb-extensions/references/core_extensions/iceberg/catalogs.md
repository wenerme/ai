...
The `iceberg` extension supports attaching Iceberg REST Catalogs. Attaching a catalog is required for [writing to Iceberg](https://duckdb.org/docs/current/core_extensions/iceberg/writing_to_iceberg.html). Before attaching an Iceberg REST Catalog, you must install the `iceberg` extension by following the instructions located in the [overview](https://duckdb.org/docs/current/core_extensions/iceberg/overview.html).
...
You can attach the Iceberg catalog with the following [`ATTACH`](https://duckdb.org/docs/current/sql/statements/attach.html) statement.
...
A REST Catalog with OAuth2 authorization can also be attached with just an `ATTACH` statement. For the complete list of `ATTACH` and `CREATE SECRET` options, see the [Iceberg Options](https://duckdb.org/docs/current/core_extensions/iceberg/iceberg_options.html#attach-options) page.

## Amazon S3 Tables

The `iceberg` extension supports reading Iceberg tables stored in [Amazon S3 Tables](https://aws.amazon.com/s3/features/tables/).

You can let DuckDB detect your AWS credentials and configuration based on the default profile in your `~/.aws` directory by creating the following secret using the [Secrets Manager](https://duckdb.org/docs/current/configuration/secrets_manager.html):
...
For the full range of credential options (assumed roles, SSO, web identity, and more), see the [aws extension](https://duckdb.org/docs/current/core_extensions/aws.html#credential_chain-provider).
...
Create an S3 secret using the [Secrets Manager](https://duckdb.org/docs/current/configuration/secrets_manager.html):
...
In this example we use an STS token, but [other authentication methods are supported](https://duckdb.org/docs/current/core_extensions/aws.html).
...
You can learn more about the `WITH` clause at [Creating Tables](https://duckdb.org/docs/current/core_extensions/iceberg/writing_to_iceberg.html#creating-tables).
