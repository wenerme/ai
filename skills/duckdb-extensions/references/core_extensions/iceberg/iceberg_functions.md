...
The examples on this page use the [`iceberg_data.zip`](https://duckdb.org/data/iceberg_data.zip) file; download and unzip it to run them.
...
The path can also point directly to a metadata file, or to an object store such as S3 or Azure Blob Storage. See the [Iceberg Options](https://duckdb.org/docs/current/core_extensions/iceberg/iceberg_options.html#scan-options) page for the parameters accepted by this function.
...
The functions `iceberg_metadata` and `iceberg_snapshots` are also available to use with an [attached Iceberg catalog](https://duckdb.org/docs/current/core_extensions/iceberg/catalogs.html) using a fully qualified path, e.g.:
...
The DuckDB `iceberg` extension exposes a function to do metadata only copies of the Iceberg metadata to [DuckLake](https://duckdb.org/docs/current/core_extensions/ducklake.html), which enables users to query Iceberg tables as if they were DuckLake tables.
