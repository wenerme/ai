...
To access object store URIs (e.g., `s3://...`), configure a `TYPE lance` secret using the [Secrets Manager](https://duckdb.org/docs/current/sql/statements/create_secret.html):
...
Use the [`COPY ... TO ...` statement](https://duckdb.org/docs/current/sql/statements/copy.html#copy--to) to materialize query results as a Lance dataset.
...
To write to `s3://...` paths, configure a `TYPE lance` secret for that scope using the [Secrets Manager](https://duckdb.org/docs/current/sql/statements/create_secret.html):
