...
> Warning Using `odbc_copy` from [Python Relational API](https://duckdb.org/docs/current/clients/python/relational_api.html).
>
> `odbc_copy` is a table function that [returns](#returns-5) one row for each 2048 copied rows.
> When it is used from Python with `duckdb.sql()` the [Lazy Evaluation](https://duckdb.org/docs/current/clients/python/relational_api.html#lazy-evaluation)
> is taking place. Thus no rows will be copied until [a method that triggers execution](https://duckdb.org/docs/current/clients/python/relational_api.html#output)
...
> then run `odbc_copy` on that file.
 ...
 - `source_conn_string` (`VARCHAR`, default: `:memory:`): DuckDB connection string to the source DB, example: `ducklake:postgres:postgresql://EXAMPLE_USER:EXAMPLE_PASSWORD@127.0.0.1:5432/lake1`
...
```sql
FROM odbc_copy(getvariable('conn'),
  source_conn_string='ducklake:postgres:postgresql://EXAMPLE_USER:EXAMPLE_PASSWORD@127.0.0.1:5432/lake1',
  source_queries=[
    'CREATE SECRET s (TYPE s3 [...])',
    'FROM nl_train_stations'
  ],
