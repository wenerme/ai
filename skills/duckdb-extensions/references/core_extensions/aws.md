...
The `aws` extension adds functionality, e.g., authentication, on top of the `httpfs` extension's [S3 capabilities](https://duckdb.org/docs/current/core_extensions/httpfs/overview.html#s3-api), using the AWS SDK.
...
> In most cases, the `aws` extension works in conjunction with the [`httpfs` extension](https://duckdb.org/docs/current/core_extensions/httpfs/overview.html).

## Configuration and Authentication

The preferred way to configure and authenticate to AWS S3 endpoints is to use [secrets](https://duckdb.org/docs/current/sql/statements/create_secret.html).
There are two S3 secret providers:

* The `config` provider, where you supply the access key and secret manually. It is part of the `httpfs` extension and is documented in the [S3 API page](https://duckdb.org/docs/current/core_extensions/httpfs/s3api.html#config-provider). Use it when you already have static credentials.
...
The full list of S3 secret parameters that apply to *both* providers (`ENDPOINT`, `REGION`, `URL_STYLE`, `USE_SSL`, `KMS_KEY_ID`, `REQUESTER_PAYS`, …) is documented in the [S3 API page](https://duckdb.org/docs/current/core_extensions/httpfs/s3api.html#overview-of-s3-secret-parameters).
...
This approach is equivalent to the [deprecated S3 API](https://duckdb.org/docs/current/core_extensions/httpfs/s3api_legacy_authentication.html)'s method `load_aws_credentials('⟨my_profile⟩')`.
...
2. The `s3_region` [setting](https://duckdb.org/docs/current/configuration/overview.html) (`SET s3_region = '⟨region⟩'`).
...
> The `rds` secret _type_ is registered by the [`postgres` extension](https://duckdb.org/docs/current/core_extensions/postgres/overview.html), not by `aws`, so the `postgres` extension must be installed and loaded before the statement above will run. For the complete end-to-end setup — passing the secret to a connection via `AWS_RDS_SECRET`, attaching, and querying — see the [Amazon RDS with IAM authentication guide](https://duckdb.org/docs/current/guides/database_integration/rds_iam.html) or the [Postgres secrets documentation](https://duckdb.org/docs/current/core_extensions/postgres/secrets.html#aws-rds-iam-authentication).

## Legacy Features

> Deprecated The `load_aws_credentials` function is deprecated and is removed in later releases. Use a [secret](https://duckdb.org/docs/current/sql/statements/create_secret.html) with the [`credential_chain` provider](#credential_chain-provider) instead.

Prior to version 0.10.0, DuckDB did not have a [Secrets manager](https://duckdb.org/docs/current/sql/statements/create_secret.html), to load the credentials automatically, the AWS extension provided
a special function to load the AWS credentials in the [legacy authentication method](https://duckdb.org/docs/current/core_extensions/httpfs/s3api_legacy_authentication.html).
