...
See the [announcement blog post](https://duckdb.org/2024/05/29/access-150k-plus-datasets-from-hugging-face-with-duckdb.html) for details.
...
To avoid accessing the remote endpoint for every query, you can save the data in a DuckDB table by running a [`CREATE TABLE ... AS` command](https://duckdb.org/docs/current/sql/statements/create_table.html#create-table--as-select-ctas). For example:
...
To query all files under a specific directory, you can use a [glob pattern](https://duckdb.org/docs/current/data/multiple_files/overview.html#multi-file-reads-and-globs). For example:
...
Second, set it in your DuckDB session using DuckDB’s [Secrets Manager](https://duckdb.org/docs/current/configuration/secrets_manager.html). DuckDB supports two providers for managing secrets:
...
CREATE SECRET hf_token (
    TYPE huggingface,
    TOKEN 'EXAMPLE_HUGGING_FACE_TOKEN'
);
```
