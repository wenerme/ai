...
The preferred way to configure and authenticate to S3 endpoints is to use [secrets](https://duckdb.org/docs/current/sql/statements/create_secret.html). Multiple secret providers are available.

To migrate from the [deprecated S3 API](https://duckdb.org/docs/current/core_extensions/httpfs/s3api_legacy_authentication.html), use a defined secret with a profile.
See [Selecting a Profile](https://duckdb.org/docs/current/core_extensions/aws.html#selecting-a-profile).
...
The `credential_chain` provider automatically fetches credentials using the AWS SDK (profiles, SSO, assumed roles, web identities, instance metadata, and more). It is provided by the [`aws` extension](https://duckdb.org/docs/current/core_extensions/aws.html). For example, to use the AWS SDK default provider:
...
For the full set of `credential_chain` options — `CHAIN` values, profile selection, assuming roles, SSO, web identity (IRSA), region resolution, validation, and auto-refresh — see the [AWS extension page](https://duckdb.org/docs/current/core_extensions/aws.html#credential_chain-provider).
...
| `REFRESH`                     | Set to `auto` to periodically refresh credentials (see the [`aws` extension](https://duckdb.org/docs/current/core_extensions/aws.html#auto-refresh)) | `S3`, `GCS`, `R2` | `STRING`  | -                                           |
...
The `httpfs` extension supports [partial reading](https://duckdb.org/docs/current/core_extensions/httpfs/https.html#partial-reading) from S3 buckets.
...
File [globbing](https://duckdb.org/docs/current/sql/functions/pattern_matching.html#globbing) is implemented using the ListObjectsV2 API call and allows using filesystem-like glob patterns to match multiple files, for example:
...
This query matches all files in the root of the bucket with the [Parquet extension](https://duckdb.org/docs/current/data/parquet/overview.html).
...
DuckDB also offers support for the [Hive partitioning scheme](https://duckdb.org/docs/current/data/partitioning/hive_partitioning.html), which is available when using HTTP(S) and S3 endpoints.
...
Further S3-related settings are available and documented in the [configuration reference](https://duckdb.org/docs/current/configuration/overview.html), including `enable_global_s3_configuration`, `merge_http_secret_into_s3_request`, `s3_allow_recursive_globbing`, `httpfs_enable_credential_refresh`, `s3_version_id_pinning`, and `unsafe_disable_etag_checks`.
