...
Prior to version 0.10.0, DuckDB did not have a [Secrets manager](https://duckdb.org/docs/current/sql/statements/create_secret.html). Hence, the configuration of and authentication to S3 endpoints was handled via variables. This page documents the legacy authentication scheme for the S3 API.
...
> The recommended way to configure and authenticate of S3 endpoints is to use [secrets](https://duckdb.org/docs/current/core_extensions/httpfs/s3api.html#configuration-and-authentication).
...
The [`aws` extension](https://duckdb.org/docs/current/core_extensions/aws.html) allows for loading AWS credentials.
...
```sql
SELECT *
FROM 's3://bucket/file.parquet?s3_access_key_id=EXAMPLE_S3_ACCESS_KEY_ID&s3_secret_access_key=EXAMPLE_S3_SECRET_ACCESS_KEY';
```
...
```sql
SELECT *
FROM 's3://bucket/file.parquet?s3_access_key_id=EXAMPLE_S3_ACCESS_KEY_ID&s3_secret_access_key=EXAMPLE_S3_SECRET_ACCESS_KEY' t1
INNER JOIN 's3://bucket/file.csv?s3_access_key_id=EXAMPLE_S3_ACCESS_KEY_ID&s3_secret_access_key=EXAMPLE_S3_SECRET_ACCESS_KEY' t2;
```
