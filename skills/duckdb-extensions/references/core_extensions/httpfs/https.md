...
For Parquet files, DuckDB supports [partial reading](https://duckdb.org/docs/current/data/parquet/overview.html#partial-reading), i.e., it can use a combination of the Parquet metadata and [HTTP range requests](https://developer.mozilla.org/en-US/docs/Web/HTTP/Range_requests) to only download the parts of the file that are actually required by the query. For example, the following query will only read the Parquet metadata and the data for the `column_a` column:
...
To authenticate for an HTTP(S) endpoint, create an `HTTP` secret using the [Secrets Manager](https://duckdb.org/docs/current/configuration/secrets_manager.html):
...
You can add an HTTP proxy using the [Secrets Manager](https://duckdb.org/docs/current/configuration/secrets_manager.html):
...
Alternatively, you can add it via [configuration options](https://duckdb.org/docs/current/configuration/pragmas.html):
...
To use the `httpfs` extension with a custom certificate file, set the following [configuration options](https://duckdb.org/docs/current/configuration/pragmas.html) prior to loading the extension:
