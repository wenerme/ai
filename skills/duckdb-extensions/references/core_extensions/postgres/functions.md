...
Executes the `⟨sql_query⟩` in the specified remote Postgres instance previously attached with `ATTACH ... AS ⟨attached_db_name⟩`.
...
Executes the query in the specified remote DB that was previously attached with `ATTACH .. AS ⟨attached_db_name⟩` and returns the query result as a table.
