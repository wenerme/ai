...
User credentials and other PostgreSQL database connection details can be stored using the DuckDB [Secrets Manager](https://duckdb.org/docs/current/configuration/secrets_manager.html). The following syntax can be used to create a secret:
...
By default, created secrets are temporary. Secrets can be persisted using the [`CREATE PERSISTENT SECRET` command](https://duckdb.org/docs/current/configuration/secrets_manager.html#persistent-secrets). Persistent secrets can be used across sessions.
...
The secret of type `rds` requires the `aws` extension to be installed and allows to configure AWS Credential Chain the same way as with the secret of type `s3`, see details in the [AWS extension documentation](https://duckdb.org/docs/current/core_extensions/aws.html#credential_chain-provider).

## Storing Secrets inside a PostgreSQL Database

DuckDB [Secrets Manager](https://duckdb.org/docs/current/configuration/secrets_manager.html) supports pluggable Storage Providers.
...
```bash
duckdb postgres:postgresql://EXAMPLE_USER:EXAMPLE_PASSWORD@127.0.0.1:5432/db1
```
...
```bash
duckdb ducklake:postgres:postgresql://EXAMPLE_USER:EXAMPLE_PASSWORD@127.0.0.1:5432/db1
```
