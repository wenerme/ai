...
The `azure` extension will be transparently [autoloaded](https://duckdb.org/docs/current/core_extensions/overview.html#autoloading-extensions) on first use from the official extension repository.
...
You can write data directly to Azure Blob or ADLSv2 Storage using the [`COPY` statement](https://duckdb.org/docs/current/sql/statements/copy.html).
...
Use the following [configuration options](https://duckdb.org/docs/current/configuration/overview.html) to control how the extension reads remote files:

| Name | Description | Type | Default |
|:---|:---|:---|:---|
| `azure_http_stats` | Include HTTP info from Azure Storage in the [`EXPLAIN ANALYZE` statement](https://duckdb.org/docs/current/dev/profiling.html). | `BOOLEAN` | `false` |
...
Multiple [Secret Providers](https://duckdb.org/docs/current/configuration/secrets_manager.html#secret-providers) are available for the Azure extension:

* If you need to define different secrets for different storage accounts, use the [`SCOPE` configuration](https://duckdb.org/docs/current/configuration/secrets_manager.html#creating-multiple-secrets-for-the-same-service-type). Note that the `SCOPE` requires a trailing slash (`SCOPE 'azure://some_container/'`).
