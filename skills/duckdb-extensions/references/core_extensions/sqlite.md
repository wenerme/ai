...
The `sqlite` extension will be transparently [autoloaded](https://duckdb.org/docs/current/extensions/overview.html#autoloading-extensions) on first use from the official extension repository.
...
> Deprecated The old `sqlite_attach` function is deprecated. It is recommended to switch over to the new [`ATTACH` syntax](https://duckdb.org/docs/current/sql/statements/attach.html).
