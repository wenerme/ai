...
If you plan to embed DuckDB in your application, also consult the ["Embedding DuckDB"](https://duckdb.org/docs/current/operations_manual/securing_duckdb/embedding_duckdb.html) page.
...
Besides SQL, DuckDB also has several non-SQL APIs that can be used to interact with the database. For example, there is a [relational API](https://duckdb.org/docs/current/clients/python/relational_api.html) in Python that allows building queries programmatically.
...
DuckDB has a flexible [extension mechanism](https://duckdb.org/docs/current/extensions/overview.html) that adds functionality such as new file formats, functions, and remote file system access. Extensions run with the same privileges as the DuckDB process itself, so they warrant careful consideration in security-sensitive environments.

### Autoloading

DuckDB can automatically load [core extensions](https://duckdb.org/docs/current/core_extensions/overview.html) when certain SQL statements require them. To maintain full control over which extensions are loaded, you can disable autoloading:
...
* **[Community extensions](https://duckdb.org/community_extensions/):** Contributed by third parties and installed via `INSTALL extension_name FROM community`. These are not maintained by the DuckDB team, so only install community extensions from sources you trust.
...
DuckDB's CLI client supports [“safe mode”](https://duckdb.org/docs/current/clients/cli/safe_mode.html), which prevents DuckDB from accessing external files other than the database file.
This can be activated via a command line argument or a [dot command](https://duckdb.org/docs/current/clients/cli/dot_commands.html):
...
DuckDB can list directories and read arbitrary files via its CSV parser’s [`read_csv` function](https://duckdb.org/docs/current/data/csv/overview.html) or read text via the [`read_text` function](https://duckdb.org/docs/current/sql/functions/text.html#read_textsource).
...
Second, you can also completely disable external access by setting the [`enable_external_access` option](https://duckdb.org/docs/current/configuration/overview.html#configuration-reference) option to `false`.
...
Security-related configuration settings generally lock themselves for safety reasons. For example, while we can disable [community extensions](https://duckdb.org/community_extensions/) using the `SET allow_community_extensions = false`, we cannot re-enable them again after the fact without restarting the database. Trying to do so will result in an error:
...
[Persistent secrets](https://duckdb.org/docs/current/configuration/secrets_manager.html#persistent-secrets) are stored in unencrypted binary format on the disk. These have the same permissions as SSH keys, `600`, i.e., only the user who is running the DuckDB (parent) process can read and write them.

## Prepared Statements to Prevent SQL Injection

Similarly to other SQL databases, it's recommended to use [prepared statements](https://duckdb.org/docs/current/sql/query_syntax/prepared_statements.html) in DuckDB to prevent [SQL injection](https://en.wikipedia.org/wiki/SQL_injection).
