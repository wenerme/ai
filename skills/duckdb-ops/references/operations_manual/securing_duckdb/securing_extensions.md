...
* Signed with a `community` key. These are open-source extensions distributed via the [DuckDB Community Extensions repository](https://duckdb.org/community_extensions/).
* Unsigned.
...
Security-related configuration settings [lock themselves](https://duckdb.org/docs/current/operations_manual/securing_duckdb/overview.html#locking-configurations), i.e., it is only possible to restrict capabilities in the current process.
...
DuckDB has a [Community Extensions repository](https://duckdb.org/community_extensions/), which allows convenient installation of third-party extensions.
...
To lock this configuration, use the [`lock_configuration` option](https://duckdb.org/docs/current/operations_manual/securing_duckdb/overview.html#locking-configurations):
...
The [`allow_unsigned_extensions` setting](https://duckdb.org/docs/current/extensions/overview.html#unsigned-extensions) can be enabled on start-up to allow loading unsigned extensions.
