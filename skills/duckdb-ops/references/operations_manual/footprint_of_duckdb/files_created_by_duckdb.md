...
|-------|-------------------|--|--|
| `~/.duckdbrc` | The content of this file is executed when starting the [DuckDB CLI client](https://duckdb.org/docs/current/clients/cli/overview.html). The commands can be both [dot command](https://duckdb.org/docs/current/clients/cli/dot_commands.html) and SQL statements. The naming of this file follows the `~/.bashrc` and `~/.zshrc` “run commands” files. | Yes | Only used by CLI |
| `~/.duckdb_history` | History file, similar to `~/.bash_history` and `~/.zsh_history`. Used by the [DuckDB CLI client](https://duckdb.org/docs/current/clients/cli/overview.html). | Yes | Only used by CLI |
| `~/.duckdb/extensions` | Binaries of installed [extensions](https://duckdb.org/docs/current/extensions/overview.html). | No | Yes |
| `~/.duckdb/stored_secrets` | [Persistent secrets](https://duckdb.org/docs/current/configuration/secrets_manager.html#persistent-secrets) created by the [Secrets manager](https://duckdb.org/docs/current/configuration/secrets_manager.html). | Yes | Yes |
...
see the instructions on using [`.gitignore` for DuckDB](https://duckdb.org/docs/current/operations_manual/footprint_of_duckdb/gitignore_for_duckdb.html).
