...
which indicates version, architecture, client, buildref in the agent string. The user-agent string can also be modified via the `custom_user_agent` setting, see [Configuration](https://duckdb.org/docs/current/configuration/overview.html). The currently generated user-agent string can be seen via `PRAGMA user_agent;`, see [Configuration/Pragmas](https://duckdb.org/docs/current/configuration/pragmas.html#user-agent).
