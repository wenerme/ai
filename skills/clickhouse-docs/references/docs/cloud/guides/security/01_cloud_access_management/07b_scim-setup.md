...
> **note**: SCIM provisioning is in private preview.
...
[Navigate to the SCIM configuration tab in Organization settings]
...
[Set provisioning mode to SCIM in the Okta application settings]

The application now shows a **Provisioning** tab.

[Provisioning tab now appears on the application]
...
[Enter SCIM connector URL and set the unique identifier to userName]

[Enter the API credentials for SCIM authentication]
...
[Click Assign people to the group]
...
[Set up Group Push by name on the application's Push Groups tab]
...
[Verify user synchronization in Users and roles]
