...
> **note**: For read/write workloads, the IAM policy must include `s3:PutObject`, `s3:DeleteObject`, and metadata-modifying actions for Iceberg. The above sample is conservative read-only.
...
> **note**: DataLake Catalog for Glue is coming in version 26.2.
```sql
