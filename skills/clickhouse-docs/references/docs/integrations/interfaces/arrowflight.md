...
> **note**: The current implementation blocks until a data block is available rather than returning immediately with no data.
...
> **note**: Only the `Arrow` format is accepted for data transfer. Specifying other formats in SQL (e.g., `FORMAT JSON`) results in an error.
