...
> **note**: The grants check queries `system.grants` and only matches direct user grants. Privileges assigned through a ClickHouse role are not detected. See the [role-based grants](/integrations/fivetran/troubleshooting#role-based-grants) section for more details.
