...
> **note**: Check the [supported AWS regions](#aws-privatelink-regions) before you start.
...
> **note**: For Kafka, the [schema registry](/integrations/clickpipes/kafka/schema-registries) can be reached over the same reverse private endpoint as the brokers — it does not need to be publicly accessible — provided its hostname resolves to the endpoint's private IP addresses.
...
> **info**: Custom private DNS is in **Private Preview**. Reach out to the [ClickHouse support team](https://clickhouse.com/support/program) to enable it for your service.
...
> **note**: The ClickHouse Cloud console currently supports one custom private DNS name per reverse private endpoint.
