...
> **note**: Here are the various states that a service can be in:
...
> **note**: A Cloud service that is **"idle"** is considered started, so a `start` command will not resume/wake it up. Use the `ping` endpoint shown in Step 1 to wake up a service.
