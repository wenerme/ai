...
> **note**: Filtering on keys later in an ordering key will not be as efficient as filtering on those that are earlier in the tuple. For reasons why, see [here](/guides/best-practices/sparse-primary-indexes#secondary-key-columns-can-not-be-inefficient)
