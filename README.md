...
| `biome-lint` | Use when fixing Biome linter errors, TypeScript/tsgo type-check errors, or running lint/typecheck workflows | local |
| `bun-ffi-interop-pattern` | Use when calling native libraries via bun:ffi, loading shared libraries with dlopen, or working with FFI pointers and C... | local |
...
| `lark-node-sdk` | Use when developing with Lark/Feishu Open Platform using @larksuiteoapi/node-sdk, including API calls, event handling,... | local |
| `orpc-implementation-sops` | Use when building, updating, or refactoring oRPC contracts, server handlers, clients, or React Query integration | local |
| `tmux-session-manager` | Use when executing commands, running builds, starting services, or monitoring logs in a visible tmux pane | local |
| `wode-db-schema-pattern` | Use when designing, creating, or modifying PostgreSQL table schemas in the Wode project, including ID strategy,... | local |
| `wode-emittery-pattern` | Use when implementing event-driven communication between React components using Emittery, including event types, sidecar... | local |
...
| `zustand-mutative-pattern` | Use when implementing React state management with Zustand, including context-scoped stores, mutative updates, or actions... | local |
